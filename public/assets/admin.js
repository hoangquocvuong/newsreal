...
async function loadCommerceCategories(){const sel=document.getElementById('commerceProductCategory'),box=document.getElementById('commerceCategoriesPanel'),prev=sel?.value||'0';if(sel&&!sel.options.length)sel.innerHTML='<option value="0">Chọn danh mục...</option>';try{const d=await Promise.race([api('/commerce/categories'),new Promise((_,rej)=>setTimeout(()=>rej(new Error('timeout')),8000))]);COM_ADMIN_CATEGORIES=d.categories||[];if(sel){sel.innerHTML='<option value="0">Chọn danh mục...</option>'+COM_ADMIN_CATEGORIES.map(c=>`<option value="${c.id}">${nrEsc(c.name)}</option>`).join('');if(prev&&[...sel.options].some(o=>o.value===String(prev)))sel.value=prev;sel.onchange=comRenderDynamicFields}if(box)box.innerHTML=COM_ADMIN_CATEGORIES.map(c=>`<div class="commerce-admin-card"><b>${nrEsc(c.name)}</b> · ${comSchemaParse(c).length} trường <button class="smallbtn" onclick="commerceEditCategory(${c.id})">Sửa schema</button></div>`).join('')||'<p class="muted">Chưa có danh mục riêng. Sản phẩm vẫn có thể đăng vào nhóm Sản phẩm chung.</p>'}catch(e){COM_ADMIN_CATEGORIES=[];if(sel)sel.innerHTML='<option value="0">Chọn danh mục...</option>';if(box)box.innerHTML='<p class="muted">Chưa tải được danh mục nâng cao. Form đăng nhanh vẫn hoạt động bình thường.</p>'}comRenderDynamicFields()}
...
async function loadCommerceProducts(){const box=document.getElementById('commerceProductsPanel');if(!box)return;box.innerHTML='<div class="muted">Đang tải sản phẩm...</div>';try{await loadCommerceCategories();const d=await api('/commerce/products');box.innerHTML=(d.products||[]).map(p=>`<div class="commerce-admin-card"><div class="row"><div><b>${nrEsc(p.name)}</b><br><small>${nrEsc(p.category_name||'Chưa phân loại')} · ${nrEsc(p.sku||'Không SKU')} · ${comAdminMoney(p.price)} · Tồn ${Number(p.stock_qty||0)}</small></div><div><button class="smallbtn" onclick="commerceToggleProduct(${p.id},${p.is_active?0:1})">${p.is_active?'Ẩn':'Hiện'}</button> <button class="smallbtn danger" onclick="commerceDeleteProduct(${p.id})">Xóa</button></div></div></div>`).join('')||'<div class="empty">Chưa có sản phẩm.</div>'}catch(e){box.innerHTML='<div class="form-errors">'+nrEsc(e.message)+'</div>'}}
