...
let allPosts=[];
let editorContext='create'; // create | edit_sample | edit_post
function nrAdminToast(message){let el=document.getElementById('nrAdminGlobalToast');if(!el){el=document.createElement('div');el.id='nrAdminGlobalToast';el.style.cssText='position:fixed;right:22px;top:86px;z-index:99999;max-width:420px;padding:14px 18px;border-radius:12px;background:#e9fbf1;color:#087443;border:1px solid #b8ebce;box-shadow:0 12px 35px #0f172a22;font-weight:800';document.body.appendChild(el)}el.textContent=message;el.hidden=false;clearTimeout(nrAdminToast._t);nrAdminToast._t=setTimeout(()=>{el.hidden=true},5000)}
function resetPostEditor(){postForm?.reset();setRichContent('');uploadedImages=[];renderImages();if(editingId)editingId.value='';editorContext='create';renderProfileFields();if(isProductTemplate())postType.value='product';if(isNewsTemplate())postType.value='news';if(isServiceTemplate())postType.value='service';if(isGameTemplate())postType.value='game';updateContentTypeUI();clearFieldErrors();if(postMsg)postMsg.classList.add('hidden');updateSubmitLabel()}
function showEditorContext(ctx='create'){editorContext=ctx;document.querySelectorAll('.tab').forEach(x=>x.classList.add('hidden'));document.getElementById('tab-newpost')?.classList.remove('hidden');const active=ctx==='edit_sample'?'samples':ctx==='edit_post'?'posts':'newpost';document.querySelectorAll('.menu-btn').forEach(x=>x.classList.toggle('active',x.dataset.tab===active));updateContentTypeUI();updateSubmitLabel()}
...
const BUILTIN_CONTENT_PROFILES={
 ...
 product:{
  id:'product-affiliate',label:'Sản phẩm / Affiliate',content_type:'product',
  categories:['Điện tử & Công nghệ','Nhà cửa & Đời sống','Thời trang & Làm đẹp','Mẹ & Bé','Thể thao & Du lịch','Phụ kiện & Khác'],
  contentLabel:'Mô tả / Review sản phẩm',
  contentHelp:'Viết mô tả, ưu nhược điểm, hướng dẫn chọn mua và nội dung SEO cho sản phẩm.',
  custom_fields:[
   {key:'product_price',label:'Giá hiện tại',type:'number',required:true,placeholder:'1490000'},
   {key:'product_old_price',label:'Giá gốc',type:'number',placeholder:'1990000'},
   {key:'product_brand',label:'Thương hiệu',type:'text',placeholder:'Ví dụ: Logitech'},
   {key:'product_sku',label:'Mã / Model',type:'text',placeholder:'Ví dụ: G304'},
   {key:'product_affiliate_url',label:'Link mua / Affiliate',type:'url',required:true,placeholder:'https://...'},
   {key:'product_badge',label:'Nhãn nổi bật',type:'text',placeholder:'BÁN CHẠY / GIẢM GIÁ'},
   {key:'product_rating',label:'Điểm đánh giá',type:'number',placeholder:'4.8'},
   {key:'product_sold',label:'Đã bán',type:'number',placeholder:'1250'},
   {key:'product_promo',label:'Ưu đãi / Voucher',type:'textarea',placeholder:'Nội dung ưu đãi, voucher hoặc lưu ý giá.'},
   {key:'product_specs',label:'Thông số sản phẩm',type:'textarea',placeholder:'Mỗi dòng: Tên thông số: Giá trị'}
  ]
 },
 game:{
...
function resolvedContentProfile(){
 ...
 else if(CLIENT_PROFILE?.content_type==='product'||CLIENT_PROFILE?.id==='product-affiliate'||isProductTemplate())base=BUILTIN_CONTENT_PROFILES.product;
 ...
 const productBase=base.content_type==='product'||remote.content_type==='product'||/^san-pham-\d+$/i.test(adminTemplateOverride())||isProductTemplate();
...
function telecomCategoryFields(category=''){
 const c=String(category||'').toLowerCase();
 const price={key:'service_price',label:'Giá / tháng',type:'text',required:true,placeholder:'Ví dụ: 220.000đ/tháng'};
 const promo={key:'service_promo',label:'Khuyến mãi / quyền lợi',type:'textarea',placeholder:'Ưu đãi, quà tặng và điều kiện áp dụng'};
 const area={key:'service_area',label:'Khu vực áp dụng',type:'text',placeholder:'Hải Phòng / Toàn quốc'};
 const cta={key:'service_cta',label:'Nhãn nút CTA',type:'text',placeholder:'Đăng ký tư vấn'};
 if(c.includes('camera'))return [price,{key:'camera_type',label:'Loại Camera',type:'select',options:['Camera trong nhà','Camera ngoài trời','Camera quay quét','Camera cố định','Bộ Camera']},{key:'camera_resolution',label:'Độ phân giải',type:'text',placeholder:'2MP / 3MP / 4MP / 2K / 3K'},{key:'camera_count',label:'Số lượng Camera',type:'text',placeholder:'1 / 2 / 4 thiết bị'},{key:'service_camera',label:'Camera / tính năng AI',type:'text',placeholder:'AI phát hiện người, đàm thoại 2 chiều...'},{key:'service_cloud',label:'Cloud / lưu trữ',type:'text',placeholder:'Cloud 1 / 3 / 7 ngày, thẻ nhớ...'},{key:'service_install',label:'Phí hòa mạng / lắp đặt',type:'text',placeholder:'Miễn phí / theo khu vực'},promo,area,cta];
 if(c.includes('truyền hình')||c.includes('mytv')||c.includes('tv360'))return [price,{key:'service_tv',label:'Gói truyền hình / nội dung',type:'text',placeholder:'TV360 / MyTV / FPT Play...'},{key:'service_device',label:'Thiết bị / Box',type:'text',placeholder:'Android Box / Smart TV / App'},{key:'service_channels',label:'Kênh / nội dung nổi bật',type:'text',placeholder:'130+ kênh, thể thao, phim...'},{key:'service_term',label:'Chu kỳ / thời hạn',type:'text',placeholder:'30 ngày / 12 tháng'},promo,area,cta];
...
const ADMIN_MENU_SPECS={
 overview:{tab:'overview',icon:'▦',label:'Tổng quan'},
 newpost:{tab:'newpost',icon:'＋',label:'Đăng nội dung mới',labelId:'menuNewPostText'},
 posts:{tab:'posts',icon:'▤',label:'Quản lý nội dung',labelId:'menuPostsText'},
 ...
 support:{tab:'support',icon:'☎',label:'Hỗ trợ'},
 settings:{tab:'settings',icon:'⚙',label:'Cài đặt website'},
 password:{tab:'password',icon:'⌁',label:'Đổi mật khẩu'}
};
function adminCapabilityProfile(){
 ...
 if(commerce)return {kind:'commerce',modules:['overview','products','orders','commerceSettings','leads','samples','stats','service','support','settings','password']};
 if(game)return {kind:'game',modules:['overview','newpost','posts','samples','stats','support','settings','password']};
 if(product)return {kind:'product',modules:['overview','newpost','posts','samples','stats','leads','service','support','settings','password']};
 if(news)return {kind:'news',modules:['overview','newpost','posts','samples','stats',...(isProfessionalContactTemplate()?['leads']:[]),'service','support','settings','password']};
 if(service)return {kind:'service',modules:['overview','newpost','posts','samples','stats','leads','service','support','settings','password']};
 return {kind:'default',modules:['overview','newpost','posts','samples','stats','service','support','settings','password']};
}
function renderAdminSidebar(){
 const side=document.getElementById('adminSidebar');if(!side)return;
 const cap=adminCapabilityProfile();
 const title=document.createElement('div');title.className='sidebar-title';title.innerHTML='<b>Quản trị website</b><small>Nội dung & vận hành</small>';
 const frag=document.createDocumentFragment();frag.appendChild(title);
 cap.modules.forEach((key,i)=>{
   const spec=ADMIN_MENU_SPECS[key];if(!spec)return;
   if(key==='settings'){const d=document.createElement('div');d.className='menu-divider';frag.appendChild(d)}
   const b=document.createElement('button');b.className='menu-btn'+(i===0?' active':'');b.dataset.tab=spec.tab;b.dataset.adminModule=key;
   b.innerHTML=`<span>${spec.icon}</span><span${spec.labelId?` id="${spec.labelId}"`:''}>${spec.label}</span>`;frag.appendChild(b);
 });
 side.replaceChildren(frag);
 side.onclick=e=>{const b=e.target.closest('.menu-btn');if(!b||!side.contains(b))return;const n=b.dataset.tab;if(n==='newpost')resetPostEditor();else if(editingId?.value){editingId.value='';editorContext='create'}showTab(n)};
 document.body.dataset.adminProfile=cap.kind;
}
...
function configureAdminForTemplate(){
 ...
 renderAdminSidebar();
 // Re-apply template labels after rebuilding the sidebar from the capability allow-list.
 const rebuiltNew=document.getElementById('menuNewPostText'),rebuiltPosts=document.getElementById('menuPostsText');
 if(rebuiltNew)rebuiltNew.textContent=commerce?'Thêm sản phẩm':product?'Thêm sản phẩm':game?'Đăng base mới':service?'Đăng gói mới':'Đăng bài mới';
 if(rebuiltPosts)rebuiltPosts.textContent=product?'Quản lý sản phẩm':game?'Quản lý Base':service?(professionalAdminKey()==='dich-vu-6'?'Quản lý gói & bài viết':'Quản lý gói & bài viết'):'Quản lý bài viết';
 updateContentTypeUI();
}
function showTab(n){if(!assertAdminCapability(n)){console.warn('Blocked Admin module outside template capability:',n);n='overview'}document.querySelectorAll('.tab').forEach(x=>x.classList.add('hidden'));const target=document.getElementById('tab-'+n);if(!target)return;target.classList.remove('hidden');document.querySelectorAll('.menu-btn').forEach(x=>x.classList.toggle('active',x.dataset.tab===n));if(n==='posts')loadPosts();if(n==='samples')loadSamplePosts();if(n==='stats')loadStats();if(n==='service')loadService();if(n==='serviceleads')loadServiceLeads();if(n==='commerce-products')loadCommerceProducts();if(n==='commerce-orders')loadCommerceOrders();if(n==='commerce-settings')loadCommerceSettings()}
...
function nrEscAttr(v=''){return String(v??'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/"/g,'&quot;')}
function nrImageListValue(v){if(Array.isArray(v))return v;try{const a=JSON.parse(String(v||'[]'));return Array.isArray(a)?a:[]}catch{return []}}
function renderTemplateSettingsFields(values={}){const panel=document.getElementById('templateSettingsPanel'),host=document.getElementById('templateSettingsFields'),schema=templateSettingsSchema();if(!panel||!host)return;panel.classList.toggle('hidden',!schema.length);host.innerHTML=schema.map(def=>{const key=String(def.key||''),type=String(def.type||'text'),raw=values[key]??def.default??'',val=String(raw??''),help=def.help?`<small class="field-help">${def.help}</small>`:'';if(type==='textarea')return `<label>${def.label||key}<textarea data-template-setting="${key}" rows="5" disabled>${nrEscAttr(val)}</textarea>${help}</label>`;if(type==='image')return `<label class="template-image-setting">${def.label||key}<input data-template-setting="${key}" type="text" value="${nrEscAttr(val)}" disabled><span class="template-image-tools">${val?`<img src="${nrEscAttr(val)}" alt="Ảnh hiện tại">`:''}<input type="file" data-template-image-file="${key}" accept="image/jpeg,image/png,image/webp" disabled><small data-template-image-status="${key}"></small></span>${help}</label>`;if(type==='image-list'){const arr=nrImageListValue(raw);return `<div class="template-image-setting template-image-list" data-template-image-list="${key}"><b>${def.label||key}</b>${help}<div data-template-image-list-items="${key}">${arr.map((url,i)=>`<div class="template-slide-row" data-slide-row><img src="${nrEscAttr(url)}" alt="Slide ${i+1}"><input type="text" data-slide-url value="${nrEscAttr(url)}" disabled><button type="button" data-slide-up disabled>↑</button><button type="button" data-slide-down disabled>↓</button><button type="button" data-slide-remove disabled>Xóa</button></div>`).join('')}</div><input type="file" data-template-image-list-file="${key}" accept="image/jpeg,image/png,image/webp" disabled><small data-template-image-list-status="${key}"></small></div>`}return `<label>${def.label||key}<input data-template-setting="${key}" type="${type==='url'?'url':'text'}" value="${nrEscAttr(val)}" placeholder="${nrEscAttr(def.placeholder||'')}" disabled>${help}</label>`}).join('')}
function collectTemplateSettings(){const out={};document.querySelectorAll('[data-template-setting]').forEach(el=>out[el.dataset.templateSetting]=el.value||'');document.querySelectorAll('[data-template-image-list]').forEach(box=>out[box.dataset.templateImageList]=[...box.querySelectorAll('[data-slide-url]')].map(x=>x.value||'').filter(Boolean));return out}
document.addEventListener('change',async e=>{const fileInput=e.target?.closest?.('[data-template-image-file]');if(!fileInput)return;const file=fileInput.files?.[0],key=fileInput.dataset.templateImageFile,status=document.querySelector(`[data-template-image-status="${key}"]`),target=document.querySelector(`[data-template-setting="${key}"]`);if(!file||!target)return;if(file.size>8*1024*1024){if(status)status.textContent='Ảnh vượt quá 8 MB';fileInput.value='';return}if(status)status.textContent='Đang tải ảnh...';const fd=new FormData();fd.append('file',file);try{const tk=nrGetAdminToken(),r=await fetch(tenantUrl('/upload'),{method:'POST',body:fd,credentials:'include',headers:tk?{'Authorization':'Bearer '+tk}:{}}),d=await r.json();if(!r.ok)throw new Error(d.error||'Tải ảnh thất bại');target.value=d.url||'';const tools=fileInput.closest('.template-image-tools');let img=tools?.querySelector('img');if(!img&&tools){img=document.createElement('img');tools.prepend(img)}if(img)img.src=d.url;if(status)status.textContent='Đã tải ảnh. Bấm Lưu thay đổi để áp dụng.'}catch(err){if(status)status.textContent=err.message||'Tải ảnh thất bại'}finally{fileInput.value=''}});
document.addEventListener('click',e=>{const row=e.target?.closest?.('[data-slide-row]');if(!row)return;const box=row.parentElement;if(e.target.matches('[data-slide-remove]'))row.remove();if(e.target.matches('[data-slide-up]')&&row.previousElementSibling)box.insertBefore(row,row.previousElementSibling);if(e.target.matches('[data-slide-down]')&&row.nextElementSibling)box.insertBefore(row.nextElementSibling,row)});
document.addEventListener('change',async e=>{const input=e.target?.closest?.('[data-template-image-list-file]');if(!input)return;const file=input.files?.[0],key=input.dataset.templateImageListFile,status=document.querySelector(`[data-template-image-list-status="${key}"]`),host=document.querySelector(`[data-template-image-list-items="${key}"]`);if(!file||!host)return;if(file.size>8*1024*1024){if(status)status.textContent='Ảnh vượt quá 8 MB';return}if(status)status.textContent='Đang tải ảnh...';const fd=new FormData();fd.append('file',file);try{const tk=nrGetAdminToken(),r=await fetch(tenantUrl('/upload'),{method:'POST',body:fd,credentials:'include',headers:tk?{'Authorization':'Bearer '+tk}:{}}),d=await r.json();if(!r.ok)throw new Error(d.error||'Tải ảnh thất bại');host.insertAdjacentHTML('beforeend',`<div class="template-slide-row" data-slide-row><img src="${nrEscAttr(d.url)}" alt="Slide mới"><input type="text" data-slide-url value="${nrEscAttr(d.url)}"><button type="button" data-slide-up>↑</button><button type="button" data-slide-down>↓</button><button type="button" data-slide-remove>Xóa</button></div>`);if(status)status.textContent='Đã thêm ảnh. Bấm Lưu thay đổi để áp dụng.'}catch(err){if(status)status.textContent=err.message||'Tải ảnh thất bại'}finally{input.value=''}});
function captureWebsiteSettings(){return {name:setName.value,phone:setPhone.value,zalo:setZalo.value,email:setEmail.value,facebook:setFacebook.value,seo_title:setSeoTitle?.value||'',seo_description:setSeoDescription?.value||'',seo_og_image:setSeoOgImage?.value||'',seo_index:!!setSeoIndex?.checked,template_settings:collectTemplateSettings()}}
function setWebsiteSettingsEditing(editing){
 [setName,setPhone,setZalo,setEmail,setFacebook,setSeoTitle,setSeoDescription,setSeoOgImage,setSeoIndex].forEach(el=>{if(el)el.disabled=!editing});document.querySelectorAll('[data-template-setting],[data-template-image-file],[data-template-image-list-file],[data-slide-url],[data-slide-up],[data-slide-down],[data-slide-remove]').forEach(el=>el.disabled=!editing);
...
function updateSubmitLabel(){
  ...
  submitPostBtn.textContent=postStatus.value==='draft'?'Lưu bản nháp':(editingId.value?(isProductTemplate()?'Cập nhật sản phẩm':isNewsTemplate()?'Cập nhật bài viết':'Cập nhật tin'):(isProductTemplate()?'Đăng sản phẩm':isNewsTemplate()?'Đăng bài':'Đăng tin'));
}
...
function fillCategoryOptions(keep=''){
  ...
  if(isNewsTemplate()||isServiceTemplate()||isGameTemplate()||isProductTemplate()||['news','service','game','product'].includes(profile.content_type))list=Array.isArray(profile.categories)?profile.categories:[];
...
const savedContext=editorContext;const message=b.status==='draft'?'✓ Đã lưu bản nháp.':(id?(savedContext==='edit_sample'?'✓ Đã cập nhật tin mẫu thành công.':'✓ Đã cập nhật tin thành công.'):'✓ Đã đăng tin thành công.');
resetPostEditor();
const target=savedContext==='edit_sample'?'samples':'posts';showTab(target);nrAdminToast(message);
}catch(err){
...
function adminViewPostUrl(x){const u=new URL('/xem-bai',location.origin);u.searchParams.set('id',String(x?.id||''));if(trialParam)u.searchParams.set('nr_trial',trialParam);else if(tenantParam)u.searchParams.set('tenant',tenantParam);return u.pathname+u.search}
function viewPost(id){const x=allPosts.find(p=>Number(p.id)===Number(id));if(!x)return;window.open(adminViewPostUrl(x),'_blank','noopener')}
...
cancelWebsiteSettings?.addEventListener('click',()=>{
 if(websiteSettingsSnapshot){
  ...
  if(setSeoTitle)setSeoTitle.value=websiteSettingsSnapshot.seo_title||'';if(setSeoDescription)setSeoDescription.value=websiteSettingsSnapshot.seo_description||'';if(setSeoOgImage)setSeoOgImage.value=websiteSettingsSnapshot.seo_og_image||'';if(setSeoIndex)setSeoIndex.checked=websiteSettingsSnapshot.seo_index!==false;
...
settingsForm.addEventListener('submit',async e=>{
 ...
 try{
   await api('/settings',{method:'PUT',body:JSON.stringify({
     name:setName.value,phone:setPhone.value,zalo:setZalo.value,email:setEmail.value,facebook:setFacebook.value,seo_title:setSeoTitle?.value||'',seo_description:setSeoDescription?.value||'',seo_og_image:setSeoOgImage?.value||'',seo_index:!!setSeoIndex?.checked,template_settings:collectTemplateSettings()
   })});
   ...
   if(setSeoTitle)setSeoTitle.value=fresh.site.seo_title||'';if(setSeoDescription)setSeoDescription.value=fresh.site.seo_description||'';if(setSeoOgImage)setSeoOgImage.value=fresh.site.seo_og_image||'';if(setSeoIndex)setSeoIndex.checked=Number(fresh.site.seo_index??1)!==0;
...
// V20.9.27.44 — Universal Commerce Admin
function comAdminMoney(n){return new Intl.NumberFormat('vi-VN').format(Number(n||0))+'đ'}
let COM_ADMIN_CATEGORIES=[];
function comSchemaParse(c){try{return JSON.parse(c?.fields_schema_json||'[]')}catch{return []}}
function comSchemaRow(f={}){const d=document.createElement('div');d.className='commerce-schema-row';d.innerHTML=`<input data-sf="label" placeholder="Tên trường, VD Màu sắc" value="${nrEsc(f.label||'')}"><select data-sf="type"><option>text</option><option>number</option><option>select</option><option>multiselect</option><option>boolean</option><option>color</option><option>size</option><option>date</option></select><input data-sf="options" placeholder="Lựa chọn: Đen,Trắng,M,L" value="${nrEsc((f.options||[]).join(','))}"><label><input data-sf="required" type="checkbox" ${f.required?'checked':''}> Bắt buộc</label><label><input data-sf="filterable" type="checkbox" ${f.filterable?'checked':''}> Bộ lọc</label><label><input data-sf="variant" type="checkbox" ${f.variant?'checked':''}> Biến thể</label><button type="button" class="smallbtn danger">×</button>`;d.querySelector('[data-sf=type]').value=f.type||'text';d.querySelector('button').onclick=()=>d.remove();return d}
function comCollectSchema(){return [...document.querySelectorAll('#commerceSchemaRows .commerce-schema-row')].map(r=>({label:r.querySelector('[data-sf=label]').value.trim(),type:r.querySelector('[data-sf=type]').value,options:r.querySelector('[data-sf=options]').value.split(',').map(x=>x.trim()).filter(Boolean),required:r.querySelector('[data-sf=required]').checked,filterable:r.querySelector('[data-sf=filterable]').checked,variant:r.querySelector('[data-sf=variant]').checked,show_in_specs:true})).filter(x=>x.label)}
function comRenderDynamicFields(){const id=Number(document.getElementById('commerceProductCategory')?.value||0),c=COM_ADMIN_CATEGORIES.find(x=>Number(x.id)===id),box=document.getElementById('commerceDynamicFields');if(!box)return;const fs=comSchemaParse(c);box.innerHTML=fs.length?'<h3>Thông tin theo danh mục '+nrEsc(c.name)+'</h3><div class="commerce-form-grid">'+fs.map(f=>{const key=nrEsc(f.key),lab=nrEsc(f.label)+(f.required?' *':'');if(['select','size','color'].includes(f.type))return `<label>${lab}<select data-attr="${key}" ${f.required?'required':''}><option value="">Chọn</option>${(f.options||[]).map(o=>`<option>${nrEsc(o)}</option>`).join('')}</select></label>`;if(f.type==='multiselect')return `<label>${lab}<input data-attr="${key}" placeholder="${nrEsc((f.options||[]).join(', '))}"></label>`;if(f.type==='boolean')return `<label><input type="checkbox" data-attr="${key}"> ${lab}</label>`;return `<label>${lab}<input type="${f.type==='number'?'number':f.type==='date'?'date':'text'}" data-attr="${key}" ${f.required?'required':''}></label>`}).join('')+'</div>':'<p class="muted">Danh mục này chưa cấu hình trường riêng.</p>'}
const COMMERCE_FALLBACK_CATEGORY_NAMES=['Công nghệ','Điện thoại','Thời trang','Mỹ phẩm','Gia dụng','Nội thất','Phụ kiện','Thể thao','Khác'];
function comLocalCategoryNames(){try{return JSON.parse(localStorage.getItem('nr_commerce_custom_categories')||'[]').filter(Boolean)}catch(e){return []}}
function comSaveLocalCategory(name){const a=[...new Set([...comLocalCategoryNames(),String(name||'').trim()].filter(Boolean))];localStorage.setItem('nr_commerce_custom_categories',JSON.stringify(a.slice(0,60)))}
function comCategoryValue(c){return Number(c?.id||0)>0?String(c.id):'name:'+String(c?.name||'')}
function comSelectedCategoryName(){const sel=document.getElementById('commerceProductCategory');return sel?.selectedOptions?.[0]?.dataset?.name||sel?.selectedOptions?.[0]?.textContent||''}
function comMergeCategories(remote=[]){const byName=new Map();for(const name of [...COMMERCE_FALLBACK_CATEGORY_NAMES,...comLocalCategoryNames()])byName.set(name.toLowerCase(),{id:0,name,is_active:1,local:true});for(const c of remote||[])if(Number(c.is_active)!==0&&c.name)byName.set(String(c.name).toLowerCase(),c);return [...byName.values()]}
function comPaintCategories(categories,keepValue){const sel=document.getElementById('commerceProductCategory'),box=document.getElementById('commerceCategoriesPanel'),help=document.getElementById('commerceCategoryHelp');COM_ADMIN_CATEGORIES=categories||[];if(sel){const prev=keepValue??sel.value??'0';sel.innerHTML='<option value="0">Chọn danh mục...</option>'+COM_ADMIN_CATEGORIES.map(c=>`<option value="${nrEsc(comCategoryValue(c))}" data-name="${nrEsc(c.name)}">${nrEsc(c.name)}</option>`).join('');if(prev&&[...sel.options].some(o=>o.value===String(prev)))sel.value=String(prev);sel.onchange=()=>{comRenderDynamicFields();const name=comSelectedCategoryName();if(help)help.textContent=name&&sel.value!=='0'?'Sản phẩm sẽ hiển thị trong “'+name+'”.':'Chọn nhóm để khách dễ tìm sản phẩm.'}}if(box)box.innerHTML=COM_ADMIN_CATEGORIES.map((c,i)=>`<div class="commerce-admin-card commerce-category-row"><div><b>${nrEsc(c.name)}</b><small>${Number(c.product_count||0)} sản phẩm</small></div>${Number(c.id||0)>0?`<div class="commerce-category-actions"><button class="smallbtn" type="button" onclick="commerceMoveCategory(${c.id},-1)" ${i===0?'disabled':''}>↑</button><button class="smallbtn" type="button" onclick="commerceMoveCategory(${c.id},1)" ${i===COM_ADMIN_CATEGORIES.length-1?'disabled':''}>↓</button><button class="smallbtn" type="button" onclick="commerceRenameCategory(${c.id})">✏️ Sửa</button><button class="smallbtn danger" type="button" onclick="commerceRemoveCategory(${c.id})">🗑 Xóa</button></div>`:'<small class="muted">Sẽ đồng bộ khi kết nối máy chủ</small>'}</div>`).join('');comRenderDynamicFields()}
function comPaintLocalCategories(){const cats=comMergeCategories([]);comPaintCategories(cats);return cats}
async function loadCommerceCategories(){const sel=document.getElementById('commerceProductCategory'),prev=sel?.value||'0';comPaintCategories(comMergeCategories([]),prev);try{const d=await Promise.race([api('/commerce/categories'),new Promise((_,rej)=>setTimeout(()=>rej(new Error('timeout')),5000))]);comPaintCategories(comMergeCategories(d.categories||[]),prev);return COM_ADMIN_CATEGORIES}catch(e){return COM_ADMIN_CATEGORIES}}
function comInstallSimpleAdminStyle(){if(document.getElementById('commerceSimpleV29Style'))return;const st=document.createElement('style');st.id='commerceSimpleV29Style';st.textContent=`#commerceProductForm{display:grid!important;grid-template-columns:1fr 1fr!important;gap:16px 20px!important}#commerceProductForm>label,#commerceProductForm>.commerce-field,#commerceProductForm>details,#commerceProductForm>#commerceDynamicFields,#commerceProductForm>#commerceVariantMatrix,#commerceProductForm>#commerceSuccess{min-width:0}#commerceProductForm label{display:flex!important;flex-direction:column!important;gap:7px!important;font-weight:700!important}#commerceProductForm input:not([type=checkbox]):not([type=file]),#commerceProductForm select,#commerceProductForm textarea{width:100%!important;box-sizing:border-box!important;min-height:44px!important;padding:10px 12px!important;border:1px solid #d7deea!important;border-radius:10px!important;background:#fff!important;font:inherit!important}#commerceProductForm textarea{min-height:130px!important;resize:vertical!important}#commerceProductForm input[type=file]{padding:11px!important;border:1px dashed #b8c5d9!important;border-radius:10px!important;background:#f8fbff!important;width:100%!important;box-sizing:border-box!important}#commerceProductForm button[type=submit]{min-height:46px!important;padding:0 22px!important;border-radius:10px!important;font-weight:800!important}.commerce-product-row{display:grid!important;grid-template-columns:72px minmax(0,1fr) auto!important;gap:14px!important;align-items:center!important}.commerce-product-row img{width:72px!important;height:72px!important;object-fit:cover!important;border-radius:10px!important}.commerce-product-row-main{display:flex!important;flex-direction:column!important;gap:5px!important}.commerce-category-row{display:flex!important;align-items:center!important;justify-content:space-between!important;gap:12px!important;margin:8px 0!important}.commerce-category-row>div:first-child{display:flex!important;flex-direction:column!important;gap:3px!important}.commerce-category-actions{display:flex!important;gap:6px!important;flex-wrap:wrap!important}.commerce-inline-create{display:flex!important;gap:8px!important;margin-top:8px!important;align-items:center!important}.commerce-inline-create.hidden{display:none!important}.commerce-inline-create input{min-width:220px!important}.commerce-option-builder{grid-column:1/-1!important;padding:14px!important;border:1px solid #e1e7f0!important;border-radius:12px!important;background:#fafcff!important}.commerce-option-head{display:flex!important;flex-direction:column!important;gap:3px!important;margin-bottom:10px!important}.commerce-simple-option-row{display:grid!important;grid-template-columns:180px minmax(220px,1fr) auto!important;gap:8px!important;margin:8px 0!important;align-items:center!important}.commerce-variant-row{display:grid!important;grid-template-columns:minmax(140px,1fr) 160px 120px!important;gap:10px!important;align-items:end!important;padding:10px 0!important;border-bottom:1px solid #edf1f6!important}@media(max-width:760px){#commerceProductForm{grid-template-columns:1fr!important}.commerce-product-row{grid-template-columns:58px minmax(0,1fr)!important}.commerce-product-row>div:last-child{grid-column:1/-1!important}}`;document.head.appendChild(st)}
comInstallSimpleAdminStyle();comPaintLocalCategories();
async function commerceRenameCategory(id){const c=COM_ADMIN_CATEGORIES.find(x=>Number(x.id)===Number(id));if(!c)return;const name=String(prompt('Đổi tên danh mục',c.name)||'').trim();if(!name||name===c.name)return;await api('/commerce/categories',{method:'PUT',body:JSON.stringify({id,name,description:c.description||'',fields_schema:comSchemaParse(c),sort_order:Number(c.sort_order||0),is_active:true})});await loadCommerceCategories();nrAdminToast('Đã đổi tên danh mục thành '+name)}
async function commerceMoveCategory(id,dir){const ordered=COM_ADMIN_CATEGORIES.filter(c=>Number(c.id||0)>0),i=ordered.findIndex(c=>Number(c.id)===Number(id)),j=i+Number(dir||0);if(i<0||j<0||j>=ordered.length)return;const a=ordered[i],b=ordered[j],sa=Number(a.sort_order??i),sb=Number(b.sort_order??j);await api('/commerce/categories',{method:'PUT',body:JSON.stringify({id:a.id,name:a.name,description:a.description||'',fields_schema:comSchemaParse(a),sort_order:sb,is_active:true})});await api('/commerce/categories',{method:'PUT',body:JSON.stringify({id:b.id,name:b.name,description:b.description||'',fields_schema:comSchemaParse(b),sort_order:sa,is_active:true})});await loadCommerceCategories()}
async function commerceRemoveCategory(id){const c=COM_ADMIN_CATEGORIES.find(x=>Number(x.id)===Number(id));if(!c)return;const count=Number(c.product_count||0);let targetId=0;if(count>0){const choices=COM_ADMIN_CATEGORIES.filter(x=>Number(x.id||0)>0&&Number(x.id)!==Number(id));if(!choices.length){nrAdminToast('Cần có ít nhất một danh mục khác để chuyển sản phẩm.');return}const answer=String(prompt(`Danh mục “${c.name}” đang có ${count} sản phẩm.\nNhập tên danh mục muốn chuyển sản phẩm sang:\n\n${choices.map(x=>x.name).join(' · ')}`)||'').trim().toLowerCase();const target=choices.find(x=>String(x.name).trim().toLowerCase()===answer);if(!target){if(answer)nrAdminToast('Không tìm thấy danh mục chuyển đến.');return}targetId=Number(target.id);if(!confirm(`Chuyển ${count} sản phẩm sang “${target.name}” rồi xóa “${c.name}”?`))return}else if(!confirm(`Xóa danh mục “${c.name}”?`))return;await api('/commerce/categories?id='+id+(targetId?'&target_id='+targetId:''),{method:'DELETE'});await loadCommerceCategories();await loadCommerceProducts();nrAdminToast(count?'Đã chuyển sản phẩm và xóa danh mục.':'Đã xóa danh mục.')}
function commerceEditCategory(id){const c=COM_ADMIN_CATEGORIES.find(x=>Number(x.id)===Number(id));if(!c)return;const f=document.getElementById('commerceCategoryForm');f.dataset.id=id;f.elements.name.value=c.name;f.elements.description.value=c.description||'';const rows=document.getElementById('commerceSchemaRows');rows.innerHTML='';comSchemaParse(c).forEach(x=>rows.appendChild(comSchemaRow(x)));f.scrollIntoView({behavior:'smooth'})}
document.getElementById('commerceAddSchemaField')?.addEventListener('click',()=>document.getElementById('commerceSchemaRows').appendChild(comSchemaRow()));
function comToggleQuickCategory(show){const box=document.getElementById('commerceQuickCategoryBox'),input=document.getElementById('commerceQuickCategoryName');if(!box)return;box.classList.toggle('hidden',!show);if(show)setTimeout(()=>input?.focus(),0)}
document.getElementById('commerceQuickAddCategory')?.addEventListener('click',()=>comToggleQuickCategory(true));
document.getElementById('commerceQuickCategoryCancel')?.addEventListener('click',()=>comToggleQuickCategory(false));
document.getElementById('commerceQuickCategorySave')?.addEventListener('click',async()=>{const input=document.getElementById('commerceQuickCategoryName'),btn=document.getElementById('commerceQuickCategorySave'),name=String(input?.value||'').trim();if(!name)return nrAdminToast('Nhập tên danh mục');comSaveLocalCategory(name);comPaintCategories(comMergeCategories(COM_ADMIN_CATEGORIES), 'name:'+name);const sel=document.getElementById('commerceProductCategory');if(sel){const local=[...sel.options].find(o=>o.dataset.name?.toLowerCase()===name.toLowerCase());if(local){sel.value=local.value;sel.dispatchEvent(new Event('change'))}}comToggleQuickCategory(false);if(input)input.value='';nrAdminToast('Đã thêm danh mục '+name);try{const created=await Promise.race([api('/commerce/categories',{method:'POST',body:JSON.stringify({name,fields_schema:[]})}),new Promise((_,rej)=>setTimeout(()=>rej(new Error('timeout')),5000))]);if(created?.ok)await loadCommerceCategories()}catch(e){/* local category remains usable and is persisted with product */}});
function comVariantFields(){const id=Number(document.getElementById('commerceProductCategory')?.value||0),c=COM_ADMIN_CATEGORIES.find(x=>Number(x.id)===id);return comSchemaParse(c).filter(f=>f.variant&&(f.options||[]).length)}
function comVariantCombos(fs){let out=[{}];for(const f of fs){const next=[];for(const base of out)for(const o of f.options||[])next.push({...base,[f.key]:o});out=next.slice(0,200)}return out}
function comGenerateVariantMatrix(){const box=document.getElementById('commerceVariantMatrix');if(!box)return;const fs=comVariantFields(),rows=comVariantCombos(fs);if(!rows.length){box.innerHTML='<p class="muted">Đánh dấu trường là “Biến thể” và khai báo các lựa chọn trong schema để tạo ma trận.</p>';return}box.innerHTML='<h3>Ma trận biến thể ('+rows.length+')</h3><p class="muted">Mỗi biến thể có SKU, giá, tồn kho và ảnh riêng. Có thể sửa từng dòng trước khi lưu.</p><div class="commerce-variant-table">'+rows.map((r,i)=>{const name=fs.map(f=>r[f.key]).join(' / ');return `<div class="commerce-variant-row" data-variant-row><b>${nrEsc(name)}</b><input data-v="name" type="hidden" value="${nrEsc(name)}"><input data-v="options" type="hidden" value='${nrEsc(JSON.stringify(r))}'><input data-v="sku" placeholder="SKU" value="VAR-${String(i+1).padStart(3,'0')}"><input data-v="price" inputmode="numeric" placeholder="Giá biến thể"><input data-v="stock_qty" type="number" min="0" value="10" placeholder="Tồn"><input data-v="image_url" placeholder="URL ảnh biến thể"></div>`}).join('')+'</div>'}
function comSimpleOptionRow(name='',values=''){const row=document.createElement('div');row.className='commerce-simple-option-row';row.innerHTML=`<input data-opt-name placeholder="Tên tùy chọn, VD: Màu sắc" value="${nrEsc(name)}"><input data-opt-values placeholder="Giá trị: Đen, Trắng, Xanh" value="${nrEsc(values)}"><button type="button" class="smallbtn danger">×</button>`;row.querySelector('button').onclick=()=>row.remove();return row}
function comAddSimpleOption(name='',values=''){const box=document.getElementById('commerceSimpleOptionRows');if(box)box.appendChild(comSimpleOptionRow(name,values))}
document.getElementById('commerceAddSimpleOption')?.addEventListener('click',()=>comAddSimpleOption());
function comSimpleOptionGroups(){return [...document.querySelectorAll('#commerceSimpleOptionRows .commerce-simple-option-row')].map(r=>({name:r.querySelector('[data-opt-name]').value.trim(),values:r.querySelector('[data-opt-values]').value.split(',').map(x=>x.trim()).filter(Boolean)})).filter(x=>x.name&&x.values.length)}
function comGenerateSimpleVariants(){const groups=comSimpleOptionGroups();if(!groups.length)return false;let combos=[{}];for(const g of groups){const next=[];for(const c of combos)for(const v of g.values)next.push({...c,[g.name]:v});combos=next.slice(0,100)}const box=document.getElementById('commerceVariantMatrix');if(!box)return true;box.innerHTML='<h3>Các lựa chọn bán ('+combos.length+')</h3>'+combos.map((opts,i)=>{const label=Object.values(opts).join(' / ');return `<div class="commerce-variant-row" data-variant-row><b>${nrEsc(label)}</b><input type="hidden" data-v="name" value="${nrEsc(label)}"><input type="hidden" data-v="options" value='${nrEsc(JSON.stringify(opts))}'><label>Giá <input data-v="price" type="number" min="0" placeholder="Theo giá sản phẩm"></label><label>Tồn kho <input data-v="stock_qty" type="number" min="0" value="0"></label><input type="hidden" data-v="sku" value=""><input type="hidden" data-v="image_url" value=""></div>`}).join('');return true}
function comCollectVariants(){return [...document.querySelectorAll('[data-variant-row]')].map(r=>{let options={};try{options=JSON.parse(r.querySelector('[data-v=options]').value||'{}')}catch{};return {name:r.querySelector('[data-v=name]').value,options,sku:r.querySelector('[data-v=sku]').value.trim(),price:Number(String(r.querySelector('[data-v=price]').value||0).replace(/[^0-9]/g,'')),stock_qty:Math.max(0,Number(r.querySelector('[data-v=stock_qty]').value||0)),image_url:r.querySelector('[data-v=image_url]').value.trim()}})}
document.getElementById('commerceGenerateVariants')?.addEventListener('click',()=>{if(!comGenerateSimpleVariants())comGenerateVariantMatrix()});
document.getElementById('commerceCategoryForm')?.addEventListener('submit',async e=>{e.preventDefault();const f=e.target,b={name:f.elements.name.value,description:f.elements.description.value,fields_schema:comCollectSchema(),is_active:true};if(f.dataset.id)b.id=Number(f.dataset.id);await api('/commerce/categories',{method:f.dataset.id?'PUT':'POST',body:JSON.stringify(b)});delete f.dataset.id;f.reset();document.getElementById('commerceSchemaRows').innerHTML='';await loadCommerceCategories();nrAdminToast('Đã lưu danh mục và schema')});
async function loadCommerceProducts(){const box=document.getElementById('commerceProductsPanel');if(!box)return;comPaintLocalCategories();box.innerHTML='<div class="muted">Đang tải sản phẩm...</div>';loadCommerceCategories();try{const d=await Promise.race([api('/commerce/products'),new Promise((_,rej)=>setTimeout(()=>rej(new Error('Không tải được sản phẩm. Bấm Tải lại để thử lại.')),7000))]);box.innerHTML=(d.products||[]).map(p=>`<div class="commerce-admin-card commerce-product-row">${p.image_url?`<img src="${nrEsc(p.image_url)}" alt="">`:'<div></div>'}<div class="commerce-product-row-main"><b>${nrEsc(p.name)}</b><small>${nrEsc(p.category_name||'Chưa phân loại')} · ${comAdminMoney(p.price)} · Còn ${Number(p.stock_qty||0)}</small><span class="${p.is_active?'is-live':'is-hidden'}">${p.is_active?'● Đang bán':'○ Đang ẩn'}</span></div><div><button class="smallbtn" onclick="commerceToggleProduct(${p.id},${p.is_active?0:1})">${p.is_active?'Ẩn':'Hiện'}</button> <button class="smallbtn danger" onclick="commerceDeleteProduct(${p.id})">Xóa</button></div></div>`).join('')||'<div class="empty">Chưa có sản phẩm. Hãy đăng sản phẩm đầu tiên ở phía trên.</div>'}catch(e){box.innerHTML='<div class="form-errors"><b>Không tải được danh sách sản phẩm.</b><br><button type="button" class="smallbtn" onclick="loadCommerceProducts()">Tải lại</button></div>'}}
async function commerceToggleProduct(id,on){const d=await api('/commerce/products');const p=(d.products||[]).find(x=>Number(x.id)===Number(id));if(!p)return;await api('/commerce/products',{method:'PUT',body:JSON.stringify({...p,id,is_active:!!on,attributes:JSON.parse(p.attributes_json||'{}'),variants:JSON.parse(p.variants_json||'[]')})});loadCommerceProducts()}
async function commerceDeleteProduct(id){if(!confirm('Xóa sản phẩm này?'))return;await api('/commerce/products?id='+id,{method:'DELETE'});loadCommerceProducts()}

// V20.9.27.50 — simple multi-image product gallery for small shops.
const commerceGalleryFiles=document.getElementById('commerceGalleryFiles');
let commerceGalleryImages=[];
function commerceRenderGallery(){
 const box=document.getElementById('commerceGalleryPreview'),cover=document.getElementById('commerceImageUrl'),galleryInput=document.getElementById('commerceGallery');
 if(!box||!cover||!galleryInput)return;
 box.innerHTML=commerceGalleryImages.map((url,i)=>`<div class="upload-thumb"><img src="${nrEsc(url)}" alt=""><div><button type="button" class="smallbtn" data-com-cover="${i}">${i===0?'Ảnh đại diện':'Đặt đại diện'}</button><button type="button" class="smallbtn danger" data-com-remove="${i}">Xóa</button></div></div>`).join('');
 cover.value=commerceGalleryImages[0]||cover.value||''; galleryInput.value=commerceGalleryImages.slice(1).join(', ');
 box.querySelectorAll('[data-com-cover]').forEach(b=>b.onclick=()=>{const x=commerceGalleryImages.splice(+b.dataset.comCover,1)[0];commerceGalleryImages.unshift(x);commerceRenderGallery()});
 box.querySelectorAll('[data-com-remove]').forEach(b=>b.onclick=()=>{commerceGalleryImages.splice(+b.dataset.comRemove,1);commerceRenderGallery()});
}
commerceGalleryFiles?.addEventListener('change',async()=>{
 const status=document.getElementById('commerceGalleryStatus'),files=[...commerceGalleryFiles.files];if(!files.length)return;status.textContent='Đang tải '+files.length+' ảnh...';
...
function commerceSkuPreview(name=''){const base=String(name||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/đ/gi,'d').toUpperCase().replace(/[^A-Z0-9]+/g,'-').replace(/^-|-$/g,'').slice(0,18);return base?('SP-'+base):''}
const commerceProductName=document.querySelector('#commerceProductForm [name=name]'),commerceProductSku=document.getElementById('commerceProductSku');
commerceProductName?.addEventListener('input',()=>{if(commerceProductSku)commerceProductSku.value=commerceSkuPreview(commerceProductName.value)});
document.getElementById('commerceProductForm')?.addEventListener('submit',async e=>{e.preventDefault();const f=e.target,btn=f.querySelector('button[type=submit]'),b=Object.fromEntries(new FormData(f).entries()),attrs={};if(!String(b.name||'').trim())return nrAdminToast('Vui lòng nhập tên sản phẩm');if(!String(b.category_id||'')||String(b.category_id)==='0')return nrAdminToast('Vui lòng chọn danh mục');const selectedCategoryName=comSelectedCategoryName();if(String(b.category_id).startsWith('name:'))b.category_id=0;b.category_name=selectedCategoryName;if(!Number(String(b.price||'').replace(/[^0-9]/g,'')))return nrAdminToast('Vui lòng nhập giá bán');b.short_description=String(b.description||'').replace(/<[^>]+>/g,' ').replace(/\s+/g,' ').trim().slice(0,180);if(btn){btn.disabled=true;btn.dataset.oldText=btn.textContent;btn.textContent='Đang đăng sản phẩm...'};document.querySelectorAll('#commerceDynamicFields [data-attr]').forEach(x=>attrs[x.dataset.attr]=x.type==='checkbox'?x.checked:x.value);b.attributes=attrs;b.is_featured=!!f.querySelector('[name=is_featured]')?.checked;b.variants=comCollectVariants();if(commerceGalleryImages.length){b.image_url=commerceGalleryImages[0];b.gallery=commerceGalleryImages.slice(1)}else b.gallery=String(b.gallery||'').split(',').map(x=>x.trim()).filter(Boolean);b.stock_qty=b.variants.length?b.variants.reduce((a,v)=>a+Number(v.stock_qty||0),0):Number(b.stock_qty||0);try{const saved=await api('/commerce/products',{method:'POST',body:JSON.stringify(b)}),p=saved.product||{};f.reset();commerceGalleryImages=[];commerceRenderGallery();await loadCommerceCategories();comRenderDynamicFields();await loadCommerceProducts();const ok=document.getElementById('commerceSuccess');if(ok){ok.classList.remove('hidden');ok.innerHTML=`<div><strong>✓ Đã thêm sản phẩm thành công</strong><span>${nrEsc(p.name||b.name)} đã được đăng vào <b>${nrEsc(p.category_name||selectedCategoryName||'cửa hàng')}</b>.</span></div><div class="commerce-success-actions"><button type="button" id="commerceAddAnother">＋ Thêm sản phẩm khác</button><button type="button" id="commerceViewList">Xem sản phẩm vừa đăng ↓</button></div>`;ok.scrollIntoView({behavior:'smooth',block:'center'});document.getElementById('commerceAddAnother').onclick=()=>{ok.classList.add('hidden');f.querySelector('[name=name]')?.focus()};document.getElementById('commerceViewList').onclick=()=>document.getElementById('commerceProductsPanel')?.scrollIntoView({behavior:'smooth',block:'start'})}nrAdminToast('Đã thêm sản phẩm thành công')}catch(err){catchCommerce(err)}finally{if(btn){btn.disabled=false;btn.textContent=btn.dataset.oldText||'＋ Thêm sản phẩm'}}});
...
document.getElementById('commerceSettingsForm')?.addEventListener('submit',async e=>{e.preventDefault();const f=e.target,b={};for(const k of ['cod_enabled','bank_enabled','store_pickup_enabled','online_enabled'])b[k]=f.elements[k].checked;for(const k of ['bank_name','bank_account','bank_holder','bank_qr_url','shipping_flat_fee','free_shipping_from'])b[k]=f.elements[k].value;try{await api('/commerce/settings',{method:'PUT',body:JSON.stringify(b)});document.getElementById('commerceSettingsMsg').textContent='✓ Đã lưu cấu hình'}catch(err){document.getElementById('commerceSettingsMsg').textContent=err.message}});
