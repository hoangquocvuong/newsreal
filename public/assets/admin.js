...
async function loadCommerceCategories(){const d=await api('/commerce/categories');COM_ADMIN_CATEGORIES=d.categories||[];const sel=document.getElementById('commerceProductCategory');if(sel){const v=sel.value;sel.innerHTML='<option value="">Chọn danh mục *</option>'+COM_ADMIN_CATEGORIES.map(c=>`<option value="${c.id}">${nrEsc(c.name)}</option>`).join('');if(v&&COM_ADMIN_CATEGORIES.some(c=>String(c.id)===String(v)))sel.value=v;else if(COM_ADMIN_CATEGORIES.length===1)sel.value=String(COM_ADMIN_CATEGORIES[0].id);sel.onchange=comRenderDynamicFields}const box=document.getElementById('commerceCategoriesPanel');if(box)box.innerHTML=COM_ADMIN_CATEGORIES.map(c=>`<div class="commerce-admin-card"><b>${nrEsc(c.name)}</b> · ${comSchemaParse(c).length} trường <button class="smallbtn" onclick="commerceEditCategory(${c.id})">Sửa schema</button></div>`).join('');comRenderDynamicFields()}
