...
if(trialParam){
 document.addEventListener('DOMContentLoaded',()=>{
  const bar=document.createElement('div');bar.className='admin-trial-bar';bar.innerHTML='<div class="admin-trial-info"><span class="admin-trial-badge">DÙNG THỬ MIỄN PHÍ</span><div class="admin-trial-time"><small>Thời gian còn lại</small><strong id="adminTrialClock"><span>--</span><i>:</i><span>--</span><i>:</i><span>--</span></strong></div></div><div class="admin-trial-actions"><a id="adminTrialViewSite" href="#">← Xem website</a><button type="button" id="adminTrialBuy">Đăng ký giao diện này</button></div>';document.body.prepend(bar);
  ...
  const buy=()=>location.href='/?template='+encodeURIComponent(new URLSearchParams(location.search).get('template')||'')+'&trial_token='+encodeURIComponent(trialParam)+'#dang-ky';
