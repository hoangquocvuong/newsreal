...
document.getElementById('commerceQuickAddCategory')?.addEventListener('click',async()=>{const name=prompt('Tên danh mục mới, ví dụ: Đồ mẹ & bé');if(!String(name||'').trim())return;try{await api('/commerce/categories',{method:'POST',body:JSON.stringify({name:String(name).trim(),fields_schema:[]})});await loadCommerceCategories();const sel=document.getElementById('commerceProductCategory'),c=COM_ADMIN_CATEGORIES.find(x=>x.name===String(name).trim());if(sel&&c){sel.value=String(c.id);sel.dispatchEvent(new Event('change'))}nrAdminToast('Đã tạo danh mục '+String(name).trim())}catch(e){nrAdminToast(e.message||'Không tạo được danh mục')}});
