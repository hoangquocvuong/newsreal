...
async function loadCommerceProducts(){const box=document.getElementById('commerceProductsPanel');if(!box)return;box.innerHTML='<div class="muted">Đang tải sản phẩm...</div>';try{const d=await api('/commerce/products');box.innerHTML=(d.products||[]).map(p=>`<div class="commerce-admin-card"><div class="row"><div><b>${nrEsc(p.name)}</b><br><small>${nrEsc(p.sku||'Không SKU')} · ${comAdminMoney(p.price)} · Tồn ${Number(p.stock_qty||0)}</small></div><div><button class="smallbtn" onclick="commerceToggleProduct(${p.id},${p.is_active?0:1})">${p.is_active?'Ẩn':'Hiện'}</button> <button class="smallbtn danger" onclick="commerceDeleteProduct(${p.id})">Xóa</button></div></div></div>`).join('')||'<div class="empty">Chưa có sản phẩm.</div>'}catch(e){box.innerHTML='<div class="form-errors">'+nrEsc(e.message)+'</div>'}}
...
document.getElementById('commerceProductForm')?.addEventListener('submit',async e=>{e.preventDefault();const b=Object.fromEntries(new FormData(e.target).entries());try{b.attributes=JSON.parse(b.attributes||'{}')}catch{alert('Thuộc tính JSON chưa đúng');return}b.stock_qty=Number(b.stock_qty||0);try{await api('/commerce/products',{method:'POST',body:JSON.stringify(b)});e.target.reset();loadCommerceProducts();nrAdminToast('Đã thêm sản phẩm')}catch(err){catchCommerce(err)}});
function catchCommerce(e){alert(e?.message||'Có lỗi khi xử lý dữ liệu bán hàng')}
