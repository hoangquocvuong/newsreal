...
document.getElementById('commerceProductForm')?.addEventListener('submit',async e=>{e.preventDefault();const f=e.target,b=Object.fromEntries(new FormData(f).entries()),attrs={};document.querySelectorAll('#commerceDynamicFields [data-attr]').forEach(x=>attrs[x.dataset.attr]=x.type==='checkbox'?x.checked:x.value);b.attributes=attrs;b.variants=comCollectVariants();b.gallery=String(b.gallery||'').split(',').map(x=>x.trim()).filter(Boolean);b.stock_qty=b.variants.length?b.variants.reduce((a,v)=>a+Number(v.stock_qty||0),0):Number(b.stock_qty||0);try{await api('/commerce/products',{method:'POST',body:JSON.stringify(b)});f.reset();comRenderDynamicFields();loadCommerceProducts();nrAdminToast('Đã thêm sản phẩm')}catch(err){catchCommerce(err)}});
