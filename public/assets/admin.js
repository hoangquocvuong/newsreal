...
async function loadCommerceCategories(){const sel=document.getElementById('commerceProductCategory'),box=document.getElementById('commerceCategoriesPanel'),help=document.getElementById('commerceCategoryHelp'),prev=sel?.value||'0';if(sel)sel.innerHTML='<option value="0">Đang tải danh mục...</option>';try{const d=await Promise.race([api('/commerce/categories'),new Promise((_,rej)=>setTimeout(()=>rej(new Error('Danh mục tải quá lâu. Vui lòng thử lại.')),20000))]);COM_ADMIN_CATEGORIES=(d.categories||[]).filter(c=>Number(c.is_active)!==0);if(sel){sel.innerHTML='<option value="0">Chọn danh mục...</option>'+COM_ADMIN_CATEGORIES.map(c=>`<option value="${c.id}">${nrEsc(c.name)}</option>`).join('');if(prev&&[...sel.options].some(o=>o.value===String(prev)))sel.value=prev;sel.onchange=()=>{comRenderDynamicFields();const c=COM_ADMIN_CATEGORIES.find(x=>String(x.id)===sel.value);if(help)help.textContent=c?'Sản phẩm sẽ hiển thị trong “'+c.name+'”.':'Chọn nhóm để khách dễ tìm sản phẩm.'}}if(box)box.innerHTML=COM_ADMIN_CATEGORIES.map(c=>`<div class="commerce-admin-card"><b>${nrEsc(c.name)}</b> <button class="smallbtn" onclick="commerceEditCategory(${c.id})">Tùy chỉnh</button></div>`).join('')||'<p class="muted">Chưa có danh mục.</p>';if(!COM_ADMIN_CATEGORIES.length)throw new Error('Danh mục chưa được khởi tạo')}catch(e){COM_ADMIN_CATEGORIES=[];if(sel)sel.innerHTML='<option value="0">Không tải được danh mục — bấm tải lại</option>';if(help){help.innerHTML='Không tải được danh mục. <button type="button" class="smallbtn" id="commerceRetryCategories">Tải lại</button>';document.getElementById('commerceRetryCategories')?.addEventListener('click',loadCommerceCategories)}if(box)box.innerHTML='<p class="muted">Không tải được danh mục. Hãy bấm Tải lại ở form sản phẩm.</p>'}comRenderDynamicFields()}
...
document.getElementById('commerceQuickAddCategory')?.addEventListener('click',async()=>{const name=prompt('Tên danh mục mới, ví dụ: Đồ mẹ & bé');if(!String(name||'').trim())return;try{const created=await api('/commerce/categories',{method:'POST',body:JSON.stringify({name:String(name).trim(),fields_schema:[]})});if(!created?.ok||!created?.category?.id)throw new Error(created?.error||'Máy chủ chưa xác nhận danh mục');await loadCommerceCategories();const sel=document.getElementById('commerceProductCategory'),c=COM_ADMIN_CATEGORIES.find(x=>Number(x.id)===Number(created.category.id))||COM_ADMIN_CATEGORIES.find(x=>x.name===String(name).trim());if(!c)throw new Error('Đã lưu nhưng chưa đọc lại được danh mục. Bấm Tải lại.');if(sel){sel.value=String(c.id);sel.dispatchEvent(new Event('change'))}nrAdminToast('Đã tạo danh mục '+c.name)}catch(e){nrAdminToast(e.message||'Không tạo được danh mục')}});
