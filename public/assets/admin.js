...
async function loadCommerceCategories(){const sel=document.getElementById('commerceProductCategory'),box=document.getElementById('commerceCategoriesPanel'),help=document.getElementById('commerceCategoryHelp'),prev=sel?.value||'0';let remote=[];try{const d=await Promise.race([api('/commerce/categories'),new Promise((_,rej)=>setTimeout(()=>rej(new Error('timeout')),7000))]);remote=(d.categories||[]).filter(c=>Number(c.is_active)!==0)}catch(e){}
 const byName=new Map();for(const name of [...COMMERCE_FALLBACK_CATEGORY_NAMES,...comLocalCategoryNames()])byName.set(name.toLowerCase(),{id:0,name,is_active:1,local:true});for(const c of remote)byName.set(String(c.name||'').toLowerCase(),c);COM_ADMIN_CATEGORIES=[...byName.values()];
 if(sel){sel.innerHTML='<option value="0">Chọn danh mục...</option>'+COM_ADMIN_CATEGORIES.map(c=>`<option value="${nrEsc(comCategoryValue(c))}" data-name="${nrEsc(c.name)}">${nrEsc(c.name)}</option>`).join('');if(prev&&[...sel.options].some(o=>o.value===String(prev)))sel.value=prev;sel.onchange=()=>{comRenderDynamicFields();const name=comSelectedCategoryName();if(help)help.textContent=name&&sel.value!=='0'?'Sản phẩm sẽ hiển thị trong “'+name+'”.':'Chọn nhóm để khách dễ tìm sản phẩm.'}}
 if(box)box.innerHTML=COM_ADMIN_CATEGORIES.map(c=>`<div class="commerce-admin-card"><b>${nrEsc(c.name)}</b>${Number(c.id||0)>0?` <button class="smallbtn" onclick="commerceEditCategory(${c.id})">Tùy chỉnh</button>`:''}</div>`).join('');comRenderDynamicFields()}
