...
async function loadStats(){if(isCommerceTemplate()){const d=await api('/commerce/stats');const cards=document.querySelectorAll('#tab-stats .kpi span');stat7.textContent=d.total||0;stat30.textContent=d.active||0;statAll.textContent=d.orders||0;if(cards[0])cards[0].textContent='Sản phẩm của bạn';if(cards[1])cards[1].textContent='Đang bán';if(cards[2])cards[2].textContent='Đơn hàng';const sub=document.querySelector('#tab-stats .admin-subtext');if(sub)sub.textContent='Theo dõi sản phẩm, tồn kho, đơn hàng và hoạt động bán hàng.';topPosts.innerHTML=`<div class="commerce-stats-extra"><b>Hết hàng: ${d.out_of_stock||0}</b><b>Doanh thu: ${Number(d.revenue||0).toLocaleString('vi-VN')}đ</b></div><h3>Sản phẩm mới đăng</h3><table class="table">${(d.recent||[]).map(x=>`<tr><td>${nrEsc(x.name)}</td><td>${nrEsc(x.category_name||'Chưa phân loại')}</td><td>${Number(x.price||0).toLocaleString('vi-VN')}đ</td><td>${Number(x.stock_qty||0)} tồn</td></tr>`).join('')||'<tr><td>Chưa có sản phẩm của bạn.</td></tr>'}</table>`;return}const d=await api('/stats');stat7.textContent=d.last7;stat30.textContent=d.last30;statAll.textContent=d.all;topPosts.innerHTML=`<table class="table">${d.top.map(x=>`<tr><td>${x.title}</td><td>${x.views||0}</td></tr>`).join('')}</table>`}
...
document.getElementById('commerceQuickAddCategory')?.addEventListener('click',async()=>{const name=String(prompt('Tên danh mục mới, ví dụ: Đồ chơi')||'').trim();if(!name)return;comSaveLocalCategory(name);let created=null;try{created=await api('/commerce/categories',{method:'POST',body:JSON.stringify({name,fields_schema:[]})})}catch(e){}await loadCommerceCategories();const sel=document.getElementById('commerceProductCategory'),c=COM_ADMIN_CATEGORIES.find(x=>String(x.name).toLowerCase()===name.toLowerCase());if(sel&&c){sel.value=comCategoryValue(c);sel.dispatchEvent(new Event('change'))}nrAdminToast(created?.ok?'Đã tạo danh mục '+name:'Đã thêm '+name+' vào danh mục. Danh mục sẽ được lưu cùng sản phẩm khi đăng.')});
...
document.getElementById('commerceGenerateVariants')?.addEventListener('click',comGenerateVariantMatrix);
...
let COM_ADMIN_PRODUCTS=[];
async function commerceRenameCategory(id){const c=COM_ADMIN_CATEGORIES.find(x=>Number(x.id)===Number(id));if(!c)return;const name=prompt('Tên danh mục mới',c.name);if(!name||name.trim()===c.name)return;await api('/commerce/categories',{method:'PUT',body:JSON.stringify({id:c.id,name:name.trim(),description:c.description||'',fields_schema:comSchemaParse(c),sort_order:c.sort_order||0})});await loadCommerceCategories();nrAdminToast('Đã đổi tên danh mục')}
async function commerceRemoveCategory(id){const c=COM_ADMIN_CATEGORIES.find(x=>Number(x.id)===Number(id));if(!c)return;const used=COM_ADMIN_PRODUCTS.filter(p=>Number(p.category_id)===Number(id));if(used.length){nrAdminToast('Danh mục đang có '+used.length+' sản phẩm. Hãy chuyển sản phẩm sang danh mục khác trước khi xóa.');return}if(!confirm('Xóa danh mục “'+c.name+'”?'))return;await api('/commerce/categories?id='+encodeURIComponent(id),{method:'DELETE'});await loadCommerceCategories();nrAdminToast('Đã xóa danh mục')}
window.commerceRenameCategory=commerceRenameCategory;window.commerceRemoveCategory=commerceRemoveCategory;
function commerceEditProduct(id){const p=COM_ADMIN_PRODUCTS.find(x=>Number(x.id)===Number(id));if(!p)return;const f=document.getElementById('commerceProductForm'),card=f?.closest('.commerce-quick-card');if(card)card.style.display='';f.dataset.editId=String(id);f.elements.name.value=p.name||'';f.elements.price.value=p.price||'';f.elements.compare_at_price.value=p.compare_at_price||'';f.elements.stock_qty.value=p.stock_qty||0;f.elements.description.value=p.description||p.short_description||'';f.elements.badge.value=p.badge||'';const sel=document.getElementById('commerceProductCategory');if(sel)sel.value=String(p.category_id||0);try{commerceGalleryImages=[p.image_url,...JSON.parse(p.gallery_json||'[]')].filter(Boolean)}catch{commerceGalleryImages=[p.image_url].filter(Boolean)}commerceRenderGallery();const btn=f.querySelector('.commerce-publish-btn');if(btn)btn.textContent='Lưu thay đổi';f.scrollIntoView({behavior:'smooth',block:'start'})}
window.commerceEditProduct=commerceEditProduct;
async function loadCommerceProducts(){const box=document.getElementById('commerceProductsPanel');if(!box)return;comPaintLocalCategories();box.innerHTML='<div class="muted">Đang tải sản phẩm...</div>';loadCommerceCategories();try{const d=await Promise.race([api('/commerce/products'),new Promise((_,rej)=>setTimeout(()=>rej(new Error('Không tải được sản phẩm. Bấm Tải lại để thử lại.')),7000))]);COM_ADMIN_PRODUCTS=d.products||[];box.innerHTML=COM_ADMIN_PRODUCTS.map(p=>`<div class="commerce-admin-card commerce-product-row">${p.image_url?`<img src="${nrEsc(p.image_url)}" alt="">`:'<div></div>'}<div class="commerce-product-row-main"><b>${nrEsc(p.name)}</b><small>${nrEsc(p.category_name||'Chưa phân loại')} · ${comAdminMoney(p.price)} · Còn ${Number(p.stock_qty||0)}</small><span class="${p.is_active?'is-live':'is-hidden'}">${p.is_active?'● Đang bán':'○ Đang ẩn'}</span></div><div><button class="smallbtn" onclick="commerceEditProduct(${p.id})">Sửa</button> <button class="smallbtn" onclick="commerceToggleProduct(${p.id},${p.is_active?0:1})">${p.is_active?'Ẩn':'Hiện'}</button> <button class="smallbtn danger" onclick="commerceDeleteProduct(${p.id})">Xóa</button></div></div>`).join('')||'<div class="empty">Chưa có sản phẩm. Hãy đăng sản phẩm đầu tiên ở phía trên.</div>'}catch(e){box.innerHTML='<div class="form-errors"><b>Không tải được danh sách sản phẩm.</b><br><button type="button" class="smallbtn" onclick="loadCommerceProducts()">Tải lại</button></div>'}}
...
function commerceRenderGallery(){
 ...
 box.innerHTML=commerceGalleryImages.map((url,i)=>`<div class="upload-thumb"><img src="${nrEsc(url)}" alt=""><div><button type="button" class="smallbtn" data-com-cover="${i}">${i===0?'⭐ Ảnh đại diện':'☆ Đặt làm ảnh đại diện'}</button><button type="button" class="smallbtn danger" data-com-remove="${i}">Xóa</button></div></div>`).join('');
...
window.commerceV37Publish=async function(e){e?.preventDefault?.();e?.stopPropagation?.();e.preventDefault();const f=document.getElementById('commerceProductForm'),btn=f?.querySelector('.commerce-publish-btn'),b=Object.fromEntries(new FormData(f).entries()),attrs={};if(!String(b.name||'').trim())return nrAdminToast('Vui lòng nhập tên sản phẩm');if(!String(b.category_id||'')||String(b.category_id)==='0')return nrAdminToast('Vui lòng chọn danh mục');const selectedCategoryName=comSelectedCategoryName();if(String(b.category_id).startsWith('name:'))b.category_id=0;b.category_name=selectedCategoryName;if(!Number(String(b.price||'').replace(/[^0-9]/g,'')))return nrAdminToast('Vui lòng nhập giá bán');b.short_description=String(b.description||'').replace(/<[^>]+>/g,' ').replace(/\s+/g,' ').trim().slice(0,180);if(btn){btn.disabled=true;btn.dataset.oldText=btn.textContent;btn.textContent='Đang đăng sản phẩm...'};document.querySelectorAll('#commerceDynamicFields [data-attr]').forEach(x=>attrs[x.dataset.attr]=x.type==='checkbox'?x.checked:x.value);b.attributes=attrs;b.is_featured=!!f.querySelector('[name=is_featured]')?.checked;b.variants=comCollectVariants();if(commerceGalleryImages.length){b.image_url=commerceGalleryImages[0];b.gallery=commerceGalleryImages.slice(1)}else b.gallery=String(b.gallery||'').split(',').map(x=>x.trim()).filter(Boolean);b.stock_qty=b.variants.length?b.variants.reduce((a,v)=>a+Number(v.stock_qty||0),0):Number(b.stock_qty||0);try{const editId=Number(f.dataset.editId||0);if(editId)b.id=editId;const saved=await api('/commerce/products',{method:editId?'PUT':'POST',body:JSON.stringify(b)}),p=saved.product||b;delete f.dataset.editId;f.reset();commerceGalleryImages=[];commerceRenderGallery();await loadCommerceCategories();comRenderDynamicFields();await loadCommerceProducts();const ok=document.getElementById('commerceSuccess');if(ok){ok.classList.remove('hidden');ok.innerHTML=`<div><strong>✓ Đã thêm sản phẩm thành công</strong><span>${nrEsc(p.name||b.name)} đã được đăng vào <b>${nrEsc(p.category_name||selectedCategoryName||'cửa hàng')}</b>.</span></div><div class="commerce-success-actions"><button type="button" id="commerceAddAnother">＋ Thêm sản phẩm khác</button><button type="button" id="commerceViewList">Xem sản phẩm vừa đăng ↓</button></div>`;ok.scrollIntoView({behavior:'smooth',block:'center'});document.getElementById('commerceAddAnother').onclick=()=>{ok.classList.add('hidden');const card=f.closest('.commerce-quick-card');if(card)card.style.display='';f.querySelector('[name=name]')?.focus()};document.getElementById('commerceViewList').onclick=()=>document.getElementById('commerceProductsPanel')?.scrollIntoView({behavior:'smooth',block:'start'});const card=f.closest('.commerce-quick-card');if(card)card.style.display='none';document.getElementById('commerceProductsPanel')?.scrollIntoView({behavior:'smooth',block:'start'})}nrAdminToast('Đã thêm sản phẩm thành công')}catch(err){catchCommerce(err)}finally{if(btn){btn.disabled=false;btn.textContent=btn.dataset.oldText||'＋ Thêm sản phẩm'}};return false};
document.getElementById('commerceProductForm')?.addEventListener('submit',e=>{e.preventDefault();e.stopImmediatePropagation();return false});
