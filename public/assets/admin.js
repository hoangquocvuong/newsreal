...
function renderTemplateUsageGuide(){
 ...
 const sections=Array.isArray(g.sections)?g.sections:[],cats=Array.isArray(g.category_rules)?g.category_rules:[],checks=Array.isArray(g.publish_checklist)?g.publish_checklist:[],quick=Array.isArray(g.quick_start)?g.quick_start:[],fields=Array.isArray(g.field_guide)?g.field_guide:[],visual=Array.isArray(g.visual_map)?g.visual_map:[],examples=Array.isArray(g.category_examples)?g.category_examples:[];
 const map=sections.map((x,i)=>`<div class="template-guide-node"><div class="guide-order">${Number(x.order||i+1)}</div><div><h3>${escGuide(x.title)}</h3><p>${escGuide(x.instruction)}</p>${x.slots?`<p><b>Số vị trí mẫu:</b> ${x.slots}${x.columns?.desktop?` · PC ${x.columns.desktop} cột`:''}${x.columns?.tablet?` · Tablet ${x.columns.tablet} cột`:''}${x.columns?.mobile?` · Mobile ${x.columns.mobile} cột`:''}</p>`:''}</div><div class="template-guide-source"><b>Bạn cần làm gì?</b>${escGuide(x.source)}</div></div>${i<sections.length-1?'<div class="template-guide-arrow">↓</div>':''}`).join('');
 ...
 const quickHtml=quick.length?`<div class="guide-beginner"><div class="guide-beginner-title"><span>1</span><div><b>Bắt đầu từ đây</b><small>Làm lần lượt 5 bước này, không cần hiểu kỹ thuật.</small></div></div><ol>${quick.map(x=>`<li>${escGuide(x)}</li>`).join('')}</ol></div>`:'';
 const visualHtml=visual.length?`<div class="template-guide-card guide-wide"><h3>Nhìn card ngoài website → điền ô nào trong Admin?</h3><p class="guide-help">Bảng này là phần quan trọng nhất. Dòng nào xuất hiện ngoài card thì điền đúng ô tương ứng bên trái.</p><div class="guide-map-table"><div class="head">Ô trong Admin</div><div class="head">Hiện ngoài website</div><div class="head">Ví dụ điền</div>${visual.map(v=>`<div><b>${escGuide(v.admin)}</b></div><div>${escGuide(v.front)}</div><div><code>${escGuide(v.example)}</code></div>`).join('')}</div></div>`:'';
 const fieldHtml=fields.length?`<div class="template-guide-card guide-wide"><h3>Giải thích từng ô khi đăng bài</h3><div class="guide-field-list">${fields.map(f=>`<div class="guide-field-row"><div><b>${escGuide(f.label)}</b><small>${escGuide(f.key)}</small></div><p>${escGuide(f.where)}</p><code>${escGuide(f.example||'Điền theo nội dung thực tế')}</code></div>`).join('')}</div></div>`:'';
 const exampleHtml=examples.length?`<div class="guide-examples"><h2>Ví dụ đăng đúng từng chuyên mục</h2><p class="guide-help">Bạn có thể nhìn đúng mẫu này rồi thay bằng thông tin thật của mình.</p>${examples.map((ex,idx)=>{const entries=Object.entries(ex.fields||{});return `<details class="guide-example" ${idx===0?'open':''}><summary><span>${idx+1}</span><div><b>${escGuide(ex.category)}</b><small>${escGuide(ex.purpose)}</small></div></summary><div class="guide-example-body"><div class="guide-example-title"><small>TIÊU ĐỀ MẪU</small><code>${escGuide(ex.title)}</code></div>${entries.length?`<div class="guide-example-grid">${entries.map(([k,v])=>{const f=fields.find(x=>x.key===k);return `<div><small>${escGuide(f?.label||k)}</small><b>${escGuide(v)}</b></div>`}).join('')}</div>`:'<p>Loại bài này chỉ cần Tiêu đề, Ảnh đại diện và Nội dung bài viết; không cần điền thông số gói dịch vụ.</p>'}</div></details>`}).join('')}</div>`:'';
 host.innerHTML=`<div class="template-guide-head"><div><small>HƯỚNG DẪN CHO NGƯỜI MỚI</small><h2>${escGuide(g.template_name||CLIENT_TEMPLATE_KEY)}</h2><p>${escGuide(g.intro||'')}</p></div><span class="template-guide-badge">Theo đúng mẫu đang dùng</span></div>${quickHtml}<div class="template-guide-note"><b>Website trắng vẫn giữ layout 1:1</b><br>${escGuide(g.empty_mode_note||'')}</div>${visualHtml}${exampleHtml}${fieldHtml}<h2>Sơ đồ trang chủ & vị trí đăng bài: bài sẽ chạy vào đâu?</h2><div class="template-guide-map">${map}</div><div class="template-guide-grid"><div class="template-guide-card"><h3>Chọn đúng chuyên mục</h3>${catHtml}</div><div class="template-guide-card"><h3>Kiểm tra trước khi bấm Đăng</h3><ol class="template-guide-list">${checks.map(x=>`<li>${escGuide(x)}</li>`).join('')}</ol></div>${tx}</div>`;
}

function renderPostTemplateGuide(){
 const host=document.getElementById('postTemplateGuide');if(!host)return;
 const g=CLIENT_PROFILE?.usage_guide,cat=String(postCategory?.value||'');
 const examples=Array.isArray(g?.category_examples)?g.category_examples:[];
 const ex=examples.find(x=>String(x.category||'')===cat);
 if(!ex){host.classList.add('hidden');host.innerHTML='';return}
 const fields=Array.isArray(g?.field_guide)?g.field_guide:[];
 const rows=Object.entries(ex.fields||{}).map(([k,v])=>{const f=fields.find(x=>x.key===k);return `<div class="post-guide-row"><span>${escGuide(f?.label||k)}</span><b>${escGuide(v)}</b></div>`}).join('');
 host.innerHTML=`<div class="post-guide-head"><div><b>Ví dụ điền đúng cho: ${escGuide(ex.category)}</b><small>${escGuide(ex.purpose||'')}</small></div><button type="button" onclick="showTab('guide')">Xem hướng dẫn đầy đủ</button></div><div class="post-guide-title"><small>Tiêu đề ví dụ</small><code>${escGuide(ex.title||'')}</code></div>${rows?`<div class="post-guide-rows">${rows}</div>`:'<p>Loại bài này chỉ cần Tiêu đề, Ảnh đại diện và Nội dung bài viết.</p>'}`;
 host.classList.remove('hidden');
}
...
function setWebsiteSettingsEditing(editing){
 [setName,setPhone,setZalo,setEmail,setFacebook,setSeoTitle,setSeoDescription,setSeoOgImage,setSeoIndex].forEach(el=>{if(el)el.disabled=!editing});document.querySelectorAll('[data-template-setting]').forEach(el=>el.disabled=!editing);
...
async function boot(){try{
 ...
 configureAdminForTemplate();renderTemplateUsageGuide();renderPostTemplateGuide();
...
function updateSubmitLabel(){
  ...
  submitPostBtn.textContent=postStatus.value==='draft'?'Lưu bản nháp':(editingId.value?(isProductTemplate()?'Cập nhật sản phẩm':isNewsTemplate()||isServiceTemplate()?'Cập nhật bài viết':'Cập nhật tin'):(isProductTemplate()?'Đăng sản phẩm':isNewsTemplate()||isServiceTemplate()?'Đăng bài':'Đăng tin'));
}
...
postType.addEventListener('change',updateContentTypeUI);transaction.addEventListener('change',()=>fillCategoryOptions(postCategory.value));postCategory.addEventListener('change',()=>{if(isGameTemplate()||postType.value==='game')renderProfileFields({});renderPostTemplateGuide()});
...
postMsg.classList.remove('hidden');postForm.reset();setRichContent('');renderProfileFields();uploadedImages=[];renderImages();editingId.value='';editorTitle.textContent=isProductTemplate()?'Thêm sản phẩm':isGameTemplate()?'Đăng base Clash of Clans':isServiceTemplate()?'Thêm gói dịch vụ':isNewsTemplate()?'Đăng bài tin tức':'Đăng tin bất động sản';if(isProductTemplate())postType.value='product';if(isProductTemplate())postType.value='product';if(isNewsTemplate())postType.value='news';if(isServiceTemplate())postType.value='service';if(isGameTemplate())postType.value='game';updateContentTypeUI();clearFieldErrors();
}catch(err){
