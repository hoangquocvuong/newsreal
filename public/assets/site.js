...
function nrGameHeader(prefix,admin){const home=prefix||'/',bases=nrGameRememberedBrowseUrl(prefix);return `<header class="coc-top coc-nav-shell"><div class="coc-wrap coc-nav-wrap"><a class="coc-brand" href="${home}" aria-label="COC Base Portal home"><b>COC</b><span>BASE PORTAL</span></a><nav class="coc-desktop-nav" aria-label="Clash of Clans navigation"><a href="${home}">Home</a><a href="${bases}">Browse Bases</a><a href="${prefix}/bases?th=all">Town Hall</a><a href="${prefix}/bases?bh=all">Builder Hall</a><a href="${prefix}/bases?ch=all">Clan Capital</a><button class="coc-saved-nav" type="button" data-coc-open-saved="1">Saved</button></nav><div class="coc-nav-actions">${admin?`<a class="coc-admin" href="${esc(admin)}" target="_blank" rel="noopener"><span>+</span><b>Đăng base</b></a>`:''}<a class="coc-donate-top" href="https://buymeacoffee.com/cocbase" target="_blank" rel="noopener noreferrer nofollow"><span aria-hidden="true">☕</span><b>Donate</b></a><button class="coc-menu-toggle" type="button" data-coc-menu-toggle="1" aria-expanded="false" aria-controls="coc-mobile-menu" aria-label="Mở menu"><span></span><span></span><span></span></button></div></div><div id="coc-mobile-menu" class="coc-mobile-menu" data-coc-mobile-menu="1" aria-hidden="true"><div class="coc-wrap"><a href="${home}">Home</a><a href="${bases}">Browse Bases</a><a href="${prefix}/bases?th=all">Town Hall</a><a href="${prefix}/bases?bh=all">Builder Hall</a><a href="${prefix}/bases?ch=all">Clan Capital</a><button type="button" data-coc-open-saved="1">🔖 Saved Bases</button><a href="${nrGameHomeHash(prefix,'about')}">About</a><a href="${nrGameHomeHash(prefix,'privacy')}">Privacy</a></div></div></header><a class="coc-donate-float" href="https://buymeacoffee.com/cocbase" target="_blank" rel="noopener noreferrer nofollow" aria-label="Support COC Base Portal on Buy Me a Coffee" title="Support us on Buy Me a Coffee"><span>☕</span></a><div class="coc-saved-sheet" data-coc-saved-sheet="1" aria-hidden="true"><button class="coc-saved-overlay" type="button" data-coc-close-saved="1" aria-label="Close saved bases"></button><section class="coc-saved-panel" role="dialog" aria-modal="true" aria-labelledby="coc-saved-title"><header><div><small>SAVED BASES</small><h3 id="coc-saved-title">Base đã lưu</h3></div><button type="button" data-coc-close-saved="1" aria-label="Close">×</button></header><div class="coc-saved-list" data-coc-saved-list="1"></div></section></div><div class="coc-toast-region" data-coc-toast-region="1" aria-live="polite" aria-atomic="true"></div>`}
function nrGameFooter(prefix=''){return `<footer class="coc-footer coc-footer-full"><div class="coc-wrap coc-footer-grid"><div><a class="coc-brand coc-footer-brand" href="${prefix||'/'}"><b>COC</b><span>BASE PORTAL</span></a><p>Kho chia sẻ base Clash of Clans miễn phí cho cộng đồng: Town Hall, Builder Hall và Clan Capital.</p></div><div><b>BASE LAYOUTS</b><a href="${prefix}/bases?th=all">Town Hall Bases</a><a href="${prefix}/bases?bh=all">Builder Hall Bases</a><a href="${prefix}/bases?ch=all">Clan Capital Bases</a></div><div><b>CỘNG ĐỒNG</b><button type="button" class="coc-footer-saved" data-coc-open-saved="1">Saved Bases</button><a href="https://buymeacoffee.com/cocbase" target="_blank" rel="noopener noreferrer nofollow">Donate</a></div><div><b>THÔNG TIN</b><a href="${nrGameHomeHash(prefix,'about')}">About</a><a href="${nrGameHomeHash(prefix,'privacy')}">Privacy</a></div></div><div class="coc-footer-bottom coc-wrap"><span>Community Clash of Clans base sharing</span><span>Not affiliated with Supercell.</span></div></footer>`}
function nrGameHomeExtras(prefix,posts){return `<section class="coc-info-section"><div class="coc-wrap coc-info-grid coc-info-grid-simple"><article id="about"><small>ABOUT</small><h3>About COC Base Portal</h3><p>Thư viện base cộng đồng dành cho Town Hall, Builder Hall và Clan Capital. Mục tiêu là tìm base nhanh, xem rõ và mở Copy Link thuận tiện trên cả máy tính lẫn điện thoại.</p></article><article id="privacy"><small>PRIVACY</small><h3>Simple, private browsing</h3><p>Không cần đăng nhập để duyệt hoặc lưu base trên thiết bị. Saved mặc định lưu local-first trong trình duyệt và không phụ thuộc Firebase.</p></article></div></section>`}
function nrGameSeedStats(post){const key=String(post.slug||post.id||post.title||'coc-base');let h=2166136261;for(let i=0;i<key.length;i++){h^=key.charCodeAt(i);h=Math.imul(h,16777619)}h=h>>>0;return{rating:Number(post.rating||((44+(h%6))/10)).toFixed(1),views:Number(post.views||650+(h%5400)),downloads:Number(post.downloads||120+(h%1450))}}
...
function nrGameDetail(post,prefix,posts=[]){const e=nrGameExtra(post),level=nrGameLevel(post),copy=e.copy_link||'#',related=nrGameRelatedPosts(post,posts),slug=nrGameBaseSlug(post),stats=nrGameSeedStats(post),href=post.demo_url||post.url||location.pathname;return `<div class="coc-site">${nrGameHeader(prefix,'')}<main class="coc-detail coc-wrap"><div class="coc-breadcrumb"><a href="${prefix||'/'}">Home</a> / ${esc(level)} / ${esc(post.title||'Base')}</div><div class="coc-detail-grid"><article data-coc-base-card="1" data-coc-slug="${esc(slug)}" data-coc-title="${esc(post.title||level+' Base Layout')}" data-coc-image="${esc(post.image||'')}" data-coc-url="${esc(href)}"><img class="coc-detail-image" src="${esc(post.image||GAME_CLASH_LEVEL_ART[level]||GAME_CLASH_LEVEL_ART.TH18)}" alt="${esc(post.title||level+' Base Layout')}"><div class="coc-detail-copy"><div class="coc-tags"><span>${esc(level)}</span><span>${esc(e.game_purpose||'Base')}</span><span>${esc(e.game_style||'Original')}</span><span>${esc(e.game_defense||'Balanced Defense')}</span></div><h1>${esc(post.title||level+' Base Layout')}</h1><div class="coc-stats big"><span>⭐ ${stats.rating}</span><span>👁 ${Number(stats.views).toLocaleString()}</span><span>⬇ ${Number(stats.downloads).toLocaleString()}</span><button type="button" class="coc-bookmark-btn coc-bookmark-detail" data-coc-bookmark="${esc(slug)}" aria-label="Save base" aria-pressed="false">🔖 <b>Save</b></button></div><div class="coc-article-body">${post.content||''}</div></div></article><aside class="coc-detail-side"><div class="coc-access-card"><small>BASE LINK</small><h3>${esc(level)} · ${esc(e.game_purpose||'Base')}</h3><a href="${esc(copy)}" target="_blank" rel="noopener">Copy Base Link</a></div></aside></div>${related.length?`<section class="coc-related"><div class="coc-heading"><div><span>RELATED BASES</span><h2>Bài viết liên quan</h2></div></div><div class="coc-related-grid">${related.map(nrGameBaseCard).join('')}</div></section>`:''}</main><div class="coc-mobile-copy-dock"><a href="${esc(copy)}" target="_blank" rel="noopener">Copy Base</a></div>${nrGameFooter(prefix)}</div>`}
