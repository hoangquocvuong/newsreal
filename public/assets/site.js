...
function comProductCards(items=[]){return (items||[]).map(p=>`<article class="com-card"><a class="com-card-img" href="${comProductUrl(p)}"><img src="${esc(p.image_url||'')}" alt="${esc(p.name)}" loading="lazy"></a><div class="com-card-body"><small class="com-badge">${esc(p.badge||p.category_name||'Sản phẩm')}</small><h3>${esc(p.name)}</h3><p class="com-desc">${esc(p.short_description||'')}</p><div class="com-price"><strong>${comMoney(p.price)}</strong>${Number(p.compare_at_price)>Number(p.price)?`<del>${comMoney(p.compare_at_price)}</del>`:''}</div><div class="com-actions"><button class="com-detail" onclick="location.href='${comProductUrl(p)}'">Chi tiết</button><button class="com-add" onclick="comAdd(${Number(p.id)},false)">+ Giỏ hàng</button></div></div></article>`).join('')}
let COM_PAGE=1;const COM_PAGE_SIZE=12;
function comPagination(total){const pages=Math.max(1,Math.ceil(total/COM_PAGE_SIZE)),box=document.getElementById('comPagination');if(!box)return;if(COM_PAGE>pages)COM_PAGE=pages;box.innerHTML=pages<=1?'':Array.from({length:pages},(_,i)=>`<button class="${i+1===COM_PAGE?'active':''}" onclick="COM_PAGE=${i+1};comFilter();document.getElementById('products')?.scrollIntoView({behavior:'smooth'})">${i+1}</button>`).join('')}
