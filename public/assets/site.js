...
async function renderCommerceStore(site={}){
 ...
 function detail(id){const x=get(id),box=document.getElementById('v24Detail');if(!x){box.classList.remove('open');return}const related=products.filter(p=>p.cat===x.cat&&String(p.id)!==String(x.id)).slice(0,4),gallery=(x.images&&x.images.length?x.images:[x.img]),attrs=Object.entries(x.attributes||{}).filter(([,v])=>String(v??'').trim()),variants=(x.variants||[]).filter(v=>v&&v.is_active!==false);const specRows=[['SKU',x.sku||'Đang cập nhật'],['Danh mục',x.cat],...attrs].map(([k,v])=>`<tr><th>${esc(k)}</th><td>${esc(Array.isArray(v)?v.join(', '):String(v))}</td></tr>`).join('');const variantHtml=variants.length?`<div class="v25-variants"><h4>Chọn phân loại</h4><div class="v25-variant-list">${variants.map((v,i)=>`<button class="v25-variant ${i?'':'active'}" data-v25-variant="${i}" data-price="${Number(v.price||x.price)}" data-stock="${Number(v.stock_qty??v.stock??x.stock)}">${esc(v.name||v.label||v.sku||('Phân loại '+(i+1)))}</button>`).join('')}</div></div>`:'';document.getElementById('v24DetailBody').innerHTML=`<div class="v24-wrap v24-detailgrid"><div class="v25-gallery"><div class="v25-thumbs">${gallery.map((im,i)=>`<button class="v25-thumb ${i?'':'active'}" data-v25-thumb="${i}"><img src="${esc(im)}" alt="${esc(x.name)} - ảnh ${i+1}"></button>`).join('')}</div><div class="v25-main"><img id="v25MainImage" src="${esc(gallery[0])}" alt="${esc(x.name)}"></div></div><div><small>${esc(x.cat)}</small><h1>${esc(x.name)}</h1><div class="v24-detailprice"><b id="v25DetailPrice">${money(x.price)}</b>${x.old>x.price?`<del>${money(x.old)}</del>`:''}</div><div class="v25-meta"><span>SKU: ${esc(x.sku||'Đang cập nhật')}</span>${x.badge?`<span>${esc(x.badge)}</span>`:''}<span id="v25Stock">${x.stock>0?'✓ Còn '+x.stock+' sản phẩm':'Tạm hết hàng'}</span></div><p class="v24-desc">${esc(x.shortDesc||x.desc)}</p>${variantHtml}<div class="v24-buy"><button class="buy" data-detail-add="${esc(x.id)}">Thêm vào giỏ hàng</button><button data-detail-buy="${esc(x.id)}">Mua ngay</button></div><button class="v24-tool" data-wish="${esc(x.id)}">${wish.includes(String(x.id))?'♥ Đã yêu thích':'♡ Lưu yêu thích'}</button><div class="v25-tabs"><div class="v25-tabnav"><button class="active" data-v25-tab="desc">Mô tả chi tiết</button><button data-v25-tab="spec">Thông số & thuộc tính</button></div><div class="v25-tabpanel" id="v25TabPanel">${x.desc||'<p>Thông tin chi tiết đang được cập nhật.</p>'}</div></div></div></div><div class="v24-wrap v24-related"><div class="v25-related-head"><div><small>CÓ THỂ BẠN SẼ THÍCH</small><h2>Sản phẩm liên quan</h2></div><p>Cùng danh mục ${esc(x.cat)}</p></div><div class="v24-grid">${related.map(card).join('')||'<p>Chưa có sản phẩm liên quan.</p>'}</div></div>`;box.classList.add('open');document.documentElement.classList.add('commerce-detail-open');document.body.classList.add('commerce-detail-open');document.querySelectorAll('[data-v25-thumb]').forEach(b=>b.onclick=()=>{document.querySelectorAll('[data-v25-thumb]').forEach(n=>n.classList.remove('active'));b.classList.add('active');document.getElementById('v25MainImage').src=gallery[+b.dataset.v25Thumb]});document.querySelectorAll('[data-v25-variant]').forEach(b=>b.onclick=()=>{document.querySelectorAll('[data-v25-variant]').forEach(n=>n.classList.remove('active'));b.classList.add('active');document.getElementById('v25DetailPrice').textContent=money(b.dataset.price);document.getElementById('v25Stock').textContent=Number(b.dataset.stock)>0?'✓ Còn '+b.dataset.stock+' sản phẩm':'Tạm hết hàng'});document.querySelectorAll('[data-v25-tab]').forEach(b=>b.onclick=()=>{document.querySelectorAll('[data-v25-tab]').forEach(n=>n.classList.remove('active'));b.classList.add('active');document.getElementById('v25TabPanel').innerHTML=b.dataset.v25Tab==='spec'?`<table class="v25-specs">${specRows||'<tr><td>Chưa có thông số.</td></tr>'}</table>`:(x.desc||'<p>Thông tin chi tiết đang được cập nhật.</p>')});document.querySelector('[data-detail-add]').onclick=()=>{add(x.id);openCart()};document.querySelector('[data-detail-buy]').onclick=()=>{add(x.id);openCart();checkout()};bindDynamic()}
