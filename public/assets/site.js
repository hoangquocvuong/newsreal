...
function nrGameHeader(prefix,admin){const home=prefix||'/',bases=`${prefix}/bases?th=all`;return `<header class="coc-top coc-nav-shell"><div class="coc-wrap coc-nav-wrap"><a class="coc-brand" href="${home}" aria-label="COC Base Portal home"><b>COC</b><span>BASE PORTAL</span></a><nav class="coc-desktop-nav" aria-label="Clash of Clans navigation"><a href="${home}">Home</a><a href="${bases}">Browse Bases</a><a href="${prefix}/bases?th=all">Town Hall</a><a href="${prefix}/bases?bh=all">Builder Hall</a><a href="${prefix}/bases?ch=all">Clan Capital</a><a href="${nrGameHomeHash(prefix,'news')}">News</a><a href="${nrGameHomeHash(prefix,'saved')}">Saved</a></nav><div class="coc-nav-actions">${admin?`<a class="coc-admin" href="${esc(admin)}" target="_blank" rel="noopener">+ Đăng base</a>`:''}<a class="coc-donate-top" href="https://buymeacoffee.com/cocbase" target="_blank" rel="noopener noreferrer nofollow"><span aria-hidden="true">☕</span> Donate</a></div></div></header><nav class="coc-mobile-nav" aria-label="Mobile navigation"><a href="${home}"><span>⌂</span><b>Home</b></a><a href="${bases}"><span>▦</span><b>Bases</b></a><a href="${nrGameHomeHash(prefix,'saved')}"><span>★</span><b>Saved</b></a><details class="coc-mobile-more"><summary><span>☰</span><b>More</b></summary><div class="coc-mobile-menu-panel"><a href="${prefix}/bases?th=all">Town Hall</a><a href="${prefix}/bases?bh=all">Builder Hall</a><a href="${prefix}/bases?ch=all">Clan Capital</a><a href="${nrGameHomeHash(prefix,'news')}">News</a><a href="${nrGameHomeHash(prefix,'find-source')}">Find Source</a><a href="${nrGameHomeHash(prefix,'events')}">Events</a><a href="${nrGameHomeHash(prefix,'rankings')}">Rankings</a><a href="${nrGameHomeHash(prefix,'about')}">About</a></div></details><a class="donate" href="https://buymeacoffee.com/cocbase" target="_blank" rel="noopener noreferrer nofollow"><span>☕</span><b>Donate</b></a></nav><a class="coc-donate-float" href="https://buymeacoffee.com/cocbase" target="_blank" rel="noopener noreferrer nofollow" aria-label="Support COC Base Portal on Buy Me a Coffee"><span>☕</span><strong>Support us</strong><small>Keep base links free</small></a>`}
function nrGameFooter(prefix=''){return `<footer class="coc-footer coc-footer-full"><div class="coc-wrap coc-footer-grid"><div><a class="coc-brand coc-footer-brand" href="${prefix||'/'}"><b>COC</b><span>BASE PORTAL</span></a><p>Kho chia sẻ base Clash of Clans miễn phí cho cộng đồng: Town Hall, Builder Hall và Clan Capital.</p></div><div><b>BASE LAYOUTS</b><a href="${prefix}/bases?th=all">Town Hall Bases</a><a href="${prefix}/bases?bh=all">Builder Hall Bases</a><a href="${prefix}/bases?ch=all">Clan Capital Bases</a></div><div><b>KHÁM PHÁ</b><a href="${nrGameHomeHash(prefix,'news')}">News</a><a href="${nrGameHomeHash(prefix,'rankings')}">Rankings</a><a href="${nrGameHomeHash(prefix,'hero-skins')}">Hero Skins</a><a href="${nrGameHomeHash(prefix,'events')}">Events</a></div><div><b>HỖ TRỢ</b><a href="${nrGameHomeHash(prefix,'find-source')}">Find Source</a><a href="${nrGameHomeHash(prefix,'saved')}">Saved Bases</a><a href="${nrGameHomeHash(prefix,'donate-center')}">Donate</a><a href="${nrGameHomeHash(prefix,'privacy')}">Privacy</a></div></div><div class="coc-footer-bottom coc-wrap"><span>Community Clash of Clans base sharing</span><span>Not affiliated with Supercell.</span></div></footer>`}
function nrGameHomeExtras(prefix,posts){const recent=posts.slice(0,4);return `<section id="news" class="coc-community-section"><div class="coc-wrap"><div class="coc-heading"><div><span>NEWS & UPDATES</span><h2>Latest base updates</h2></div></div><div class="coc-community-grid">${recent.map(p=>`<a class="coc-community-card" href="${p.demo_url||p.url||'#'}"><img src="${esc(p.image||'')}" alt="${esc(p.title||'Clash of Clans base')}" loading="lazy"><b>${esc(p.title||'Clash of Clans Base')}</b></a>`).join('')}</div></div></section><section class="coc-tools-section"><div class="coc-wrap coc-tool-grid"><article id="find-source"><small>FIND SOURCE</small><h3>Find the original base source</h3><p>Use the base image, title and Copy Link to trace the shared layout quickly.</p><a href="${prefix}/bases?th=all">Browse bases →</a></article><article id="saved"><small>SAVED</small><h3>Keep useful layouts together</h3><p>Saved-base support remains part of the portal navigation for returning players.</p><a href="${prefix}/bases?th=all">Choose a base →</a></article><article id="donate-center"><small>SUPPORT</small><h3>Support the community portal</h3><p>The site shares layouts for free. Support is optional and never unlocks paid bases.</p><a href="#about">About the project →</a></article></div></section><section id="events" class="coc-mini-section"><div class="coc-wrap coc-mini-grid"><article><small>EVENTS</small><h3>Clash events & seasonal layouts</h3><p>Quick access point for event-related layouts and seasonal collections.</p></article><article id="rankings"><small>RANKINGS</small><h3>Popular community bases</h3><p>Ranking area for most viewed, voted and copied layouts.</p></article><article id="hero-skins"><small>HERO SKINS</small><h3>Hero skins showcase</h3><p>A dedicated category retained from the original portal navigation.</p></article></div></section><section class="coc-info-section"><div class="coc-wrap coc-info-grid"><article id="about"><small>ABOUT</small><h3>About COC Base Portal</h3><p>A community-focused Clash of Clans layout library for TH, BH and Clan Capital. Every base is shared free with direct copy links when available.</p></article><article id="privacy"><small>PRIVACY</small><h3>Simple, transparent browsing</h3><p>Base browsing does not require payment. Privacy and analytics information can be managed from the site settings.</p></article><article id="android-app"><small>ANDROID APP</small><h3>Android access</h3><p>Mobile app entry retained as part of the original navigation structure.</p></article><article id="ios-app"><small>IOS APP</small><h3>iOS access</h3><p>iOS app entry retained as part of the original navigation structure.</p></article></div></section>`}
...
function nrGameDetail(post,prefix,posts=[]){const e=nrGameExtra(post),level=nrGameLevel(post),copy=e.copy_link||'#',related=nrGameRelatedPosts(post,posts);return `<div class="coc-site">${nrGameHeader(prefix,'')}<main class="coc-detail coc-wrap"><div class="coc-breadcrumb"><a href="${prefix||'/'}">Home</a> / ${esc(level)} / ${esc(post.title||'Base')}</div><div class="coc-detail-grid"><article><img class="coc-detail-image" src="${esc(post.image||GAME_CLASH_LEVEL_ART[level]||GAME_CLASH_LEVEL_ART.TH18)}" alt="${esc(post.title||level+' Base Layout')}"><div class="coc-detail-copy"><div class="coc-tags"><span>${esc(level)}</span><span>${esc(e.game_purpose||'Base')}</span><span>${esc(e.game_style||'Original')}</span><span>${esc(e.game_defense||'Balanced Defense')}</span></div><h1>${esc(post.title||level+' Base Layout')}</h1><div class="coc-stats big"><span>⭐ ${Number(post.rating||4.8).toFixed(1)}</span><span>👁 ${Number(post.views||0).toLocaleString()}</span><span>⬇ ${Number(post.downloads||e.downloads||0).toLocaleString()}</span></div><div class="coc-article-body">${post.content||''}</div></div></article><aside class="coc-detail-side"><div class="coc-access-card"><small>BASE LINK</small><h3>${esc(level)} · ${esc(e.game_purpose||'Base')}</h3><a href="${esc(copy)}" target="_blank" rel="noopener">Copy Base Link</a></div></aside></div>${related.length?`<section class="coc-related"><div class="coc-heading"><div><span>RELATED BASES</span><h2>Bài viết liên quan</h2></div></div><div class="coc-related-grid">${related.map(nrGameBaseCard).join('')}</div></section>`:''}</main>${nrGameFooter(prefix)}</div>`}
function renderGameClash1(site={}){const root=document.querySelector('main');if(!root)return;const posts=nrGamePosts(),isDemo=window.NR_DEMO_THEME==='game-1',prefix=isDemo?(window.NR_DEMO_PREFIX||'/demo/game/clash-of-clans'):'',rel=isDemo&&location.pathname.startsWith(prefix)?(location.pathname.slice(prefix.length)||'/'):location.pathname,admin=isDemo?nrDemoAdminUrl('game-1','newpost'):'/admin?tab=newpost';if(/^\/base\/[^/]+\.html$/i.test(rel)){const slug=rel.split('/').pop().replace(/\.html$/i,''),post=posts.find(p=>String(p.demo_url||p.url||'').includes('/'+slug+'.html'))||posts[0];if(post){root.innerHTML=nrGameDetail(post,prefix,posts);return}}if(/^\/(bases|free-bases|premium-bases)/i.test(rel)){if(/\/(free|premium)-bases/i.test(rel))history.replaceState({},'',`${prefix}/bases${location.search||'?th=all'}`);root.innerHTML=`<div class="coc-site">${nrGameHeader(prefix,admin)}${nrGameFilterPage(posts,prefix)}${nrGameFooter(prefix)}</div>`;nrGameBindFastFilters(posts,prefix);if(location.hash==='#base-results')nrGameScrollResults('auto');if(!window.__NR_COC_POPSTATE__){window.__NR_COC_POPSTATE__=1;window.addEventListener('popstate',()=>{const b=document.querySelector('[data-coc-browser="1"]');if(b){b.outerHTML=nrGameFilterPage(posts,prefix);nrGameBindFastFilters(posts,prefix)}})}return}root.innerHTML=`<div class="coc-site">${nrGameHeader(prefix,admin)}<section class="coc-hero"><div class="coc-wrap coc-hero-grid"><div class="coc-hero-copy"><span class="coc-kicker">CLASH OF CLANS · COMMUNITY BASE LAYOUTS</span><h1>Choose your Hall.<br><em>Find a base fast.</em></h1><p>Kho chia sẻ base Clash of Clans miễn phí cho cộng đồng. Chọn đúng cấp nhà, lọc theo mục đích và mở Copy Link chỉ trong vài giây.</p><div class="coc-hero-actions"><a href="${prefix}/bases?th=18#base-results">Browse TH18 Bases</a><a class="soft" href="#town-hall">Choose another Hall</a></div><div class="coc-hero-benefits"><span>✓ Free copy links</span><span>✓ No login required</span><span>✓ Instant filters</span></div><div class="coc-proof"><span><b>TH2–TH18</b><small>Town Hall</small></span><span><b>BH2–BH10</b><small>Builder Hall</small></span><span><b>CH1–CH10</b><small>Clan Capital</small></span></div></div><div class="coc-hero-art"><div class="coc-hero-art-badge">LATEST HALL</div><picture><source srcset="${GAME_CLASH_LEVEL_ART.TH18.replace('/s600/','/s1600/')} 1600w, ${GAME_CLASH_LEVEL_ART.TH18.replace('/s600/','/s1200/')} 1200w, ${GAME_CLASH_LEVEL_ART.TH18.replace('/s600/','/s800/')} 800w" sizes="(max-width:720px) 90vw, 42vw"><img src="${GAME_CLASH_LEVEL_ART.TH18.replace('/s600/','/s1200/')}" alt="Town Hall 18 Clash of Clans" fetchpriority="high" decoding="async"></picture><div class="coc-hero-art-meta"><div><b>Town Hall 18</b><span>Full image · High resolution</span></div><a href="${prefix}/bases?th=18#base-results">View TH18 bases →</a></div></div></div></section><section id="town-hall" class="coc-section" data-structure-key="town-hall"><div class="coc-wrap"><div class="coc-heading"><div><span>TOWN HALL</span><h2>Choose Town Hall</h2></div></div><div class="coc-level-grid" data-contract-grid="1">${nrGameLevelCards('th',prefix)}</div></div></section><section id="builder-hall" class="coc-section coc-dark" data-structure-key="builder-hall"><div class="coc-wrap"><div class="coc-heading light"><div><span>BUILDER BASE</span><h2>Choose Builder Hall</h2></div></div><div class="coc-level-grid" data-contract-grid="1">${nrGameLevelCards('bh',prefix)}</div></div></section><section id="clan-capital" class="coc-section" data-structure-key="clan-capital"><div class="coc-wrap"><div class="coc-heading"><div><span>CLAN CAPITAL</span><h2>Choose Capital Hall</h2></div></div><div class="coc-level-grid" data-contract-grid="1">${nrGameLevelCards('ch',prefix)}</div></div></section>${nrGameHomeExtras(prefix,posts)}${nrGameFooter(prefix)}</div>`;try{nrApplyStructureGeometry(site,'game-1');nrAuditStructureContract(site,'game-1')}catch(e){}}
