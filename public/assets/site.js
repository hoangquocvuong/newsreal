...
function nrGameFilterPage(posts,prefix){const state=nrGameFilterState(),{group,level,type,year,sort}=state,groupName=group==='th'?'Town Hall':group==='bh'?'Builder Hall':'Clan Capital',types=nrGameFilterTypes(group),years=['all','2026','2025'],sorts=[['latest','Latest'],['rating','Vote'],['download','Download'],['view','View']];nrGameRememberFilter(state);nrGameRememberRecentFilter(state);let arr=posts.filter(p=>{const e=nrGameExtra(p),g=String(e.game_group||p.category||'').toLowerCase(),lv=nrGameLevel(p).replace(/\D/g,''),pt=String(e.game_purpose||'').toLowerCase();return g.includes(group==='th'?'town hall':group==='bh'?'builder hall':'clan capital')&&(level==='all'||lv===level)&&(type==='all'||pt===type||pt.startsWith(type))&&(year==='all'||String(e.game_year||'2026')===year)});if(sort==='rating')arr.sort((a,b)=>Number(b.rating||0)-Number(a.rating||0));else if(sort==='download')arr.sort((a,b)=>Number(nrGameExtra(b).downloads||b.downloads||0)-Number(nrGameExtra(a).downloads||a.downloads||0));else if(sort==='view')arr.sort((a,b)=>Number(b.views||0)-Number(a.views||0));const link=(label,changes,active,step)=>`<a data-coc-filter="1" data-coc-filter-option="${step}" class="${active?'active':''}" href="${nrGameFilterUrl(prefix,state,changes)}">${label}</a>`;const controls=[['group','Group',groupName],['level','Level',nrGameFilterLabel(state,'level')],[group==='ch'?'district':'type',group==='ch'?'District':'Type',nrGameFilterLabel(state,'type')],['year','Year',nrGameFilterLabel(state,'year')],['sort','Sort',nrGameFilterLabel(state,'sort')]];const suggested=level!=='all'&&type==='all'?'type':type!=='all'&&year==='all'?'year':year!=='all'&&sort==='latest'?'sort':'';const groupOpts=['th','bh','ch'].map(g=>link(g==='th'?'Town Hall':g==='bh'?'Builder Hall':'Clan Capital',{group:g},g===group,'group')).join('');const levelOpts=['all',...GAME_LEVELS[group].map(String)].map(v=>link(v==='all'?'All':group.toUpperCase()+v,{level:v},v===level,'level')).join('');const typeOpts=types.map(([v,t])=>link(t,{type:v},v===type,'type')).join('');const yearOpts=years.map(v=>link(v==='all'?'All':v,{year:v},v===year,'year')).join('');const sortOpts=sorts.map(([v,t])=>link(t,{sort:v},v===sort,'sort')).join('');const reset=nrGameFilterUrl(prefix,{group,level:'all',type:'all',year:'all',sort:'latest'}),activeSummary=[level==='all'?groupName:group.toUpperCase()+level,type==='all'?'':(types.find(x=>x[0]===type)?.[1]||type),year==='all'?'':year,sort==='latest'?'':nrGameFilterLabel(state,'sort')].filter(Boolean);const recent=nrGameRecentFiltersRead().filter(x=>x&&['th','bh','ch'].includes(x.group)).slice(0,3),recentHtml=recent.length?`<div class="coc-filter-recent"><small>GẦN ĐÂY</small>${recent.map(x=>`<a data-coc-filter="1" class="coc-recent-chip" href="${nrGameFilterUrl(prefix,x)}">${esc((x.group||'th').toUpperCase()+(x.level&&x.level!=='all'?x.level:''))}${x.type&&x.type!=='all'?' · '+esc(nrGameFilterTypes(x.group).find(t=>t[0]===x.type)?.[1]||x.type):''}${x.year&&x.year!=='all'?' · '+esc(x.year):''}</a>`).join('')}</div>`:'';const recovery=!arr.length?`<div class="coc-filter-recovery"><b>Không có base phù hợp</b><span>Thử nới điều kiện để xem nhiều kết quả hơn.</span><div>${type!=='all'?`<a data-coc-filter="1" href="${nrGameFilterUrl(prefix,state,{type:'all'})}">Bỏ ${esc(nrGameFilterLabel(state,'type'))}</a>`:''}${year!=='all'?`<a data-coc-filter="1" href="${nrGameFilterUrl(prefix,state,{year:'all'})}">Bỏ ${esc(year)}</a>`:''}<a data-coc-filter="1" href="${nrGameFilterUrl(prefix,state,{level:'all',type:'all',year:'all'})}">Xem tất cả ${esc(groupName)}</a></div></div>`:'';return `<section class="coc-browser" data-coc-browser="1"><div class="coc-wrap"><div class="coc-browser-head"><div><small>COMMUNITY BASES</small><h1>${groupName} ${level==='all'?'':group.toUpperCase()+level} Base Layouts</h1><p>Chọn nhanh theo từng bước. Bạn có thể dừng ngay sau Level hoặc tiếp tục lọc sâu hơn.</p></div></div><div class="coc-smart-filter" data-coc-smart-filter="1"><div class="coc-filter-steps" role="toolbar" aria-label="Smart progressive base filters">${controls.map(([key,label,value])=>{const normalized=key==='district'?'type':key;return `<button type="button" class="coc-filter-step ${suggested===normalized?'is-suggested':''}" data-coc-filter-step="${normalized}" aria-expanded="false"><small>${label}</small><b>${esc(value)}</b><span>⌄</span></button>`}).join('')}</div><div class="coc-filter-popovers"><div class="coc-filter-popover" data-coc-filter-popover="group"><div class="coc-filter-popover-head"><b>1 · Chọn Group</b><span>TH / BH / CH</span></div><div class="coc-filter-options coc-filter-options-compact">${groupOpts}</div></div><div class="coc-filter-popover" data-coc-filter-popover="level"><div class="coc-filter-popover-head"><b>2 · Chọn Level</b><span>${groupName}</span></div><div class="coc-filter-options coc-filter-options-level">${levelOpts}</div></div><div class="coc-filter-popover" data-coc-filter-popover="type"><div class="coc-filter-popover-head"><b>3 · ${group==='ch'?'Chọn District':'Chọn Type'}</b><span>Tùy chọn nâng cao</span></div><div class="coc-filter-options">${typeOpts}</div></div><div class="coc-filter-popover" data-coc-filter-popover="year"><div class="coc-filter-popover-head"><b>4 · Chọn Year</b><span>Tùy chọn</span></div><div class="coc-filter-options coc-filter-options-compact">${yearOpts}</div></div><div class="coc-filter-popover" data-coc-filter-popover="sort"><div class="coc-filter-popover-head"><b>5 · Sắp xếp</b><span>Tùy chọn</span></div><div class="coc-filter-options coc-filter-options-compact">${sortOpts}</div></div></div><div class="coc-filter-feedback"><div><small>ĐANG LỌC</small><b>${activeSummary.join(' · ')}</b><span>${arr.length} base phù hợp</span></div><a data-coc-filter="1" href="${reset}">Reset</a></div>${recentHtml}</div><div class="coc-results-toolbar" id="base-results" data-coc-results="1"><div><small>RESULTS</small><strong>${arr.length} base phù hợp</strong><span>${activeSummary.join(' · ')}</span></div><a data-coc-filter="1" href="${reset}">Reset filters</a></div>${recovery}<div class="coc-grid coc-result-grid">${arr.length?arr.map(nrGameBaseCard).join(''):''}</div></div></section>`}
