...
const NR_FAST_NAV_VERSION='20.9.5';
...
function nrParseExtraObject(raw){
  if(raw&&typeof raw==='object'&&!Array.isArray(raw))return raw;
  try{const x=JSON.parse(String(raw||'{}'));return x&&typeof x==='object'&&!Array.isArray(x)?x:{}}catch(e){return {}}
}
function nrNormalizeContentPost(post={}){
  const raw=post&&typeof post==='object'?post:{};
  const extra=nrParseExtraObject(raw.extra_json);
  const merged={...extra,...raw};
  const images=[merged.image,...String(merged.gallery||'').split(',').map(v=>v.trim())].filter(Boolean);
  const displayPrice=String(merged.service_price||merged.product_price||merged.price||'').trim();
  const ctaLabel=String(merged.service_cta||merged.product_cta_label||merged.cta_label||'').trim();
  return {...merged,extra_json:raw.extra_json??extra,service_price:String(merged.service_price||displayPrice||''),service_cta:String(merged.service_cta||ctaLabel||''),_nr:{images,image:images[0]||'',excerpt:sxText(merged.content||'').slice(0,160),display_price:displayPrice,cta_label:ctaLabel,detail_label:String(merged.detail_label||'Xem chi tiết').trim(),is_sample:Number(merged.is_sample||0)===1||String(merged.sample_key||'')!==''}};
}
function nrNormalizeContentPosts(posts=[]){return Array.isArray(posts)?posts.map(nrNormalizeContentPost):[]}
function nrContentMeta(post={}){return post?._nr||nrNormalizeContentPost(post)._nr}
function getImages(x){
  const meta=x?._nr;
  if(meta&&Array.isArray(meta.images))return meta.images.filter(Boolean);
...
async function nrBootMain(){
 try{
   const demoTemplateKey=window.NR_DEMO_THEME&&(/^tin-tuc-[1-4]$/.test(window.NR_DEMO_THEME)||/^mau-[1-5]$/.test(window.NR_DEMO_THEME)||/^dich-vu-\d+$/.test(window.NR_DEMO_THEME)||/^blog-ca-nhan-[1-2]$/.test(window.NR_DEMO_THEME)||/^doanh-nghiep-[1-2]$/.test(window.NR_DEMO_THEME)||/^game-\d+$/.test(window.NR_DEMO_THEME)||/^san-pham-\d+$/.test(window.NR_DEMO_THEME))?window.NR_DEMO_THEME:'';
   ...
   SITE_DATA=d;
   // V20.9.27.26 — Universal Content Contract: all templates receive normalized post objects.
   if(Array.isArray(SITE_DATA?.posts))SITE_DATA.posts=nrNormalizeContentPosts(SITE_DATA.posts);
   ...
   const DEMO_PRESETS={
     ...
     'dich-vu-5':'universal_commerce_5',
     'dich-vu-6':'service_lion_dance_6',
     'blog-ca-nhan-1':'personal_blog_1',
     'blog-ca-nhan-2':'personal_blog_2',
     'doanh-nghiep-1':'corporate_modern_1',
     'doanh-nghiep-2':'corporate_industry_2',
     'game-1':'game_clash_1',
     'san-pham-1':'product_affiliate_1'
   };
   ...
   document.body.classList.toggle('theme-showcase-new',['personal_blog_1','personal_blog_2','corporate_modern_1','corporate_industry_2','universal_commerce_5','service_lion_dance_6'].includes(effectivePreset));
   document.body.classList.toggle('theme-game-clash',effectivePreset==='game_clash_1');
   document.body.classList.toggle('theme-product-affiliate',effectivePreset==='product_affiliate_1');
   fillPublicFooter(s);
   ...
   }else if(effectivePreset==='service_lion_dance_6'){
     try{renderLionDanceTemplate(s)}catch(e){console.error('LION SERVICE',e);showError('Không tải được giao diện Lân Sư Rồng.')}
   }else if(['personal_blog_1','personal_blog_2','corporate_modern_1','corporate_industry_2','universal_commerce_5'].includes(effectivePreset)){
     try{renderShowcaseTemplate(effectivePreset,s)}catch(e){console.error('SHOWCASE TEMPLATE',e);showError('Không tải được giao diện mẫu.')}
   }else if(effectivePreset==='product_affiliate_1'){
     try{renderProductAffiliate1(s)}catch(e){console.error('PRODUCT AFFILIATE',e);showError('Không tải được giao diện Sản phẩm / Affiliate.')}
   ...
   if(window.NR_DEMO_THEME==='game-1'){
     ...
     try{
       ...
       const sample=nrBuildProductShowroomPosts(),fallbackSite={name:'Product Store',template_key:'san-pham-1',preset:'product_affiliate_1',template_settings:{product_cta_label:'Mua Ngay'}};
...
function nrScheduleBoot(){
 if(window.__NR_MAIN_BOOT_SCHEDULED__)return;
 window.__NR_MAIN_BOOT_SCHEDULED__=1;
 setTimeout(()=>nrBootMain().catch(e=>{console.error('NR BOOT FATAL',e);try{nrTemplateBootReady()}catch(_){}}),0);
}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',nrScheduleBoot,{once:true});else nrScheduleBoot();
...
/* =========================================================
   V20.8.8 — GAME PORTAL MOBILE UX / CLASH OF CLANS CONTRACT
...
function nrGamePosts(){const live=(SITE_DATA?.posts||[]).filter(x=>['game','base'].includes(String(x.type||'').toLowerCase())||nrGameExtra(x).game_group);let local=[];try{if(typeof GAME_REAL_SAMPLE_POSTS!=='undefined'&&Array.isArray(GAME_REAL_SAMPLE_POSTS))local=GAME_REAL_SAMPLE_POSTS}catch(e){}/* V20.9.17 — Public showroom must always use the packaged real Atom samples. This prevents a stale/shared demo payload from replacing the 8 TH18 posts or producing 0-result Hall filters. Trial/client still consume only their own real posts. */if(window.NR_DEMO_THEME==='game-1'&&!window.NR_TRIAL_TOKEN&&!window.NR_CLIENT_SIM&&local.length)return local;if(window.NR_DEMO_THEME==='game-1')return live.length?live:local;return live}
...
function nrGameLevelCards(group,prefix=''){const levels=Array.isArray(GAME_LEVELS[group])?GAME_LEVELS[group]:[];return levels.slice().reverse().map(lv=>{const code=group.toUpperCase()+lv,href=`${prefix}/bases?${group}=${lv}#base-results`;return `<article class="coc-level-card"><a class="coc-level-art" href="${href}"><img src="${esc(nrGameArt(group,lv))}" alt="${code} Clash of Clans" loading="lazy" decoding="async"></a><h3><a href="${href}">${code}</a></h3></article>`}).join('')}
function nrGameHomeHash(prefix,id){const home=prefix||'/';return `${home}${id?'#'+id:''}`}
function nrGameLegacyMenu(prefix){const base=prefix||'';return `<details class="coc-more"><summary>More</summary><div class="coc-more-menu"><a href="${base}/bases?th=all">Town Hall</a><a href="${base}/bases?bh=all">Builder Hall</a><a href="${base}/bases?ch=all">Capital Hall</a><a href="${nrGameHomeHash(prefix,'events')}">Events</a><a href="${nrGameHomeHash(prefix,'rankings')}">Rankings</a><a href="${nrGameHomeHash(prefix,'hero-skins')}">Hero Skins</a><a href="${nrGameHomeHash(prefix,'about')}">About</a><a href="${nrGameHomeHash(prefix,'privacy')}">Privacy</a><a href="${nrGameHomeHash(prefix,'android-app')}">Android App</a><a href="${nrGameHomeHash(prefix,'ios-app')}">iOS App</a></div></details>`}
function nrGameRememberedBrowseUrl(prefix=''){try{const x=JSON.parse(localStorage.getItem('NR_COC_LAST_FILTER_V1')||'null');if(x&&['th','bh','ch'].includes(x.group)){return nrGameFilterUrl(prefix,{group:x.group,level:x.level||'all',type:x.type||'all',year:x.year||'all',sort:x.sort||'latest'})}}catch(e){}return `${prefix}/bases?th=all`}
function nrGameTemplateSettings(site={}){let raw=site?.template_settings;if(!raw){try{raw=JSON.parse(String(site?.template_settings_json||'{}'))}catch(e){raw={}}}raw=raw&&typeof raw==='object'?raw:{};return{donate_url:String(raw.donate_url||'https://buymeacoffee.com/cocbase'),about_title:String(raw.about_title||'About COC Base Portal'),about_content:String(raw.about_content||'Thư viện base cộng đồng dành cho Town Hall, Builder Hall và Clan Capital.'),terms_title:String(raw.terms_title||'Điều khoản sử dụng'),terms_content:String(raw.terms_content||'Base được chia sẻ cho cộng đồng. Người dùng tự chịu trách nhiệm khi sử dụng liên kết bên thứ ba.'),footer_text:String(raw.footer_text||'Community Clash of Clans base sharing · Not affiliated with Supercell.')}}
function nrGameHeader(prefix,admin,settings={}){const home=prefix||'/',bases=nrGameRememberedBrowseUrl(prefix),donate=esc(settings.donate_url||'https://buymeacoffee.com/cocbase');return `<header class="coc-top coc-nav-shell"><div class="coc-wrap coc-nav-wrap"><a class="coc-brand" href="${home}" aria-label="COC Base Portal home"><b>COC</b><span>BASE PORTAL</span></a><nav class="coc-desktop-nav" aria-label="Clash of Clans navigation"><a href="${home}">Home</a><a href="${bases}">Browse Bases</a><a href="${prefix}/bases?th=all">Town Hall</a><a href="${prefix}/bases?bh=all">Builder Hall</a><a href="${prefix}/bases?ch=all">Clan Capital</a><button class="coc-saved-nav" type="button" data-coc-open-saved="1">Saved</button></nav><div class="coc-nav-actions">${admin?`<a class="coc-admin" href="${esc(admin)}" target="_blank" rel="noopener"><span>+</span><b>Đăng base</b></a>`:''}<a class="coc-donate-top" href="${donate}" target="_blank" rel="noopener noreferrer nofollow"><span aria-hidden="true">☕</span><b>Donate</b></a><button class="coc-menu-toggle" type="button" data-coc-menu-toggle="1" aria-expanded="false" aria-controls="coc-mobile-menu" aria-label="Mở menu"><span></span><span></span><span></span></button></div></div><div id="coc-mobile-menu" class="coc-mobile-menu" data-coc-mobile-menu="1" aria-hidden="true"><div class="coc-wrap"><a href="${home}">Home</a><a href="${bases}">Browse Bases</a><a href="${prefix}/bases?th=all">Town Hall</a><a href="${prefix}/bases?bh=all">Builder Hall</a><a href="${prefix}/bases?ch=all">Clan Capital</a><button type="button" data-coc-open-saved="1">Saved Bases</button><a href="${prefix}/about">Thông tin</a><a href="${prefix}/terms">Điều khoản</a></div></div></header><a class="coc-donate-float" href="${donate}" target="_blank" rel="noopener noreferrer nofollow" aria-label="Support COC Base Portal" title="Donate"><span>☕</span></a><div class="coc-saved-sheet" data-coc-saved-sheet="1" aria-hidden="true"><button class="coc-saved-overlay" type="button" data-coc-close-saved="1" aria-label="Close saved bases"></button><section class="coc-saved-panel" role="dialog" aria-modal="true" aria-labelledby="coc-saved-title"><header><div><small>SAVED BASES</small><h3 id="coc-saved-title">Base đã lưu</h3></div><button type="button" data-coc-close-saved="1" aria-label="Close">×</button></header><div class="coc-saved-list" data-coc-saved-list="1"></div></section></div><div class="coc-toast-region" data-coc-toast-region="1" aria-live="polite" aria-atomic="true"></div>`}
function nrGameFooter(prefix='',settings={}){const donate=esc(settings.donate_url||'https://buymeacoffee.com/cocbase');return `<footer class="coc-footer coc-footer-full"><div class="coc-wrap coc-footer-grid"><div><a class="coc-brand coc-footer-brand" href="${prefix||'/'}"><b>COC</b><span>BASE PORTAL</span></a><p>${esc(settings.footer_text||'Community Clash of Clans base sharing')}</p></div><div><b>BASE LAYOUTS</b><a href="${prefix}/bases?th=all">Town Hall Bases</a><a href="${prefix}/bases?bh=all">Builder Hall Bases</a><a href="${prefix}/bases?ch=all">Clan Capital Bases</a></div><div><b>CỘNG ĐỒNG</b><button type="button" class="coc-footer-saved" data-coc-open-saved="1">Saved Bases</button><a href="${donate}" target="_blank" rel="noopener noreferrer nofollow">Donate</a></div><div><b>THÔNG TIN</b><a href="${prefix}/about">Thông tin</a><a href="${prefix}/terms">Điều khoản</a></div></div><div class="coc-footer-bottom coc-wrap"><span>${esc(settings.footer_text||'Community Clash of Clans base sharing')}</span></div></footer>`}
function nrGameInfoPage(kind,prefix,settings,admin=''){const terms=kind==='terms',title=terms?settings.terms_title:settings.about_title,content=terms?settings.terms_content:settings.about_content;return `<div class="coc-site">${nrGameHeader(prefix,admin,settings)}<main class="coc-info-page coc-wrap"><div class="coc-breadcrumb"><a href="${prefix||'/'}">Home</a> / ${terms?'Điều khoản':'Thông tin'}</div><article><small>${terms?'TERMS':'ABOUT'}</small><h1>${esc(title)}</h1><div class="coc-info-content">${esc(content).split('\n').join('<br>')}</div></article></main>${nrGameFooter(prefix,settings)}</div>`}
function nrGameHomeExtras(prefix,posts,settings={}){return `<section class="coc-info-section"><div class="coc-wrap coc-info-grid coc-info-grid-simple"><article id="about"><small>ABOUT</small><h3>${esc(settings.about_title||'About COC Base Portal')}</h3><p>${esc(settings.about_content||'Thư viện base cộng đồng dành cho Town Hall, Builder Hall và Clan Capital.').slice(0,260)}</p><a href="${prefix}/about">Xem thông tin →</a></article><article id="terms"><small>TERMS</small><h3>${esc(settings.terms_title||'Điều khoản sử dụng')}</h3><p>${esc(settings.terms_content||'Base được chia sẻ cho cộng đồng.').slice(0,260)}</p><a href="${prefix}/terms">Xem điều khoản →</a></article></div></section>`}
function nrGameSeedStats(post){if(window.NR_DEMO_THEME!=='game-1')return{rating:Number(post.rating||0).toFixed(1),views:Number(post.views||0),downloads:Number(post.downloads||0)};const key=String(post.slug||post.id||post.title||'coc-base');let h=2166136261;for(let i=0;i<key.length;i++){h^=key.charCodeAt(i);h=Math.imul(h,16777619)}h=h>>>0;return{rating:Number(post.rating||((44+(h%6))/10)).toFixed(1),views:Number(post.views||650+(h%5400)),downloads:Number(post.downloads||120+(h%1450))}}
function nrGameBaseSlug(post){const href=String(post.demo_url||post.url||'');const m=href.match(/\/base\/([^/?#]+?)(?:\.html)?(?:[?#]|$)/i);return String(post.slug||post.slugKey||(m&&m[1])||post.id||post.title||'base').toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'')}
function nrGameBaseCard(post){const e=nrGameExtra(post),level=nrGameLevel(post),href=post.demo_url||post.url||'#',slug=nrGameBaseSlug(post),stats=nrGameSeedStats(post);return `<article class="coc-card" data-coc-base-card="1" data-coc-slug="${esc(slug)}" data-coc-title="${esc(post.title||level+' Base Layout')}" data-coc-image="${esc(post.image||'')}" data-coc-url="${esc(href)}"><a class="coc-thumb" href="${esc(href)}"><img src="${esc(post.image||nrGameArt(level.slice(0,2).toLowerCase(),Number(level.replace(/\D/g,''))))}" alt="${esc(post.title||level+' Base')}" loading="lazy" decoding="async"><span class="coc-level">${esc(level)}</span></a><div class="coc-card-body"><h3><a href="${esc(href)}">${esc(post.title||level+' Base Layout')}</a></h3><div class="coc-tags"><span>${esc(e.game_purpose||'Base')}</span><span>${esc(e.game_style||'Original')}</span><span>${esc(e.game_defense||'Balanced Defense')}</span></div><div class="coc-card-stats" aria-label="Base stats"><button type="button" class="coc-vote-btn" data-coc-vote="${esc(slug)}" title="Vote">⭐ <b data-coc-rating>${stats.rating}</b></button><span title="Views">👁 <b data-coc-views>${Number(stats.views).toLocaleString()}</b></span><span title="Downloads">⬇ <b data-coc-downloads>${Number(stats.downloads).toLocaleString()}</b></span><button type="button" class="coc-bookmark-btn" data-coc-bookmark="${esc(slug)}" aria-label="Save base" aria-pressed="false">🔖</button></div></div></article>`}
function nrGameFilterState(){const params=new URLSearchParams(location.search),paramGroup=['th','bh','ch'].find(g=>params.has(g));let remembered=null;try{remembered=JSON.parse(localStorage.getItem('NR_COC_LAST_FILTER_V1')||'null')}catch(e){}const group=paramGroup||(['th','bh','ch'].includes(remembered?.group)?remembered.group:'th');const rawPage=Math.max(1,Number(params.get('page')||1)||1);return {group,level:paramGroup?(params.get(group)||'all'):(remembered?.level||'all'),type:params.get('type')||(paramGroup?'all':remembered?.type||'all'),year:params.get('year')||(paramGroup?'all':remembered?.year||'all'),sort:params.get('sort')||(paramGroup?'latest':remembered?.sort||'latest'),page:rawPage}}
function nrGameRememberFilter(state){try{localStorage.setItem('NR_COC_LAST_FILTER_V1',JSON.stringify({group:state.group,level:state.level||'all',type:state.type||'all',year:state.year||'all',sort:state.sort||'latest'}))}catch(e){}}
...
function nrGameFilterUrl(prefix,state,changes={}){const n={...state,...changes};if(changes.group&&changes.group!==state.group){n.level='all';n.type='all'}if(Object.keys(changes).some(k=>['group','level','type','year','sort'].includes(k))&&!Object.prototype.hasOwnProperty.call(changes,'page'))n.page=1;const q=new URLSearchParams();q.set(n.group,n.level||'all');if(n.type&&n.type!=='all')q.set('type',n.type);if(n.year&&n.year!=='all')q.set('year',n.year);if(n.sort&&n.sort!=='latest')q.set('sort',n.sort);if(Number(n.page||1)>1)q.set('page',String(Math.max(1,Number(n.page)||1)));return `${prefix}/bases?${q.toString()}`}
function nrGameFilterLabel(state,key){const groupName=state.group==='th'?'Town Hall':state.group==='bh'?'Builder Hall':'Clan Capital';if(key==='group')return groupName;if(key==='level')return state.level==='all'?'All':state.group.toUpperCase()+state.level;if(key==='type'){const hit=nrGameFilterTypes(state.group).find(x=>x[0]===state.type);return state.type==='all'?'All':(hit?.[1]||state.type)}if(key==='year')return state.year==='all'?'All':state.year;if(key==='sort')return state.sort==='rating'?'Vote':state.sort==='download'?'Download':state.sort==='view'?'View':'Latest';return 'All'}
function nrGameRecentFiltersRead(){try{const x=JSON.parse(localStorage.getItem('NR_COC_RECENT_FILTERS_V1')||'[]');return Array.isArray(x)?x:[]}catch(e){return[]}}
function nrGameRecentFiltersWrite(items){try{localStorage.setItem('NR_COC_RECENT_FILTERS_V1',JSON.stringify(items.slice(0,3)))}catch(e){}}
function nrGameRememberRecentFilter(state){if(!state||state.level==='all')return;const item={group:state.group,level:state.level||'all',type:state.type||'all',year:state.year||'all',sort:state.sort||'latest'},key=JSON.stringify(item),list=nrGameRecentFiltersRead().filter(x=>JSON.stringify(x)!==key);list.unshift(item);nrGameRecentFiltersWrite(list)}
function nrGameFilterPage(posts,prefix){const state=nrGameFilterState(),{group,level,type,year,sort}=state,groupName=group==='th'?'Town Hall':group==='bh'?'Builder Hall':'Clan Capital',types=nrGameFilterTypes(group),currentYear=String(new Date().getFullYear()),dataYears=posts.map(p=>String(nrGameExtra(p).game_year||'').trim()).filter(y=>/^\d{4}$/.test(y)),years=['all',...Array.from(new Set([currentYear,...dataYears])).sort((a,b)=>Number(b)-Number(a))],sorts=[['latest','Latest'],['rating','Vote'],['download','Download'],['view','View']],PAGE_SIZE=12;nrGameRememberFilter(state);nrGameRememberRecentFilter(state);let arr=posts.filter(p=>{const e=nrGameExtra(p),g=String(e.game_group||p.category||'').toLowerCase(),lv=nrGameLevel(p).replace(/\D/g,''),pt=String(e.game_purpose||'').toLowerCase();return g.includes(group==='th'?'town hall':group==='bh'?'builder hall':'clan capital')&&(level==='all'||lv===level)&&(type==='all'||pt===type||pt.startsWith(type))&&(year==='all'||String(e.game_year||currentYear)===year)});if(sort==='rating')arr.sort((a,b)=>Number(b.rating||0)-Number(a.rating||0));else if(sort==='download')arr.sort((a,b)=>Number(nrGameExtra(b).downloads||b.downloads||0)-Number(nrGameExtra(a).downloads||a.downloads||0));else if(sort==='view')arr.sort((a,b)=>Number(b.views||0)-Number(a.views||0));const total=arr.length,totalPages=Math.max(1,Math.ceil(total/PAGE_SIZE)),page=Math.min(Math.max(1,Number(state.page||1)),totalPages),pageItems=arr.slice((page-1)*PAGE_SIZE,page*PAGE_SIZE),pageState={...state,page};const link=(label,changes,active,step)=>`<a data-coc-filter="1" data-coc-filter-option="${step}" class="${active?'active':''}" href="${nrGameFilterUrl(prefix,pageState,changes)}">${label}</a>`;const controls=[['group','Group',groupName],['level','Level',nrGameFilterLabel(pageState,'level')],[group==='ch'?'district':'type',group==='ch'?'District':'Type',nrGameFilterLabel(pageState,'type')],['year','Year',nrGameFilterLabel(pageState,'year')],['sort','Sort',nrGameFilterLabel(pageState,'sort')]];const suggested=level!=='all'&&type==='all'?'type':type!=='all'&&year==='all'?'year':year!=='all'&&sort==='latest'?'sort':'';const groupOpts=['th','bh','ch'].map(g=>link(g==='th'?'Town Hall':g==='bh'?'Builder Hall':'Clan Capital',{group:g},g===group,'group')).join('');const levelOpts=['all',...GAME_LEVELS[group].map(String)].map(v=>link(v==='all'?'All':group.toUpperCase()+v,{level:v},v===level,'level')).join('');const typeOpts=types.map(([v,t])=>link(t,{type:v},v===type,'type')).join('');const yearOpts=years.map(v=>link(v==='all'?'All':v,{year:v},v===year,'year')).join('');const sortOpts=sorts.map(([v,t])=>link(t,{sort:v},v===sort,'sort')).join('');const reset=nrGameFilterUrl(prefix,{group,level:'all',type:'all',year:'all',sort:'latest',page:1}),activeSummary=[level==='all'?groupName:group.toUpperCase()+level,type==='all'?'':(types.find(x=>x[0]===type)?.[1]||type),year==='all'?'':year,sort==='latest'?'':nrGameFilterLabel(pageState,'sort')].filter(Boolean);const recent=nrGameRecentFiltersRead().filter(x=>x&&['th','bh','ch'].includes(x.group)).slice(0,3),recentHtml=recent.length?`<div class="coc-filter-recent"><small>GẦN ĐÂY</small>${recent.map(x=>`<a data-coc-filter="1" class="coc-recent-chip" href="${nrGameFilterUrl(prefix,{...x,page:1})}">${esc((x.group||'th').toUpperCase()+(x.level&&x.level!=='all'?x.level:''))}${x.type&&x.type!=='all'?' · '+esc(nrGameFilterTypes(x.group).find(t=>t[0]===x.type)?.[1]||x.type):''}${x.year&&x.year!=='all'?' · '+esc(x.year):''}</a>`).join('')}</div>`:'';const recovery=!total?`<div class="coc-filter-recovery"><b>Không có base phù hợp</b><span>Thử nới điều kiện để xem nhiều kết quả hơn.</span><div>${type!=='all'?`<a data-coc-filter="1" href="${nrGameFilterUrl(prefix,pageState,{type:'all'})}">Bỏ ${esc(nrGameFilterLabel(pageState,'type'))}</a>`:''}${year!=='all'?`<a data-coc-filter="1" href="${nrGameFilterUrl(prefix,pageState,{year:'all'})}">Bỏ ${esc(year)}</a>`:''}<a data-coc-filter="1" href="${nrGameFilterUrl(prefix,pageState,{level:'all',type:'all',year:'all'})}">Xem tất cả ${esc(groupName)}</a></div></div>`:'';const pageNums=[];for(let i=Math.max(1,page-2);i<=Math.min(totalPages,page+2);i++)pageNums.push(i);if(pageNums[0]>1)pageNums.unshift(1);if(pageNums[pageNums.length-1]<totalPages)pageNums.push(totalPages);const pagination=totalPages>1?`<nav class="coc-pagination" aria-label="Phân trang kết quả"><a data-coc-filter="1" class="coc-page-prev ${page<=1?'is-disabled':''}" href="${page<=1?'#':nrGameFilterUrl(prefix,pageState,{page:page-1})}" ${page<=1?'aria-disabled="true"':''}>‹</a>${pageNums.map((n,i)=>`${i&&pageNums[i-1]!==n-1?'<span class="coc-page-gap">…</span>':''}<a data-coc-filter="1" class="${n===page?'active':''}" href="${nrGameFilterUrl(prefix,pageState,{page:n})}">${n}</a>`).join('')}<a data-coc-filter="1" class="coc-page-next ${page>=totalPages?'is-disabled':''}" href="${page>=totalPages?'#':nrGameFilterUrl(prefix,pageState,{page:page+1})}" ${page>=totalPages?'aria-disabled="true"':''}>›</a><small>Trang ${page}/${totalPages}</small></nav>`:'';return `<section class="coc-browser" data-coc-browser="1"><div class="coc-wrap"><div class="coc-browser-head"><div><small>COMMUNITY BASES</small><h1>${groupName} ${level==='all'?'':group.toUpperCase()+level} Base Layouts</h1><p>Chọn nhanh theo từng bước. Bạn có thể dừng ngay sau Level hoặc tiếp tục lọc sâu hơn.</p></div></div><div class="coc-smart-filter" data-coc-smart-filter="1"><div class="coc-filter-steps" role="toolbar" aria-label="Smart progressive base filters">${controls.map(([key,label,value])=>{const normalized=key==='district'?'type':key;return `<button type="button" class="coc-filter-step ${suggested===normalized?'is-suggested':''}" data-coc-filter-step="${normalized}" aria-expanded="false"><small>${label}</small><b>${esc(value)}</b><span>⌄</span></button>`}).join('')}</div><div class="coc-filter-popovers"><div class="coc-filter-popover" data-coc-filter-popover="group"><div class="coc-filter-popover-head"><b>1 · Chọn Group</b><span>TH / BH / CH</span></div><div class="coc-filter-options coc-filter-options-compact">${groupOpts}</div></div><div class="coc-filter-popover" data-coc-filter-popover="level"><div class="coc-filter-popover-head"><b>2 · Chọn Level</b><span>${groupName}</span></div><div class="coc-filter-options coc-filter-options-level">${levelOpts}</div></div><div class="coc-filter-popover" data-coc-filter-popover="type"><div class="coc-filter-popover-head"><b>3 · ${group==='ch'?'Chọn District':'Chọn Type'}</b><span>Tùy chọn nâng cao</span></div><div class="coc-filter-options">${typeOpts}</div></div><div class="coc-filter-popover" data-coc-filter-popover="year"><div class="coc-filter-popover-head"><b>4 · Chọn Year</b><span>Tùy chọn</span></div><div class="coc-filter-options coc-filter-options-compact">${yearOpts}</div></div><div class="coc-filter-popover" data-coc-filter-popover="sort"><div class="coc-filter-popover-head"><b>5 · Sắp xếp</b><span>Tùy chọn</span></div><div class="coc-filter-options coc-filter-options-compact">${sortOpts}</div></div></div><div class="coc-filter-feedback"><div><small>ĐANG LỌC</small><b>${activeSummary.join(' · ')}</b><span>${total} base phù hợp</span></div><a data-coc-filter="1" href="${reset}">Reset</a></div>${recentHtml}</div><div class="coc-results-toolbar" id="base-results" data-coc-results="1"><div><small>RESULTS</small><strong>${total} base phù hợp</strong><span>${activeSummary.join(' · ')}${totalPages>1?` · Trang ${page}/${totalPages}`:''}</span></div><a data-coc-filter="1" href="${reset}">Reset filters</a></div>${recovery}<div class="coc-grid coc-result-grid">${pageItems.length?pageItems.map(nrGameBaseCard).join(''):(!posts.length?Array.from({length:12},(_,i)=>`<article class="coc-empty-base-card"><div><b>Base ${String(i+1).padStart(2,'0')}</b><span>Chưa có bài viết</span></div></article>`).join(''):'')}</div>${pagination}</div></section>`}
...
function nrGameCloseFilterSteps(){document.querySelectorAll('[data-coc-filter-popover]').forEach(x=>x.classList.remove('is-open'));document.querySelectorAll('[data-coc-filter-step]').forEach(x=>x.setAttribute('aria-expanded','false'))}
function nrGameOpenFilterStep(step){nrGameCloseFilterSteps();const pop=document.querySelector(`[data-coc-filter-popover="${step}"]`),btn=document.querySelector(`[data-coc-filter-step="${step}"]`);if(pop)pop.classList.add('is-open');if(btn)btn.setAttribute('aria-expanded','true')}
function nrGameBindFastFilters(posts,prefix){const browser=document.querySelector('[data-coc-browser="1"]');if(!browser)return;browser.addEventListener('click',e=>{const step=e.target.closest('[data-coc-filter-step]');if(step){e.preventDefault();const key=step.dataset.cocFilterStep,pop=document.querySelector(`[data-coc-filter-popover="${key}"]`),was=!!pop?.classList.contains('is-open');nrGameCloseFilterSteps();if(!was)nrGameOpenFilterStep(key);return}const a=e.target.closest('a[data-coc-filter="1"]');if(!a)return;const u=new URL(a.href,location.origin);if(u.origin!==location.origin)return;e.preventDefault();const selectedStep=a.dataset.cocFilterOption||'';history.pushState({cocFilter:1},'',u.pathname+u.search);const old=document.querySelector('[data-coc-browser="1"]');if(old)old.outerHTML=nrGameFilterPage(posts,prefix);nrGameBindFastFilters(posts,prefix);nrGameSavedSync();nrGameHydrateStats();if(!selectedStep&&u.searchParams.has('page')){nrGameScrollResults('smooth');return}if(selectedStep==='group'){nrGameOpenFilterStep('level');return}if(selectedStep==='level'){nrGameCloseFilterSteps();document.querySelector('[data-coc-filter-step="type"]')?.classList.add('is-suggested');return}nrGameCloseFilterSteps()});document.addEventListener('click',function cocProgressiveOutside(e){if(!document.querySelector('[data-coc-browser="1"]')){document.removeEventListener('click',cocProgressiveOutside,true);return}if(e.target.closest('[data-coc-smart-filter="1"]'))return;nrGameCloseFilterSteps()},true)}
function nrGameRelatedPosts(post,posts){const e=nrGameExtra(post),group=String(e.game_group||post.category||'').toLowerCase(),level=nrGameLevel(post),purpose=String(e.game_purpose||'').toLowerCase();const sameLevel=posts.filter(p=>p!==post&&nrGameLevel(p)===level&&String(nrGameExtra(p).game_group||p.category||'').toLowerCase()===group);return sameLevel.sort((a,b)=>{const ap=String(nrGameExtra(a).game_purpose||'').toLowerCase()===purpose?1:0,bp=String(nrGameExtra(b).game_purpose||'').toLowerCase()===purpose?1:0;return bp-ap}).slice(0,6)}
function nrGameDetailSidebar(post,copy){
 const e=nrGameExtra(post),level=nrGameLevel(post),group=String(e.game_group||post.category||'Town Hall'),purpose=String(e.game_purpose||'Base'),year=String(e.game_year||'—'),style=String(e.game_style||'Original'),defense=String(e.game_defense||'Balanced Defense'),thirdLabel=group.toLowerCase().includes('clan capital')?'District':'Purpose';
 const row=(k,v)=>`<div class="coc-side-meta-row"><span>${esc(k)}</span><b>${esc(v||'—')}</b></div>`;
 return `<section class="coc-side-info"><div class="coc-side-box-title"><small>BASE INFORMATION</small><strong>Thông tin layout</strong></div>${row('Group',group)}${row('Level',level)}${row(thirdLabel,purpose)}${row('Year',year)}${row('Style',style)}${row('Defense',defense)}</section><section class="coc-side-guide"><div class="coc-side-box-title"><small>QUICK GUIDE</small><strong>Cách sử dụng</strong></div><ol><li>Nhấn <b>Copy Base Link</b>.</li><li>Cho phép mở Clash of Clans.</li><li>Chọn một Layout Slot trống.</li><li>Kiểm tra lại layout rồi lưu.</li></ol><p>Base được chia sẻ miễn phí cho cộng đồng. Bạn có thể dùng <b>Saved</b> để đánh dấu và xem lại sau.</p></section><div class="coc-access-card coc-access-card-rich"><small>BASE LINK</small><h3>${esc(level)} · ${esc(purpose)}</h3><p class="coc-side-intro">Mở layout trực tiếp trong Clash of Clans. Kiểm tra đúng cấp nhà trước khi sao chép.</p><a href="${esc(copy)}" data-coc-download="${esc(nrGameBaseSlug(post))}" target="_blank" rel="noopener">Copy Base Link</a></div>`
}
function nrGameArticleContent(post){
 const e=nrGameExtra(post),level=nrGameLevel(post),raw=String(post?.content||''),sample=String(post?.sample_key||'');
 if(sample.includes('atom-th18')){
  const purpose=String(e.game_purpose||'Base'),defense=String(e.game_defense||'Balanced Defense'),style=String(e.game_style||'Original');
  return `<p>${esc(post.title||level+' Base Layout')} is shared for Clash of Clans players looking for a practical ${esc(level)} layout. Use the Copy Base Link on this page to open the layout directly in game.</p><h2>${esc(level)} base layout overview</h2><p>This page keeps the original base image and copy link while presenting the layout in the same clean article structure as the rest of the Game template. Purpose: ${esc(purpose)} · Style: ${esc(style)} · Defense: ${esc(defense)}.</p>`;
 }
 return raw;
}
function nrGameDetail(post,prefix,posts=[],settings={}){const e=nrGameExtra(post),level=nrGameLevel(post),copy=e.copy_link||'#',related=nrGameRelatedPosts(post,posts),slug=nrGameBaseSlug(post),stats=nrGameSeedStats(post),href=post.demo_url||post.url||location.pathname;return `<div class="coc-site">${nrGameHeader(prefix,'',settings)}<main class="coc-detail coc-wrap"><div class="coc-breadcrumb"><a href="${prefix||'/'}">Home</a> / ${esc(level)} / ${esc(post.title||'Base')}</div><div class="coc-detail-grid"><article data-coc-base-card="1" data-coc-slug="${esc(slug)}" data-coc-title="${esc(post.title||level+' Base Layout')}" data-coc-image="${esc(post.image||'')}" data-coc-url="${esc(href)}"><img class="coc-detail-image" src="${esc(post.image||GAME_CLASH_LEVEL_ART[level]||GAME_CLASH_LEVEL_ART.TH18)}" alt="${esc(post.title||level+' Base Layout')}"><div class="coc-detail-copy"><div class="coc-tags"><span>${esc(level)}</span><span>${esc(e.game_purpose||'Base')}</span><span>${esc(e.game_style||'Original')}</span><span>${esc(e.game_defense||'Balanced Defense')}</span></div><h1>${esc(post.title||level+' Base Layout')}</h1><div class="coc-stats big"><button type="button" class="coc-vote-btn" data-coc-vote="${esc(slug)}">⭐ <b data-coc-rating>${stats.rating}</b></button><span>👁 <b data-coc-views>${Number(stats.views).toLocaleString()}</b></span><span>⬇ <b data-coc-downloads>${Number(stats.downloads).toLocaleString()}</b></span><button type="button" class="coc-bookmark-btn coc-bookmark-detail" data-coc-bookmark="${esc(slug)}" aria-label="Save base" aria-pressed="false">🔖 <b>Save</b></button></div><div class="coc-article-body">${nrGameArticleContent(post)}</div></div></article><aside class="coc-detail-side">${nrGameDetailSidebar(post,copy)}</aside></div>${related.length?`<section class="coc-related"><div class="coc-heading"><div><span>RELATED BASES</span><h2>Bài viết liên quan</h2></div></div><div class="coc-related-grid">${related.map(nrGameBaseCard).join('')}</div></section>`:''}</main><div class="coc-mobile-copy-dock"><a class="coc-dock-copy" href="${esc(copy)}" data-coc-download="${esc(slug)}" target="_blank" rel="noopener">Copy Base</a><a class="coc-dock-donate" href="${esc(settings.donate_url||'https://buymeacoffee.com/cocbase')}" target="_blank" rel="noopener noreferrer nofollow"><span aria-hidden="true">☕</span><b>Donate</b></a></div>${nrGameFooter(prefix,settings)}</div>`}
...
function nrGameSavedWrite(items){try{localStorage.setItem('NR_COC_SAVED_V1',JSON.stringify(items.slice(0,120)))}catch(e){}}
function nrGameSavedSync(){const saved=nrGameSavedRead(),set=new Set(saved.map(x=>x.slug));document.querySelectorAll('[data-coc-bookmark]').forEach(btn=>{const on=set.has(btn.dataset.cocBookmark);btn.classList.toggle('is-saved',on);btn.setAttribute('aria-pressed',on?'true':'false');if(btn.classList.contains('coc-bookmark-detail'))btn.innerHTML=on?'🔖 <b>Saved</b>':'🔖 <b>Save</b>'});const list=document.querySelector('[data-coc-saved-list="1"]');if(list){list.innerHTML=saved.length?saved.map(x=>`<article class="coc-saved-item" data-saved-slug="${esc(x.slug)}"><a href="${esc(x.url||'#')}"><img src="${esc(x.image||'')}" alt="${esc(x.title||'Saved base')}" loading="lazy"><span>${esc(x.title||'Saved base')}</span></a><button type="button" data-coc-remove-saved="${esc(x.slug)}" aria-label="Remove saved base">×</button></article>`).join(''):`<div class="coc-saved-empty"><b>Chưa lưu base nào</b><p>Nhấn biểu tượng 🔖 trên card hoặc trong trang chi tiết để lưu vào thiết bị này.</p></div>`}}
function nrGameToast(message,type='saved'){let region=document.querySelector('[data-coc-toast-region="1"]');if(!region){region=document.createElement('div');region.className='coc-toast-region';region.dataset.cocToastRegion='1';region.setAttribute('aria-live','polite');document.body.appendChild(region)}const node=document.createElement('div');node.className='coc-toast '+type;node.innerHTML=`<span>${type==='saved'?'✓':'↩'}</span><b>${esc(message)}</b>`;region.appendChild(node);requestAnimationFrame(()=>node.classList.add('show'));setTimeout(()=>{node.classList.remove('show');setTimeout(()=>node.remove(),220)},2300)}
function nrGameToggleSaved(btn){const card=btn.closest('[data-coc-base-card="1"]');if(!card)return;const slug=btn.dataset.cocBookmark||card.dataset.cocSlug;if(!slug)return;let saved=nrGameSavedRead();const i=saved.findIndex(x=>x.slug===slug),wasSaved=i>=0;if(wasSaved)saved.splice(i,1);else saved.unshift({slug,title:card.dataset.cocTitle||'Saved base',image:card.dataset.cocImage||'',url:card.dataset.cocUrl||location.href,saved_at:Date.now()});nrGameSavedWrite(saved);nrGameSavedSync();nrGameToast(wasSaved?'Base đã được bỏ khỏi Saved':'Base đã được lưu',wasSaved?'removed':'saved')}
function nrGameSetSavedOpen(open){const sheet=document.querySelector('[data-coc-saved-sheet="1"]');if(!sheet)return;sheet.classList.toggle('show',!!open);sheet.setAttribute('aria-hidden',open?'false':'true');document.documentElement.classList.toggle('coc-saved-open',!!open);if(open)nrGameSavedSync()}
function nrGameSetMenuOpen(open){const menu=document.querySelector('[data-coc-mobile-menu="1"]'),toggle=document.querySelector('[data-coc-menu-toggle="1"]');if(!menu||!toggle)return;menu.classList.toggle('show',!!open);menu.setAttribute('aria-hidden',open?'false':'true');toggle.classList.toggle('is-open',!!open);toggle.setAttribute('aria-expanded',open?'true':'false');document.documentElement.classList.toggle('coc-menu-open',!!open)}
function nrGameStatsClientId(){let id='';try{id=localStorage.getItem('NR_COC_STATS_CLIENT_V1')||'';if(!id){id=(crypto.randomUUID?crypto.randomUUID():Math.random().toString(36).slice(2)+Date.now());localStorage.setItem('NR_COC_STATS_CLIENT_V1',id)}}catch(e){id='anon-'+Math.random().toString(36).slice(2)}return id}
function nrGameApplyStats(slug,stats){if(!slug||!stats)return;document.querySelectorAll(`[data-coc-base-card="1"][data-coc-slug="${CSS.escape(slug)}"]`).forEach(card=>{const r=card.querySelector('[data-coc-rating]'),v=card.querySelector('[data-coc-views]'),d=card.querySelector('[data-coc-downloads]');if(r&&Number(stats.rating)>0)r.textContent=Number(stats.rating).toFixed(1);if(v)v.textContent=Number(stats.views||0).toLocaleString();if(d)d.textContent=Number(stats.downloads||0).toLocaleString()})}
async function nrGameHydrateStats(){if(window.NR_DEMO_THEME==='game-1')return;const slugs=[...new Set([...document.querySelectorAll('[data-coc-base-card="1"][data-coc-slug]')].map(x=>x.dataset.cocSlug).filter(Boolean))].slice(0,60);if(!slugs.length)return;try{const url=tenantApiUrl('/api/game/stats?slugs='+encodeURIComponent(slugs.join(','))),r=await fetch(url,{headers:{'Accept':'application/json'}});if(!r.ok)return;const d=await r.json();for(const [slug,st] of Object.entries(d.stats||{}))nrGameApplyStats(slug,st)}catch(e){}}
async function nrGameRecordStat(action,slug,value){if(window.NR_DEMO_THEME==='game-1'||!slug)return null;try{const r=await fetch(tenantApiUrl('/api/game/stats/action'),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action,slug,value,client_id:nrGameStatsClientId()})});if(!r.ok)return null;const d=await r.json();if(d.stats)nrGameApplyStats(slug,d.stats);return d}catch(e){return null}}
function nrGameTrackDetailView(slug){if(window.NR_DEMO_THEME==='game-1'||!slug)return;const day=new Date().toISOString().slice(0,10),k='NR_COC_VIEW_'+day+'_'+slug;try{if(localStorage.getItem(k))return;localStorage.setItem(k,'1')}catch(e){}nrGameRecordStat('view',slug)}
function nrGameOpenVote(slug){document.querySelector('.coc-vote-popover')?.remove();const pop=document.createElement('div');pop.className='coc-vote-popover';pop.innerHTML=`<b>Đánh giá base này</b><div>${[1,2,3,4,5].map(n=>`<button type="button" data-coc-rate="${n}">${n}★</button>`).join('')}</div>`;pop.addEventListener('click',async e=>{const b=e.target.closest('[data-coc-rate]');if(!b)return;const n=Number(b.dataset.cocRate);await nrGameRecordStat('vote',slug,n);nrGameToast(`Đã ghi nhận đánh giá ${n} sao`,'saved');pop.remove()});document.body.appendChild(pop);setTimeout(()=>{document.addEventListener('click',function close(e){if(!pop.contains(e.target)&&!e.target.closest('[data-coc-vote]')){pop.remove();document.removeEventListener('click',close)}},true)},0)}
function nrGameBindLocalFirst(){nrGameSavedSync();nrGameHydrateStats();if(window.__NR_COC_LOCAL_FIRST_BOUND__)return;window.__NR_COC_LOCAL_FIRST_BOUND__=1;document.addEventListener('click',e=>{const vote=e.target.closest('[data-coc-vote]');if(vote){e.preventDefault();nrGameOpenVote(vote.dataset.cocVote);return}const download=e.target.closest('[data-coc-download]');if(download){nrGameRecordStat('download',download.dataset.cocDownload)}const bookmark=e.target.closest('[data-coc-bookmark]');if(bookmark){e.preventDefault();e.stopPropagation();nrGameToggleSaved(bookmark);return}const menuToggle=e.target.closest('[data-coc-menu-toggle="1"]');if(menuToggle){e.preventDefault();nrGameSetMenuOpen(menuToggle.getAttribute('aria-expanded')!=='true');return}const menuLink=e.target.closest('[data-coc-mobile-menu="1"] a');if(menuLink){nrGameSetMenuOpen(false);return}const open=e.target.closest('[data-coc-open-saved="1"]');if(open){e.preventDefault();nrGameSetMenuOpen(false);nrGameSetSavedOpen(true);return}const close=e.target.closest('[data-coc-close-saved="1"]');if(close){e.preventDefault();nrGameSetSavedOpen(false);return}const remove=e.target.closest('[data-coc-remove-saved]');if(remove){e.preventDefault();let saved=nrGameSavedRead().filter(x=>x.slug!==remove.dataset.cocRemoveSaved);nrGameSavedWrite(saved);nrGameSavedSync();nrGameToast('Base đã được bỏ khỏi Saved','removed');return}if(!e.target.closest('[data-coc-mobile-menu="1"]')&&!e.target.closest('[data-coc-menu-toggle="1"]'))nrGameSetMenuOpen(false)},true);document.addEventListener('keydown',e=>{if(e.key==='Escape'){nrGameSetSavedOpen(false);nrGameSetMenuOpen(false);nrGameCloseFilterSteps()}})}
async function nrGameLoadHeroSkinShowcase(){const box=document.querySelector('[data-coc-hero-skin="1"]');if(!box||box.dataset.loaded==='1')return;box.dataset.loaded='1';const img=box.querySelector('img'),name=box.querySelector('[data-coc-skin-name]'),meta=box.querySelector('[data-coc-skin-meta]'),now=new Date(),day=[now.getFullYear(),String(now.getMonth()+1).padStart(2,'0'),String(now.getDate()).padStart(2,'0')].join('-'),dayIndex=Math.floor(Date.UTC(now.getFullYear(),now.getMonth(),now.getDate())/86400000),cacheKey='NR_COC_HERO_SHOWCASE_V2_'+day;function apply(s){if(!s||!s.image)return false;img.src=s.image;img.alt=(s.name||'Clash of Clans Hero Skin')+(s.hero?' · '+s.hero:'');name.textContent=s.name||'Hero Skin';meta.textContent=[s.hero,s.year,s.rarity,'Skin of the day'].filter(Boolean).join(' · ');box.classList.add('is-ready');return true}try{const cached=JSON.parse(localStorage.getItem(cacheKey)||'null');if(cached&&apply(cached))return}catch(e){}try{const base='https://hoangquocvuong.github.io/coc-hero-skins/';const index=await fetch(base+'hero-skins.json',{cache:'force-cache'}).then(r=>{if(!r.ok)throw new Error('hero index');return r.json()});const heroes=(index.heroes||[]).filter(x=>x&&x.file);if(!heroes.length)throw new Error('no heroes');const sets=await Promise.all(heroes.map(async h=>{try{const d=await fetch(base+h.file,{cache:'force-cache'}).then(r=>{if(!r.ok)throw new Error('hero file');return r.json()});return{hero:d.hero||h.name||'',skins:(d.skins||[]).filter(x=>x&&x.image)}}catch(e){return{hero:h.name||'',skins:[]}}}));let pool=[];for(const set of sets){if(!set.skins.length)continue;const maxYear=Math.max(...set.skins.map(x=>Number(x.year)||0));const recent=set.skins.filter(x=>(Number(x.year)||0)===maxYear);for(const skin of recent)pool.push({...skin,hero:set.hero})}if(!pool.length)throw new Error('no skins');pool.sort((a,b)=>String(a.hero).localeCompare(String(b.hero))||String(a.name).localeCompare(String(b.name))||String(a.image).localeCompare(String(b.image)));const skin=pool[((dayIndex%pool.length)+pool.length)%pool.length];if(!apply(skin))throw new Error('invalid skin');try{Object.keys(localStorage).filter(k=>k.startsWith('NR_COC_HERO_SHOWCASE_V2_')&&k!==cacheKey).forEach(k=>localStorage.removeItem(k));localStorage.setItem(cacheKey,JSON.stringify(skin))}catch(e){}}catch(e){box.classList.add('is-fallback')}}
function nrGameEmergencyHome(){
 const main=document.querySelector('main');if(!main)return;
 const prefix=window.NR_DEMO_PREFIX||'/demo/game/clash-of-clans';
 main.innerHTML=`<div class="coc-site coc-emergency"><header class="coc-top"><div class="coc-wrap coc-nav-wrap"><a class="coc-brand" href="${prefix}/"><b>COC</b><span>BASE PORTAL</span></a><div class="coc-nav-actions"><a class="coc-donate-top" href="https://buymeacoffee.com/cocbase" target="_blank" rel="noopener noreferrer nofollow"><span>☕</span><b>Donate</b></a></div></div></header><section class="coc-hero"><div class="coc-wrap"><div class="coc-hero-copy"><span class="coc-kicker">CLASH OF CLANS · COMMUNITY BASE LAYOUTS</span><h1>Choose your Hall.<br><em>Find a base fast.</em></h1><p>Kho chia sẻ base Clash of Clans miễn phí cho cộng đồng.</p><div class="coc-hero-actions"><a href="${prefix}/bases?th=all">Browse Town Hall Bases</a></div></div></div></section><section class="coc-section"><div class="coc-wrap"><div class="coc-heading"><div><span>BASE PORTAL</span><h2>Đang khôi phục dữ liệu giao diện</h2></div></div><p>Giao diện game đã được giữ đúng template. Hãy tải lại sau khi asset mới được cập nhật.</p></div></section></div>`;
}
function renderGameClash1(site={}){const root=document.querySelector('main');if(!root)return;const posts=nrGamePosts(),isDemo=window.NR_DEMO_THEME==='game-1',prefix=isDemo?(window.NR_DEMO_PREFIX||'/demo/game/clash-of-clans'):'',rel=isDemo&&location.pathname.startsWith(prefix)?(location.pathname.slice(prefix.length)||'/'):location.pathname,admin=isDemo?nrDemoAdminUrl('game-1','newpost'):'/admin?tab=newpost',settings=nrGameTemplateSettings(site);if(/^\/(about|terms)\/?$/i.test(rel)){root.innerHTML=nrGameInfoPage(rel.toLowerCase().includes('terms')?'terms':'about',prefix,settings,admin);nrGameBindLocalFirst();return}if(/^\/base\/[^/]+\.html$/i.test(rel)){const slug=rel.split('/').pop().replace(/\.html$/i,''),post=posts.find(p=>String(p.demo_url||p.url||'').includes('/'+slug+'.html'))||posts[0];if(post){root.innerHTML=nrGameDetail(post,prefix,posts,settings);nrGameBindLocalFirst();nrGameTrackDetailView(slug);return}}if(/^\/(bases|free-bases|premium-bases)/i.test(rel)){if(/\/(free|premium)-bases/i.test(rel))history.replaceState({},'',`${prefix}/bases${location.search||'?th=all'}`);root.innerHTML=`<div class="coc-site">${nrGameHeader(prefix,admin,settings)}${nrGameFilterPage(posts,prefix)}${nrGameFooter(prefix,settings)}</div>`;nrGameBindFastFilters(posts,prefix);nrGameBindLocalFirst();if(location.hash==='#base-results')nrGameScrollResults('auto');if(!window.__NR_COC_POPSTATE__){window.__NR_COC_POPSTATE__=1;window.addEventListener('popstate',()=>{const b=document.querySelector('[data-coc-browser="1"]');if(b){b.outerHTML=nrGameFilterPage(posts,prefix);nrGameBindFastFilters(posts,prefix);nrGameSavedSync();nrGameHydrateStats()}})}return}root.innerHTML=`<div class="coc-site">${nrGameHeader(prefix,admin,settings)}<section class="coc-hero"><div class="coc-wrap coc-hero-grid"><div class="coc-hero-copy"><span class="coc-kicker">CLASH OF CLANS · COMMUNITY BASE LAYOUTS</span><h1>Choose your Hall.<br><em>Find a base fast.</em></h1><p>Kho chia sẻ base Clash of Clans miễn phí cho cộng đồng. Chọn đúng cấp nhà, lọc theo mục đích và mở Copy Link chỉ trong vài giây.</p><div class="coc-hero-actions"><a href="${prefix}/bases?th=18#base-results">Browse TH18 Bases</a><a class="soft" href="#town-hall">Choose another Hall</a></div><div class="coc-hero-benefits"><span>✓ Free copy links</span><span>✓ No login required</span><span>✓ Instant filters</span></div><div class="coc-proof"><span><b>TH2–TH18</b><small>Town Hall</small></span><span><b>BH2–BH10</b><small>Builder Hall</small></span><span><b>CH1–CH10</b><small>Clan Capital</small></span></div></div><div class="coc-hero-art coc-hero-skin" data-coc-hero-skin="1"><div class="coc-hero-art-badge">HERO SHOWCASE</div><div class="coc-skin-stage"><img src="/assets/demo/game-hero-placeholder.svg" alt="Clash of Clans Hero Showcase" fetchpriority="high" decoding="async"></div><div class="coc-hero-art-meta"><div><b data-coc-skin-name>Loading latest skin…</b><span data-coc-skin-meta>Randomized daily from recent Hero Skins</span></div><a href="#town-hall">Choose Hall →</a></div></div></div></section><section id="town-hall" class="coc-section" data-structure-key="town-hall"><div class="coc-wrap"><div class="coc-heading"><div><span>TOWN HALL</span><h2>Choose Town Hall</h2></div></div><div class="coc-level-grid" data-contract-grid="1">${nrGameLevelCards('th',prefix)}</div></div></section><section id="builder-hall" class="coc-section coc-dark" data-structure-key="builder-hall"><div class="coc-wrap"><div class="coc-heading light"><div><span>BUILDER BASE</span><h2>Choose Builder Hall</h2></div></div><div class="coc-level-grid" data-contract-grid="1">${nrGameLevelCards('bh',prefix)}</div></div></section><section id="clan-capital" class="coc-section" data-structure-key="clan-capital"><div class="coc-wrap"><div class="coc-heading"><div><span>CLAN CAPITAL</span><h2>Choose Capital Hall</h2></div></div><div class="coc-level-grid" data-contract-grid="1">${nrGameLevelCards('ch',prefix)}</div></div></section>${nrGameHomeExtras(prefix,posts,settings)}${nrGameFooter(prefix,settings)}</div>`;nrGameBindLocalFirst();nrGameLoadHeroSkinShowcase();try{nrApplyStructureGeometry(site,'game-1');nrAuditStructureContract(site,'game-1')}catch(e){}}
...
function nrProductPrefix(){return window.NR_DEMO_THEME==='san-pham-1'?(window.NR_DEMO_PREFIX||'/demo/san-pham/mau-1'):''}
function nrProductUrl(p){return `${nrProductPrefix()}/san-pham/${nrProductSlug(p.title)}.html`}
function nrProductPosts(){return (SITE_DATA?.posts||[]).filter(x=>String(x.type||'').toLowerCase()==='product')}
function nrProductSettings(site={}){const v=site.template_settings&&typeof site.template_settings==='object'?site.template_settings:(()=>{try{return JSON.parse(site.template_settings_json||'{}')}catch(e){return {}}})();const raw=String(v.product_cta_label||'').trim();const cta=!raw||/^xem\s*nơi\s*bán$/i.test(raw)?'Mua Ngay':raw;return {cta,note:v.product_affiliate_note||'Website có thể nhận hoa hồng khi người xem mua hàng qua liên kết giới thiệu.'}}
function nrProductCard(p){const e=nrProductExtra(p),price=nrProductMoney(e.product_price),old=Number(e.product_old_price)>Number(e.product_price)?nrProductMoney(e.product_old_price):'',img=getImages(p)[0]||'/assets/product-affiliate/product-1.svg';return `<article class="pa-card"><a class="pa-card-media" href="${nrProductUrl(p)}"><img src="${esc(img)}" alt="${esc(p.title)}" loading="lazy">${e.product_badge?`<span>${esc(e.product_badge)}</span>`:''}</a><div class="pa-card-body"><small>${esc(e.product_brand||p.category||'Sản phẩm')}</small><a class="pa-card-title" href="${nrProductUrl(p)}">${esc(p.title)}</a><div class="pa-card-rating">★ ${esc(e.product_rating||'4.8')} <span>· Đã bán ${esc(e.product_sold||'0')}</span></div><div class="pa-card-price"><b>${esc(price)}</b>${old?`<del>${esc(old)}</del>`:''}</div></div></article>`}
function nrProductHeader(site={}){const prefix=nrProductPrefix(),admin=window.NR_DEMO_THEME?nrDemoAdminUrl('san-pham-1','newpost'):'/admin?tab=newpost';return `<header class="pa-header"><div class="pa-wrap pa-head"><a class="pa-brand" href="${prefix||'/'}"><span>SHOP</span><b>${esc(cleanSiteName(site.name||'PRODUCT'))}</b></a><form class="pa-search" data-pa-search action="${prefix||'/'}#paFilterResults"><input name="q" value="${esc(new URLSearchParams(location.search).get('q')||'')}" placeholder="Tìm sản phẩm, thương hiệu..."><button>Tìm kiếm</button></form><a class="pa-admin" href="${admin}" target="_blank" rel="noopener">+ Thêm sản phẩm</a></div><nav class="pa-nav"><div class="pa-wrap"><a href="${prefix||'/'}" data-pa-home>Trang chủ</a>${['Điện tử & Công nghệ','Nhà cửa & Đời sống','Thời trang & Làm đẹp','Mẹ & Bé','Thể thao & Du lịch'].map(c=>`<a href="${prefix||'/'}?category=${encodeURIComponent(c)}#paFilterResults" data-pa-category="${esc(c)}">${esc(c)}</a>`).join('')}</div></nav></header>`}
function nrProductFooter(site={},settings={}){const prefix=nrProductPrefix()||'/';const phone=String(site.phone||'').trim(),email=String(site.email||site.public_email||'').trim();return `<footer class="pa-footer"><div class="pa-wrap pa-footer-grid"><div class="pa-footer-about"><div class="pa-footer-brand"><span>SHOP</span><b>${esc(cleanSiteName(site.name||'PRODUCT STORE'))}</b></div><p>Website giới thiệu, review sản phẩm, voucher và ưu đãi qua liên kết tiếp thị. Chủ website tự quản lý toàn bộ nội dung trong Trang quản trị.</p><div class="pa-footer-contact">${phone?`<a href="tel:${esc(phone.replace(/\s/g,''))}">☎ ${esc(phone)}</a>`:''}${email?`<a href="mailto:${esc(email)}">✉ ${esc(email)}</a>`:''}</div></div><div><b>Danh mục</b><a href="${prefix}" data-pa-home>Tất cả sản phẩm</a><a href="${prefix}?category=${encodeURIComponent('Điện tử & Công nghệ')}#paFilterResults" data-pa-category="Điện tử & Công nghệ">Điện tử & Công nghệ</a><a href="${prefix}?category=${encodeURIComponent('Nhà cửa & Đời sống')}#paFilterResults" data-pa-category="Nhà cửa & Đời sống">Nhà cửa & Đời sống</a><a href="${prefix}?category=${encodeURIComponent('Thời trang & Làm đẹp')}#paFilterResults" data-pa-category="Thời trang & Làm đẹp">Thời trang & Làm đẹp</a><a href="${prefix}?category=${encodeURIComponent('Mẹ & Bé')}#paFilterResults" data-pa-category="Mẹ & Bé">Mẹ & Bé</a></div><div><b>Thông tin</b><a href="${prefix}#san-pham">Sản phẩm nổi bật</a><a href="${prefix}?q=voucher#paFilterResults">Voucher & ưu đãi</a><span>Hướng dẫn mua hàng</span><span>Câu hỏi thường gặp</span><span>Liên hệ quản trị</span></div><div><b>Chính sách & Affiliate</b><span>Chính sách bảo mật</span><span>Điều khoản sử dụng</span><span>Tuyên bố Affiliate</span><span>Miễn trừ trách nhiệm</span><p class="pa-footer-note">${esc(settings.note)}</p></div></div><div class="pa-footer-bottom"><div class="pa-wrap">© ${new Date().getFullYear()} ${esc(cleanSiteName(site.name||'Product Store'))} · Giá, ưu đãi và tồn kho được xác nhận tại trang bán hàng.</div></div></footer>`}
function nrProductSpecs(raw=''){return String(raw||'').split(/\n+/).map(x=>x.trim()).filter(Boolean).map(line=>{const i=line.indexOf(':');return i>0?`<tr><th>${esc(line.slice(0,i).trim())}</th><td>${esc(line.slice(i+1).trim())}</td></tr>`:`<tr><td colspan="2">${esc(line)}</td></tr>`}).join('')}
function nrProductText(html=''){const d=document.createElement('div');d.innerHTML=String(html||'');return String(d.textContent||'').replace(/\s+/g,' ').trim()}
function nrProductReviewCard(p){const e=nrProductExtra(p),img=getImages(p)[0]||'/assets/product-affiliate/product-1.svg',text=nrProductText(p.content).slice(0,155);return `<article class="pa-home-review-card"><a class="pa-home-review-media" href="${nrProductUrl(p)}"><img src="${esc(img)}" alt="${esc(p.title)}" loading="lazy"></a><div><small>${esc(p.category||'REVIEW SẢN PHẨM')}</small><h3><a href="${nrProductUrl(p)}">${esc(p.title)}</a></h3><p>${esc(text||'Bài review mẫu giúp khách hàng hình dung cách trình bày nội dung chi tiết của sản phẩm.')}${text.length>=155?'…':''}</p><div class="pa-home-review-meta"><span>★ ${esc(e.product_rating||'4.8')}</span><span>${esc(nrProductMoney(e.product_price))}</span></div><a class="pa-home-review-link" href="${nrProductUrl(p)}">Xem bài review chi tiết →</a></div></article>`}
function nrProductFilterState(){const u=new URLSearchParams(location.search);return {q:String(u.get('q')||'').trim(),category:String(u.get('category')||'').trim(),brand:String(u.get('brand')||'').trim(),price:String(u.get('price')||'').trim(),sort:String(u.get('sort')||'popular').trim()||'popular'}}
function nrProductPriceRanges(category=''){if(category==='Điện tử & Công nghệ')return [['under-500','Dưới 500.000đ',0,500000],['500-1m','500.000đ – 1 triệu',500000,1000000],['1-2m','1 – 2 triệu',1000000,2000000],['2-5m','2 – 5 triệu',2000000,5000000],['5m-plus','Trên 5 triệu',5000000,null]];if(category==='Nhà cửa & Đời sống'||category==='Thời trang & Làm đẹp'||category==='Mẹ & Bé')return [['under-300','Dưới 300.000đ',0,300000],['300-700','300.000đ – 700.000đ',300000,700000],['700-1500','700.000đ – 1,5 triệu',700000,1500000],['1500-plus','Trên 1,5 triệu',1500000,null]];return [['under-500','Dưới 500.000đ',0,500000],['500-1m','500.000đ – 1 triệu',500000,1000000],['1-2m','1 – 2 triệu',1000000,2000000],['2-5m','2 – 5 triệu',2000000,5000000],['5m-plus','Trên 5 triệu',5000000,null]]}
function nrProductFilterControls(posts,state={}){const categories=[...new Set(posts.map(p=>p.category).filter(Boolean))],brands=[...new Set(posts.map(p=>nrProductExtra(p).product_brand).filter(Boolean))].sort(),ranges=nrProductPriceRanges(state.category);return `<section class="pa-filter-shell" id="paFilters"><div class="pa-wrap"><div class="pa-filter-title"><div><small>BỘ LỌC SẢN PHẨM</small><b>Lọc nhanh theo nhu cầu</b></div><button type="button" data-pa-clear>Xóa bộ lọc</button></div><div class="pa-filter-grid"><label><span>Danh mục</span><select data-pa-filter="category"><option value="">Tất cả danh mục</option>${categories.map(v=>`<option value="${esc(v)}" ${state.category===v?'selected':''}>${esc(v)}</option>`).join('')}</select></label><label><span>Thương hiệu</span><select data-pa-filter="brand"><option value="">Tất cả thương hiệu</option>${brands.map(v=>`<option value="${esc(v)}" ${state.brand===v?'selected':''}>${esc(v)}</option>`).join('')}</select></label><label><span>Khoảng giá</span><select data-pa-filter="price"><option value="">Tất cả mức giá${state.category?' của '+esc(state.category):''}</option>${ranges.map(v=>`<option value="${v[0]}" ${state.price===v[0]?'selected':''}>${v[1]}</option>`).join('')}</select></label><label><span>Sắp xếp</span><select data-pa-filter="sort"><option value="popular" ${state.sort==='popular'?'selected':''}>Phổ biến</option><option value="newest" ${state.sort==='newest'?'selected':''}>Mới nhất</option><option value="price-asc" ${state.sort==='price-asc'?'selected':''}>Giá thấp</option><option value="price-desc" ${state.sort==='price-desc'?'selected':''}>Giá cao</option></select></label></div></div></section>`}
function nrProductFilterApply(posts,state){const q=String(state.q||'').toLowerCase();const ranges=nrProductPriceRanges(state.category),range=ranges.find(x=>x[0]===state.price);let out=posts.filter(p=>{const e=nrProductExtra(p),price=Number(e.product_price||0),hay=(p.title+' '+(p.category||'')+' '+(e.product_brand||'')+' '+(e.product_sku||'')+' '+nrProductText(p.content)).toLowerCase();if(state.category&&p.category!==state.category)return false;if(state.brand&&e.product_brand!==state.brand)return false;if(q&&!hay.includes(q))return false;if(range&&(!price||price<range[2]||(range[3]!==null&&price>=range[3])))return false;return true});if(state.sort==='price-asc')out.sort((a,b)=>Number(nrProductExtra(a).product_price||0)-Number(nrProductExtra(b).product_price||0));else if(state.sort==='price-desc')out.sort((a,b)=>Number(nrProductExtra(b).product_price||0)-Number(nrProductExtra(a).product_price||0));else if(state.sort==='newest')out.sort((a,b)=>Number(b.id||0)-Number(a.id||0));else out.sort((a,b)=>Number(nrProductExtra(b).product_sold||0)-Number(nrProductExtra(a).product_sold||0));return out}
function nrProductStateTitle(state){return [state.category,state.brand,state.price?((nrProductPriceRanges(state.category).find(x=>x[0]===state.price)||[])[1]||''):null,state.q?'Từ khóa: '+state.q:null].filter(Boolean).join(' · ')||'Sản phẩm phù hợp'}
function nrProductInitFilters(posts){const result=document.getElementById('paFilterResults'),home=document.getElementById('paHomeContent'),grid=document.getElementById('paFilterGrid'),title=document.getElementById('paFilterTitle'),desc=document.getElementById('paFilterDesc');if(!result||!home||!grid)return;const read=()=>{const s=nrProductFilterState();document.querySelectorAll('[data-pa-filter]').forEach(el=>{if(el.dataset.paFilter in s)el.value=s[el.dataset.paFilter]||''});return s};const syncPriceOptions=(state)=>{const el=document.querySelector('[data-pa-filter="price"]');if(!el)return;const current=state.price||'';el.innerHTML=`<option value="">Tất cả mức giá${state.category?' của '+esc(state.category):''}</option>`+nrProductPriceRanges(state.category).map(v=>`<option value="${v[0]}">${v[1]}</option>`).join('');if([...el.options].some(o=>o.value===current))el.value=current;else state.price=''};const write=(state,push=true)=>{const u=new URL(location.href);['q','category','brand','price'].forEach(k=>state[k]?u.searchParams.set(k,state[k]):u.searchParams.delete(k));if(state.sort&&state.sort!=='popular')u.searchParams.set('sort',state.sort);else u.searchParams.delete('sort');u.hash='#paFilterResults';history[push?'pushState':'replaceState']({paFilter:true},'',u.pathname+(u.search||'')+u.hash)};const render=(state,scroll=true)=>{syncPriceOptions(state);const active=!!(state.q||state.category||state.brand||state.price||state.sort&&state.sort!=='popular');if(!active){result.hidden=true;home.hidden=false;return}const list=nrProductFilterApply(posts,state);home.hidden=true;result.hidden=false;title.textContent=nrProductStateTitle(state);desc.textContent=`Tìm thấy ${list.length} sản phẩm phù hợp.`;grid.innerHTML=list.map(nrProductCard).join('')||'<div class="pa-empty">Chưa có sản phẩm phù hợp. Hãy thử tiêu chí khác.</div>';if(scroll)setTimeout(()=>result.scrollIntoView({behavior:'smooth',block:'start'}),30)};const stateFromControls=()=>{const s=nrProductFilterState();document.querySelectorAll('[data-pa-filter]').forEach(el=>s[el.dataset.paFilter]=el.value||'');const q=document.querySelector('[data-pa-search] input[name="q"]');s.q=q?String(q.value||'').trim():s.q;return s};document.querySelectorAll('[data-pa-filter]').forEach(el=>el.addEventListener('change',()=>{const s=stateFromControls();if(el.dataset.paFilter==='category')s.price='';write(s);render(s,true)}));document.querySelectorAll('[data-pa-category]').forEach(a=>a.addEventListener('click',ev=>{ev.preventDefault();const s=nrProductFilterState();s.category=a.dataset.paCategory||'';s.brand='';s.price='';const input=document.querySelector('[data-pa-search] input[name="q"]');if(input)input.value='';s.q='';write(s);document.querySelector('[data-pa-filter="category"]')&&(document.querySelector('[data-pa-filter="category"]').value=s.category);render(s,true)}));document.querySelectorAll('[data-pa-home]').forEach(a=>a.addEventListener('click',ev=>{if(location.pathname.includes('/san-pham/'))return;ev.preventDefault();const u=new URL(location.href);u.search='';u.hash='';history.pushState({},'',u.pathname);const input=document.querySelector('[data-pa-search] input[name="q"]');if(input)input.value='';document.querySelectorAll('[data-pa-filter]').forEach(x=>x.value=x.dataset.paFilter==='sort'?'popular':'');result.hidden=true;home.hidden=false;window.scrollTo({top:0,behavior:'smooth'})}));const form=document.querySelector('[data-pa-search]');if(form)form.addEventListener('submit',ev=>{ev.preventDefault();const s=stateFromControls();write(s);render(s,true)});const clear=document.querySelector('[data-pa-clear]');if(clear)clear.addEventListener('click',()=>{const s={q:'',category:'',brand:'',price:'',sort:'popular'};const input=document.querySelector('[data-pa-search] input[name="q"]');if(input)input.value='';document.querySelectorAll('[data-pa-filter]').forEach(x=>x.value=x.dataset.paFilter==='sort'?'popular':'');const u=new URL(location.href);u.search='';u.hash='';history.pushState({},'',u.pathname);result.hidden=true;home.hidden=false;document.getElementById('san-pham')?.scrollIntoView({behavior:'smooth',block:'start'})});window.addEventListener('popstate',()=>render(read(),false));const initial=read();render(initial,false);if(location.hash==='#paFilterResults'&&(initial.q||initial.category||initial.brand||initial.price))setTimeout(()=>result.scrollIntoView({behavior:'auto',block:'start'}),80)}
function nrProductInitDetail(){document.querySelectorAll('[data-pa-thumb]').forEach(b=>b.onclick=()=>{document.querySelectorAll('[data-pa-thumb]').forEach(x=>x.classList.remove('active'));b.classList.add('active');const im=b.querySelector('img');if(im&&document.getElementById('paMainImage'))document.getElementById('paMainImage').src=im.src});document.querySelectorAll('.pa-mobile-collapse>button').forEach(btn=>btn.addEventListener('click',()=>{const box=btn.closest('.pa-mobile-collapse');const open=box.classList.toggle('is-open');btn.setAttribute('aria-expanded',open?'true':'false')}))}
function nrProductDetail(p,site={}){const e=nrProductExtra(p),settings=nrProductSettings(site),images=getImages(p);while(images.length<3)images.push(`/assets/product-affiliate/product-${images.length+1}.svg`);const price=nrProductMoney(e.product_price),old=Number(e.product_old_price)>Number(e.product_price)?nrProductMoney(e.product_old_price):'';const related=nrProductPosts().filter(x=>x.id!==p.id&&(x.category===p.category)).slice(0,5),rating=Number(e.product_rating||4.8)||4.8,sold=String(e.product_sold||'0'),brand=e.product_brand||'Đang cập nhật',sku=e.product_sku||'Đang cập nhật';const pct=[Math.min(96,Math.round(rating/5*94)),Math.max(2,Math.round((5-rating)*9)),3,1,0],collapse=(label,body,open=false)=>`<section class="pa-detail-block pa-mobile-collapse${open?' is-open':''}"><button type="button" aria-expanded="${open?'true':'false'}">${esc(label)}</button><div class="pa-mobile-collapse-content">${body}</div></section>`;const desc=collapse('Mô tả / Review sản phẩm',`<div class="pa-content">${p.content||'<p>Chưa có nội dung mô tả. Chủ website có thể bổ sung bài review đầy đủ trong Trang quản trị.</p>'}</div>`,true);const highlights=collapse('Điểm nổi bật',`<div class="pa-feature-list"><div><b>Thiết kế & trải nghiệm</b><span>Trình bày các ưu điểm chính, đối tượng sử dụng và tình huống phù hợp.</span></div><div><b>Giá & ưu đãi</b><span>Hiển thị giá hiện tại, giá cũ và voucher do chủ website cập nhật.</span></div><div><b>Đánh giá trước khi mua</b><span>Nên đối chiếu phân loại, vận chuyển và chính sách mới nhất tại nơi bán.</span></div></div>`);const guide=collapse('Hướng dẫn mua hàng',`<ol class="pa-buy-guide"><li>Kiểm tra đúng mẫu, phiên bản và thông số cần mua.</li><li>Bấm <b>${esc(settings.cta)}</b> để mở trang bán hàng.</li><li>Đối chiếu voucher, phí vận chuyển, tồn kho và giá thanh toán cuối.</li><li>Hoàn tất đơn hàng trực tiếp tại nền tảng bán hàng.</li></ol>`);const specs=collapse('Thông số sản phẩm',`<table>${nrProductSpecs(e.product_specs)||'<tr><th>Thương hiệu</th><td>'+esc(brand)+'</td></tr><tr><th>Model</th><td>'+esc(sku)+'</td></tr><tr><td colspan="2">Chủ website có thể thêm thông số chi tiết trong Trang quản trị.</td></tr>'}</table>`);return `<div class="pa-site">${nrProductHeader(site)}<main class="pa-wrap pa-detail"><nav class="pa-crumb"><a href="${nrProductPrefix()||'/'}">Trang chủ</a> / <a href="${nrProductPrefix()||'/'}?category=${encodeURIComponent(p.category||'')}#paFilterResults">${esc(p.category||'Sản phẩm')}</a> / ${esc(p.title)}</nav><section class="pa-detail-top"><div class="pa-gallery"><div class="pa-thumbs">${images.slice(0,5).map((im,i)=>`<button class="${i?'':'active'}" data-pa-thumb="${i}"><img src="${esc(im)}" alt="${esc(p.title)} - ảnh ${i+1}"></button>`).join('')}</div><div class="pa-main-image"><img id="paMainImage" src="${esc(images[0])}" alt="${esc(p.title)}"></div></div><div class="pa-summary"><span class="pa-category">${esc(p.category||'Sản phẩm')}</span><h1>${esc(p.title)}</h1><div class="pa-rating"><b>${esc(rating.toFixed(1))} ★</b><span>Đã bán ${esc(sold)}</span></div><p class="pa-brand-line">Thương hiệu: <b>${esc(brand)}</b> · Model: <b>${esc(sku)}</b></p><div class="pa-price"><strong>${esc(price)}</strong>${old?`<del>${esc(old)}</del>`:''}</div>${e.product_promo?`<div class="pa-promo"><b>🎟 Ưu đãi hiện có</b><p>${esc(e.product_promo)}</p></div>`:''}<div class="pa-trust-grid"><span>✓ Thông tin rõ ràng</span><span>✓ Giá dễ kiểm tra</span><span>✓ Link mua trực tiếp</span><span>✓ Có disclosure Affiliate</span></div><a class="pa-buy" href="${esc(e.product_affiliate_url||'#')}" ${e.product_affiliate_url&&e.product_affiliate_url!=='#'?'target="_blank" rel="nofollow sponsored noopener"':''}>${esc(settings.cta)}</a><small class="pa-aff-note">${esc(settings.note)}</small></div></section><section class="pa-detail-lower pa-detail-rich"><article>${desc}${highlights}${guide}</article><aside>${specs}<div class="pa-detail-block pa-source-card"><h2>Thông tin cập nhật</h2><dl><div><dt>Thương hiệu</dt><dd>${esc(brand)}</dd></div><div><dt>Model / SKU</dt><dd>${esc(sku)}</dd></div><div><dt>Nguồn giá & ưu đãi</dt><dd>Kiểm tra tại trang bán hàng</dd></div><div><dt>Trạng thái</dt><dd>Được chủ website cập nhật</dd></div></dl></div><div class="pa-detail-block pa-affiliate-box"><h2>Tuyên bố Affiliate</h2><p>${esc(settings.note)}</p><p>Website không trực tiếp xử lý đơn hàng; giá, voucher, tồn kho và chính sách được xác nhận tại nơi bán.</p></div></aside></section><section class="pa-detail-block pa-review-section"><div class="pa-heading"><div><small>ĐÁNH GIÁ</small><h2>Đánh giá sản phẩm</h2><p>Thông tin tổng hợp để tham khảo trước khi quyết định mua.</p></div></div><div class="pa-review-panel"><div class="pa-review-score"><strong>${esc(rating.toFixed(1))}</strong><span>trên 5</span><b>★★★★★</b><small>Đã bán ${esc(sold)}</small></div><div class="pa-review-bars">${[5,4,3,2,1].map((star,i)=>`<div><span>${star} sao</span><i><em style="width:${pct[i]}%"></em></i><b>${pct[i]}%</b></div>`).join('')}</div></div></section><section class="pa-detail-block pa-faq"><div class="pa-heading"><div><small>HỖ TRỢ</small><h2>Câu hỏi thường gặp</h2></div></div><details open><summary>Giá của ${esc(p.title)} có thể thay đổi không?</summary><p>Có. Giá, voucher và tồn kho có thể thay đổi theo thời điểm. Hãy kiểm tra giá thanh toán cuối cùng tại nơi bán.</p></details><details><summary>Mua ${esc(p.title)} ở đâu?</summary><p>Nút ${esc(settings.cta)} sẽ chuyển người xem tới liên kết bán hàng do chủ website cấu hình.</p></details><details><summary>Thông tin sản phẩm được cập nhật thế nào?</summary><p>Chủ website quản lý nội dung, giá, thông số, hình ảnh và liên kết mua hàng trực tiếp trong Trang quản trị.</p></details></section>${related.length?`<section class="pa-section pa-related-section"><div class="pa-heading"><div><small>CÙNG CHUYÊN MỤC</small><h2>Sản phẩm liên quan</h2><p>Gợi ý các sản phẩm cùng nhóm để người xem so sánh nhanh.</p></div></div><div class="pa-grid">${related.map(nrProductCard).join('')}</div></section>`:''}</main><div class="pa-mobile-sticky-buy"><div><small>Giá tham khảo</small><strong>${esc(price)}</strong></div><a href="${esc(e.product_affiliate_url||'#')}" ${e.product_affiliate_url&&e.product_affiliate_url!=='#'?'target="_blank" rel="nofollow sponsored noopener"':''}>${esc(settings.cta)}</a></div>${nrProductFooter(site,settings)}</div>`}

function renderProductAffiliate1(site={}){document.querySelectorAll('footer.footer,footer.public-footer').forEach(x=>x.remove());const root=document.querySelector('main');if(!root)return;const posts=nrProductPosts(),prefix=nrProductPrefix(),rel=prefix&&location.pathname.startsWith(prefix)?(location.pathname.slice(prefix.length)||'/'):location.pathname;const detail=rel.match(/^\/san-pham\/([^/]+)\.html$/i);document.body.classList.toggle('pa-product-detail',!!detail);if(detail){const slug=detail[1],p=posts.find(x=>nrProductSlug(x.title)===slug);root.innerHTML=p?nrProductDetail(p,site):`<div class="pa-site">${nrProductHeader(site)}<div class="pa-wrap pa-empty"><h1>Không tìm thấy sản phẩm</h1><a href="${prefix||'/'}">Về trang chủ</a></div>${nrProductFooter(site,nrProductSettings(site))}</div>`;nrProductInitDetail();return}const state=nrProductFilterState(),featured=posts.filter(x=>x.featured).slice(0,10),hero=featured[0]||posts[0];const section=(cat,key)=>{const items=posts.filter(x=>x.category===cat).slice(0,5);return `<section class="pa-section" data-structure-key="${key}"><div class="pa-heading"><div><small>${esc(cat.toUpperCase())}</small><h2>${esc(cat)}</h2></div><a href="${prefix||'/'}?category=${encodeURIComponent(cat)}#paFilterResults" data-pa-category="${esc(cat)}">Xem tất cả →</a></div><div class="pa-grid" data-contract-grid="1">${items.map(nrProductCard).join('')||Array.from({length:5},()=>'<div class="pa-card pa-skeleton"></div>').join('')}</div></section>`};root.innerHTML=`<div class="pa-site">${nrProductHeader(site)}<section class="pa-hero" data-structure-key="hero"><div class="pa-wrap pa-hero-grid"><div><span class="pa-kicker">PRODUCT STORE · AFFILIATE READY</span><h1>Sản phẩm rõ ràng.<br><em>Giá & ưu đãi dễ thấy.</em></h1><p>Giao diện catalog thương mại dành cho website review, giới thiệu sản phẩm và affiliate. Toàn bộ dữ liệu do chủ website quản lý trong Trang quản trị.</p><div class="pa-hero-actions"><a href="#san-pham">Xem sản phẩm</a><a class="soft" href="${window.NR_DEMO_THEME?nrDemoAdminUrl('san-pham-1','newpost'):'/admin?tab=newpost'}" target="_blank">Thêm sản phẩm</a></div></div><div class="pa-hero-product">${hero?`<img src="${esc(getImages(hero)[0]||'/assets/product-affiliate/product-1.svg')}" alt="${esc(hero.title)}"><small>${esc(hero.category||'Sản phẩm')}</small><b>${esc(hero.title)}</b><strong>${esc(nrProductMoney(nrProductExtra(hero).product_price))}</strong>`:'<div class="pa-empty">Chưa có sản phẩm</div>'}</div></div></section><section class="pa-category-strip" data-structure-key="categories"><div class="pa-wrap">${['Điện tử & Công nghệ','Nhà cửa & Đời sống','Thời trang & Làm đẹp','Mẹ & Bé','Thể thao & Du lịch','Phụ kiện & Khác'].map((c,i)=>`<a href="${prefix||'/'}?category=${encodeURIComponent(c)}#paFilterResults" data-pa-category="${esc(c)}"><span>${['⌁','⌂','◇','♡','△','＋'][i]}</span><b>${esc(c)}</b></a>`).join('')}</div></section>${nrProductFilterControls(posts,state)}<section id="paFilterResults" class="pa-section pa-filter-results" hidden><div class="pa-wrap"><div class="pa-heading"><div><small>KẾT QUẢ LỌC</small><h2 id="paFilterTitle">Sản phẩm phù hợp</h2><p id="paFilterDesc"></p></div><button type="button" class="pa-filter-reset" data-pa-clear>Xóa lọc</button></div><div id="paFilterGrid" class="pa-grid pa-filter-grid-results"></div></div></section><div id="paHomeContent"><div id="san-pham" class="pa-wrap"><section class="pa-section" data-structure-key="featured"><div class="pa-heading"><div><small>NỔI BẬT</small><h2>Sản phẩm nổi bật</h2></div></div><div class="pa-grid" data-contract-grid="1">${featured.map(nrProductCard).join('')||Array.from({length:10},()=>'<div class="pa-card pa-skeleton"></div>').join('')}</div></section></div><div class="pa-wrap">${section('Điện tử & Công nghệ','electronics')}${section('Nhà cửa & Đời sống','home')}${section('Thời trang & Làm đẹp','beauty')}${section('Mẹ & Bé','baby')}</div></div>${nrProductFooter(site,nrProductSettings(site))}</div>`;nrProductInitFilters(posts);try{nrApplyStructureGeometry(site,'san-pham-1');nrAuditStructureContract(site,'san-pham-1')}catch(e){}}
...
function renderServiceProviderLegacy(site={},key='dich-vu-1'){
...
function renderServiceProvider(site={},key='dich-vu-1'){
 ...
 const serviceCardSpec=x=>{const e=ex(x),c=String(x.category||'').toLowerCase();if(c.includes('camera'))return e.camera_resolution||e.camera_type||e.camera_count||e.service_camera||e.service_cloud||'Camera & lưu trữ';if(c.includes('truyền hình')||c.includes('mytv')||c.includes('tv360'))return e.service_channels||e.service_tv||e.service_device||'Truyền hình & giải trí';if(c.includes('combo'))return [e.service_speed_down||e.service_speed,e.service_tv,e.service_camera].filter(Boolean).join(' · ')||'Combo nhiều dịch vụ';return e.service_speed_down||e.service_speed||e.service_wifi||e.service_device||'Tư vấn theo nhu cầu'};
 const serviceDetailBenefits=x=>{const e=ex(x),c=String(x.category||'').toLowerCase(),pairs=c.includes('camera')?[[e.camera_type,'Loại'],[e.camera_resolution,'Độ phân giải'],[e.camera_count,'Số camera'],[e.service_camera,'AI / tính năng'],[e.service_cloud,'Lưu trữ'],[e.service_install,'Lắp đặt'],[e.service_area,'Khu vực']]:c.includes('truyền hình')||c.includes('mytv')||c.includes('tv360')?[[e.service_tv,'Gói nội dung'],[e.service_channels,'Kênh / nội dung'],[e.service_device,'Thiết bị'],[e.service_term,'Chu kỳ'],[e.service_area,'Khu vực']]:c.includes('combo')?[[e.service_speed_down||e.service_speed,'Download'],[e.service_speed_up,'Upload'],[e.service_wifi,'Wi-Fi'],[e.service_tv,'Truyền hình'],[e.service_camera,'Camera'],[e.service_cloud,'Cloud'],[e.service_term,'Cam kết'],[e.service_install,'Lắp đặt']]:[[e.service_speed_down||e.service_speed,'Download'],[e.service_speed_up,'Upload'],[e.service_wifi||e.service_device,'Wi-Fi / thiết bị'],[e.service_devices,'Số thiết bị'],[e.service_term,'Cam kết'],[e.service_install,'Lắp đặt'],[e.service_area,'Khu vực']];return pairs.filter(x=>x[0]).map(([v,k])=>`${k}: ${v}`)};
 const card=(x,cl='')=>{const e=ex(x),meta=nrContentMeta(x),im=meta.image||x.image||x.image_url||x.thumbnail||nrSvcFallback(key,x.category),price=nrServiceIsShowroom(key)?nrSvcSafePrice(key,x.category,e.service_price):meta.display_price||e.service_price||'Liên hệ',promo=e.service_promo||meta.excerpt||'Thông tin đang cập nhật',spec=serviceCardSpec(x);return `<article class="tel-card ${cl}" data-service-card="1" data-contract-slot="1"><button type="button" class="tel-card-img svc-detail-open" data-package="${esc(x.title||'')}" aria-label="Xem chi tiết ${esc(x.title||'gói dịch vụ')}"><img src="${esc(im)}" alt="${esc(x.title)}" loading="lazy" onerror="this.onerror=null;this.src='${esc(x.fallback_image||nrSvcFallback(key,x.category))}'"></button><div class="tel-card-body"><span class="tel-pill">${esc(x.category||'Dịch vụ')}</span><button type="button" class="tel-title-link svc-detail-open" data-package="${esc(x.title||'')}"><h3>${esc(x.title||'Gói dịch vụ')}</h3></button><div class="tel-speed">${esc(spec)}</div><p class="tel-desc">${esc(promo)}</p><button type="button" class="tel-detail-link svc-detail-open" data-package="${esc(x.title||'')}">Xem chi tiết & khuyến mãi →</button><div class="tel-card-bottom"><div class="tel-price"><small>Chỉ từ</small><strong>${esc(price)}</strong></div><a class="tel-cta svc-lead-open" href="#dang-ky" data-package="${esc(x.title||'')}" data-category="${esc(x.category||'')}">${esc(e.service_cta||'Đăng ký')}</a></div></div></article>`};
...
/* V20.9.26.1 — Professional templates: tenant data + exact categories + contact workflow. */
function sxSlug(v=''){return String(v||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/đ/g,'d').replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'')}
function sxSettings(site={}){let x=site.template_settings;if(!x){try{x=JSON.parse(site.template_settings_json||'{}')}catch(e){x={}}}return x&&typeof x==='object'?x:{}}
function sxProfile(site={},key=''){let st=site.structure_profile;if(typeof st==='string'){try{st=JSON.parse(st)}catch(e){st={}}}const fallback={
 'blog-ca-nhan-1':['Thương hiệu cá nhân','Freelance','Năng suất','Sách & công cụ','Lối sống sáng tạo','Góc nhìn'],
 'blog-ca-nhan-2':['Kinh doanh nội dung','Video & sản xuất','Chiến lược nội dung','Câu chuyện dự án','Công cụ','Kiếm tiền nội dung'],
 'doanh-nghiep-1':['Giới thiệu & năng lực','Sản phẩm & dịch vụ','Dự án & đối tác','Tin tức & sự kiện','Tuyển dụng','Hoạt động & văn hóa'],
 'doanh-nghiep-2':['Giới thiệu & năng lực','Sản phẩm & dịch vụ','Dự án tiêu biểu','Tin tức & sự kiện','Nhân sự & tuyển dụng','Trách nhiệm xã hội'],
 'dich-vu-5':['Laptop văn phòng','Laptop gaming','Laptop mỏng nhẹ','Laptop đồ họa','MacBook','Cẩm nang laptop'],
...
function lionExtra(p){const n=nrNormalizeContentPost(p),e=nrParseExtraObject(n.extra_json);return {...e,...Object.fromEntries(['service_price','lion_count','drum_count','performers_count','performance_duration','fireworks','confetti','couplets','service_area','service_cta'].map(k=>[k,n[k]??e[k]??'']))}}
function lionMoney(v=''){return String(v||'Liên hệ báo giá')}
function lionServiceCard(p){const n=nrNormalizeContentPost(p),e=lionExtra(n),meta=nrContentMeta(n),img=getImages(n)[0]||n.image||'/assets/marketing-demo.webp',price=meta.display_price||'Liên hệ báo giá',cta=e.service_cta||'Liên hệ báo giá',detail=sxPostUrl(n);return `<article class="ld-pack"><a class="ld-pack-img" href="${detail}"><img src="${esc(img)}" alt="${esc(n.title||'Dịch vụ')}"><span>${esc(price)}</span></a><div class="ld-pack-body"><small>${esc(n.category||'Dịch vụ')}</small><h3><a href="${detail}">${esc(n.title||'Dịch vụ')}</a></h3>${meta.excerpt?`<p class="ld-pack-excerpt">${esc(meta.excerpt)}</p>`:''}<div class="ld-spec-row"><span><b>${esc(e.lion_count||'—')}</b>Đội hình</span><span><b>${esc(e.drum_count||'—')}</b>Trống</span><span><b>${esc(e.performers_count||'—')}</b>Nhân sự</span></div><ul><li>Pháo sáng: ${esc(e.fireworks||'Theo yêu cầu')}</li><li>Kim tuyến: ${esc(e.confetti||'Theo yêu cầu')}</li><li>Câu đối: ${esc(e.couplets||'Theo kịch bản')}</li></ul><div class="ld-pack-actions"><a href="#contact">${esc(cta)}</a><button type="button" data-ld-price>Giá tiền</button></div><div class="ld-pack-price"><b>${esc(price)}</b><small>Giá tham khảo · liên hệ để xác nhận theo địa điểm và kịch bản</small></div><a class="ld-pack-detail" href="${detail}">Xem chi tiết gói →</a></div></article>`}
...
function bindLionPackageCards(){document.querySelectorAll('[data-ld-price]').forEach(b=>b.onclick=()=>b.closest('.ld-pack')?.classList.toggle('show-price'))}
function bindLionHeroSlider(){const imgs=[...document.querySelectorAll('.ld-hero-slides img')],dots=[...document.querySelectorAll('.ld-hero-dots button')];if(imgs.length<2)return;let i=0,timer;const go=n=>{i=(n+imgs.length)%imgs.length;imgs.forEach((x,j)=>x.classList.toggle('active',j===i));dots.forEach((x,j)=>x.classList.toggle('active',j===i))};dots.forEach((b,j)=>b.onclick=()=>{go(j);clearInterval(timer);timer=setInterval(()=>go(i+1),4500)});timer=setInterval(()=>go(i+1),4500)}
function renderLionDanceTemplate(site={}){const root=document.querySelector('main');if(!root)return;document.querySelectorAll('footer.footer,footer.public-footer').forEach(x=>x.remove());const key='dich-vu-6',settings=sxSettings(site),cats=sxProfile(site,key),posts=(SITE_DATA?.posts||[]).filter(p=>p.status!=='draft'&&p.type==='service').sort((a,b)=>(Number(a?.is_sample||0)-Number(b?.is_sample||0))||(Number(b?.id||0)-Number(a?.id||0))),id=new URLSearchParams(location.search).get('id'),detail=id?posts.find(p=>String(p.id)===String(id)):null,brand=esc(site.name||'Đoàn Lân Sư Rồng'),phone=String(site.phone||''),email=String(site.email||site.contact_email||''),zalo=String(site.zalo||''),facebook=String(site.facebook||''),address=String(settings.contact_address||''),intro=String(settings.troupe_intro||'Đoàn Lân Sư Rồng chuyên biểu diễn khai trương, lễ hội, Trung Thu, Tết và sự kiện doanh nghiệp.'),experience=String(settings.experience_years||'15+ năm'),eventCount=String(settings.event_count||'800+ sự kiện'),memberCount=String(settings.member_count||'35+ thành viên'),awards=[settings.award_1,settings.award_2,settings.award_3].filter(Boolean),footerIntro=String(settings.footer_intro||intro),footerPhone=String(settings.footer_phone||phone||''),footerEmail=String(settings.footer_email||email||''),privacyUrl=String(settings.privacy_url||'#'),termsUrl=String(settings.terms_url||'#'),copyright=String(settings.footer_copyright||('© 2026 '+(site.name||'Đoàn Lân Sư Rồng')));const direct=[phone?`<a href="tel:${esc(phone.replace(/\s/g,''))}">☎ ${esc(phone)}</a>`:'',email?`<a href="mailto:${esc(email)}">✉ ${esc(email)}</a>`:'',zalo?`<a href="https://zalo.me/${esc(zalo.replace(/\D/g,''))}" target="_blank" rel="noopener">Zalo</a>`:'',facebook?`<a href="${esc(facebook)}" target="_blank" rel="noopener">Facebook</a>`:''].filter(Boolean).join('');if(detail){const imgs=getImages(detail);const e=lionExtra(detail);root.innerHTML=`<div class="ld-site"><header class="ld-header"><div class="ld-wrap"><a class="ld-brand" href="/">${brand}<small>LÂN · SƯ · RỒNG</small></a><nav><a href="/">Trang chủ</a><a href="/#packages">Gói dịch vụ</a><a href="/#experience">Năng lực</a><a href="/#contact">Liên hệ</a></nav><a class="ld-admin" href="${sxAdminNewPostUrl(key)}" target="_blank" rel="noopener">＋ Đăng bài</a></div></header><article class="ld-detail"><div class="ld-wrap"><a class="ld-back" href="/">← Trang chủ</a><small>${esc(detail.category||'Dịch vụ')}</small><h1>${esc(detail.title||'Dịch vụ')}</h1><p>${esc(sxText(detail.content||'').slice(0,220))}</p>${lionGallery(imgs,detail.title||'')}<div class="ld-detail-grid"><div class="ld-copy">${detail.content||'<p>Nội dung đang được cập nhật.</p>'}</div><aside><h3>Thông tin gói</h3>${Object.entries({"Giá":e.service_price,"Đội hình":e.lion_count,"Trống & bộ gõ":e.drum_count,"Nhân sự":e.performers_count,"Thời lượng":e.performance_duration,"Pháo sáng":e.fireworks,"Kim tuyến":e.confetti,"Câu đối":e.couplets}).filter(x=>x[1]).map(([k,v])=>`<div><small>${esc(k)}</small><b>${esc(v)}</b></div>`).join('')}<a href="/#contact">Liên hệ báo giá</a></aside></div></div></article><footer class="ld-footer"><div class="ld-wrap ld-footer-grid"><div><b>${brand}</b><p>${esc(footerIntro)}</p></div><div><b>Dịch vụ</b><a href="/#packages">2–7 đầu lân</a><a href="/#packages">Múa rồng</a><a href="/#packages">Trống hội</a></div><div><b>Liên hệ</b>${footerPhone?`<a href="tel:${esc(footerPhone.replace(/\s/g,''))}">☎ ${esc(footerPhone)}</a>`:''}${footerEmail?`<a href="mailto:${esc(footerEmail)}">✉ ${esc(footerEmail)}</a>`:''}${address?`<span>${esc(address)}</span>`:''}</div><div><b>Thông tin</b><a href="/#experience">Giới thiệu đoàn</a><a href="${esc(privacyUrl)}">Chính sách bảo mật</a><a href="${esc(termsUrl)}">Điều khoản sử dụng</a></div></div><div class="ld-wrap ld-footer-bottom">${esc(copyright)}</div></footer><a class="ld-float" href="/#contact">☎ Liên hệ báo giá</a></div>`;bindLionGallery();return}const packages=posts.filter(x=>x.category==='Gói múa lân').slice(0,6),dragon=posts.filter(x=>x.category==='Múa rồng').slice(0,3),drums=posts.filter(x=>x.category==='Trống hội').slice(0,3),events=posts.filter(x=>x.category==='Sự kiện đã thực hiện').slice(0,6),blog=posts.filter(x=>['Tin hoạt động','Kiến thức & phong tục'].includes(x.category)).slice(0,6),featured=posts[0],heroImgs=[...new Set(posts.flatMap(p=>getImages(p)).filter(Boolean))].slice(0,5),customHeroSlides=(()=>{const v=settings.hero_slides;if(Array.isArray(v))return v.filter(Boolean);try{const a=JSON.parse(String(v||'[]'));return Array.isArray(a)?a.filter(Boolean):[]}catch{return []}})(),hero=settings.hero_image||heroImgs[0]||'/assets/marketing-demo.webp',displayHeroImgs=customHeroSlides.length?customHeroSlides:(heroImgs.length?heroImgs:[hero]);root.innerHTML=`<div class="ld-site"><header class="ld-header"><div class="ld-wrap"><a class="ld-brand" href="/">${brand}<small>LÂN · SƯ · RỒNG</small></a><nav><a href="#packages">Gói dịch vụ</a><a href="#experience">Năng lực</a><a href="#events">Sự kiện</a><a href="#blog">Bài viết</a><a href="#contact">Liên hệ</a></nav><a class="ld-admin" href="${sxAdminNewPostUrl(key)}" target="_blank" rel="noopener">＋ Đăng bài</a></div></header><section class="ld-hero" style="--ld-hero:url('${esc(hero)}')"><div class="ld-hero-slides">${displayHeroImgs.map((x,i)=>`<img class="${i===0?'active':''}" src="${esc(x)}" alt="${brand} ${i+1}">`).join('')}</div><div class="ld-wrap"><small>LÂN SƯ RỒNG · BIỂU DIỄN SỰ KIỆN</small><h1>${esc(settings.hero_title||site.name||'Đoàn Lân Sư Rồng')}</h1><p>${esc(settings.hero_description||settings.hero_note||'Khai hội rộn ràng – đội hình linh hoạt từ 2 đến 7 đầu lân, múa rồng, trống hội và hiệu ứng theo kịch bản.')}</p><div class="ld-actions"><a href="#packages">${esc(settings.hero_cta_primary||'Xem gói biểu diễn')}</a><a class="alt" href="#contact">${esc(settings.hero_cta_secondary||'Liên hệ báo giá')}</a></div><div class="ld-stats"><div><b>${esc(experience)}</b><span>Kinh nghiệm</span></div><div><b>${esc(eventCount)}</b><span>Sự kiện</span></div><div><b>${esc(memberCount)}</b><span>Thành viên</span></div><div><b>24/7</b><span>Nhận lịch</span></div></div></div><div class="ld-hero-dots">${displayHeroImgs.map((_,i)=>`<button class="${i===0?'active':''}" data-ld-slide="${i}" aria-label="Ảnh ${i+1}"></button>`).join('')}</div></section><section class="ld-section" id="packages"><div class="ld-wrap"><div class="ld-title"><div><small>GÓI DỊCH VỤ</small><h2>Từ 2 đến 7 đầu lân</h2></div><p>Mỗi gói hiển thị rõ đội hình, trống, số người, hiệu ứng và giá để khách dễ lựa chọn.</p></div><div class="ld-pack-grid">${packages.length?packages.map(lionServiceCard).join(''):'<div class="sxp-empty">Chưa có gói dịch vụ. Vào Trang quản trị để đăng các gói 2–7 đầu lân.</div>'}</div></div></section><section class="ld-section ld-soft"><div class="ld-wrap"><div class="ld-title"><div><small>TIẾT MỤC MỞ RỘNG</small><h2>Múa rồng & Trống hội</h2></div></div><div class="ld-feature-grid">${[...dragon,...drums].map(p=>`<a href="${sxPostUrl(p)}">${p.image?`<img src="${esc(getImages(p)[0]||p.image)}" alt="${esc(p.title)}">`:''}<div><small>${esc(p.category)}</small><h3>${esc(p.title)}</h3><p>${esc(sxText(p.content).slice(0,120))}</p></div></a>`).join('')||'<div class="sxp-empty">Đăng bài Múa rồng và Trống hội để hiển thị ở đây.</div>'}</div></div></section><section class="ld-section ld-trust" id="experience"><div class="ld-wrap ld-trust-grid"><div><small>GIỚI THIỆU ĐOÀN</small><h2>Kinh nghiệm, kỷ luật và hình ảnh chuyên nghiệp.</h2><p>${esc(intro)}</p><div class="ld-actions"><a href="#contact">Nhận tư vấn</a><a class="alt" href="#events">Xem sự kiện</a></div></div><div class="ld-awards">${awards.length?awards.map((a,i)=>`<div><b>0${i+1} · THÀNH TÍCH</b><span>${esc(a)}</span></div>`).join(''):'<div><b>DANH HIỆU & THÀNH TÍCH</b><span>Cập nhật các giải thưởng, bằng khen và cột mốc nổi bật trong Trang quản trị.</span></div>'}</div></div></section><section class="ld-section" id="events"><div class="ld-wrap"><div class="ld-title"><div><small>SỰ KIỆN GẦN ĐÂY</small><h2>Những chương trình đã thực hiện</h2></div><p>Mỗi bài có thể tải nhiều ảnh. Trang chi tiết sẽ tự tạo gallery/slideshow.</p></div><div class="ld-event-grid">${events.map(sxCard).join('')||'<div class="sxp-empty">Chưa có sự kiện. Hãy đăng bài và chọn chuyên mục “Sự kiện đã thực hiện”.</div>'}</div></div></section><section class="ld-section ld-soft" id="blog"><div class="ld-wrap"><div class="ld-title"><div><small>BLOG & KINH NGHIỆM</small><h2>Nội dung giúp khách tin tưởng hơn</h2></div></div><div class="ld-blog-grid">${blog.map(sxCard).join('')||'<div class="sxp-empty">Chưa có bài blog.</div>'}</div></div></section><section class="ld-section ld-contact" id="contact"><div class="ld-wrap ld-contact-grid"><div><small>LIÊN HỆ BÁO GIÁ</small><h2>${esc(settings.contact_title||'Đặt lịch Lân Sư Rồng cho sự kiện của bạn')}</h2><p>${esc(settings.contact_intro||'Gửi thời gian, địa điểm và quy mô mong muốn. Đoàn sẽ liên hệ để tư vấn đội hình phù hợp.')}</p><div class="ld-direct">${direct||'<span>Hãy cập nhật Hotline, Email, Zalo và Facebook trong Trang quản trị.</span>'}</div>${address?`<p><b>Địa chỉ:</b> ${esc(address)}</p>`:''}</div><form id="sxpContactForm" class="ld-contact-form"><input name="customer_name" required placeholder="Họ và tên *"><input name="phone" required inputmode="tel" placeholder="Số điện thoại *"><input name="province" placeholder="Tỉnh / Thành phố"><input name="district" placeholder="Quận / Huyện"><textarea name="need" rows="5" placeholder="Ngày tổ chức, địa điểm, số đầu lân hoặc nhu cầu chương trình"></textarea><button type="submit">${esc(settings.contact_cta||'Gửi yêu cầu báo giá')} →</button><div id="sxpContactMsg" class="sxp-contact-msg"></div></form></div></section><footer class="ld-footer"><div class="ld-wrap ld-footer-grid"><div><b>${brand}</b><p>${esc(footerIntro)}</p></div><div><b>Dịch vụ</b><a href="#packages">2–7 đầu lân</a><a href="#packages">Múa rồng</a><a href="#packages">Trống hội</a></div><div><b>Liên hệ</b>${footerPhone?`<a href="tel:${esc(footerPhone.replace(/\s/g,''))}">☎ ${esc(footerPhone)}</a>`:''}${footerEmail?`<a href="mailto:${esc(footerEmail)}">✉ ${esc(footerEmail)}</a>`:''}${address?`<span>${esc(address)}</span>`:''}</div><div><b>Thông tin</b><a href="#experience">Giới thiệu đoàn</a><a href="${esc(privacyUrl)}">Chính sách bảo mật</a><a href="${esc(termsUrl)}">Điều khoản sử dụng</a></div></div><div class="ld-wrap ld-footer-bottom">${esc(copyright)}</div></footer><a class="ld-float" href="#contact">☎ Liên hệ báo giá</a></div>`;bindLionPackageCards();bindLionHeroSlider();sxBindContact(site,key);try{nrApplyStructureGeometry(site,key);nrAuditStructureContract(site,key)}catch(e){}}

// V20.9.27.44 — Universal Commerce frontend. Laptop is sample data, not engine logic.
function comApi(path){const q=new URLSearchParams(),cur=new URLSearchParams(location.search);for(const k of ['tenant','nr_trial'])if(cur.get(k))q.set(k,cur.get(k));return '/api/commerce/'+path+(q.toString()?'?'+q:'')}
function comMoney(n){return new Intl.NumberFormat('vi-VN').format(Number(n||0))+'đ'}
function comCartGet(){try{return JSON.parse(localStorage.getItem('nr_commerce_cart')||'[]')}catch{return []}}
function comCartSet(x){localStorage.setItem('nr_commerce_cart',JSON.stringify(x));comCartRender()}
let COM_DATA={products:[],categories:[],settings:{}};
function comSelectedVariant(p){let vs=[];try{vs=JSON.parse(p.variants_json||'[]')}catch{};const el=document.querySelector('input[name="comVariant"]:checked');return vs.length?{index:Number(el?.value||0),data:vs[Number(el?.value||0)]||vs[0]}:null}
// Backward compatibility contract: Mã giảm giá / voucher remains supported by API, but V15 removes it from the customer-facing checkout UI.
function comEnsureCart(){if(document.getElementById('comCart'))return;const x=document.createElement('aside');x.className='com-cart';x.id='comCart';x.innerHTML=`<div class="com-cart-head"><b>Giỏ hàng & thanh toán</b><button onclick="document.getElementById('comCart').classList.remove('open')">✕</button></div><div class="com-cart-list" id="comCartList"></div><div class="com-cart-foot"><div class="com-order-summary"><p>Tạm tính <b id="comCartTotal">0đ</b></p></div><form class="com-checkout" onsubmit="comCheckout(event)"><h3>Thông tin nhận hàng</h3><input name="customer_name" required placeholder="Họ và tên *"><input name="phone" required placeholder="Số điện thoại *"><input name="email" type="email" placeholder="Email nhận xác nhận đơn"><div class="com-address-grid"><input name="province" required placeholder="Tỉnh / Thành phố *"><input name="district" required placeholder="Quận / Huyện *"><input name="ward" required placeholder="Phường / Xã *"></div><input name="address" required placeholder="Số nhà, tên đường *"><h3>Vận chuyển</h3><label><input type="radio" name="shipping_method" value="standard" checked> Giao hàng tiêu chuẩn</label><label><input type="radio" name="shipping_method" value="express"> Giao nhanh</label><h3>Thanh toán</h3><div class="com-payment-options" id="comPaymentOptions"></div><textarea name="note" placeholder="Ghi chú đơn hàng"></textarea><button type="submit">Xác nhận đặt hàng</button><div id="comCheckoutMsg"></div></form></div>`;document.body.appendChild(x);const d=COM_DATA.settings||{},ps=[];if(Number(d.cod_enabled??1))ps.push('<label><input type="radio" name="payment_method" value="cod" checked> Thanh toán khi nhận hàng (COD)</label>');if(Number(d.bank_enabled??1))ps.push('<label><input type="radio" name="payment_method" value="bank"> Chuyển khoản / VietQR</label>');if(Number(d.store_pickup_enabled??1))ps.push('<label><input type="radio" name="payment_method" value="pickup"> Thanh toán tại cửa hàng</label>');if(Number(d.online_enabled))ps.push('<label><input type="radio" name="payment_method" value="online"> Thanh toán online</label>');document.getElementById('comPaymentOptions').innerHTML=ps.join('');comCartRender()}
function comAdd(id,buyNow=false){const p=COM_DATA.products.find(x=>Number(x.id)===Number(id));if(!p)return;const sv=comSelectedVariant(p),v=sv?.data||null,stock=v?Number(v.stock_qty??v.stock??p.stock_qty??0):Number(p.stock_qty||0);if(p.stock_status==='out_of_stock'||stock<=0){alert('Biến thể này hiện đã hết hàng');return}const variantKey=v?String(v.sku||v.name||v.label||sv.index):'',key=String(p.id)+'::'+variantKey;let c=comCartGet(),x=c.find(x=>String(x.key||x.product_id)===key);const price=Number(v?.price||p.price);if(x)x.qty=Math.min(stock,Number(x.qty||1)+1);else c.push({key,product_id:p.id,name:p.name,price,qty:1,variant_key:variantKey,variant_name:v?.name||v?.label||'',sku:v?.sku||p.sku||'',image_url:v?.image_url||v?.image||p.image_url||''});comCartSet(c);comEnsureCart();if(buyNow||document.getElementById('comCart'))document.getElementById('comCart').classList.add('open')}
function comRemove(key){comCartSet(comCartGet().filter(x=>String(x.key||x.product_id)!==String(key)))}
function comQty(key,delta){let c=comCartGet(),x=c.find(x=>String(x.key||x.product_id)===String(key));if(!x)return;x.qty=Math.max(1,Math.min(99,Number(x.qty||1)+delta));comCartSet(c)}
function comCartRender(){const c=comCartGet(),list=document.getElementById('comCartList'),count=document.getElementById('comCartCount'),subtotal=c.reduce((a,x)=>a+Number(x.price)*Number(x.qty),0);if(count)count.textContent=c.reduce((a,x)=>a+Number(x.qty),0);const detailCount=document.getElementById('comDetailCartCount');if(detailCount)detailCount.textContent=c.reduce((a,x)=>a+Number(x.qty),0);if(list)list.innerHTML=c.length?c.map(x=>`<div class="com-cart-item">${x.image_url?`<img src="${esc(x.image_url)}" alt="">`:''}<div><b>${esc(x.name)}</b>${x.variant_name?`<small>${esc(x.variant_name)} · ${esc(x.sku||'')}</small>`:''}<small>${comMoney(x.price)}</small><div class="com-qty"><button onclick="comQty('${esc(x.key||x.product_id)}',-1)">−</button><span>${x.qty}</span><button onclick="comQty('${esc(x.key||x.product_id)}',1)">+</button></div></div><b>${comMoney(Number(x.price)*Number(x.qty))}</b><button onclick="comRemove('${esc(x.key||x.product_id)}')">Xóa</button></div>`).join(''):'<p>Giỏ hàng đang trống.</p>';const t=document.getElementById('comCartTotal');if(t)t.textContent=comMoney(subtotal);const sm=document.getElementById('comSummarySubtotal');if(sm)sm.textContent=comMoney(subtotal)}
function comProductUrl(p){return location.pathname.replace(/\/san-pham\/.*$/,'').replace(/\/$/,'')+'/san-pham/'+encodeURIComponent(p.slug||p.id)+'/' }
const COM_PAGE_SIZE=8;let COM_PAGE=1;
function comProductCards(items=[]){return (items||[]).map(p=>{const old=Number(p.compare_at_price||0),price=Number(p.price||0),badge=String(p.badge||''),img=String(p.image_url||'');return `<article class="com-card">${img?`<a class="com-card-img" href="${comProductUrl(p)}"><img src="${esc(img)}" alt="${esc(p.name)}" loading="lazy"></a>`:''}<div class="com-card-body">${badge?`<small class="com-badge">${esc(badge)}</small>`:''}<small>${esc(p.category_name||'Sản phẩm')}</small><h3><a href="${comProductUrl(p)}">${esc(p.name)}</a></h3><p>${esc(p.short_description||'')}</p><div class="com-card-price"><b>${comMoney(price)}</b>${old>price?`<del>${comMoney(old)}</del>`:''}</div><div class="com-card-actions"><a href="${comProductUrl(p)}">Xem chi tiết</a><button type="button" onclick="comAdd(${Number(p.id)},false)">+ Giỏ hàng</button></div></div></article>`}).join('')}
function comPagination(total){const pages=Math.max(1,Math.ceil(total/COM_PAGE_SIZE));if(COM_PAGE>pages)COM_PAGE=pages;if(pages<=1)return '';let out='<nav class="com-pagination" aria-label="Phân trang">';out+=`<button ${COM_PAGE<=1?'disabled':''} onclick="comGoPage(${COM_PAGE-1})">‹</button>`;for(let i=1;i<=pages;i++)out+=`<button class="${i===COM_PAGE?'active':''}" onclick="comGoPage(${i})">${i}</button>`;out+=`<button ${COM_PAGE>=pages?'disabled':''} onclick="comGoPage(${COM_PAGE+1})">›</button></nav>`;return out}
function comGoPage(n){COM_PAGE=Math.max(1,Number(n)||1);comFilter(false);document.getElementById('products')?.scrollIntoView({behavior:'smooth',block:'start'})}
function comEnsureMobileCss(){if(document.getElementById('comV14Css'))return;const st=document.createElement('style');st.id='comV14Css';st.textContent=`.com-pagination{display:flex;gap:8px;justify-content:center;align-items:center;margin:28px 0 10px;flex-wrap:wrap}.com-pagination button{min-width:40px;height:40px;border:1px solid #d9e0ea;background:#fff;border-radius:10px;font-weight:800}.com-pagination button.active{background:#0f172a;color:#fff}.com-pagination button:disabled{opacity:.35}.com-discovery{margin:32px 0;padding:24px;border-radius:22px;background:#0f172a;color:#fff}.com-discovery h2{margin:0 0 8px}.com-discovery-links{display:flex;gap:10px;flex-wrap:wrap;margin-top:16px}.com-discovery-links button{border:0;border-radius:999px;padding:10px 14px;background:#fff;color:#0f172a;font-weight:800}.com-card-img{display:block;aspect-ratio:1/1;overflow:hidden;background:#f3f5f8}.com-card-img img{width:100%;height:100%;object-fit:cover}.com-card-body{padding:14px}.com-card-body h3{margin:7px 0}.com-card-body h3 a{text-decoration:none;color:inherit}.com-card-body p{display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}.com-card-price{display:flex;gap:8px;align-items:center;flex-wrap:wrap}.com-card-actions{display:flex;gap:8px;margin-top:12px}.com-card-actions>*{flex:1;text-align:center;padding:10px 8px;border-radius:10px;border:0;text-decoration:none;font-weight:800}.com-card-actions a{background:#eef2ff}.com-card-actions button{background:#0f172a;color:#fff}.com-cart{overflow:hidden}.com-cart-foot{overflow-y:auto;overscroll-behavior:contain}.com-checkout{padding-bottom:max(24px,env(safe-area-inset-bottom))}@media(max-width:680px){.com-wrap{padding-left:10px!important;padding-right:10px!important}.com-head .com-wrap{gap:8px}.com-admin-link{display:none!important}.com-search{order:3;width:100%}.com-cats{display:flex;overflow-x:auto;flex-wrap:nowrap;scrollbar-width:none;padding-bottom:5px}.com-cats::-webkit-scrollbar{display:none}.com-cat{flex:0 0 auto}.com-toolbar{align-items:center}.com-grid{grid-template-columns:repeat(2,minmax(0,1fr))!important;gap:8px!important}.com-card-body{padding:9px}.com-card-body small,.com-card-body p{font-size:11px}.com-card-body h3{font-size:14px;line-height:1.25}.com-card-price b{font-size:14px}.com-card-price del{font-size:10px}.com-card-actions{gap:5px}.com-card-actions>*{font-size:11px;padding:8px 4px}.com-discovery{margin:18px 0;padding:16px}.com-cart{width:100%!important;max-width:none!important;height:100dvh!important;max-height:100dvh!important;top:0!important;bottom:auto!important;display:flex!important;flex-direction:column}.com-cart-head{flex:0 0 auto}.com-cart-list{flex:0 0 auto;max-height:28dvh;overflow:auto}.com-cart-foot{flex:1 1 auto;min-height:0;overflow-y:auto!important;-webkit-overflow-scrolling:touch;padding-bottom:80px!important}.com-address-grid{grid-template-columns:1fr!important}.com-checkout input,.com-checkout textarea,.com-checkout button{min-height:46px;font-size:16px}.com-product-main,.com-product-content{grid-template-columns:1fr!important}.com-product-page .com-grid{grid-template-columns:repeat(2,minmax(0,1fr))!important}}.commerce21{width:100vw!important;max-width:none!important;margin-left:calc(50% - 50vw)!important;background:#fff!important;--accent:#0b57d0;--ink:#101828;--line:#e4e9f0;--soft:#f5f7fb}.c21-wrap{width:min(1280px,calc(100% - 48px))!important;max-width:none!important}.c21-ann{background:#0f172a!important;padding:8px 16px!important}.c21-head{box-shadow:0 1px 0 var(--line);border:0!important}.c21-headrow{height:78px!important;grid-template-columns:210px minmax(420px,1fr) auto!important}.c21-brand{font-size:27px!important;color:var(--ink)!important}.c21-brand em{color:var(--accent)!important}.c21-search{height:48px!important;border:2px solid var(--accent)!important;background:#fff!important;border-radius:10px!important}.c21-search button{background:var(--accent)!important}.c21-actions .bag{background:var(--ink)!important}.c21-nav{border-top:1px solid #eef1f5!important}.c21-nav .c21-wrap{height:46px!important}.c21-hero{background:var(--soft)!important;padding:24px 0 18px!important}.c21-hero-grid{grid-template-columns:minmax(0,1.65fr) minmax(280px,.55fr)!important;gap:16px!important}.c21-hero-main{min-height:390px!important;border-radius:18px!important;background:linear-gradient(135deg,#eaf2ff,#f8fbff 58%,#e8f7ef)!important;padding:46px!important}.c21-hero-main h1{font-size:48px!important;line-height:1.03!important;color:var(--ink)!important;letter-spacing:-2px!important}.c21-kicker{background:#fff!important;color:var(--accent)!important;padding:7px 10px!important;border-radius:999px!important;display:inline-flex!important}.c21-cta{background:var(--accent)!important;border-radius:9px!important}.c21-promos{gap:16px!important}.c21-promo{border-radius:18px!important;min-height:187px!important;background:#fff1df!important}.c21-promo:last-child{background:#e7f6ee!important}.c21-category-section{padding:34px 0 20px!important}.c21-section-head h2{font-size:27px!important;color:var(--ink)!important}.c21-categories{grid-template-columns:repeat(9,1fr)!important;gap:10px!important}.c21-categories button{min-height:96px!important;border:1px solid var(--line)!important;background:#fff!important;border-radius:14px!important;box-shadow:0 4px 14px rgba(16,24,40,.035)}.c21-categories button:hover{border-color:#9ec0f6!important;background:#f7faff!important}.c21-trust{width:min(1280px,calc(100% - 48px))!important;border:1px solid var(--line)!important;border-radius:14px!important;margin:14px auto 0!important}.c21-products{background:#f7f8fa!important;padding:38px 0 56px!important}.c21-pills .com-cat.active{background:var(--accent)!important;border-color:var(--accent)!important}.commerce21 .com-grid{grid-template-columns:repeat(4,minmax(0,1fr))!important;gap:18px!important}.commerce21 .com-card{border:1px solid var(--line)!important;border-radius:14px!important;box-shadow:0 4px 16px rgba(16,24,40,.04)!important}.commerce21 .com-card:hover{transform:translateY(-3px);box-shadow:0 14px 30px rgba(16,24,40,.09)!important}.commerce21 .com-card-img{border-radius:0!important;background:#f1f3f6!important}.commerce21 .com-card-body{padding:14px!important}.commerce21 .com-card-actions button{background:var(--accent)!important;border-radius:8px!important}.commerce21 .com-card-actions a{border-radius:8px!important}.commerce21 .com-pagination button.active{background:var(--accent)!important;border-color:var(--accent)!important}.c21-newsletter{width:min(1280px,calc(100% - 48px))!important;background:linear-gradient(100deg,#0b57d0,#3b82f6)!important;color:#fff!important;border-radius:18px!important;margin-top:40px!important}.c21-newsletter p{color:#dbeafe!important}.c21-newsletter a{background:#fff!important;color:#0b57d0!important;border-radius:9px!important}.c21-footer{background:#0f172a!important}.commerce21 .com-cart{z-index:100!important}
@media(max-width:1050px){.c21-categories{grid-template-columns:repeat(6,1fr)!important}.commerce21 .com-grid{grid-template-columns:repeat(3,1fr)!important}.c21-headrow{grid-template-columns:170px 1fr auto!important}}
@media(max-width:640px){.c21-wrap{width:calc(100% - 20px)!important}.c21-headrow{grid-template-columns:1fr auto!important;height:auto!important}.c21-search{grid-column:1/-1!important}.c21-hero{padding-top:10px!important}.c21-hero-grid{grid-template-columns:1fr!important}.c21-hero-main{min-height:390px!important;padding:22px 18px!important}.c21-hero-main h1{font-size:32px!important;letter-spacing:-1px!important}.c21-promos{display:none!important}.c21-categories{display:flex!important;overflow:auto!important}.c21-categories button{flex:0 0 108px!important}.c21-trust{width:calc(100% - 20px)!important;grid-template-columns:1fr 1fr!important}.commerce21 .com-grid{grid-template-columns:repeat(2,minmax(0,1fr))!important;gap:9px!important}.c21-newsletter{width:calc(100% - 20px)!important}}
`;document.head.appendChild(st)}
function comDetail(id){const p=COM_DATA.products.find(x=>Number(x.id)===Number(id));if(p)location.href=comProductUrl(p)}
function comVariantSync(p){let vs=[];try{vs=JSON.parse(p.variants_json||'[]')}catch{};if(!vs.length)return;const sv=comSelectedVariant(p),v=sv?.data||vs[0],price=Number(v.price||p.price),stock=Number(v.stock_qty??v.stock??0);const pr=document.getElementById('comDetailPrice'),sku=document.getElementById('comDetailSku'),st=document.getElementById('comDetailStock'),im=document.getElementById('comMainImage');if(pr)pr.textContent=comMoney(price);if(sku)sku.textContent='SKU '+esc(v.sku||p.sku||'—');if(st){st.textContent=stock>0?'✓ Còn hàng · '+stock+' sản phẩm':'✕ Biến thể tạm hết hàng';st.classList.toggle('out',stock<=0)}if(im&&(v.image_url||v.image))im.src=v.image_url||v.image;document.querySelectorAll('.com-buy-actions button').forEach(b=>b.disabled=stock<=0)}
function comRenderProductDetail(p){let attrs={};try{attrs=JSON.parse(p.attributes_json||'{}')}catch{};let gallery=[];try{gallery=JSON.parse(p.gallery_json||'[]')}catch{};gallery=[p.image_url,...gallery].filter(Boolean);let variants=[];try{variants=JSON.parse(p.variants_json||'[]')}catch{};const root=document.querySelector('main');if(!root)return;root.innerHTML=`<header class="com-detail-head"><div class="com-wrap"><a class="com-brand" href="${location.pathname.replace(/\/san-pham\/.*$/,'')}">← Cửa hàng</a><div class="com-detail-tools"><span>${esc(p.category_name||'Sản phẩm')}</span><button class="com-cart-btn" onclick="document.getElementById('comCart').classList.add('open')">🛒 Giỏ hàng (<span id="comDetailCartCount">0</span>)</button></div></div></header><div class="com-product-page com-wrap"><nav class="com-breadcrumb"><a href="${location.pathname.replace(/\/san-pham\/.*$/,'')}">Cửa hàng</a> / ${esc(p.category_name||'Sản phẩm')} / ${esc(p.name)}</nav><section class="com-product-main"><div class="com-gallery"><img class="com-gallery-main" id="comMainImage" src="${esc(gallery[0]||'')}">${gallery.length>1?`<div class="com-thumbs">${gallery.map(x=>`<button onclick="document.getElementById('comMainImage').src='${esc(x)}'"><img src="${esc(x)}"></button>`).join('')}</div>`:''}</div><div class="com-buybox"><small>${esc(p.category_name||'SẢN PHẨM')} · <span id="comDetailSku">SKU ${esc(p.sku||'—')}</span></small><h1>${esc(p.name)}</h1><div class="com-stars">★★★★★ <span>4.8 · Đã bán 120+</span></div><p>${esc(p.short_description||'')}</p><div class="com-product-price"><strong id="comDetailPrice">${comMoney(p.price)}</strong>${Number(p.compare_at_price)>Number(p.price)?`<del>${comMoney(p.compare_at_price)}</del>`:''}</div><div class="com-stock" id="comDetailStock">✓ ${p.stock_status==='out_of_stock'?'Tạm hết hàng':'Còn hàng · '+Number(p.stock_qty||0)+' sản phẩm'}</div>${variants.length?`<div class="com-variants"><b>Phiên bản</b>${variants.map((v,i)=>`<label><input type="radio" name="comVariant" value="${i}" onchange="comVariantSync(COM_DATA.products.find(x=>Number(x.id)===${p.id}))" ${i?'':'checked'}> ${esc(v.name||v.label||('Biến thể '+(i+1)))}</label>`).join('')}</div>`:''}<div class="com-buy-actions"><button onclick="comAdd(${p.id},false)">Thêm vào giỏ</button><button onclick="comAdd(${p.id},true)">Mua ngay</button></div><div class="com-trust">🚚 Giao hàng linh hoạt　↩ Đổi trả theo chính sách　🛡 Bảo hành rõ ràng</div></div></section><section class="com-product-content"><article><h2>Mô tả sản phẩm</h2>${p.description||`<p>${esc(p.short_description||'Thông tin sản phẩm đang được cập nhật.')}</p>`}</article><aside><h2>Thông số / thuộc tính</h2><table>${Object.entries(attrs).map(([k,v])=>`<tr><th>${esc(k.replaceAll('_',' '))}</th><td>${esc(Array.isArray(v)?v.join(', '):v)}</td></tr>`).join('')||'<tr><td>Đang cập nhật thông số.</td></tr>'}</table></aside></section><section><h2>Sản phẩm liên quan</h2><div class="com-grid">${comProductCards(COM_DATA.products.filter(x=>x.id!==p.id&&(x.category_id===p.category_id||x.category_slug===p.category_slug)).slice(0,4))}</div></section></div>`;comEnsureCart();comVariantSync(p);comCartRender()}

function comFilter(resetPage=true){if(resetPage)COM_PAGE=1;const q=String(document.getElementById('comSearch')?.value||'').toLowerCase(),cat=String(document.querySelector('.com-cat.active')?.dataset.slug||''),sort=document.getElementById('comSort')?.value||'featured';let a=COM_DATA.products.filter(p=>(!q||(p.name+' '+p.sku+' '+p.short_description).toLowerCase().includes(q))&&(!cat||p.category_slug===cat));if(sort==='price_asc')a.sort((x,y)=>x.price-y.price);if(sort==='price_desc')a.sort((x,y)=>y.price-x.price);if(sort==='newest')a.sort((x,y)=>y.id-x.id);const start=(COM_PAGE-1)*COM_PAGE_SIZE,page=a.slice(start,start+COM_PAGE_SIZE),grid=document.getElementById('comGrid');if(grid)grid.innerHTML=comProductCards(page)+(a.length?comPagination(a.length):'<p>Chưa có sản phẩm phù hợp.</p>');const c=document.getElementById('comResultCount');if(c)c.textContent=a.length+' sản phẩm'}
async function comCheckout(e){e.preventDefault();const f=e.target,c=comCartGet(),b=Object.fromEntries(new FormData(f).entries());b.items=c;try{const r=await fetch(comApi('checkout'),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(b)}),d=await r.json();if(!r.ok)throw new Error(d.error||'Không đặt được hàng');comCartSet([]);document.getElementById('comCheckoutMsg').innerHTML=`<b>✓ Đặt hàng thành công</b><br>Mã đơn: <strong>${esc(d.order_code)}</strong><br>Tổng thanh toán: ${comMoney(d.total_amount)}<br><small>Lưu mã đơn và số điện thoại để tra cứu trạng thái đơn hàng.</small>${d.bank?`<br>Chuyển khoản: ${esc(d.bank.bank_name)} · ${esc(d.bank.account)} · ${esc(d.bank.holder)}`:''}`;f.reset()}catch(err){document.getElementById('comCheckoutMsg').textContent=err.message}}
function comEnsureV15Css(){if(document.getElementById('comV15Css'))return;const st=document.createElement('style');st.id='comV15Css';st.textContent=`
.com-store{background:#f6f7fb;color:#111827;min-height:100vh}.com-wrap{width:min(1180px,calc(100% - 32px));margin:auto}.com-head{position:sticky;top:0;z-index:30;background:rgba(255,255,255,.96);backdrop-filter:blur(16px);border-bottom:1px solid #e5e7eb}.com-head .com-wrap{min-height:72px;display:flex;align-items:center;gap:16px}.com-brand{font-size:21px;font-weight:950;color:#111827;text-decoration:none;white-space:nowrap}.com-search{display:flex;flex:1;max-width:620px;margin-left:auto;border:1px solid #dbe1ea;background:#f8fafc;border-radius:14px;overflow:hidden}.com-search input{flex:1;border:0;background:transparent;padding:13px 15px;outline:0;min-width:0}.com-search button,.com-cart-btn{border:0;background:#111827;color:#fff;padding:12px 16px;font-weight:850;border-radius:12px}.com-admin-link{font-weight:800;text-decoration:none;color:#475569}.com-hero{padding:28px 0 18px;background:linear-gradient(135deg,#0b1220,#16233c);color:#fff}.com-featured-shell,.com-hero-grid{min-height:390px;position:relative;border-radius:28px;overflow:hidden}.com-featured-shell{background:linear-gradient(120deg,#101827,#172554)}#comFeaturedSlide{height:100%}#comFeaturedSlide>div,.com-featured-shell #comFeaturedSlide{min-height:390px}.com-featured-shell #comFeaturedSlide{display:grid}.com-featured-shell #comFeaturedSlide>div{display:grid;grid-template-columns:1fr 1fr;align-items:center}.com-featured-copy{padding:48px 56px}.com-featured-copy small{font-weight:900;letter-spacing:.12em;color:#93c5fd}.com-featured-copy h1{font-size:clamp(34px,5vw,58px);line-height:1.02;margin:12px 0 16px;max-width:650px}.com-featured-copy p{font-size:17px;line-height:1.7;color:#cbd5e1;max-width:580px}.com-featured-price{display:flex;align-items:center;gap:12px;margin:20px 0}.com-featured-price b{font-size:30px;color:#fbbf24}.com-featured-price del{color:#94a3b8}.com-featured-actions{display:flex;gap:10px}.com-featured-actions a{padding:12px 17px;border-radius:12px;background:#fff;color:#111827;text-decoration:none;font-weight:900}.com-featured-actions a:last-child{background:#2563eb;color:#fff}.com-featured-shell img{width:100%;height:390px;object-fit:cover}.com-featured-prev,.com-featured-next{position:absolute;top:50%;transform:translateY(-50%);width:42px;height:42px;border:0;border-radius:50%;background:#fff;color:#111827;font-size:25px}.com-featured-prev{left:14px}.com-featured-next{right:14px}.com-featured-dots{position:absolute;bottom:16px;left:50%;transform:translateX(-50%);display:flex;gap:7px}.com-featured-dots button{width:9px;height:9px;padding:0;border:0;border-radius:99px;background:#94a3b8}.com-featured-dots button.active{width:28px;background:#fff}.com-trustbar{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin:18px auto 26px}.com-trustbar div{background:#fff;border:1px solid #e5e7eb;border-radius:16px;padding:16px}.com-trustbar b{display:block;margin-bottom:4px}.com-trustbar span{font-size:13px;color:#64748b}.com-cats{display:flex;gap:8px;flex-wrap:wrap;margin:8px 0 20px}.com-cat{border:1px solid #dbe1ea;background:#fff;padding:10px 14px;border-radius:999px;font-weight:800}.com-cat.active{background:#111827;color:#fff;border-color:#111827}.com-toolbar{display:flex;justify-content:space-between;gap:14px;align-items:center;margin:24px 0 14px}.com-toolbar b{font-size:25px}.com-toolbar select{padding:10px 12px;border:1px solid #dbe1ea;border-radius:11px;background:#fff}.com-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:16px}.com-card{background:#fff;border:1px solid #e4e8ef;border-radius:18px;overflow:hidden;box-shadow:0 7px 24px rgba(15,23,42,.045);transition:.2s}.com-card:hover{transform:translateY(-3px);box-shadow:0 16px 38px rgba(15,23,42,.09)}.com-card-img{display:block;aspect-ratio:1/1;overflow:hidden;background:#f1f5f9}.com-card-img img{width:100%;height:100%;object-fit:cover;transition:.25s}.com-card:hover .com-card-img img{transform:scale(1.035)}.com-card-body{padding:15px}.com-card-body small{color:#64748b;font-weight:700}.com-card-body h3{font-size:16px;line-height:1.35;margin:8px 0;min-height:43px}.com-card-body h3 a{text-decoration:none;color:#111827}.com-card-body p{font-size:13px;color:#64748b;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;min-height:38px}.com-card-price{display:flex;gap:8px;align-items:center;flex-wrap:wrap}.com-card-price b{font-size:18px;color:#dc2626}.com-card-price del{font-size:12px;color:#94a3b8}.com-card-actions{display:flex;gap:7px;margin-top:12px}.com-card-actions>*{flex:1;text-align:center;padding:10px 6px;border-radius:10px;border:0;text-decoration:none;font-weight:850;font-size:12px}.com-card-actions a{background:#eef2ff;color:#3730a3}.com-card-actions button{background:#111827;color:#fff}.com-pagination{display:flex;gap:8px;justify-content:center;align-items:center;margin:30px 0;flex-wrap:wrap}.com-pagination button{min-width:40px;height:40px;border:1px solid #d9e0ea;background:#fff;border-radius:10px;font-weight:800}.com-pagination button.active{background:#111827;color:#fff}.com-pagination button:disabled{opacity:.35}.com-discovery{margin:36px 0;padding:30px;border-radius:24px;background:linear-gradient(120deg,#111827,#1e3a5f);color:#fff}.com-discovery h2{font-size:28px;margin:0 0 7px}.com-discovery p{color:#cbd5e1}.com-discovery-links{display:flex;gap:9px;flex-wrap:wrap;margin-top:16px}.com-discovery-links button{border:1px solid rgba(255,255,255,.22);border-radius:999px;padding:10px 14px;background:rgba(255,255,255,.1);color:#fff;font-weight:800}.com-footer{background:#0b1220;color:#cbd5e1;padding:42px 0}.com-footer-grid{display:grid;grid-template-columns:2fr 1fr 1fr;gap:28px}.com-footer b{color:#fff;font-size:17px}.com-cart{position:fixed;z-index:80;right:0;top:0;width:min(520px,100%);height:100dvh;background:#fff;box-shadow:-20px 0 60px rgba(15,23,42,.18);transform:translateX(110%);transition:.25s;display:flex;flex-direction:column;overflow:hidden}.com-cart.open{transform:none}.com-cart-head{padding:18px 20px;border-bottom:1px solid #e5e7eb;display:flex;justify-content:space-between;align-items:center}.com-cart-head button{border:0;background:#f1f5f9;border-radius:10px;padding:8px 11px}.com-cart-list{max-height:30dvh;overflow:auto;padding:12px 20px}.com-cart-foot{flex:1;min-height:0;overflow-y:auto;padding:12px 20px 70px}.com-checkout{display:grid;gap:10px}.com-checkout h3{margin:12px 0 2px}.com-checkout input,.com-checkout textarea{width:100%;box-sizing:border-box;padding:12px;border:1px solid #dbe1ea;border-radius:10px}.com-checkout>button{border:0;border-radius:12px;background:#16a34a;color:#fff;padding:14px;font-weight:900}.com-address-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:8px}.com-product-page{padding-top:24px;padding-bottom:50px}.com-product-main{display:grid;grid-template-columns:1.05fr .95fr;gap:36px;background:#fff;padding:24px;border-radius:22px;border:1px solid #e5e7eb}.com-gallery-main{width:100%;aspect-ratio:1/1;object-fit:cover;border-radius:16px}.com-buybox h1{font-size:34px;line-height:1.15}.com-product-content{display:grid;grid-template-columns:1.5fr .8fr;gap:22px;margin:22px 0}.com-product-content>*{background:#fff;border:1px solid #e5e7eb;border-radius:18px;padding:22px}.com-detail-head{background:#fff;border-bottom:1px solid #e5e7eb}.com-detail-head .com-wrap{height:66px;display:flex;align-items:center;justify-content:space-between}.com-breadcrumb{font-size:13px;color:#64748b;margin:0 0 16px}.com-buy-actions{display:flex;gap:10px;margin:18px 0}.com-buy-actions button{flex:1;padding:13px;border:0;border-radius:12px;background:#111827;color:#fff;font-weight:900}.com-buy-actions button:last-child{background:#2563eb}
@media(max-width:900px){.com-grid{grid-template-columns:repeat(3,minmax(0,1fr))}.com-featured-copy{padding:34px}.com-trustbar{grid-template-columns:repeat(2,1fr)}}
@media(max-width:680px){.com-wrap{width:min(100% - 20px,1180px)}.com-head .com-wrap{min-height:64px;gap:7px;flex-wrap:wrap;padding:8px 0}.com-brand{font-size:17px}.com-admin-link{display:none!important}.com-cart-btn{padding:10px;font-size:12px}.com-search{order:3;width:100%;max-width:none}.com-search input{padding:11px}.com-search button{padding:10px 12px}.com-hero{padding:10px 0 12px}.com-featured-shell,.com-featured-shell #comFeaturedSlide,.com-featured-shell #comFeaturedSlide>div{min-height:285px}.com-featured-shell #comFeaturedSlide>div{grid-template-columns:1.15fr .85fr}.com-featured-copy{padding:22px 16px}.com-featured-copy h1{font-size:27px}.com-featured-copy p{font-size:13px;line-height:1.45}.com-featured-price b{font-size:21px}.com-featured-actions a{font-size:11px;padding:9px}.com-featured-shell img{height:285px}.com-featured-prev,.com-featured-next{display:none}.com-trustbar{grid-template-columns:repeat(2,1fr);gap:7px;margin:10px auto 18px}.com-trustbar div{padding:11px}.com-trustbar span{font-size:11px}.com-cats{overflow-x:auto;flex-wrap:nowrap;scrollbar-width:none;margin-bottom:10px}.com-cat{flex:0 0 auto;padding:8px 11px;font-size:12px}.com-toolbar{margin:15px 0 10px}.com-toolbar b{font-size:18px}.com-toolbar select{max-width:125px;font-size:12px}.com-grid{grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.com-card{border-radius:13px}.com-card-body{padding:9px}.com-card-body small,.com-card-body p{font-size:10px}.com-card-body h3{font-size:13px;min-height:35px}.com-card-price b{font-size:14px}.com-card-price del{font-size:9px}.com-card-actions{gap:4px}.com-card-actions>*{font-size:10px;padding:8px 3px}.com-discovery{margin:18px 0;padding:17px}.com-discovery h2{font-size:21px}.com-footer-grid{grid-template-columns:1fr;gap:15px}.com-cart{width:100%;max-width:none}.com-cart-list{max-height:27dvh}.com-cart-foot{-webkit-overflow-scrolling:touch;padding-bottom:max(80px,env(safe-area-inset-bottom))}.com-address-grid{grid-template-columns:1fr}.com-checkout input,.com-checkout textarea,.com-checkout button{min-height:46px;font-size:16px}.com-product-main,.com-product-content{grid-template-columns:1fr}.com-product-main{padding:12px;gap:16px}.com-buybox h1{font-size:25px}.com-product-page .com-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}
`;document.head.appendChild(st)}

function comEnsureV16Css(){if(document.getElementById('com-v16-css'))return;const st=document.createElement('style');st.id='com-v16-css';st.textContent=`
.com-store{background:#f6f8fc;color:#0f172a}.com-head{background:rgba(255,255,255,.96);backdrop-filter:blur(14px);border-bottom:1px solid #e7ebf2;position:sticky;top:0;z-index:35}.com-head .com-wrap{min-height:76px}.com-brand{font-size:22px;letter-spacing:-.5px;color:#0f172a}.com-search{border:1px solid #dbe3ef;background:#f8fafc;border-radius:14px;padding:4px}.com-search input{background:transparent}.com-search button,.com-cart-btn{border-radius:10px;background:#0f172a!important}.com-hero{background:linear-gradient(135deg,#071225 0%,#102449 55%,#17366b 100%);padding:28px 0}.com-featured-shell{border-radius:24px;overflow:hidden;box-shadow:0 28px 70px rgba(2,8,23,.24)}.com-featured-copy{background:linear-gradient(135deg,#0b1730,#102a5a);color:#fff}.com-featured-copy small{color:#93c5fd}.com-featured-copy h1{font-size:clamp(32px,4vw,58px);line-height:1.02;letter-spacing:-1.5px}.com-featured-copy p{color:#dbeafe}.com-featured-actions a:first-child{background:#fff;color:#0f172a}.com-featured-actions a:last-child{background:#2563eb;color:#fff}.com-trustbar{margin-top:18px}.com-trustbar div{background:#fff;border:1px solid #e6eaf1;border-radius:15px;box-shadow:0 7px 24px rgba(15,23,42,.04)}.com-cats{padding:8px 0}.com-cat{background:#fff;border:1px solid #dfe5ee;border-radius:999px}.com-cat.active{background:#0f172a;color:#fff;border-color:#0f172a}.com-toolbar{padding:18px 0 10px;border-top:1px solid #e6eaf1}.com-toolbar b{font-size:26px;letter-spacing:-.5px}.com-card{background:#fff;border:1px solid #e5eaf1;border-radius:18px;overflow:hidden;box-shadow:0 8px 28px rgba(15,23,42,.05);transition:.2s transform,.2s box-shadow}.com-card:hover{transform:translateY(-4px);box-shadow:0 18px 40px rgba(15,23,42,.11)}.com-card-img{background:#f1f5f9}.com-card-img img{aspect-ratio:1/1;object-fit:cover}.com-card-body h3 a{color:#0f172a}.com-card-price b{color:#dc2626}.com-card-actions a{background:#eef2ff;color:#3730a3}.com-card-actions button{background:#0f172a}.com-pagination button{border-radius:10px}.com-discovery{background:linear-gradient(135deg,#0f172a,#172554);border-radius:20px}.com-footer{background:#081225}.com-product-page{padding-top:28px}.com-product-main,.com-product-content{background:#fff;border:1px solid #e5eaf1;border-radius:20px}.com-cart{box-shadow:-20px 0 60px rgba(15,23,42,.18)}
@media(max-width:680px){.com-head .com-wrap{min-height:62px}.com-brand{font-size:17px}.com-hero{padding:10px 0}.com-featured-shell{border-radius:16px}.com-featured-copy h1{font-size:24px}.com-card-img img{aspect-ratio:1/1}.com-card-body h3{font-size:13px}.com-toolbar b{font-size:20px}}
`;document.head.appendChild(st)}
function comRenderFeaturedHero(products=[]){const box=document.getElementById('comHero');if(!box)return;const list=(products||[]).filter(x=>Number(x.is_featured)&&Number(x.is_active)!==0).slice(0,6);if(!list.length)return;let i=0;const money=n=>Number(n||0).toLocaleString('vi-VN')+'đ';box.innerHTML='<div class="com-wrap com-featured-shell"><div id="comFeaturedSlide"></div><button class="com-featured-prev" type="button">‹</button><button class="com-featured-next" type="button">›</button><div class="com-featured-dots"></div></div>';const slide=box.querySelector('#comFeaturedSlide'),dots=box.querySelector('.com-featured-dots');function draw(){const x=list[i],old=Number(x.compare_at_price||0);slide.innerHTML='<div class="com-featured-copy"><small>'+esc(x.badge||'SẢN PHẨM NỔI BẬT')+'</small><h1>'+esc(x.name)+'</h1><p>'+esc(x.short_description||'Sản phẩm được chủ shop lựa chọn giới thiệu nổi bật.')+'</p><div class="com-featured-price"><b>'+money(x.price)+'</b>'+(old?'<del>'+money(old)+'</del>':'')+'</div><div class="com-featured-actions"><a href="'+location.pathname.replace(/\/san-pham\/[^/]+\/?$/,'').replace(/\/$/,'')+'/san-pham/'+encodeURIComponent(x.slug)+'">Xem chi tiết</a><a href="#products">Xem sản phẩm</a></div></div><img src="'+esc(x.image_url||'')+'" alt="'+esc(x.name)+'">';dots.innerHTML=list.map((_,n)=>'<button class="'+(n===i?'active':'')+'" data-i="'+n+'"></button>').join('');dots.querySelectorAll('button').forEach(b=>b.onclick=()=>{i=+b.dataset.i;draw();restart()})}let timer;function restart(){clearInterval(timer);timer=setInterval(()=>{i=(i+1)%list.length;draw()},5000)}box.querySelector('.com-featured-prev').onclick=()=>{i=(i-1+list.length)%list.length;draw();restart()};box.querySelector('.com-featured-next').onclick=()=>{i=(i+1)%list.length;draw();restart()};draw();restart()}
function comEnsureV17Css(){if(document.getElementById('commerce-fresh-v18'))return;const st=document.createElement('style');st.id='commerce-fresh-v18';st.textContent=`
body:has(.commerce21){background:#fff}.commerce21{--ink:#171717;--muted:#717171;--line:#e9e9e9;--accent:#ff5a1f;--soft:#fff5ef;font-family:Inter,system-ui,-apple-system,Segoe UI,sans-serif;color:var(--ink);background:#fff;min-height:100vh}.commerce21 *{box-sizing:border-box}.c21-wrap{width:min(1240px,calc(100% - 40px));margin:auto}.c21-ann{background:#171717;color:#fff;text-align:center;padding:9px 16px;font-size:12px;letter-spacing:.02em}.c21-head{background:#fff;border-bottom:1px solid var(--line);position:sticky;top:0;z-index:40}.c21-headrow{height:82px;display:grid;grid-template-columns:190px minmax(320px,1fr) auto;align-items:center;gap:26px}.c21-brand{text-decoration:none;color:var(--ink);font-weight:950;font-size:26px;letter-spacing:-1.2px}.c21-brand em{font-style:normal;color:var(--accent)}.c21-search{height:48px;background:#f6f6f6;border-radius:999px;display:flex;padding-left:18px;overflow:hidden;border:1px solid transparent}.c21-search:focus-within{border-color:#d8d8d8;background:#fff}.c21-search input{flex:1;border:0;outline:0;background:transparent;font-size:14px}.c21-search button{border:0;background:var(--accent);color:#fff;font-weight:850;padding:0 24px;cursor:pointer}.c21-actions{display:flex;gap:9px}.c21-actions a,.c21-actions button{height:44px;border:1px solid var(--line);background:#fff;border-radius:999px;padding:0 15px;display:flex;align-items:center;text-decoration:none;color:var(--ink);font-weight:800;cursor:pointer}.c21-actions .bag{background:var(--ink);color:#fff;border-color:var(--ink)}.c21-nav{border-top:1px solid #f3f3f3}.c21-nav .c21-wrap{height:48px;display:flex;align-items:center;gap:28px;overflow:auto;scrollbar-width:none}.c21-nav a{color:#454545;text-decoration:none;font-size:13px;font-weight:750;white-space:nowrap}.c21-nav a.hot{color:var(--accent)}.c21-hero{padding:28px 0 16px}.c21-hero-grid{display:grid;grid-template-columns:minmax(0,1.55fr) minmax(300px,.65fr);gap:16px}.c21-hero-main{min-height:410px;border-radius:30px;background:#f2efe9;overflow:hidden;display:grid;grid-template-columns:1.05fr .95fr;align-items:center;padding:54px;position:relative}.c21-kicker{font-size:12px;font-weight:900;letter-spacing:.16em;color:var(--accent)}.c21-hero-main h1{font-size:52px;line-height:1.02;letter-spacing:-2.7px;margin:13px 0 16px;max-width:620px}.c21-hero-main p{font-size:15px;line-height:1.7;color:#666;max-width:500px;margin:0 0 25px}.c21-cta{display:inline-flex;padding:13px 20px;border-radius:999px;background:var(--ink);color:#fff;text-decoration:none;font-weight:850}.c21-hero-main img{width:100%;height:330px;object-fit:contain;filter:drop-shadow(0 25px 35px rgba(0,0,0,.12))}.c21-promos{display:grid;gap:16px}.c21-promo{border-radius:30px;padding:28px;min-height:197px;display:flex;flex-direction:column;justify-content:flex-end;background:#dff4ea}.c21-promo:last-child{background:#fce7ef}.c21-promo small{font-weight:900;font-size:11px;letter-spacing:.12em}.c21-promo h3{font-size:25px;line-height:1.05;margin:8px 0}.c21-promo p{font-size:12px;color:#626262;margin:0}.c21-category-section{padding:22px 0 12px}.c21-section-head{display:flex;justify-content:space-between;align-items:end;margin-bottom:18px}.c21-section-head h2{font-size:28px;letter-spacing:-1px;margin:0}.c21-section-head p{margin:5px 0 0;color:var(--muted);font-size:13px}.c21-categories{display:grid;grid-template-columns:repeat(6,1fr);gap:12px}.c21-categories button{border:0;background:#fafafa;border-radius:20px;min-height:100px;padding:15px 8px;font-weight:800;cursor:pointer;transition:.2s}.c21-categories button:hover{background:var(--soft);transform:translateY(-2px)}.c21-categories .ico{display:block;font-size:26px;margin-bottom:8px}.c21-trust{margin:28px auto;border-top:1px solid var(--line);border-bottom:1px solid var(--line);display:grid;grid-template-columns:repeat(4,1fr)}.c21-trust div{padding:20px 18px}.c21-trust b{display:block;font-size:13px}.c21-trust span{font-size:11px;color:#888;display:block;margin-top:4px}.c21-products{padding:25px 0 48px}.c21-products .com-toolbar{padding:0 0 14px;border:0;background:transparent}.c21-products .com-toolbar select{border:1px solid var(--line);background:#fff;border-radius:999px;padding:9px 13px}.c21-pills{display:flex;gap:8px;overflow:auto;scrollbar-width:none;margin:18px 0}.c21-pills .com-cat{border:1px solid var(--line);background:#fff;border-radius:999px;padding:9px 15px;white-space:nowrap;font-weight:750}.c21-pills .com-cat.active{background:var(--ink);border-color:var(--ink);color:#fff}.commerce21 .com-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:18px}.commerce21 .com-card{border:0;border-radius:20px;background:#fff;overflow:hidden;box-shadow:none}.commerce21 .com-card-img{background:#f7f7f7;border-radius:20px;padding:12px}.commerce21 .com-card-img img{border-radius:14px}.commerce21 .com-card-body{padding:12px 4px 8px}.commerce21 .com-card-body>small:not(.com-badge){color:#999}.commerce21 .com-badge{background:var(--accent);color:#fff;border-radius:999px;padding:4px 8px}.commerce21 .com-card-body h3{font-size:14px}.commerce21 .com-card-price b{font-size:18px;color:var(--ink)}.commerce21 .com-card-actions a{background:#f4f4f4;color:#222}.commerce21 .com-card-actions button{background:var(--ink);color:#fff;border-radius:999px}.commerce21 .com-pagination button.active{background:var(--accent);border-color:var(--accent)}.c21-newsletter{background:#f3f0ea;border-radius:30px;padding:36px;display:flex;justify-content:space-between;align-items:center;gap:25px}.c21-newsletter h2{font-size:28px;margin:0 0 5px}.c21-newsletter p{color:#6d6d6d;margin:0}.c21-newsletter a{background:var(--accent);color:#fff;text-decoration:none;font-weight:850;border-radius:999px;padding:13px 20px;white-space:nowrap}.c21-footer{margin-top:50px;background:#171717;color:#fff;padding:48px 0 24px}.c21-footgrid{display:grid;grid-template-columns:1.5fr repeat(3,1fr);gap:40px}.c21-footer h4{margin:0 0 13px}.c21-footer p,.c21-footer a{display:block;color:#aaa;text-decoration:none;font-size:12px;line-height:1.9}.c21-copy{border-top:1px solid #333;margin-top:30px;padding-top:18px;color:#777;font-size:11px}.commerce21 .com-cart{z-index:100}
@media(max-width:900px){.c21-headrow{grid-template-columns:1fr auto;height:auto;padding:14px 0}.c21-search{grid-column:1/-1;grid-row:2}.c21-actions .admin{display:none}.c21-hero-grid{grid-template-columns:1fr}.c21-promos{grid-template-columns:1fr 1fr}.c21-hero-main{min-height:360px}.c21-categories{grid-template-columns:repeat(3,1fr)}.commerce21 .com-grid{grid-template-columns:repeat(3,1fr)}}
@media(max-width:640px){.c21-wrap{width:calc(100% - 20px)}.c21-ann{font-size:10px}.c21-headrow{gap:8px}.c21-brand{font-size:21px}.c21-actions a{display:none}.c21-actions button{height:38px;padding:0 11px;font-size:11px}.c21-search{height:42px}.c21-search button{padding:0 15px}.c21-nav .c21-wrap{height:40px;gap:18px}.c21-hero{padding-top:10px}.c21-hero-main{padding:25px 20px;min-height:420px;grid-template-columns:1fr}.c21-hero-main h1{font-size:34px;letter-spacing:-1.5px}.c21-hero-main img{height:180px;order:-1}.c21-promos{display:none}.c21-section-head h2{font-size:22px}.c21-categories{display:flex;overflow:auto;scrollbar-width:none}.c21-categories button{flex:0 0 110px;min-height:85px}.c21-trust{grid-template-columns:1fr 1fr;margin-top:18px}.c21-trust div{padding:13px 8px}.commerce21 .com-grid{grid-template-columns:repeat(2,minmax(0,1fr));gap:9px}.commerce21 .com-card-body{padding:8px 2px}.commerce21 .com-card-body h3{font-size:12px}.commerce21 .com-card-price b{font-size:14px}.commerce21 .com-card-actions>*{font-size:10px;padding:8px 3px}.c21-newsletter{padding:24px;display:block}.c21-newsletter h2{font-size:22px}.c21-newsletter a{display:inline-flex;margin-top:16px}.c21-footgrid{grid-template-columns:1fr 1fr;gap:24px}.c21-footgrid>div:first-child{grid-column:1/-1}}
`;document.head.appendChild(st)}


async function renderCommerceStore(site={}){
 const root=document.querySelector('main');if(!root)return;
 document.querySelectorAll('footer.footer,footer.public-footer').forEach(x=>x.remove());
 const brand=String(site.shop_name||site.name||'MIRA STORE'),money=n=>new Intl.NumberFormat('vi-VN').format(Number(n||0))+'đ';
 const seed=[
 ['demo-1','Tai nghe AirBeat Pro','Công nghệ',1290000,1690000,'https://images.unsplash.com/photo-1505740420928-5e560c06d30e?auto=format&fit=crop&w=900&q=82','BÁN CHẠY'],['demo-2','Đồng hồ Urban Classic','Phụ kiện',1890000,2390000,'https://images.unsplash.com/photo-1523275335684-37898b6baf30?auto=format&fit=crop&w=900&q=82','MỚI'],['demo-3','Sneaker Motion 02','Thời trang',1590000,1990000,'https://images.unsplash.com/photo-1542291026-7eec264c27ff?auto=format&fit=crop&w=900&q=82','-20%'],['demo-4','Camera Mini Studio','Công nghệ',3490000,3990000,'https://images.unsplash.com/photo-1516035069371-29a1b244cc32?auto=format&fit=crop&w=900&q=82','NỔI BẬT'],
 ['demo-5','Túi Everyday Canvas','Thời trang',790000,990000,'https://images.unsplash.com/photo-1553062407-98eeb64c6a62?auto=format&fit=crop&w=900&q=82','MỚI'],['demo-6','Serum Daily Glow','Mỹ phẩm',590000,750000,'https://images.unsplash.com/photo-1620916566398-39f1143ab7be?auto=format&fit=crop&w=900&q=82','YÊU THÍCH'],['demo-7','Đèn bàn Nordic','Nội thất',890000,1190000,'https://images.unsplash.com/photo-1507473885765-e6ed057f782c?auto=format&fit=crop&w=900&q=82','-25%'],['demo-8','Bình giữ nhiệt Flow','Gia dụng',390000,490000,'https://images.unsplash.com/photo-1602143407151-7111542de6e8?auto=format&fit=crop&w=900&q=82','BÁN CHẠY'],
 ['demo-9','Loa Bluetooth Mini','Công nghệ',990000,1290000,'https://images.unsplash.com/photo-1608043152269-423dbba4e7e1?auto=format&fit=crop&w=900&q=82','HOT'],['demo-10','Kính Urban Frame','Phụ kiện',690000,890000,'https://images.unsplash.com/photo-1511499767150-a48a237f0083?auto=format&fit=crop&w=900&q=82','MỚI'],['demo-11','Áo Basic Premium','Thời trang',490000,650000,'https://images.unsplash.com/photo-1521572163474-6864f9cf17ab?auto=format&fit=crop&w=900&q=82','-15%'],['demo-12','Kem dưỡng Pure Skin','Mỹ phẩm',650000,790000,'https://images.unsplash.com/photo-1556228578-8c89e6adf883?auto=format&fit=crop&w=900&q=82','BÁN CHẠY'],
 ['demo-13','Ghế thư giãn Soft','Nội thất',2490000,2990000,'https://images.unsplash.com/photo-1567538096630-e0c55bd6374c?auto=format&fit=crop&w=900&q=82','NỔI BẬT'],['demo-14','Máy pha cà phê Home','Gia dụng',2190000,2590000,'https://images.unsplash.com/photo-1495474472287-4d71bcdd2085?auto=format&fit=crop&w=900&q=82','MỚI'],['demo-15','Bàn phím Studio Keys','Công nghệ',1390000,1690000,'https://images.unsplash.com/photo-1587829741301-dc798b83add3?auto=format&fit=crop&w=900&q=82','HOT'],['demo-16','Balo City Pack','Phụ kiện',850000,1050000,'https://images.unsplash.com/photo-1553062407-98eeb64c6a62?auto=format&fit=crop&w=900&q=82','-20%']
 ];
 const fallback=seed.map(x=>({id:x[0],name:x[1],cat:x[2],price:x[3],old:x[4],img:x[5],badge:x[6],stock:12,desc:'Thiết kế hiện đại, dễ sử dụng và phù hợp cho nhu cầu hằng ngày. Sản phẩm được chọn để cửa hàng có thể giới thiệu rõ ràng, đẹp mắt và dễ mua.'}));
 let catalog=null;try{const qp=new URLSearchParams();const cur=new URLSearchParams(location.search);for(const k of ['tenant','nr_trial'])if(cur.get(k))qp.set(k,cur.get(k));qp.set('_v',String(Date.now()));const rr=await fetch('/api/commerce/catalog?'+qp.toString(),{cache:'no-store'});if(rr.ok)catalog=await rr.json()}catch(e){console.warn('Commerce catalog fallback',e)}
 const liveRaw=Array.isArray(catalog?.products)?catalog.products:[];
 const raw=liveRaw.length?liveRaw:(Array.isArray(SITE_DATA?.commerce_products)?SITE_DATA.commerce_products:[]);
 const parseJson=(v,fallback)=>{if(v==null||v==='')return fallback;if(typeof v==='object')return v;try{return JSON.parse(v)}catch(e){return fallback}};
 const normalizeGallery=(x,cover)=>{const direct=Array.isArray(x.images)?x.images:parseJson(x.gallery_json,[]);const extra=Array.isArray(direct)?direct:String(x.gallery||'').split(',').map(v=>v.trim()).filter(Boolean);return [...new Set([cover,...extra].filter(Boolean))]};
 const normalizeAttrs=x=>{const a=parseJson(x.attributes_json,x.attributes||{});return a&&typeof a==='object'&&!Array.isArray(a)?a:{}};
 const normalizeVariants=x=>{const v=parseJson(x.variants_json,x.variants||[]);return Array.isArray(v)?v:[]};
 const products=raw.length?raw.map((x,i)=>{const cover=x.cover_image||x.image_url||x.image||getImages(x)[0]||fallback[i%fallback.length].img;return {id:String(x.id||i),slug:x.slug||String(x.id||i),sku:x.sku||'',name:x.name||x.title||'Sản phẩm',cat:x.category_name||x.category||'Khác',price:Number(x.sale_price||x.price||0),old:Number(x.compare_at_price||x.compare_price||x.old_price||0),img:cover,images:normalizeGallery(x,cover),badge:x.badge||'',stock:Number(x.stock_qty??x.stock??x.stock_quantity??10),stockStatus:x.stock_status||'in_stock',shortDesc:x.short_description||'',desc:x.description||x.short_description||fallback[0].desc,attributes:normalizeAttrs(x),variants:normalizeVariants(x)}}):fallback.map((x,i)=>({...x,slug:x.id,sku:'DEMO-'+String(i+1).padStart(3,'0'),images:[x.img,x.img,x.img],shortDesc:x.desc,attributes:{'Chất liệu':'Cao cấp','Bảo hành':'12 tháng','Xuất xứ':'Chính hãng'},variants:[]}));
 const apiCats=Array.isArray(catalog?.categories)?catalog.categories.map(x=>x.name).filter(Boolean):[];
 const cats=['Tất cả',...new Set([...apiCats,...products.map(x=>x.cat).filter(Boolean)])],PAGE=8,CART='nr-commerce-v24-cart',WISH='nr-commerce-v24-wish';
 const load=k=>{try{return JSON.parse(localStorage.getItem(k)||'[]')}catch(e){return[]}},save=(k,v)=>localStorage.setItem(k,JSON.stringify(v));let cart=load(CART),wish=load(WISH),active='Tất cả',term='',page=1,sort='featured';
 const get=id=>products.find(x=>String(x.id)===String(id));
 const card=x=>`<article class="v24-card"><button class="v24-heart ${wish.includes(String(x.id))?'on':''}" data-wish="${esc(x.id)}" aria-label="Yêu thích">${wish.includes(String(x.id))?'♥':'♡'}</button><a class="v24-photo" href="#product-${encodeURIComponent(x.id)}"><img src="${esc(x.img)}" alt="${esc(x.name)}" loading="lazy">${x.badge?`<span>${esc(x.badge)}</span>`:''}</a><div class="v24-body"><small>${esc(x.cat)}</small><a class="v24-name" href="#product-${encodeURIComponent(x.id)}">${esc(x.name)}</a><div class="v24-price"><b>${money(x.price)}</b>${x.old>x.price?`<del>${money(x.old)}</del>`:''}</div><div class="v24-actions"><button data-add="${esc(x.id)}">+ Giỏ hàng</button><a href="#product-${encodeURIComponent(x.id)}">Chi tiết</a></div></div></article>`;
 const featuredProducts=products.filter(x=>/NỔI BẬT|BÁN CHẠY|HOT/i.test(x.badge||'')).slice(0,4).concat(products.slice(0,4)).filter((x,i,a)=>a.findIndex(y=>y.id===x.id)===i).slice(0,4);
 const newProducts=[...products].slice(-4).reverse();
 const showcaseCats=cats.filter(x=>x!=='Tất cả').slice(0,3);
 const showcaseSection=(title,note,items,cls='')=>items.length?`<section class="v24-showcase ${cls}"><div class="v24-wrap"><div class="v24-title"><div><h2>${esc(title)}</h2><p>${esc(note)}</p></div><a class="v46-seeall" href="#catalog">Xem tất cả →</a></div><div class="v24-grid">${items.map(card).join('')}</div></div></section>`:'';
 const categoryShowcases=showcaseCats.map((c,i)=>showcaseSection(c,`Gợi ý nổi bật trong danh mục ${c}`,products.filter(x=>x.cat===c).slice(0,4),i%2?'is-soft':'')).join('');
 root.innerHTML=`<div class="commerce24" data-commerce-v24="true"><style>
 .commerce24{--ink:#101010;--muted:#737373;--line:#e9e7e2;--soft:#f7f6f2;--accent:#ff5b35;background:#fff;color:var(--ink);font-family:Inter,system-ui,-apple-system,Segoe UI,sans-serif;margin-left:calc(50% - 50vw);margin-right:calc(50% - 50vw);min-height:100vh}.commerce24 *{box-sizing:border-box}.v24-wrap{width:min(1280px,calc(100% - 48px));margin:auto}.v24-top{background:#101010;color:#fff;font-size:12px;padding:9px 0}.v24-top .v24-wrap{display:flex;justify-content:space-between}.v24-head{position:sticky;top:0;z-index:50;background:#fff;border-bottom:1px solid var(--line)}.v24-headrow{height:78px;display:grid;grid-template-columns:190px 1fr auto;gap:24px;align-items:center}.v24-logo{font-size:25px;font-weight:950;letter-spacing:-1.3px}.v24-logo i{color:var(--accent);font-style:normal}.v24-search{height:46px;border:1px solid #ddd;border-radius:999px;display:flex;overflow:hidden;background:#fafafa}.v24-search input{flex:1;border:0;background:transparent;padding:0 18px;outline:0}.v24-search button{border:0;background:#111;color:#fff;padding:0 22px;font-weight:850}.v24-tools{display:flex;gap:8px}.v24-tool{height:44px;border:1px solid var(--line);background:#fff;border-radius:999px;padding:0 15px;font-weight:850}.v24-tool.primary{background:#111;color:#fff}.v24-nav{border-top:1px solid #f3f3f3}.v24-nav .v24-wrap{height:45px;display:flex;gap:28px;align-items:center}.v24-nav a{font-size:13px;font-weight:800;color:#333;text-decoration:none}.v24-hero{padding:24px 0}.v24-hero-grid{display:grid;grid-template-columns:1.55fr .7fr;gap:16px}.v24-mainhero{min-height:390px;background:#e8f0df;border-radius:28px;padding:52px;position:relative;overflow:hidden}.v24-mainhero h1{font-size:58px;line-height:1;letter-spacing:-3px;max-width:600px;margin:15px 0}.v24-mainhero p{max-width:470px;color:#56604f;line-height:1.7}.v24-mainhero img{position:absolute;right:2%;bottom:0;width:39%;height:88%;object-fit:cover;border-radius:140px 140px 0 0}.v24-cta{display:inline-flex;margin-top:18px;background:#111;color:#fff;border-radius:999px;padding:13px 20px;text-decoration:none;font-weight:900}.v24-promos{display:grid;gap:16px}.v24-promo{border-radius:28px;padding:27px;display:flex;flex-direction:column;justify-content:flex-end;background:#f8dfe5}.v24-promo:last-child{background:#e2e6fa}.v24-promo b{font-size:24px;margin-top:6px}.v24-section{padding:42px 0}.v24-title{display:flex;justify-content:space-between;align-items:end;gap:20px;margin-bottom:18px}.v24-title h2{font-size:32px;margin:0;letter-spacing:-1.3px}.v24-title p{margin:5px 0;color:var(--muted)}.v24-filters{display:flex;gap:8px;flex-wrap:wrap}.v24-filter{border:1px solid var(--line);background:#fff;border-radius:999px;padding:9px 14px;font-weight:800}.v24-filter.active{background:#111;color:#fff}.v24-toolbar{display:flex;justify-content:space-between;align-items:center;margin:0 0 18px;color:var(--muted);font-size:13px}.v24-toolbar select{border:1px solid var(--line);border-radius:10px;padding:9px 12px;background:#fff}.v24-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:20px}.v24-card{position:relative}.v24-photo{height:300px;display:block;background:#f5f5f3;border-radius:20px;overflow:hidden;position:relative}.v24-photo img{width:100%;height:100%;object-fit:cover;transition:.3s}.v24-card:hover img{transform:scale(1.03)}.v24-photo span{position:absolute;left:12px;top:12px;background:#111;color:#fff;padding:6px 9px;border-radius:999px;font-size:10px;font-weight:900}.v24-heart{position:absolute;right:12px;top:12px;z-index:3;width:38px;height:38px;border:0;border-radius:50%;background:#fff;font-size:20px;box-shadow:0 4px 16px #0001}.v24-heart.on{color:#e53935}.v24-body{padding:13px 3px}.v24-body small{color:#888}.v24-name{display:block;color:#111;text-decoration:none;font-weight:850;margin:6px 0 9px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.v24-price{display:flex;gap:8px;align-items:center}.v24-price b{font-size:18px}.v24-price del{font-size:12px;color:#999}.v24-actions{display:grid;grid-template-columns:1fr auto;gap:7px;margin-top:12px}.v24-actions button,.v24-actions a{border:1px solid #111;border-radius:10px;padding:10px 11px;font-weight:850;text-decoration:none;text-align:center;font-size:12px}.v24-actions button{background:#111;color:#fff}.v24-actions a{color:#111}.v24-pages{display:flex;justify-content:center;gap:7px;margin-top:30px}.v24-pages button{width:40px;height:40px;border:1px solid var(--line);background:#fff;border-radius:10px;font-weight:850}.v24-pages button.active{background:#111;color:#fff}.v24-story{background:#111;color:#fff;padding:62px 0;margin-top:20px}.v24-story .v24-wrap{display:grid;grid-template-columns:1fr 1fr;gap:55px;align-items:center}.v24-story img{width:100%;height:390px;object-fit:cover;border-radius:25px}.v24-story h2{font-size:43px;letter-spacing:-2px;margin:0 0 15px}.v24-story p{color:#bbb;line-height:1.8}.v24-footer{background:#f5f3ed;padding:48px 0}.v24-footgrid{display:grid;grid-template-columns:2fr repeat(3,1fr);gap:35px}.v24-footer a,.v24-footer p{display:block;color:#666;text-decoration:none;font-size:13px;line-height:2}.v24-overlay{position:fixed;inset:0;background:#0007;z-index:2147483600;display:none}.v24-overlay.open{display:block}.v24-drawer{position:absolute;right:0;top:0;width:min(470px,100%);height:100%;background:#fff;padding:25px;overflow:auto}.v24-drawer-head{display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid var(--line);padding-bottom:15px}.v24-close{border:0;background:#eee;border-radius:50%;width:38px;height:38px;font-size:20px}.v24-line{display:grid;grid-template-columns:72px 1fr auto;gap:12px;padding:14px 0;border-bottom:1px solid var(--line);align-items:center}.v24-line img{width:72px;height:72px;object-fit:cover;border-radius:12px}.v24-line b{display:block}.v24-line button{border:0;background:none}.v24-total{display:flex;justify-content:space-between;font-size:19px;font-weight:900;padding:20px 0}.v24-checkout{width:100%;border:0;background:#111;color:#fff;border-radius:12px;padding:14px;font-weight:900}.v24-detail{position:fixed;inset:0;background:#fff;z-index:2147483000;overflow:auto;display:none;overscroll-behavior:contain}.v24-detail.open{display:block}html.commerce-detail-open,body.commerce-detail-open{overflow:hidden!important;height:100%!important}.v24-detailbar{position:sticky;top:0;background:#fff;border-bottom:1px solid var(--line);z-index:20;padding:14px 0;box-shadow:0 4px 16px rgba(0,0,0,.04)}.v24-detailgrid{display:grid;grid-template-columns:minmax(0,1fr) minmax(380px,.82fr);gap:42px;padding:34px 0;align-items:start}.v24-detailimg{width:100%;height:600px;object-fit:cover;border-radius:25px;background:#f5f5f3}.v24-detail h1{font-size:40px;line-height:1.08;letter-spacing:-1.7px;margin:8px 0 14px}.v24-detailprice b{font-size:30px}.v24-detailprice del{color:#999;margin-left:10px}.v24-stock{display:inline-block;margin:18px 0;background:#eef7e9;padding:8px 11px;border-radius:999px;font-size:12px;font-weight:850}.v24-desc{line-height:1.8;color:#666}.v25-gallery{display:grid;grid-template-columns:92px 1fr;gap:14px}.v25-thumbs{display:flex;flex-direction:column;gap:10px}.v25-thumb{width:92px;height:92px;border:2px solid transparent;border-radius:14px;padding:3px;background:#fff;overflow:hidden}.v25-thumb.active{border-color:#111}.v25-thumb img{width:100%;height:100%;object-fit:cover;border-radius:10px}.v25-main{position:relative}.v25-main img{width:100%;height:min(68vh,620px);object-fit:contain;border-radius:22px;background:#f5f5f3}.v25-meta{display:flex;gap:8px;flex-wrap:wrap;margin:12px 0}.v25-meta span{background:#f5f5f3;border-radius:999px;padding:7px 10px;font-size:12px}.v25-variants{margin:18px 0}.v25-variants h4{margin:0 0 9px}.v25-variant-list{display:flex;gap:8px;flex-wrap:wrap}.v25-variant{border:1px solid #ddd;background:#fff;border-radius:10px;padding:9px 12px}.v25-variant.active{border-color:#111;background:#111;color:#fff}.v25-tabs{margin-top:28px;border-top:1px solid var(--line)}.v25-tabnav{display:flex;gap:24px;border-bottom:1px solid var(--line)}.v25-tabnav button{border:0;background:none;padding:16px 0;font-weight:900;color:#777}.v25-tabnav button.active{color:#111;border-bottom:2px solid #111}.v25-tabpanel{padding:22px 0;line-height:1.8;color:#555}.v25-specs{width:100%;border-collapse:collapse}.v25-specs th,.v25-specs td{padding:12px;border-bottom:1px solid var(--line);text-align:left}.v25-specs th{width:35%;color:#777;font-weight:700}.v25-related-head{display:flex;justify-content:space-between;align-items:end}.v25-related-head p{color:#888;margin:0}.v24-buy{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin:24px 0}.v24-buy button{padding:15px;border-radius:12px;font-weight:900;border:1px solid #111;background:#fff}.v24-buy .buy{background:#111;color:#fff}.v24-related{padding:25px 0 55px}.v24-related h2{font-size:28px}.v24-related .v24-grid{grid-template-columns:repeat(4,1fr)}.v24-empty{text-align:center;padding:55px;color:#777}.v24-checkform{display:grid;gap:10px;margin-top:18px}.v24-checkform input,.v24-checkform select,.v24-checkform textarea{width:100%;padding:12px;border:1px solid #ddd;border-radius:10px;font:inherit}.v24-checkform .row{display:grid;grid-template-columns:1fr 1fr;gap:10px}
.v24-showcase{padding:38px 0}.v24-showcase.is-soft{background:#faf9f6}.v46-seeall{color:#111;text-decoration:none;font-weight:900;white-space:nowrap}.v46-benefits{padding:12px 0 34px}.v46-benefitgrid{display:grid;grid-template-columns:repeat(4,1fr);gap:12px}.v46-benefit{border:1px solid var(--line);border-radius:16px;padding:18px;background:#fff}.v46-benefit b{display:block;margin-bottom:5px}.v46-benefit span{font-size:12px;color:var(--muted)}.v25-summary{padding:6px 0}.v25-tabpanel{max-width:720px}.v25-tabpanel img{max-width:100%;height:auto}.v24-related{border-top:1px solid var(--line);margin-top:12px}.v24-detailgrid{grid-template-columns:minmax(0,1.08fr) minmax(360px,.92fr)}
 @media(max-width:900px){.v46-benefitgrid{grid-template-columns:repeat(2,1fr)}.v24-headrow{grid-template-columns:1fr auto;height:auto;padding:13px 0}.v24-search{grid-column:1/-1;grid-row:2}.v24-nav{display:none}.v24-hero-grid{grid-template-columns:1fr}.v24-promos{grid-template-columns:1fr 1fr}.v24-grid{grid-template-columns:repeat(2,1fr)}.v24-detailgrid{grid-template-columns:1fr}.v24-detailimg{height:460px}.v24-story .v24-wrap{grid-template-columns:1fr}}
 @media(max-width:560px){.v24-showcase{padding:28px 0}.v46-benefitgrid{grid-template-columns:1fr 1fr}.v46-benefit{padding:13px}.v24-wrap{width:calc(100% - 20px)}.v24-top span:last-child{display:none}.v24-logo{font-size:20px}.v24-tool span{display:none}.v24-mainhero{min-height:350px;padding:25px}.v24-mainhero h1{font-size:38px;letter-spacing:-2px;max-width:75%}.v24-mainhero p{max-width:62%;font-size:13px}.v24-mainhero img{width:46%;height:58%}.v24-promos{display:none}.v24-section{padding:32px 0}.v24-title{display:block}.v24-title h2{font-size:25px}.v24-filters{overflow:auto;flex-wrap:nowrap;margin-top:14px}.v24-filter{white-space:nowrap}.v24-grid{gap:10px}.v24-photo{height:190px}.v24-actions{grid-template-columns:1fr}.v24-actions a{display:none}.v24-story{padding:42px 0}.v24-story img{height:270px}.v24-story h2{font-size:31px}.v24-footgrid{grid-template-columns:1fr 1fr}.v24-footgrid>div:first-child{grid-column:1/-1}.v24-detailgrid{padding:20px 0;gap:20px}.v24-detailimg{height:360px}.v25-gallery{grid-template-columns:1fr}.v25-thumbs{order:2;flex-direction:row;overflow:auto}.v25-thumb{flex:0 0 72px;width:72px;height:72px}.v25-main img{height:360px}.v24-detail h1{font-size:30px}.v24-related .v24-grid{grid-template-columns:repeat(2,1fr)}.v24-checkform .row{grid-template-columns:1fr}}
 </style><div class="v24-top"><div class="v24-wrap"><span>Miễn phí vận chuyển đơn từ 500.000đ</span><span>Đổi trả 7 ngày · Hỗ trợ 08:00–22:00</span></div></div><header class="v24-head"><div class="v24-wrap v24-headrow"><div class="v24-logo">${esc(brand.split(' ')[0])}<i>+</i>STORE</div><form class="v24-search" id="v24Search"><input id="v24SearchInput" placeholder="Tìm trong cửa hàng..."><button>Tìm kiếm</button></form><div class="v24-tools"><button class="v24-tool" id="v24WishBtn">♡ <span>Yêu thích</span> <b id="v24WishCount">${wish.length}</b></button><button class="v24-tool primary" id="v24CartBtn">🛍 <span>Giỏ hàng</span> <b id="v24CartCount">0</b></button></div></div><nav class="v24-nav"><div class="v24-wrap"><a href="#catalog">Danh mục</a><a href="#catalog">Sản phẩm mới</a><a href="#catalog">Bán chạy</a><a href="#catalog">Ưu đãi</a><a href="#story">Về cửa hàng</a></div></nav></header><section class="v24-hero"><div class="v24-wrap"><div class="v24-hero-grid"><div class="v24-mainhero"><div><small>BỘ SƯU TẬP MỚI · 2026</small><h1>Mỗi ngày một lựa chọn đáng thích.</h1><p>Sản phẩm được tuyển chọn cho phong cách sống hiện đại — đẹp, hữu ích và vừa túi tiền.</p><a class="v24-cta" href="#catalog">Mua sắm ngay →</a></div><img src="https://images.unsplash.com/photo-1542291026-7eec264c27ff?auto=format&fit=crop&w=900&q=85"></div><div class="v24-promos"><div class="v24-promo"><small>ƯU ĐÃI TRONG TUẦN</small><b>Phụ kiện chọn lọc đến -30%</b></div><div class="v24-promo"><small>NEW DROP</small><b>Những món mới vừa lên kệ</b></div></div></div></div></section><section class="v46-benefits"><div class="v24-wrap v46-benefitgrid"><div class="v46-benefit"><b>🚚 Giao hàng linh hoạt</b><span>Thiết lập phù hợp từng cửa hàng.</span></div><div class="v46-benefit"><b>↩ Đổi trả rõ ràng</b><span>Chính sách dễ hiểu cho khách.</span></div><div class="v46-benefit"><b>🔒 Thanh toán thuận tiện</b><span>COD hoặc chuyển khoản.</span></div><div class="v46-benefit"><b>💬 Hỗ trợ nhanh</b><span>Hotline và tư vấn trực tiếp.</span></div></div></section>${showcaseSection('Sản phẩm nổi bật','Những lựa chọn đáng chú ý của cửa hàng',featuredProducts)}${showcaseSection('Mới lên kệ','Sản phẩm mới để khách dễ khám phá',newProducts,'is-soft')}${categoryShowcases}<section class="v24-section" id="catalog"><div class="v24-wrap"><div class="v24-title"><div><h2>Khám phá sản phẩm</h2><p>Nhiều lựa chọn hơn, tìm nhanh hơn và mua thuận tiện hơn.</p></div><div class="v24-filters">${cats.map((x,i)=>`<button class="v24-filter ${i===0?'active':''}" data-filter="${esc(x)}">${esc(x)}</button>`).join('')}</div></div><div class="v24-toolbar"><span id="v24Result"></span><select id="v24Sort"><option value="featured">Nổi bật</option><option value="new">Mới nhất</option><option value="low">Giá thấp → cao</option><option value="high">Giá cao → thấp</option></select></div><div class="v24-grid" id="v24Grid"></div><div class="v24-pages" id="v24Pages"></div></div></section><section class="v24-story" id="story"><div class="v24-wrap"><img src="https://images.unsplash.com/photo-1441986300917-64674bd600d8?auto=format&fit=crop&w=1200&q=82"><div><small>SHOP DÀNH CHO THƯƠNG HIỆU NHỎ</small><h2>Trưng bày nhiều hơn. Mua sắm dễ hơn.</h2><p>Danh mục rõ ràng, sản phẩm có phân trang, yêu thích, giỏ hàng và trang chi tiết giúp khách đi từ khám phá đến đặt hàng trong một trải nghiệm liền mạch.</p><a class="v24-cta" href="#catalog">Xem sản phẩm →</a></div></div></section><footer class="v24-footer"><div class="v24-wrap v24-footgrid"><div><div class="v24-logo">${esc(brand.split(' ')[0])}<i>+</i>STORE</div><p>Website bán hàng dành cho shop nhỏ và thương hiệu online.</p></div><div><b>Mua sắm</b><a href="#catalog">Sản phẩm</a><a href="#catalog">Bán chạy</a></div><div><b>Hỗ trợ</b><a href="#">Vận chuyển</a><a href="#">Đổi trả</a></div><div><b>Liên hệ</b><p>${esc(site.phone||'0900 000 000')}</p><p>${esc(site.email||site.contact_email||'hello@store.vn')}</p></div></div></footer><div class="v24-overlay" id="v24Overlay"><aside class="v24-drawer"><div class="v24-drawer-head"><h2 id="v24DrawerTitle">Giỏ hàng</h2><button class="v24-close" data-close>×</button></div><div id="v24DrawerBody"></div></aside></div><section class="v24-detail" id="v24Detail"><div class="v24-detailbar"><div class="v24-wrap"><button class="v24-tool" id="v24Back">← Tiếp tục mua sắm</button></div></div><div id="v24DetailBody"></div></section></div>`;
 const countCart=()=>cart.reduce((n,x)=>n+Number(x.qty||1),0);function updateCounts(){document.getElementById('v24CartCount').textContent=countCart();document.getElementById('v24WishCount').textContent=wish.length}
 function add(id,qty=1){const row=cart.find(x=>String(x.id)===String(id));row?row.qty+=qty:cart.push({id:String(id),qty});save(CART,cart);updateCounts()}
 function toggleWish(id){id=String(id);wish=wish.includes(id)?wish.filter(x=>x!==id):[...wish,id];save(WISH,wish);updateCounts();renderList()}
 function filtered(){let a=products.filter(x=>(active==='Tất cả'||x.cat===active)&&(!term||x.name.toLowerCase().includes(term)));if(sort==='low')a.sort((a,b)=>a.price-b.price);if(sort==='high')a.sort((a,b)=>b.price-a.price);return a}
 function renderList(){const a=filtered(),pages=Math.max(1,Math.ceil(a.length/PAGE));page=Math.min(page,pages);document.getElementById('v24Result').textContent=`${a.length} sản phẩm · Trang ${page}/${pages}`;document.getElementById('v24Grid').innerHTML=a.slice((page-1)*PAGE,page*PAGE).map(card).join('')||'<div class="v24-empty">Không tìm thấy sản phẩm phù hợp.</div>';document.getElementById('v24Pages').innerHTML=Array.from({length:pages},(_,i)=>`<button class="${i+1===page?'active':''}" data-page="${i+1}">${i+1}</button>`).join('');bindDynamic()}
 function bindDynamic(){document.querySelectorAll('[data-add]').forEach(b=>b.onclick=()=>{add(b.dataset.add);openCart()});document.querySelectorAll('[data-wish]').forEach(b=>b.onclick=()=>toggleWish(b.dataset.wish));document.querySelectorAll('[data-page]').forEach(b=>b.onclick=()=>{page=+b.dataset.page;renderList();document.getElementById('catalog').scrollIntoView({behavior:'smooth'})})}
 function openCart(){const body=document.getElementById('v24DrawerBody');document.getElementById('v24DrawerTitle').textContent='Giỏ hàng';body.innerHTML=cart.length?cart.map(r=>{const x=get(r.id);return x?`<div class="v24-line"><img src="${esc(x.img)}"><div><b>${esc(x.name)}</b><small>${money(x.price)} × ${r.qty}</small></div><button data-remove="${esc(x.id)}">×</button></div>`:''}).join('')+`<div class="v24-total"><span>Tạm tính</span><span>${money(cart.reduce((n,r)=>n+(get(r.id)?.price||0)*r.qty,0))}</span></div><button class="v24-checkout" id="v24Checkout">Tiến hành đặt hàng</button>`:'<div class="v24-empty">Giỏ hàng đang trống.</div>';openDrawer();body.querySelectorAll('[data-remove]').forEach(b=>b.onclick=()=>{cart=cart.filter(x=>x.id!==b.dataset.remove);save(CART,cart);updateCounts();openCart()});const ck=document.getElementById('v24Checkout');if(ck)ck.onclick=checkout}
 function openWish(){const body=document.getElementById('v24DrawerBody');document.getElementById('v24DrawerTitle').textContent='Sản phẩm yêu thích';const a=products.filter(x=>wish.includes(String(x.id)));body.innerHTML=a.length?a.map(x=>`<div class="v24-line"><img src="${esc(x.img)}"><div><b>${esc(x.name)}</b><small>${money(x.price)}</small></div><button data-wadd="${esc(x.id)}">+ Giỏ</button></div>`).join(''):'<div class="v24-empty">Bạn chưa lưu sản phẩm nào.</div>';openDrawer();body.querySelectorAll('[data-wadd]').forEach(b=>b.onclick=()=>{add(b.dataset.wadd);openCart()})}
 function openDrawer(){document.getElementById('v24Overlay').classList.add('open')}function closeDrawer(){document.getElementById('v24Overlay').classList.remove('open')}
 function checkout(){const body=document.getElementById('v24DrawerBody');document.getElementById('v24DrawerTitle').textContent='Thông tin đặt hàng';body.innerHTML=`<form class="v24-checkform" id="v24CheckForm"><input required name="name" placeholder="Họ và tên *"><input required name="phone" placeholder="Số điện thoại *"><div class="row"><input name="province" placeholder="Tỉnh / Thành phố"><input name="ward" placeholder="Phường / Xã"></div><input name="address" placeholder="Địa chỉ nhận hàng"><select name="payment"><option>Thanh toán khi nhận hàng (COD)</option><option>Chuyển khoản / VietQR</option></select><textarea name="note" rows="3" placeholder="Ghi chú đơn hàng"></textarea><button class="v24-checkout">Xác nhận đặt hàng</button></form>`;document.getElementById('v24CheckForm').onsubmit=e=>{e.preventDefault();body.innerHTML='<div class="v24-empty"><h3>✓ Đã tiếp nhận đơn hàng</h3><p>Cửa hàng sẽ liên hệ xác nhận với bạn.</p></div>';cart=[];save(CART,cart);updateCounts()}}
 function detail(id){const x=get(id),box=document.getElementById('v24Detail');if(!x){box.classList.remove('open');return}const related=products.filter(p=>p.cat===x.cat&&String(p.id)!==String(x.id)).slice(0,4),gallery=(x.images&&x.images.length?x.images:[x.img]).filter(Boolean);document.getElementById('v24DetailBody').innerHTML=`<div class="v24-wrap v24-detailgrid"><div class="v25-gallery"><div class="v25-thumbs">${gallery.map((im,i)=>`<button class="v25-thumb ${i?'':'active'}" data-v25-thumb="${i}"><img src="${esc(im)}" alt="${esc(x.name)} - ảnh ${i+1}"></button>`).join('')}</div><div class="v25-main"><img id="v25MainImage" src="${esc(gallery[0]||x.img)}" alt="${esc(x.name)}"></div></div><div class="v25-summary"><small>${esc(x.cat)}</small><h1>${esc(x.name)}</h1><div class="v24-detailprice"><b>${money(x.price)}</b>${x.old>x.price?`<del>${money(x.old)}</del>`:''}</div><div class="v25-meta">${x.badge?`<span>${esc(x.badge)}</span>`:''}<span>${x.stock>0?'✓ Còn '+x.stock+' sản phẩm':'Tạm hết hàng'}</span></div><p class="v24-desc">${esc(x.shortDesc||'')}</p><div class="v24-buy"><button type="button" class="buy" data-detail-add="${esc(x.id)}">Thêm vào giỏ hàng</button><button type="button" data-detail-buy="${esc(x.id)}">Mua ngay</button></div><button type="button" class="v24-tool" data-wish="${esc(x.id)}">${wish.includes(String(x.id))?'♥ Đã yêu thích':'♡ Lưu yêu thích'}</button><div class="v25-tabs"><div class="v25-tabnav"><button type="button" class="active">Mô tả sản phẩm</button></div><div class="v25-tabpanel">${x.desc||'<p>Thông tin chi tiết đang được cập nhật.</p>'}</div></div></div></div><div class="v24-wrap v24-related"><div class="v25-related-head"><div><small>CÓ THỂ BẠN SẼ THÍCH</small><h2>Sản phẩm liên quan</h2></div><p>Cùng danh mục ${esc(x.cat)}</p></div><div class="v24-grid">${related.map(card).join('')||'<p>Chưa có sản phẩm liên quan.</p>'}</div></div>`;box.classList.add('open');document.documentElement.classList.add('commerce-detail-open');document.body.classList.add('commerce-detail-open');document.querySelectorAll('[data-v25-thumb]').forEach(b=>b.onclick=()=>{document.querySelectorAll('[data-v25-thumb]').forEach(n=>n.classList.remove('active'));b.classList.add('active');document.getElementById('v25MainImage').src=gallery[+b.dataset.v25Thumb]});const addBtn=document.querySelector('[data-detail-add]');if(addBtn)addBtn.onclick=()=>{add(x.id);addBtn.textContent='✓ Đã thêm vào giỏ';setTimeout(()=>addBtn.textContent='Thêm vào giỏ hàng',1400);openCart()};const buyBtn=document.querySelector('[data-detail-buy]');if(buyBtn)buyBtn.onclick=()=>{add(x.id);openCart();checkout()};bindDynamic()}
 function route(){const m=location.hash.match(/^#product-(.+)$/);if(m)detail(decodeURIComponent(m[1]));else{document.getElementById('v24Detail').classList.remove('open');document.documentElement.classList.remove('commerce-detail-open');document.body.classList.remove('commerce-detail-open')}}
 document.querySelectorAll('[data-filter]').forEach(b=>b.onclick=()=>{document.querySelectorAll('[data-filter]').forEach(x=>x.classList.remove('active'));b.classList.add('active');active=b.dataset.filter;page=1;renderList()});document.getElementById('v24Sort').onchange=e=>{sort=e.target.value;page=1;renderList()};document.getElementById('v24Search').onsubmit=e=>{e.preventDefault();term=document.getElementById('v24SearchInput').value.trim().toLowerCase();page=1;renderList();document.getElementById('catalog').scrollIntoView({behavior:'smooth'})};document.getElementById('v24CartBtn').onclick=openCart;document.getElementById('v24WishBtn').onclick=openWish;document.querySelector('[data-close]').onclick=closeDrawer;document.getElementById('v24Overlay').onclick=e=>{if(e.target.id==='v24Overlay')closeDrawer()};document.getElementById('v24Back').onclick=()=>{history.pushState(null,'',location.pathname+location.search);route()};addEventListener('hashchange',route);updateCounts();renderList();route();
}

function renderShowcaseTemplate(preset,site={}){
 if(preset==='universal_commerce_5'){renderCommerceStore(site);return}
 const root=document.querySelector('main');if(!root)return;document.querySelectorAll('footer.footer,footer.public-footer').forEach(x=>x.remove());
 const map={personal_blog_1:['blog-ca-nhan-1','blog','NHẬT KÝ CÁ NHÂN','Bài viết, trải nghiệm và góc nhìn được sắp xếp rõ ràng theo từng chuyên mục.','Đọc bài mới','violet'],personal_blog_2:['blog-ca-nhan-2','creator','GÓC NGƯỜI SÁNG TẠO','Không gian dành cho nội dung, dự án và hành trình xây dựng thương hiệu cá nhân.','Khám phá nội dung','orange'],corporate_modern_1:['doanh-nghiep-1','corp','DOANH NGHIỆP HIỆN ĐẠI','Bố cục dùng được cho nhiều ngành nghề với giới thiệu, dịch vụ, dự án, tin tức và tuyển dụng.','Nhận tư vấn','blue'],corporate_industry_2:['doanh-nghiep-2','industry','HỒ SƠ DOANH NGHIỆP CHUYÊN NGHIỆP','Giới thiệu năng lực, sản phẩm dịch vụ, dự án, tin tức và tuyển dụng trong một bố cục mạnh mẽ.','Nhận tư vấn','slate'],universal_commerce_5:['dich-vu-5','service','CỬA HÀNG ONLINE','Trưng bày laptop, cấu hình, giá và nhận yêu cầu tư vấn.','Nhận tư vấn','blue']};
 const c=map[preset]||map.personal_blog_1,key=c[0],kind=c[1],settings=sxSettings(site),cats=sxProfile(site,key),all=(SITE_DATA?.posts||[]).filter(p=>p.status!=='draft'&&(kind==='ev'?p.type==='service':p.type==='news'));
 const id=new URLSearchParams(location.search).get('id'),detail=id?all.find(p=>String(p.id)===String(id)):null;
 const brand=esc(site.name||c[2]),phone=String(site.phone||''),email=String(site.email||site.contact_email||''),zalo=String(site.zalo||''),facebook=String(site.facebook||''),address=String(settings.contact_address||''),hours=String(settings.working_hours||''),contactTitle=String(settings.contact_title||'Liên hệ với chúng tôi'),contactIntro=String(settings.contact_intro||'Để lại thông tin hoặc liên hệ trực tiếp để được hỗ trợ.'),cta=String(settings.contact_cta||'Gửi yêu cầu'),footerIntro=String(settings.footer_intro||'Thông tin ngắn về doanh nghiệp hoặc thương hiệu.'),footerAddress=String(settings.footer_address||address||''),footerPhone=String(settings.footer_phone||phone||''),footerEmail=String(settings.footer_email||email||''),footerFacebook=String(settings.footer_facebook||facebook||''),footerZalo=String(settings.footer_zalo||zalo||''),privacyUrl=String(settings.privacy_url||'#'),termsUrl=String(settings.terms_url||'#'),footerCopyright=String(settings.footer_copyright||('© 2026 '+(site.name||c[2])+'. Bản quyền thuộc doanh nghiệp.'));
 if(detail){const img=getImages(detail)[0]||detail.image||'/assets/marketing-demo.webp';root.innerHTML=`<div class="sx-site sx-${kind}" data-accent="${c[5]}"><header class="sx-header"><div class="sx-wrap"><a class="sx-brand" href="/">${brand}</a><nav>${cats.slice(0,5).map(x=>`<a href="/#cat-${sxSlug(x)}">${esc(x)}</a>`).join('')}</nav><div class="sx-head-actions"><a class="sx-head-admin" href="${sxAdminNewPostUrl(key)}" target="_blank" rel="noopener">＋ Đăng bài</a><a class="sx-head-cta" href="/#contact">Liên hệ</a></div></div></header><article class="sxp-detail"><div class="sx-wrap"><a class="sxp-back" href="/">← Về trang chủ</a><small>${esc(detail.category||'Bài viết')}</small><h1>${esc(detail.title||'Bài viết')}</h1><img src="${esc(img)}" alt="${esc(detail.title||'Bài viết')}"><div class="sxp-article-body">${detail.content||'<p>Nội dung đang được cập nhật.</p>'}</div></div></article><footer class="sx-footer sx-footer-pro"><div class="sx-wrap sx-footer-grid"><div><b>${brand}</b><p>${esc(footerIntro)}</p></div><div><b>Liên kết nhanh</b><a href="/">Trang chủ</a><a href="/#contact">Liên hệ</a></div><div><b>Thông tin liên hệ</b>${footerPhone?`<a href="tel:${esc(footerPhone.replace(/\s/g,''))}">${esc(footerPhone)}</a>`:''}${footerEmail?`<a href="mailto:${esc(footerEmail)}">${esc(footerEmail)}</a>`:''}${footerAddress?`<span>${esc(footerAddress)}</span>`:''}</div><div><b>Chính sách</b><a href="${esc(privacyUrl)}">Chính sách bảo mật</a><a href="${esc(termsUrl)}">Điều khoản sử dụng</a></div></div><div class="sx-wrap sx-footer-bottom"><span>${esc(footerCopyright)}</span></div></footer></div>`;return}
 const featured=all[0],heroImg=settings.hero_image||(featured?(getImages(featured)[0]||featured.image):'/assets/marketing-demo.webp');
 const categorySections=cats.map((cat,idx)=>{const arr=all.filter(x=>String(x.category||'').trim()===cat).slice(0,6);return `<section class="sxp-category ${idx%2?'is-alt':''}" id="cat-${sxSlug(cat)}" data-structure-key="cat-${idx+1}"><div class="sx-wrap"><div class="sxp-section-head"><div><small>CHUYÊN MỤC ${String(idx+1).padStart(2,'0')}</small><h2>${esc(cat)}</h2></div><span>${arr.length} bài đang hiển thị</span></div><div class="sxp-grid">${arr.length?arr.map(sxCard).join(''):`<div class="sxp-empty">Chưa có bài trong chuyên mục <b>${esc(cat)}</b>. Khi đăng bài trong Trang quản trị và chọn đúng chuyên mục, nội dung sẽ tự xuất hiện tại đây.</div>`}</div></div></section>`}).join('');
 const direct=[phone?`<a href="tel:${esc(phone.replace(/\s/g,''))}">☎ ${esc(phone)}</a>`:'',email?`<a href="mailto:${esc(email)}">✉ ${esc(email)}</a>`:'',zalo?`<a href="https://zalo.me/${esc(zalo.replace(/\D/g,''))}" target="_blank" rel="noopener">Zalo</a>`:'',facebook?`<a href="${esc(facebook)}" target="_blank" rel="noopener">Facebook</a>`:''].filter(Boolean).join('');
 root.innerHTML=`<div class="sx-site sx-${kind}" data-accent="${c[5]}"><header class="sx-header"><div class="sx-wrap"><a class="sx-brand" href="/">${brand}</a><nav>${cats.slice(0,4).map(x=>`<a href="#cat-${sxSlug(x)}">${esc(x)}</a>`).join('')}<a href="#contact">Liên hệ</a></nav><div class="sx-head-actions"><a class="sx-head-admin" href="${sxAdminNewPostUrl(key)}" target="_blank" rel="noopener">＋ Đăng bài</a><a class="sx-head-cta" href="#contact">${esc(c[4])}</a></div></div></header><section class="sxp-hero"><div class="sx-wrap sxp-hero-grid"><div><span class="sx-kicker">${c[2]}</span><h1>${brand}</h1><p>${esc(settings.hero_note||c[3])}</p><div class="sx-actions"><a href="#noi-dung">Xem nội dung</a><a class="ghost" href="#contact">Liên hệ →</a></div><div class="sxp-cat-chips">${cats.map(x=>`<a href="#cat-${sxSlug(x)}">${esc(x)}</a>`).join('')}</div></div><div class="sxp-hero-photo"><img src="${esc(heroImg)}" alt="${brand}">${featured?`<a href="${sxPostUrl(featured)}"><small>${esc(featured.category||'Nổi bật')}</small><b>${esc(featured.title)}</b></a>`:''}</div></div></section><section class="sxp-featured" id="noi-dung"><div class="sx-wrap"><div class="sxp-section-head"><div><small>NỘI DUNG MỚI</small><h2>${kind==='ev'?'Kiến thức & dịch vụ mới':'Bài viết mới nhất'}</h2></div><span>${all.length} nội dung</span></div><div class="sxp-grid sxp-grid-featured">${all.slice(0,6).map(sxCard).join('')||'<div class="sxp-empty">Chưa có bài viết. Hãy vào Trang quản trị để đăng nội dung đầu tiên.</div>'}</div></div></section>${categorySections}<section class="sxp-contact" id="contact" data-structure-key="contact"><div class="sx-wrap sxp-contact-grid"><div><small>LIÊN HỆ</small><h2>${esc(contactTitle)}</h2><p>${esc(contactIntro)}</p><div class="sxp-contact-direct">${direct||'<span>Hãy cập nhật số điện thoại, email hoặc Zalo trong Trang quản trị.</span>'}</div>${address?`<p><b>Địa chỉ:</b> ${esc(address)}</p>`:''}${hours?`<p><b>Giờ làm việc:</b> ${esc(hours)}</p>`:''}${settings.map_url?`<a class="sxp-map" href="${esc(settings.map_url)}" target="_blank" rel="noopener">Xem bản đồ →</a>`:''}</div><form id="sxpContactForm" class="sxp-contact-form"><input name="customer_name" required placeholder="Họ và tên *"><input name="phone" required inputmode="tel" placeholder="Số điện thoại *"><input name="province" placeholder="Tỉnh / Thành phố"><textarea name="need" rows="5" placeholder="Nội dung cần tư vấn"></textarea><button type="submit">${esc(cta)} →</button><div id="sxpContactMsg" class="sxp-contact-msg"></div></form></div></section><footer class="sx-footer sx-footer-pro"><div class="sx-wrap sx-footer-grid"><div><b>${brand}</b><p>${esc(footerIntro)}</p></div><div><b>Liên kết nhanh</b><a href="/">Trang chủ</a><a href="#noi-dung">Bài viết</a><a href="#contact">Liên hệ</a></div><div><b>Thông tin liên hệ</b>${footerPhone?`<a href="tel:${esc(footerPhone.replace(/\s/g,''))}">☎ ${esc(footerPhone)}</a>`:''}${footerEmail?`<a href="mailto:${esc(footerEmail)}">✉ ${esc(footerEmail)}</a>`:''}${footerAddress?`<span>${esc(footerAddress)}</span>`:''}${footerZalo?`<a href="https://zalo.me/${esc(footerZalo.replace(/\D/g,''))}" target="_blank" rel="noopener">Zalo</a>`:''}</div><div><b>Thông tin khác</b>${footerFacebook?`<a href="${esc(footerFacebook)}" target="_blank" rel="noopener">Facebook</a>`:''}<a href="${esc(privacyUrl)}">Chính sách bảo mật</a><a href="${esc(termsUrl)}">Điều khoản sử dụng</a></div></div><div class="sx-wrap sx-footer-bottom"><span>${esc(footerCopyright)}</span><span>Website quản trị bằng HoangVuongTech</span></div></footer></div>`;sxBindContact(site,key);try{nrApplyStructureGeometry(site,key);nrAuditStructureContract(site,key)}catch(e){}
}
