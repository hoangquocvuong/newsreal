...
async function nrBootMain(){
 try{
   ...
   document.body.classList.toggle('theme-showcase-new',['personal_blog_1','personal_blog_2','corporate_modern_1','corporate_industry_2','service_ev_charge_5','service_lion_dance_6'].includes(effectivePreset));
   ...
   }else if(['personal_blog_1','personal_blog_2','corporate_modern_1','corporate_industry_2','service_ev_charge_5'].includes(effectivePreset)){
...
// V20.9.27.13 — Template Contract V2: runtime defaults come from the same settings_schema used by Admin.
function sxStructure(site={}){let st=site.structure_profile;if(typeof st==='string'){try{st=JSON.parse(st||'{}')}catch(e){st={}}}return st&&typeof st==='object'?st:{}}
function sxSchemaDefault(site,key,fallback=''){const st=sxStructure(site),row=(Array.isArray(st.settings_schema)?st.settings_schema:[]).find(x=>String(x?.key||'')===String(key||''));return row&&Object.prototype.hasOwnProperty.call(row,'default')?String(row.default??''):String(fallback??'')}
function sxSetting(site,settings,key,fallback=''){return settings&&Object.prototype.hasOwnProperty.call(settings,key)?String(settings[key]??''):sxSchemaDefault(site,key,fallback)}
function sxHeroContract(site={}){const st=sxStructure(site);return st&&st.contract_version==='template-ssot-v2'&&st.hero&&typeof st.hero==='object'?st.hero:{}}
function sxProfile(site={},key=''){let st=site.structure_profile;if(typeof st==='string'){try{st=JSON.parse(st)}catch(e){st={}}}const fallback={
 ...
 'dich-vu-5':['Kiến thức sạc','Lắp đặt tại nhà','Điểm sạc công cộng','Kỹ thuật điện','Bảo trì','Vận hành đội xe'],
...
function lionServiceCard(p){const e=lionExtra(p),img=getImages(p)[0]||p.image||'/assets/marketing-demo.webp',price=e.service_price||'Liên hệ báo giá',excerpt=sxText(p.excerpt||p.content||'').slice(0,165);return `<article class="ld-pack"><div class="ld-pack-img"><a href="${sxPostUrl(p)}"><img src="${esc(img)}" alt="${esc(p.title||'Dịch vụ')}"></a><span>${esc(price)}</span></div><div class="ld-pack-body"><small>${esc(p.category||'Dịch vụ')}</small><h3><a href="${sxPostUrl(p)}">${esc(p.title||'Dịch vụ')}</a></h3>${excerpt?`<p class="ld-pack-excerpt">${esc(excerpt)}</p>`:''}<div class="ld-spec-row"><span><b>${esc(e.lion_count||'—')}</b>Lân</span><span><b>${esc(e.drum_count||'—')}</b>Trống</span><span><b>${esc(e.performers_count||'—')}</b>Nhân sự</span></div><ul><li>Pháo sáng: ${esc(e.fireworks||'Theo yêu cầu')}</li><li>Kim tuyến: ${esc(e.confetti||'Theo yêu cầu')}</li><li>Câu đối: ${esc(e.couplets||'Theo kịch bản')}</li></ul><div class="ld-pack-actions"><a href="#contact">Liên hệ báo giá</a><button type="button" data-ld-price>Giá tiền</button></div><div class="ld-pack-price">${esc(price)}<small>Giá có thể thay đổi theo địa điểm, thời lượng và hạng mục bổ sung.</small></div><a class="ld-pack-detail" href="${sxPostUrl(p)}">Xem chi tiết gói →</a></div></article>`}
function lionVideoEmbedUrl(raw=''){try{const u=new URL(String(raw).trim());const h=u.hostname.replace(/^www\./,'').toLowerCase();if(h==='youtu.be'){const id=u.pathname.split('/').filter(Boolean)[0];return id?'https://www.youtube-nocookie.com/embed/'+encodeURIComponent(id):''}if(h==='youtube.com'||h==='m.youtube.com'||h==='music.youtube.com'){let id=u.searchParams.get('v');if(!id&&u.pathname.startsWith('/shorts/'))id=u.pathname.split('/')[2];if(!id&&u.pathname.startsWith('/embed/'))id=u.pathname.split('/')[2];return id?'https://www.youtube-nocookie.com/embed/'+encodeURIComponent(id):''}if(h==='vimeo.com'||h==='player.vimeo.com'){const parts=u.pathname.split('/').filter(Boolean),id=parts.reverse().find(x=>/^\d+$/.test(x));return id?'https://player.vimeo.com/video/'+id:''}}catch(e){}return ''}
function lionVideoUrls(detail={}){const e=lionExtra(detail),raw=String(e.video_embeds||'');return raw.split(/[\n,]+/).map(x=>x.trim()).filter(Boolean).map(lionVideoEmbedUrl).filter(Boolean).slice(0,8)}
function lionGallery(images=[],title='',videos=[]){const media=[...images.map(src=>({type:'image',src})),...videos.map(src=>({type:'video',src}))];if(!media.length)return '';const first=media[0];const main=first.type==='video'?`<iframe id="ldGalleryVideo" src="${esc(first.src)}" title="${esc(title)}" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen loading="lazy"></iframe>`:`<img id="ldGalleryMain" src="${esc(first.src)}" alt="${esc(title)}">`;return `<div class="ld-gallery"><div class="ld-gallery-main" id="ldGalleryStage">${main}</div><div class="ld-gallery-thumbs">${media.map((x,i)=>x.type==='video'?`<button type="button" class="${i===0?'active':''}" data-ld-video="${esc(x.src)}"><span class="ld-video-thumb">▶ Video</span></button>`:`<button type="button" class="${i===0?'active':''}" data-ld-src="${esc(x.src)}"><img src="${esc(x.src)}" alt="${esc(title)} ${i+1}"></button>`).join('')}</div></div>`}
function bindLionGallery(){const stage=document.getElementById('ldGalleryStage');if(!stage)return;const buttons=document.querySelectorAll('.ld-gallery-thumbs button');buttons.forEach(b=>b.onclick=function(){buttons.forEach(x=>x.classList.remove('active'));this.classList.add('active');if(this.dataset.ldVideo){stage.innerHTML=`<iframe src="${esc(this.dataset.ldVideo)}" title="Video" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen loading="lazy"></iframe>`}else{stage.innerHTML=`<img id="ldGalleryMain" src="${esc(this.dataset.ldSrc||'')}" alt="Ảnh bài viết">`}})}
function bindLionHeroSlider(){const imgs=[...document.querySelectorAll('.ld-hero-slides img')],dots=[...document.querySelectorAll('[data-ld-hero-slide]')];if(imgs.length<2)return;let i=0,t=null;const go=n=>{i=(n+imgs.length)%imgs.length;imgs.forEach((x,j)=>x.classList.toggle('active',j===i));dots.forEach((x,j)=>x.classList.toggle('active',j===i))};dots.forEach((b,j)=>b.onclick=()=>{go(j);if(t)clearInterval(t);t=setInterval(()=>go(i+1),4500)});t=setInterval(()=>go(i+1),4500)}
function renderLionDanceTemplate(site={}){const root=document.querySelector('main');if(!root)return;document.querySelectorAll('footer.footer,footer.public-footer').forEach(x=>x.remove());const key='dich-vu-6',settings=sxSettings(site),cats=sxProfile(site,key),posts=(SITE_DATA?.posts||[]).filter(p=>p.status!=='draft'&&p.type==='service'),id=new URLSearchParams(location.search).get('id'),detail=id?posts.find(p=>String(p.id)===String(id)):null,brand=esc(site.name||'Đoàn Lân Sư Rồng'),phone=String(site.phone||''),email=String(site.email||site.contact_email||''),zalo=String(site.zalo||''),facebook=String(site.facebook||''),address=String(settings.contact_address||''),intro=String(settings.troupe_intro||'Đoàn Lân Sư Rồng chuyên biểu diễn khai trương, lễ hội, Trung Thu, Tết và sự kiện doanh nghiệp.'),experience='15+ năm biểu diễn',eventCount='800+ sự kiện',memberCount='35+ thành viên',awards=[settings.award_1,settings.award_2,settings.award_3].filter(Boolean),footerIntro=String(settings.footer_intro||intro),footerPhone=String(settings.footer_phone||phone||''),footerEmail=String(settings.footer_email||email||''),privacyUrl=String(settings.privacy_url||'#'),termsUrl=String(settings.terms_url||'#'),copyright=String(settings.footer_copyright||('© 2026 '+(site.name||'Đoàn Lân Sư Rồng')));const direct=[phone?`<a href="tel:${esc(phone.replace(/\s/g,''))}">☎ ${esc(phone)}</a>`:'',email?`<a href="mailto:${esc(email)}">✉ ${esc(email)}</a>`:'',zalo?`<a href="https://zalo.me/${esc(zalo.replace(/\D/g,''))}" target="_blank" rel="noopener">Zalo</a>`:'',facebook?`<a href="${esc(facebook)}" target="_blank" rel="noopener">Facebook</a>`:''].filter(Boolean).join('');if(detail){const imgs=getImages(detail);const e=lionExtra(detail);root.innerHTML=`<div class="ld-site"><header class="ld-header"><div class="ld-wrap"><a class="ld-brand" href="/">${brand}<small>LÂN · SƯ · RỒNG</small></a><nav><a href="/">Trang chủ</a><a href="/#packages">Gói dịch vụ</a><a href="/#experience">Năng lực</a><a href="/#contact">Liên hệ</a></nav><a class="ld-admin" href="${sxAdminNewPostUrl(key)}" target="_blank" rel="noopener">＋ Đăng bài</a></div></header><article class="ld-detail"><div class="ld-wrap"><a class="ld-back" href="/">← Trang chủ</a><small>${esc(detail.category||'Dịch vụ')}</small><h1>${esc(detail.title||'Dịch vụ')}</h1><p>${esc(sxText(detail.content||'').slice(0,220))}</p>${lionGallery(imgs,detail.title||'',lionVideoUrls(detail))}<div class="ld-detail-grid"><div class="ld-copy">${detail.content||'<p>Nội dung đang được cập nhật.</p>'}</div><aside><h3>Thông tin gói</h3>${Object.entries({"Giá":e.service_price,"Đội hình":e.lion_count,"Trống & bộ gõ":e.drum_count,"Nhân sự":e.performers_count,"Thời lượng":e.performance_duration,"Pháo sáng":e.fireworks,"Kim tuyến":e.confetti,"Câu đối":e.couplets}).filter(x=>x[1]).map(([k,v])=>`<div><small>${esc(k)}</small><b>${esc(v)}</b></div>`).join('')}<a href="/#contact">Liên hệ báo giá</a></aside></div></div></article><footer class="ld-footer"><div class="ld-wrap"><b>${brand}</b><span>${esc(copyright)}</span></div></footer></div>`;bindLionGallery();return}const packages=posts.filter(x=>x.category==='Gói múa lân').slice(0,7),dragon=posts.filter(x=>x.category==='Múa rồng').slice(0,2),drums=posts.filter(x=>x.category==='Trống hội').slice(0,2),events=posts.filter(x=>x.category==='Sự kiện đã thực hiện').slice(0,3),blog=posts.filter(x=>['Tin hoạt động','Kiến thức & phong tục'].includes(x.category)).slice(0,6),featured=posts[0],heroImages=['https://images.unsplash.com/photo-1675605849420-8637ece6a0a1?auto=format&fit=crop&w=1800&q=86','https://images.unsplash.com/photo-1772374998724-a8d939b12acb?auto=format&fit=crop&w=1800&q=84','https://images.unsplash.com/photo-1758776935722-77bd8efa6339?auto=format&fit=crop&w=1800&q=84'],heroBadge='LÂN SƯ RỒNG · BIỂU DIỄN SỰ KIỆN',heroTitle='Khai hội rộn ràng – Khởi đầu may mắn cùng Lân Sư Rồng chuyên nghiệp.',heroNote='Nhận biểu diễn khai trương, động thổ, Trung Thu, Tết, lễ hội và sự kiện doanh nghiệp. Gói dịch vụ rõ nhân sự, nhạc cụ, đạo cụ và hạng mục đi kèm để khách dễ chọn.',heroPrimary='Xem gói biểu diễn',heroPrimaryUrl='#packages',heroSecondary='Liên hệ báo giá',heroSecondaryUrl='#contact',heroStat4='24/7',heroStat4Label='Nhận lịch sự kiện';root.innerHTML=`<div class="ld-site"><header class="ld-header"><div class="ld-wrap"><a class="ld-brand" href="/">${brand}<small>LÂN · SƯ · RỒNG</small></a><nav><a href="#packages">Gói dịch vụ</a><a href="#experience">Năng lực</a><a href="#events">Sự kiện</a><a href="#blog">Bài viết</a><a href="#contact">Liên hệ</a></nav><a class="ld-admin" href="${sxAdminNewPostUrl(key)}" target="_blank" rel="noopener">＋ Đăng bài</a></div></header><section class="ld-hero${heroImages.length?'':' ld-hero-empty'}"><div class="ld-hero-slides">${heroImages.map((src,i)=>`<img class="${i===0?'active':''}" src="${esc(src)}" alt="${esc(heroTitle)} ${i+1}">`).join('')}</div><div class="ld-wrap"><small>${esc(heroBadge)}</small><h1>${esc(heroTitle)}</h1><p>${esc(heroNote)}</p><div class="ld-actions"><a href="${esc(heroPrimaryUrl)}">${esc(heroPrimary)}</a><a class="alt" href="${esc(heroSecondaryUrl)}">${esc(heroSecondary)}</a></div><div class="ld-stats"><div><b>${esc(experience)}</b><span>Kinh nghiệm</span></div><div><b>${esc(eventCount)}</b><span>Sự kiện</span></div><div><b>${esc(memberCount)}</b><span>Thành viên</span></div><div><b>${esc(heroStat4)}</b><span>${esc(heroStat4Label)}</span></div></div></div>${heroImages.length>1?`<div class="ld-hero-dots">${heroImages.map((_,i)=>`<button type="button" class="${i===0?'active':''}" data-ld-hero-slide="${i}" aria-label="Ảnh Hero ${i+1}"></button>`).join('')}</div>`:''}</section><section class="ld-section" id="packages"><div class="ld-wrap"><div class="ld-title"><div><small>GÓI MÚA LÂN</small><h2>Chọn đội hình theo quy mô sự kiện</h2></div><p>Mỗi gói nêu rõ số đầu lân, nhạc cụ, nhân sự và hiệu ứng đi kèm. Giá hiển thị là mức tham khảo để khách dễ dự trù ngân sách.</p></div><div class="ld-pack-grid">${packages.length?packages.map(lionServiceCard).join(''):'<div class="sxp-empty">Chưa có gói dịch vụ. Vào Trang quản trị để đăng các gói 2–7 đầu lân.</div>'}</div></div></section><section class="ld-section ld-soft"><div class="ld-wrap"><div class="ld-title"><div><small>MỞ RỘNG CHƯƠNG TRÌNH</small><h2>Múa rồng & Trống hội</h2></div><p>Kết hợp thành chương trình khai hội trọn gói cho sân khấu lớn, lễ hội và sự kiện doanh nghiệp.</p></div><div class="ld-feature-grid">${[...dragon,...drums].map(p=>`<a href="${sxPostUrl(p)}">${p.image?`<img src="${esc(getImages(p)[0]||p.image)}" alt="${esc(p.title)}">`:''}<div><small>${esc(p.category)}</small><h3>${esc(p.title)}</h3><p>${esc(sxText(p.content).slice(0,120))}</p></div></a>`).join('')||'<div class="sxp-empty">Đăng bài Múa rồng và Trống hội để hiển thị ở đây.</div>'}</div></div></section><section class="ld-section ld-trust" id="experience"><div class="ld-wrap ld-trust-grid"><div><small>GIỚI THIỆU ĐOÀN</small><h2>Kinh nghiệm, kỷ luật và hình ảnh chuyên nghiệp.</h2><p>${esc(intro)}</p><div class="ld-actions"><a href="#contact">Nhận tư vấn</a><a class="alt" href="#events">Xem sự kiện</a></div></div><div class="ld-awards">${awards.length?awards.map((a,i)=>`<div><b>0${i+1} · THÀNH TÍCH</b><span>${esc(a)}</span></div>`).join(''):'<div><b>DANH HIỆU & THÀNH TÍCH</b><span>Cập nhật các giải thưởng, bằng khen và cột mốc nổi bật trong Trang quản trị.</span></div>'}</div></div></section><section class="ld-section" id="events"><div class="ld-wrap"><div class="ld-title"><div><small>SỰ KIỆN GẦN ĐÂY</small><h2>Những chương trình đã thực hiện</h2></div><p>Mỗi bài có thể tải nhiều ảnh. Trang chi tiết sẽ tự tạo gallery/slideshow.</p></div><div class="ld-event-grid">${events.map(sxCard).join('')||'<div class="sxp-empty">Chưa có sự kiện. Hãy đăng bài và chọn chuyên mục “Sự kiện đã thực hiện”.</div>'}</div></div></section><section class="ld-section ld-soft" id="blog"><div class="ld-wrap"><div class="ld-title"><div><small>BLOG & KINH NGHIỆM</small><h2>Nội dung giúp khách tin tưởng hơn</h2></div></div><div class="ld-blog-grid">${blog.map(sxCard).join('')||'<div class="sxp-empty">Chưa có bài blog.</div>'}</div></div></section><section class="ld-section ld-contact" id="contact"><div class="ld-wrap ld-contact-grid"><div><small>LIÊN HỆ BÁO GIÁ</small><h2>${esc(settings.contact_title||'Kết nối với HẢI PHÒNG LÂN ĐƯỜNG')}</h2><p>${esc(settings.contact_intro||'Trao đổi nhu cầu, nhận tư vấn và thông tin phù hợp. Hotline, email, Zalo và nội dung khối này đều có thể chỉnh trong Trang quản trị.')}</p><div class="ld-direct">${direct||'<span>Hãy cập nhật Hotline, Email, Zalo và Facebook trong Trang quản trị.</span>'}</div>${address?`<p><b>Địa chỉ:</b> ${esc(address)}</p>`:''}</div><form id="sxpContactForm" class="ld-contact-form"><input name="customer_name" required placeholder="Họ và tên *"><input name="phone" required inputmode="tel" placeholder="Số điện thoại *"><input name="province" placeholder="Tỉnh / Thành phố"><input name="district" placeholder="Quận / Huyện"><textarea name="need" rows="5" placeholder="Ngày tổ chức, địa điểm, số đầu lân hoặc nhu cầu chương trình"></textarea><button type="submit">${esc(settings.contact_cta||'Gửi yêu cầu báo giá')} →</button><div id="sxpContactMsg" class="sxp-contact-msg"></div></form></div></section><footer class="ld-footer"><div class="ld-wrap ld-footer-grid"><div><b>${brand}</b><p>${esc(footerIntro)}</p></div><div><b>Dịch vụ</b><a href="#packages">2–7 đầu lân</a><a href="#packages">Múa rồng</a><a href="#packages">Trống hội</a></div><div><b>Liên hệ</b>${footerPhone?`<a href="tel:${esc(footerPhone.replace(/\s/g,''))}">☎ ${esc(footerPhone)}</a>`:''}${footerEmail?`<a href="mailto:${esc(footerEmail)}">✉ ${esc(footerEmail)}</a>`:''}${address?`<span>${esc(address)}</span>`:''}</div><div><b>Thông tin</b><a href="#experience">Giới thiệu đoàn</a><a href="${esc(privacyUrl)}">Chính sách bảo mật</a><a href="${esc(termsUrl)}">Điều khoản sử dụng</a></div></div><div class="ld-wrap ld-footer-bottom">${esc(copyright)}</div></footer><a class="ld-float" href="#contact">☎ Liên hệ báo giá</a></div>`;bindLionHeroSlider();root.querySelectorAll('[data-ld-price]').forEach(btn=>btn.addEventListener('click',()=>btn.closest('.ld-pack')?.classList.toggle('show-price')));sxBindContact(site,key);try{nrApplyStructureGeometry(site,key);nrAuditStructureContract(site,key)}catch(e){}}
function renderShowcaseTemplate(preset,site={}){
 ...
 const featured=all[0],heroContract=sxHeroContract(site),heroImg=sxSetting(site,settings,'hero_image_url',featured?(getImages(featured)[0]||featured.image||''):''),heroTitle=sxSetting(site,settings,'hero_title',site.name||c[0]),heroPrimary=sxSetting(site,settings,'hero_primary_label','Xem nội dung'),heroPrimaryUrl=sxSetting(site,settings,'hero_primary_url','#noi-dung'),heroSecondary=sxSetting(site,settings,'hero_secondary_label','Liên hệ'),heroSecondaryUrl=sxSetting(site,settings,'hero_secondary_url','#contact'),heroNote=sxSetting(site,settings,'hero_note',c[3]);
