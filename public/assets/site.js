...
function nrGameBindLocalFirst(){nrGameSavedSync();nrGameHydrateStats();if(window.__NR_COC_LOCAL_FIRST_BOUND__)return;window.__NR_COC_LOCAL_FIRST_BOUND__=1;document.addEventListener('click',e=>{const vote=e.target.closest('[data-coc-vote]');if(vote){e.preventDefault();nrGameOpenVote(vote.dataset.cocVote);return}const download=e.target.closest('[data-coc-download]');if(download){nrGameRecordStat('download',download.dataset.cocDownload)}const bookmark=e.target.closest('[data-coc-bookmark]');if(bookmark){e.preventDefault();e.stopPropagation();nrGameToggleSaved(bookmark);return}const menuToggle=e.target.closest('[data-coc-menu-toggle="1"]');if(menuToggle){e.preventDefault();nrGameSetMenuOpen(menuToggle.getAttribute('aria-expanded')!=='true');return}const menuLink=e.target.closest('[data-coc-mobile-menu="1"] a');if(menuLink){nrGameSetMenuOpen(false);return}const open=e.target.closest('[data-coc-open-saved="1"]');if(open){e.preventDefault();nrGameSetMenuOpen(false);nrGameSetSavedOpen(true);return}const close=e.target.closest('[data-coc-close-saved="1"]');if(close){e.preventDefault();nrGameSetSavedOpen(false);return}const remove=e.target.closest('[data-coc-remove-saved]');if(remove){e.preventDefault();let saved=nrGameSavedRead().filter(x=>x.slug!==remove.dataset.cocRemoveSaved);nrGameSavedWrite(saved);nrGameSavedSync();nrGameToast('Base đã được bỏ khỏi Saved','removed');return}if(!e.target.closest('[data-coc-mobile-menu="1"]')&&!e.target.closest('[data-coc-menu-toggle="1"]'))nrGameSetMenuOpen(false)},true);document.addEventListener('keydown',e=>{if(e.key==='Escape'){nrGameSetSavedOpen(false);nrGameSetMenuOpen(false);nrGameSetFilterOpen(false)}})}
async function nrGameLoadHeroSkinShowcase(){const box=document.querySelector('[data-coc-hero-skin="1"]');if(!box||box.dataset.loaded==='1')return;box.dataset.loaded='1';const img=box.querySelector('img'),name=box.querySelector('[data-coc-skin-name]'),meta=box.querySelector('[data-coc-skin-meta]');const day=new Date().toISOString().slice(0,10),cacheKey='NR_COC_HERO_SHOWCASE_'+day;function apply(s,hero){if(!s||!s.image)return false;img.src=s.image;img.alt=(s.name||'Clash of Clans Hero Skin')+(hero?' · '+hero:'');name.textContent=s.name||'Hero Skin';meta.textContent=[hero,s.year,s.rarity].filter(Boolean).join(' · ');box.classList.add('is-ready');return true}try{const cached=JSON.parse(localStorage.getItem(cacheKey)||'null');if(cached&&apply(cached.skin,cached.hero))return}catch(e){}try{const base='https://hoangquocvuong.github.io/coc-hero-skins/';const index=await fetch(base+'hero-skins.json',{cache:'force-cache'}).then(r=>{if(!r.ok)throw new Error('hero index');return r.json()});const heroes=(index.heroes||[]).filter(x=>x&&x.file);if(!heroes.length)throw new Error('no heroes');const seed=Number(day.replace(/-/g,''))||1;const heroInfo=heroes[seed%heroes.length];const data=await fetch(base+heroInfo.file,{cache:'force-cache'}).then(r=>{if(!r.ok)throw new Error('hero file');return r.json()});const skins=(data.skins||[]).filter(x=>x&&x.image);if(!skins.length)throw new Error('no skins');const years=skins.map(x=>Number(x.year)||0),maxYear=Math.max(...years),latest=skins.filter(x=>(Number(x.year)||0)===maxYear),skin=latest[seed%latest.length]||skins[seed%skins.length];apply(skin,data.hero||heroInfo.name);try{localStorage.setItem(cacheKey,JSON.stringify({skin,hero:data.hero||heroInfo.name}))}catch(e){}}catch(e){box.classList.add('is-fallback')}}
