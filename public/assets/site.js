...
function comAdd(id){const p=COM_DATA.products.find(x=>Number(x.id)===Number(id));if(!p)return;let c=comCartGet(),x=c.find(x=>Number(x.product_id)===Number(id));if(x)x.qty=Math.min(Number(p.stock_qty||99),Number(x.qty||1)+1);else c.push({product_id:p.id,name:p.name,price:Number(p.price),qty:1});comCartSet(c);document.getElementById('comCart')?.classList.add('open')}
function comRemove(id){comCartSet(comCartGet().filter(x=>Number(x.product_id)!==Number(id)))}
function comQty(id,delta){let c=comCartGet(),x=c.find(x=>Number(x.product_id)===Number(id));if(!x)return;x.qty=Math.max(1,Math.min(99,Number(x.qty||1)+delta));comCartSet(c)}
function comCartRender(){const c=comCartGet(),list=document.getElementById('comCartList'),count=document.getElementById('comCartCount'),subtotal=c.reduce((a,x)=>a+Number(x.price)*Number(x.qty),0);if(count)count.textContent=c.reduce((a,x)=>a+Number(x.qty),0);if(list)list.innerHTML=c.length?c.map(x=>`<div class="com-cart-item"><div><b>${esc(x.name)}</b><small>${comMoney(x.price)}</small><div class="com-qty"><button onclick="comQty(${x.product_id},-1)">−</button><span>${x.qty}</span><button onclick="comQty(${x.product_id},1)">+</button></div></div><b>${comMoney(Number(x.price)*Number(x.qty))}</b><button onclick="comRemove(${x.product_id})">Xóa</button></div>`).join(''):'<p>Giỏ hàng đang trống.</p>';const t=document.getElementById('comCartTotal');if(t)t.textContent=comMoney(subtotal);const s=document.getElementById('comSummarySubtotal');if(s)s.textContent=comMoney(subtotal)}
function comProductCards(rows){return rows.map(p=>`<article class="com-card"><div class="com-card-img"><img src="${esc(p.image_url||'/assets/marketing-demo.webp')}" alt="${esc(p.name)}"></div><div class="com-card-body"><span class="com-badge">${esc(p.badge||p.category_name||'SẢN PHẨM')}</span><h3>${esc(p.name)}</h3><p class="com-desc">${esc(p.short_description||'Sản phẩm chất lượng, thông tin có thể tùy chỉnh trong Admin.')}</p><div class="com-stock">${p.stock_status==='out_of_stock'?'Tạm hết hàng':'✓ Còn hàng · '+Number(p.stock_qty||0)+' sản phẩm'}</div><div class="com-price"><strong>${comMoney(p.price)}</strong>${Number(p.compare_at_price)>Number(p.price)?`<del>${comMoney(p.compare_at_price)}</del>`:''}</div><div class="com-actions"><button class="com-detail" onclick="comDetail(${p.id})">Chi tiết</button><button class="com-add" ${p.stock_status==='out_of_stock'?'disabled':''} onclick="comAdd(${p.id})">+ Giỏ hàng</button></div></div></article>`).join('')}
function comProductUrl(p){const base=location.pathname.replace(/\/san-pham\/.*$/,'').replace(/\/$/,'');return base+'/san-pham/'+encodeURIComponent(p.slug||String(p.id))+'/' + location.search}
function comDetail(id){const p=COM_DATA.products.find(x=>Number(x.id)===Number(id));if(p)location.href=comProductUrl(p)}
function comRenderProductDetail(p){let attrs={};try{attrs=JSON.parse(p.attributes_json||'{}')}catch{};let gallery=[];try{gallery=JSON.parse(p.gallery_json||'[]')}catch{};gallery=[p.image_url,...gallery].filter(Boolean);let variants=[];try{variants=JSON.parse(p.variants_json||'[]')}catch{};const root=document.querySelector('main');if(!root)return;root.innerHTML=`<div class="com-product-page com-wrap"><nav class="com-breadcrumb"><a href="${location.pathname.replace(/\/san-pham\/.*$/,'')}">Cửa hàng</a> / ${esc(p.category_name||'Sản phẩm')} / ${esc(p.name)}</nav><section class="com-product-main"><div class="com-gallery"><img class="com-gallery-main" id="comMainImage" src="${esc(gallery[0]||'')}">${gallery.length>1?`<div class="com-thumbs">${gallery.map(x=>`<button onclick="document.getElementById('comMainImage').src='${esc(x)}'"><img src="${esc(x)}"></button>`).join('')}</div>`:''}</div><div class="com-buybox"><small>${esc(p.category_name||'SẢN PHẨM')} · SKU ${esc(p.sku||'—')}</small><h1>${esc(p.name)}</h1><div class="com-stars">★★★★★ <span>4.8 · Đã bán 120+</span></div><p>${esc(p.short_description||'')}</p><div class="com-product-price"><strong>${comMoney(p.price)}</strong>${Number(p.compare_at_price)>Number(p.price)?`<del>${comMoney(p.compare_at_price)}</del>`:''}</div><div class="com-stock">✓ ${p.stock_status==='out_of_stock'?'Tạm hết hàng':'Còn hàng · '+Number(p.stock_qty||0)+' sản phẩm'}</div>${variants.length?`<div class="com-variants"><b>Phiên bản</b>${variants.map((v,i)=>`<label><input type="radio" name="comVariant" value="${i}" ${i?'':'checked'}> ${esc(v.name||v.label||('Biến thể '+(i+1)))}</label>`).join('')}</div>`:''}<div class="com-buy-actions"><button onclick="comAdd(${p.id})">Thêm vào giỏ</button><button onclick="comAdd(${p.id});document.getElementById('comCart').classList.add('open')">Mua ngay</button></div><div class="com-trust">🚚 Giao hàng linh hoạt　↩ Đổi trả theo chính sách　🛡 Bảo hành rõ ràng</div></div></section><section class="com-product-content"><article><h2>Mô tả sản phẩm</h2>${p.description||`<p>${esc(p.short_description||'Thông tin sản phẩm đang được cập nhật.')}</p>`}</article><aside><h2>Thông số / thuộc tính</h2><table>${Object.entries(attrs).map(([k,v])=>`<tr><th>${esc(k.replaceAll('_',' '))}</th><td>${esc(Array.isArray(v)?v.join(', '):v)}</td></tr>`).join('')||'<tr><td>Đang cập nhật thông số.</td></tr>'}</table></aside></section><section><h2>Sản phẩm liên quan</h2><div class="com-grid">${comProductCards(COM_DATA.products.filter(x=>x.id!==p.id&&(x.category_id===p.category_id||x.category_slug===p.category_slug)).slice(0,4))}</div></section></div>`}
