...
function nrGameHeader(prefix,admin){return `<header class="coc-top"><div class="coc-wrap"><a class="coc-brand" href="${prefix||'/'}"><b>COC</b><span>BASE PORTAL</span></a><nav><a href="${prefix||'/'}#town-hall">Town Hall</a><a href="${prefix||'/'}#builder-hall">Builder Hall</a><a href="${prefix||'/'}#clan-capital">Clan Capital</a><a href="${prefix}/free-bases">Free</a><a href="${prefix}/premium-bases">Premium</a></nav>${admin?`<a class="coc-admin" href="${esc(admin)}" target="_blank" rel="noopener">+ Đăng base</a>`:''}</div></header>`}
function nrGameBaseCard(post){const e=nrGameExtra(post),level=nrGameLevel(post),premium=String(e.access_tier||'free').toLowerCase()==='premium',href=post.demo_url||post.url||'#';return `<article class="coc-card"><a class="coc-thumb" href="${esc(href)}"><img src="${esc(post.image||nrGameArt(level.slice(0,2).toLowerCase(),Number(level.replace(/\D/g,''))))}" alt="${esc(post.title||level+' Base')}" loading="lazy"><span class="coc-level">${esc(level)}</span>${premium?'<span class="coc-premium">PREMIUM</span>':'<span class="coc-free">FREE</span>'}</a><div class="coc-card-body"><h3><a href="${esc(href)}">${esc(post.title||level+' Base Layout')}</a></h3><div class="coc-tags"><span>${esc(e.game_purpose||'War')}</span><span>${esc(e.game_style||'Compact')}</span><span>${esc(e.game_defense||'Anti 3 Star')}</span></div></div></article>`}
function nrGameLevelCards(group,prefix){return GAME_LEVELS[group].slice().reverse().map(lv=>{const code=group.toUpperCase()+lv,q=`${group}=${lv}`,free=`${prefix}/free-bases?${q}`,premium=`${prefix}/premium-bases?${q}`;return `<article class="coc-level-card"><a class="coc-level-art" href="${free}"><img src="${esc(nrGameArt(group,lv))}" alt="${code} Clash of Clans" loading="lazy"></a><h3><a href="${free}">${code}</a></h3><div class="coc-level-actions"><a href="${free}">FREE</a><a class="premium" href="${premium}">PREMIUM</a></div></article>`}).join('')}
function nrGameFilterPage(posts,prefix,rel){const premium=/premium-bases/i.test(rel),params=new URLSearchParams(location.search),group=['th','bh','ch'].find(g=>params.has(g))||'th',level=params.get(group)||'all',type=params.get('type')||'all',year=params.get('year')||'all',sort=params.get('sort')||'latest',purposes=group==='th'?['all','war','farming','hybrid','trophy','legend','cwl','troll']:['all','war','trophy','hybrid'],years=['all','2026','2025'];let arr=posts.filter(p=>{const e=nrGameExtra(p),g=String(e.game_group||p.category||'').toLowerCase(),lv=nrGameLevel(p).replace(/\D/g,''),tier=String(e.access_tier||'free').toLowerCase(),pt=String(e.game_purpose||'').toLowerCase();return g.includes(group==='th'?'town hall':group==='bh'?'builder hall':'clan capital')&&(level==='all'||lv===level)&&(type==='all'||pt.startsWith(type))&&(year==='all'||String(e.game_year||'2026')===year)&&tier===(premium?'premium':'free')});if(sort==='rating')arr.sort((a,b)=>Number(b.rating||0)-Number(a.rating||0));else if(sort==='download')arr.sort((a,b)=>Number(nrGameExtra(b).downloads||b.downloads||0)-Number(nrGameExtra(a).downloads||a.downloads||0));else if(sort==='view')arr.sort((a,b)=>Number(b.views||0)-Number(a.views||0));const url=(changes={})=>{const q=new URLSearchParams(location.search);['th','bh','ch'].forEach(g=>q.delete(g));q.set(changes.group||group,changes.level??level);[['type',changes.type??type],['year',changes.year??year],['sort',changes.sort??sort]].forEach(([k,v])=>v&&v!=='all'?q.set(k,v):q.delete(k));return `${prefix}/${premium?'premium-bases':'free-bases'}?${q}`};const levelBtns=['all',...GAME_LEVELS[group].map(String)].map(v=>`<a class="${v===level?'active':''}" href="${url({level:v})}">${v==='all'?'All':group.toUpperCase()+v}</a>`).join('');return `<section class="coc-browser"><div class="coc-wrap"><div class="coc-browser-head"><div><small>${premium?'PREMIUM':'FREE'} BASES</small><h1>${group==='th'?'Town Hall':group==='bh'?'Builder Hall':'Clan Capital'} ${level==='all'?'':group.toUpperCase()+level} Base Layouts</h1></div><div class="coc-mode-switch"><a class="${!premium?'active':''}" href="${prefix}/free-bases?${group}=${level}">FREE</a><a class="${premium?'active':''}" href="${prefix}/premium-bases?${group}=${level}">PREMIUM</a></div></div><div class="coc-filter-panel"><div class="coc-filter-row"><b>Group</b>${['th','bh','ch'].map(g=>`<a class="${g===group?'active':''}" href="${url({group:g,level:'all',type:'all'})}">${g==='th'?'Town Hall':g==='bh'?'Builder Hall':'Clan Capital'}</a>`).join('')}</div><div class="coc-filter-row"><b>Level</b>${levelBtns}</div><div class="coc-filter-row"><b>Type</b>${purposes.map(v=>`<a class="${v===type?'active':''}" href="${url({type:v})}">${v==='all'?'All':v.toUpperCase()}</a>`).join('')}</div><div class="coc-filter-row"><b>Year</b>${years.map(v=>`<a class="${v===year?'active':''}" href="${url({year:v})}">${v==='all'?'All':v}</a>`).join('')}</div><div class="coc-filter-row"><b>Sort</b>${[['latest','Latest'],['rating','Vote'],['download','Download'],['view','View']].map(([v,t])=>`<a class="${v===sort?'active':''}" href="${url({sort:v})}">${t}</a>`).join('')}</div></div><div class="coc-result-meta"><b>${arr.length}</b> base phù hợp</div><div class="coc-grid coc-result-grid">${arr.length?arr.map(nrGameBaseCard).join(''):'<div class="coc-no-results">Chưa có base phù hợp bộ lọc này.</div>'}</div></div></section>`}
function renderGameClash1(site={}){const root=document.querySelector('main');if(!root)return;const posts=nrGamePosts(),isDemo=window.NR_DEMO_THEME==='game-1',prefix=isDemo?(window.NR_DEMO_PREFIX||'/demo/game/clash-of-clans'):'',rel=isDemo&&location.pathname.startsWith(prefix)?(location.pathname.slice(prefix.length)||'/'):location.pathname,admin=isDemo?nrDemoAdminUrl('game-1','newpost'):'/admin?tab=newpost';if(/^\/base\/[^/]+\.html$/i.test(rel)){const slug=rel.split('/').pop().replace(/\.html$/i,''),post=posts.find(p=>String(p.demo_url||p.url||'').includes('/'+slug+'.html'))||posts[0];if(post){const e=nrGameExtra(post),level=nrGameLevel(post),premium=String(e.access_tier||'free').toLowerCase()==='premium';root.innerHTML=`<div class="coc-site">${nrGameHeader(prefix,'')}<main class="coc-detail coc-wrap"><div class="coc-breadcrumb"><a href="${prefix||'/'}">Home</a> / ${esc(level)} / ${esc(post.title||'Base')}</div><div class="coc-detail-grid"><article><img class="coc-detail-image" src="${esc(post.image||GAME_CLASH_LEVEL_ART[level]||GAME_CLASH_LEVEL_ART.TH18)}"><div class="coc-detail-copy"><div class="coc-tags"><span>${esc(level)}</span><span>${esc(e.game_purpose||'War')}</span><span>${esc(e.game_style||'Compact')}</span><span>${esc(e.game_defense||'Anti 3 Star')}</span></div><h1>${esc(post.title||level+' Base Layout')}</h1><div class="coc-stats big"><span>⭐ ${Number(post.rating||4.8).toFixed(1)}</span><span>👁 ${Number(post.views||0).toLocaleString()}</span><span>⬇ ${Number(post.downloads||e.downloads||0).toLocaleString()}</span></div><div class="coc-article-body">${post.content||''}</div></div></article><aside class="coc-detail-side"><div class="coc-access-card ${premium?'premium':''}"><small>${premium?'PREMIUM BASE':'FREE BASE'}</small><h3>${esc(level)} · ${esc(e.game_purpose||'War')}</h3><a href="${esc(premium?(e.premium_link||'#'):(e.copy_link||'#'))}">${premium?'Mở quyền Premium':'Copy Base Link'}</a></div></aside></div></main></div>`;return}}if(/\/(free|premium)-bases/i.test(rel)){root.innerHTML=`<div class="coc-site">${nrGameHeader(prefix,admin)}${nrGameFilterPage(posts,prefix,rel)}<footer class="coc-footer"><div class="coc-wrap"><b>COC BASE PORTAL</b><span>Game website template · HoangVuongTech</span></div></footer></div>`;return}root.innerHTML=`<div class="coc-site">${nrGameHeader(prefix,admin)}<section class="coc-hero"><div class="coc-wrap coc-hero-grid"><div><span class="coc-kicker">CLASH OF CLANS · BASE LAYOUT PORTAL</span><h1>Choose your Hall.<br><em>Then find your base.</em></h1><p>Trang chủ chỉ chọn cấp nhà. Sau khi chọn TH/BH/CH, bạn mới vào kho base với đầy đủ bộ lọc Free/Premium, Type, Year và Sort.</p><div class="coc-proof"><span><b>TH2–TH18</b><small>Town Hall</small></span><span><b>BH2–BH10</b><small>Builder Hall</small></span><span><b>CH1–CH10</b><small>Clan Capital</small></span></div></div><div class="coc-hero-art"><img src="${GAME_CLASH_LEVEL_ART.TH18}" alt="TH18"><div><b>Choose Hall Level</b><span>Free · Premium · Filters</span></div></div></div></section><section id="town-hall" class="coc-section" data-structure-key="town-hall"><div class="coc-wrap"><div class="coc-heading"><div><span>TOWN HALL</span><h2>Choose Town Hall</h2></div></div><div class="coc-level-grid" data-contract-grid="1">${nrGameLevelCards('th',prefix)}</div></div></section><section id="builder-hall" class="coc-section coc-dark" data-structure-key="builder-hall"><div class="coc-wrap"><div class="coc-heading light"><div><span>BUILDER BASE</span><h2>Choose Builder Hall</h2></div></div><div class="coc-level-grid" data-contract-grid="1">${nrGameLevelCards('bh',prefix)}</div></div></section><section id="clan-capital" class="coc-section" data-structure-key="clan-capital"><div class="coc-wrap"><div class="coc-heading"><div><span>CLAN CAPITAL</span><h2>Choose Capital Hall</h2></div></div><div class="coc-level-grid" data-contract-grid="1">${nrGameLevelCards('ch',prefix)}</div></div></section><footer class="coc-footer"><div class="coc-wrap"><b>COC BASE PORTAL</b><span>Game website template · HoangVuongTech</span></div></footer></div>`;try{nrApplyStructureGeometry(site,'game-1');nrAuditStructureContract(site,'game-1')}catch(e){}}
