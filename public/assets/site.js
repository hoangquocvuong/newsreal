...
function nrProductFooter(site={},settings={}){return `<footer class="pa-footer"><div class="pa-wrap"><div><b>${esc(cleanSiteName(site.name||'PRODUCT STORE'))}</b><p>Catalog sản phẩm do chủ website tự quản lý trong Trang quản trị.</p></div><div><b>Danh mục</b><a href="${nrProductPrefix()||'/'}">Tất cả sản phẩm</a><a href="${nrProductPrefix()||'/'}?category=${encodeURIComponent('Điện tử & Công nghệ')}">Điện tử & Công nghệ</a></div><div><b>Thông tin</b><span>${esc(settings.note)}</span></div></div></footer>`}
