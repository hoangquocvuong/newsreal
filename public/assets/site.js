...
function comProductCards(items=[]){return (items||[]).map(p=>{const sold=Number(p.sold_count||0),old=Number(p.compare_at_price||0),img=p.image_url||'/assets/marketing-demo.webp';return `<article class="com-card"><a class="com-card-image" href="${comProductUrl(p)}"><img src="${esc(img)}" alt="${esc(p.name)}" loading="lazy">${p.badge?`<span>${esc(p.badge)}</span>`:''}</a><div class="com-card-body"><small>${esc(p.category_name||'Sản phẩm')}</small><h3><a href="${comProductUrl(p)}">${esc(p.name)}</a></h3><div class="com-card-price"><b>${comMoney(p.price)}</b>${old>Number(p.price)?`<del>${comMoney(old)}</del>`:''}</div><div class="com-card-meta">${Number(p.stock_qty||0)>0?'Còn hàng':'Tạm hết hàng'}${sold?' · Đã bán '+sold:''}</div><div class="com-card-actions"><button onclick="comAdd(${Number(p.id)},false)">+ Giỏ hàng</button><a href="${comProductUrl(p)}">Chi tiết</a></div></div></article>`}).join('')}
let COM_PAGE=1;const COM_PAGE_SIZE=12;
function comRenderPager(total){const host=document.getElementById('comPager');if(!host)return;const pages=Math.max(1,Math.ceil(total/COM_PAGE_SIZE));if(COM_PAGE>pages)COM_PAGE=pages;host.innerHTML=pages<=1?'':`<button ${COM_PAGE<=1?'disabled':''} onclick="COM_PAGE--;comFilter(false)">‹ Trước</button><span>Trang ${COM_PAGE} / ${pages}</span><button ${COM_PAGE>=pages?'disabled':''} onclick="COM_PAGE++;comFilter(false)">Sau ›</button>`}
