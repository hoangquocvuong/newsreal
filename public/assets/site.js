...
async function renderCommerceStore(site={}){comEnsureMobileCss();comEnsureV15Css();comEnsureV16Css();const root=document.querySelector('main');if(!root)return;document.querySelectorAll('footer.footer,footer.public-footer').forEach(x=>x.remove());const settings=sxSettings(site),isPreview=/\/demo\//.test(location.pathname)||new URLSearchParams(location.search).has('nr_trial'),brand=esc(settings.store_name||(isPreview?'NOVA SHOP':(site.name||'Cửa hàng Online'))),hero=settings.hero_image||'/assets/marketing-demo.webp',admin=sxAdminNewPostUrl('dich-vu-5').replace('tab=newpost','tab=commerce-products');root.innerHTML=`<div class="com-store"><header class="com-head"><div class="com-wrap"><a class="com-brand" href="#">${brand}</a><div class="com-search"><input id="comSearch" placeholder="Tìm sản phẩm, mã SKU..."><button onclick="comFilter()">Tìm kiếm</button></div><a class="com-admin-link" href="${admin}" target="_blank">Quản trị</a><button class="com-cart-btn" onclick="document.getElementById('comCart').classList.add('open')">Giỏ hàng (<span id="comCartCount">0</span>)</button></div></header><section class="com-hero" id="comHero"><div class="com-wrap com-hero-grid"><div><small>SẢN PHẨM NỔI BẬT</small><h1>${esc(settings.hero_title||'Sản phẩm nổi bật của cửa hàng')}</h1><p>${esc(settings.hero_description||'Chủ shop chỉ cần đánh dấu sản phẩm nổi bật trong Trang quản trị. Slider sẽ tự cập nhật.')}</p></div><img src="${esc(hero)}" alt="${brand}"></div></section><section class="com-wrap com-trustbar"><div><b>🚚 Giao hàng linh hoạt</b><span>Thiết lập phí vận chuyển trong Admin</span></div><div><b>↩ Đổi trả rõ ràng</b><span>Chính sách minh bạch cho khách mua</span></div><div><b>🛡 Thanh toán an toàn</b><span>COD, chuyển khoản và VietQR</span></div><div><b>💬 Hỗ trợ nhanh</b><span>Hotline, Zalo và hộp tư vấn</span></div></section><main class="com-wrap" id="products"><div id="comCats" class="com-cats"></div><div class="com-toolbar"><div><b>Sản phẩm</b> · <span id="comResultCount">0 sản phẩm</span></div><select id="comSort" onchange="comFilter()"><option value="featured">Nổi bật</option><option value="newest">Mới nhất</option><option value="price_asc">Giá thấp → cao</option><option value="price_desc">Giá cao → thấp</option></select></div><div id="comGrid" class="com-grid"><p>Đang tải sản phẩm...</p></div></main><section class="com-wrap"><div class="com-discovery"><h2>Khám phá nhanh</h2><p>Chọn nhóm sản phẩm để tìm đúng thứ bạn cần nhanh hơn.</p><div id="comDiscoveryLinks" class="com-discovery-links"></div></div></section><footer class="com-footer"><div class="com-wrap com-footer-grid"><div><b>${brand}</b><p>${esc(settings.footer_intro||'Cửa hàng trực tuyến chuyên nghiệp. Chủ website có thể thay toàn bộ sản phẩm, danh mục và nội dung.')}</p></div><div><b>Liên hệ</b><p>${esc(site.phone||'Hotline đang cập nhật')}</p><p>${esc(site.email||'Email đang cập nhật')}</p></div><div><b>Chính sách</b><p>Giao hàng · Đổi trả · Bảo hành</p></div></div></footer></div><aside class="com-cart" id="comCart"><div class="com-cart-head"><b>Giỏ hàng</b><button onclick="document.getElementById('comCart').classList.remove('open')">✕</button></div><div class="com-cart-list" id="comCartList"></div><div class="com-cart-foot"><div class="com-order-summary"><p>Tạm tính <b id="comCartTotal">0đ</b></p><p>Phí vận chuyển <b>Tính theo cấu hình cửa hàng</b></p></div><form class="com-checkout" onsubmit="comCheckout(event)"><h3>Thông tin nhận hàng</h3><input name="customer_name" required placeholder="Họ và tên *"><input name="phone" required placeholder="Số điện thoại *"><input name="email" type="email" placeholder="Email nhận xác nhận đơn"><div class="com-address-grid"><input name="province" required placeholder="Tỉnh / Thành phố *"><input name="district" required placeholder="Quận / Huyện *"><input name="ward" required placeholder="Phường / Xã *"></div><input name="address" required placeholder="Số nhà, tên đường *"><h3>Vận chuyển</h3><label><input type="radio" name="shipping_method" value="standard" checked> Giao hàng tiêu chuẩn</label><label><input type="radio" name="shipping_method" value="express"> Giao nhanh (nếu cửa hàng hỗ trợ)</label><h3>Thanh toán</h3><div class="com-payment-options" id="comPaymentOptions"></div><textarea name="note" placeholder="Ghi chú cho cửa hàng / đơn vị giao hàng"></textarea><button type="submit">Xác nhận đặt hàng</button><div id="comCheckoutMsg"></div></form></div></aside>`;try{const r=await fetch(comApi('catalog')),d=await r.json();if(!r.ok)throw new Error(d.error||'Không tải được sản phẩm');COM_DATA=d;comRenderFeaturedHero(d.products||[]);const detailSlug=decodeURIComponent((location.pathname.match(/\/san-pham\/([^/]+)/)||[])[1]||'');if(detailSlug){const dp=d.products.find(x=>String(x.slug)===detailSlug||String(x.id)===detailSlug);if(dp){comRenderProductDetail(dp);return}}document.getElementById('comCats').innerHTML=`<button class="com-cat active" data-slug="">Tất cả</button>`+d.categories.map(c=>`<button class="com-cat" data-slug="${esc(c.slug)}">${esc(c.name)}</button>`).join('');document.querySelectorAll('.com-cat').forEach(b=>b.onclick=()=>{document.querySelectorAll('.com-cat').forEach(x=>x.classList.remove('active'));b.classList.add('active');comFilter()});const dl=document.getElementById('comDiscoveryLinks');if(dl)dl.innerHTML=d.categories.slice(0,8).map(c=>`<button type="button" data-discovery="${esc(c.slug)}">${esc(c.name)}</button>`).join('');dl?.querySelectorAll('[data-discovery]').forEach(b=>b.onclick=()=>{const target=document.querySelector(`.com-cat[data-slug="${CSS.escape(b.dataset.discovery)}"]`);target?.click();document.getElementById('products')?.scrollIntoView({behavior:'smooth'})});const ps=[];if(Number(d.settings.cod_enabled))ps.push('<label><input type="radio" name="payment_method" value="cod" checked> Thanh toán khi nhận hàng (COD)</label>');if(Number(d.settings.bank_enabled))ps.push('<label><input type="radio" name="payment_method" value="bank"> Chuyển khoản ngân hàng / VietQR</label>');if(Number(d.settings.store_pickup_enabled))ps.push('<label><input type="radio" name="payment_method" value="pickup"> Thanh toán tại cửa hàng</label>');if(Number(d.settings.online_enabled))ps.push('<label><input type="radio" name="payment_method" value="online"> Thanh toán online</label>');document.getElementById('comPaymentOptions').innerHTML=ps.join('');document.getElementById('comSearch').oninput=comFilter;comFilter();comCartRender()}catch(e){document.getElementById('comGrid').innerHTML='<p>'+esc(e.message)+'</p>'}}
