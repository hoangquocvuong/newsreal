...
function nrGameHeader(prefix,admin){const home=prefix||'/',bases=`${prefix}/bases?th=all`;return `<header class="coc-top coc-nav-shell"><div class="coc-wrap coc-nav-wrap"><a class="coc-brand" href="${home}" aria-label="COC Base Portal home"><b>COC</b><span>BASE PORTAL</span></a><nav class="coc-desktop-nav" aria-label="Clash of Clans navigation"><a href="${home}">Home</a><a href="${bases}">Browse Bases</a><a href="${prefix}/bases?th=all">Town Hall</a><a href="${prefix}/bases?bh=all">Builder Hall</a><a href="${prefix}/bases?ch=all">Clan Capital</a><button class="coc-saved-nav" type="button" data-coc-open-saved="1">Saved</button></nav><div class="coc-nav-actions">${admin?`<a class="coc-admin" href="${esc(admin)}" target="_blank" rel="noopener">+ Đăng base</a>`:''}<a class="coc-donate-top" href="https://buymeacoffee.com/cocbase" target="_blank" rel="noopener noreferrer nofollow"><span aria-hidden="true">☕</span><b>Donate</b></a></div></div></header><nav class="coc-mobile-nav" aria-label="Mobile navigation"><a href="${home}"><span>⌂</span><b>Home</b></a><a href="${bases}"><span>▦</span><b>Bases</b></a><button type="button" data-coc-open-saved="1"><span>🔖</span><b>Saved</b></button><details class="coc-mobile-more"><summary><span>☰</span><b>More</b></summary><div class="coc-mobile-menu-panel"><a href="${prefix}/bases?th=all">Town Hall</a><a href="${prefix}/bases?bh=all">Builder Hall</a><a href="${prefix}/bases?ch=all">Clan Capital</a><a href="${nrGameHomeHash(prefix,'about')}">About</a><a href="${nrGameHomeHash(prefix,'privacy')}">Privacy</a></div></details><a class="donate" href="https://buymeacoffee.com/cocbase" target="_blank" rel="noopener noreferrer nofollow"><span>☕</span><b>Donate</b></a></nav><a class="coc-donate-float" href="https://buymeacoffee.com/cocbase" target="_blank" rel="noopener noreferrer nofollow" aria-label="Support COC Base Portal on Buy Me a Coffee" title="Support us on Buy Me a Coffee"><span>☕</span></a><div class="coc-saved-sheet" data-coc-saved-sheet="1" aria-hidden="true"><button class="coc-saved-overlay" type="button" data-coc-close-saved="1" aria-label="Close saved bases"></button><section class="coc-saved-panel" role="dialog" aria-modal="true" aria-labelledby="coc-saved-title"><header><div><small>SAVED BASES</small><h3 id="coc-saved-title">Base đã lưu</h3></div><button type="button" data-coc-close-saved="1" aria-label="Close">×</button></header><div class="coc-saved-list" data-coc-saved-list="1"></div></section></div>`}
...
function nrGameToggleSaved(btn){const card=btn.closest('[data-coc-base-card="1"]');if(!card)return;const slug=btn.dataset.cocBookmark||card.dataset.cocSlug;if(!slug)return;let saved=nrGameSavedRead();const i=saved.findIndex(x=>x.slug===slug);if(i>=0)saved.splice(i,1);else saved.unshift({slug,title:card.dataset.cocTitle||'Saved base',image:card.dataset.cocImage||'',url:card.dataset.cocUrl||location.href,saved_at:Date.now()});nrGameSavedWrite(saved);nrGameSavedSync()}
...
function nrGameBindLocalFirst(){nrGameSavedSync();if(window.__NR_COC_LOCAL_FIRST_BOUND__)return;window.__NR_COC_LOCAL_FIRST_BOUND__=1;document.addEventListener('click',e=>{const bookmark=e.target.closest('[data-coc-bookmark]');if(bookmark){e.preventDefault();e.stopPropagation();nrGameToggleSaved(bookmark);return}const open=e.target.closest('[data-coc-open-saved="1"]');if(open){e.preventDefault();nrGameSetSavedOpen(true);return}const close=e.target.closest('[data-coc-close-saved="1"]');if(close){e.preventDefault();nrGameSetSavedOpen(false);return}const remove=e.target.closest('[data-coc-remove-saved]');if(remove){e.preventDefault();let saved=nrGameSavedRead().filter(x=>x.slug!==remove.dataset.cocRemoveSaved);nrGameSavedWrite(saved);nrGameSavedSync();return}},true);document.addEventListener('keydown',e=>{if(e.key==='Escape')nrGameSetSavedOpen(false)})}
