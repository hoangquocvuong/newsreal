...
function renderServiceProvider(site={},key='dich-vu-1'){
 ...
 if(nrServiceIsShowroom(key)) posts=cfg.samples.map((x,i)=>({title:x[1],category:x[0],content:x[4],image:x[5]||({'dich-vu-1':['https://shop.fpt.vn/Content/images/fpt-telecom.jpg','https://shop.fpt.vn/Content/images/fpt-telecom.jpg','https://hi-static.fpt.vn/sys/shop/prod/2025-04-08/67f4b9178361d_mien-phi-camera-khi-dang-ky-internet-thang-4.jpg','https://shop.fpt.vn/Content/images/fpt-telecom.jpg'],'dich-vu-2':['https://media-vnpt.vnptvas.vn/Media/Images/upload_images/images/202605/img_vm_2605291839585263.jpg?mode=crop&w=700','https://media-vnpt.vnptvas.vn/Media/Images/upload_images/images/202605/img_vm_2605291842257354.jpg?mode=crop&w=700','https://media-vnpt.vnptvas.vn/Media/Images/upload_images/images/202605/img_vm_2605291839585263.jpg?mode=crop&w=700','https://media-vnpt.vnptvas.vn/Media/Images/upload_images/images/202605/img_vm_2605291842257354.jpg?mode=crop&w=700'],'dich-vu-3':['https://media.vietteltelecom.vn/upload/ckfinder/images/20_1%281%29.png','https://media.vietteltelecom.vn/upload/ckfinder/images/20_1%281%29.png','https://media.vietteltelecom.vn/upload/ckfinder/images/87_ab.jpg','https://media.vietteltelecom.vn/upload/ckfinder/images/20_1%281%29.png']}[key]||[])[Math.max(0,cfg.cats.indexOf(x[0]))]||nrSvcImg(i,i<6?'net':i<12?'tv':i<18?'cam':'combo'),extra_json:{service_speed_down:x[2],service_price:x[3],service_promo:x[4]}}));
 ...
 const card=(x,cl='')=>{const e=ex(x),im=x.image||x.image_url||x.thumbnail||'';return `<article class="tel-card ${cl}">${im?`<a class="tel-card-img" href="#dang-ky"><img src="${esc(im)}" alt="${esc(x.title)}" loading="lazy" onerror="this.onerror=null;this.src='${esc(x.fallback_image||svcFallback(0))}'"></a>`:''}<div class="tel-card-body"><span class="tel-pill">${esc(x.category||'Dịch vụ')}</span><h3>${esc(x.title||'Gói dịch vụ')}</h3><div class="tel-speed">${esc(e.service_speed_down||'Tư vấn theo nhu cầu')}</div><p class="tel-desc">${esc(e.service_promo||String(x.content||'').replace(/<[^>]+>/g,' ').slice(0,180)||'Thông tin đang cập nhật')}</p><div class="tel-card-bottom"><div class="tel-price"><small>Chỉ từ</small><strong>${esc(e.service_price||'Liên hệ')}</strong></div><a class="tel-cta svc-lead-open" href="#dang-ky" data-package="${esc(x.title||'')}" data-category="${esc(x.category||'')}">Đăng ký</a></div></div></article>`};
 const empty=(cat,n=3)=>Array.from({length:n},()=>`<div class="tel-empty"><b>${esc(cat)}</b><span>Chưa có gói dịch vụ</span><small>Khung giao diện được giữ nguyên khi website chưa đăng nội dung.</small></div>`).join('');
 const cards=(i,n=6,cl='')=>arr(i).length?arr(i).slice(0,n).map((x,j)=>card(x,cl+(j===0?' is-first':''))).join(''):empty(cfg.cats[i],Math.min(n,4));
 ...
 root.innerHTML=`<div class="tel-site tel-${key}">${nav}${body}<footer class="tel-footer"><div class="tel-wrap">${brand}<p>${esc(cfg.kicker.replace(/ · /g,' • '))}</p><div><a href="#internet">Internet</a><a href="#tv">Truyền hình</a><a href="#camera">Camera</a><a href="#dang-ky">Đăng ký tư vấn</a></div></div></footer><div class="svc-mobile-contact"><a href="tel:${phoneHref}">☎ Gọi ngay</a><a href="#dang-ky">Đăng ký tư vấn</a></div></div>`;
