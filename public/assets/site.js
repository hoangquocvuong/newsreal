...
function renderServiceProvider(site={},key='dich-vu-1'){
 ...
 const card=(x,variant='')=>{const e=ex(x),im=img(x);return `<article class="svc2-card ${variant}">${im?`<div class="svc2-thumb"><img src="${esc(im)}" alt="${esc(x.title||'Dịch vụ')}" loading="lazy"></div>`:''}<div class="svc2-body"><span class="tag">${esc(x.category||'Dịch vụ')}</span><h3>${esc(x.title||'Gói dịch vụ')}</h3>${e.service_speed?`<div class="spec">${esc(e.service_speed)}</div>`:''}<p>${esc(e.service_promo||String(x.content||'').replace(/<[^>]+>/g,' ').slice(0,150)||'Thông tin chi tiết đang được cập nhật.')}</p><div class="price">${e.service_price?`Chỉ từ <strong>${esc(e.service_price)}</strong>`:'Liên hệ để nhận báo giá'}</div><a class="svc1-btn primary" href="#dang-ky">${esc(e.service_cta||'Đăng ký tư vấn')}</a></div></article>`};
