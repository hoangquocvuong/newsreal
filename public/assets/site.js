...
async function nrBootMain(){
 try{
   ...
   const DEMO_PRESETS={
     ...
     'dich-vu-5':'service_laptop_store_5',
   ...
   document.body.classList.toggle('theme-showcase-new',['personal_blog_1','personal_blog_2','corporate_modern_1','corporate_industry_2','service_laptop_store_5','service_lion_dance_6'].includes(effectivePreset));
   ...
   }else if(['personal_blog_1','personal_blog_2','corporate_modern_1','corporate_industry_2','service_laptop_store_5'].includes(effectivePreset)){
...
function comCartRender(){const c=comCartGet(),list=document.getElementById('comCartList'),count=document.getElementById('comCartCount'),total=c.reduce((a,x)=>a+Number(x.price)*Number(x.qty),0);if(count)count.textContent=c.reduce((a,x)=>a+Number(x.qty),0);if(list)list.innerHTML=c.length?c.map(x=>`<div class="com-cart-item"><div><b>${esc(x.name)}</b><small>${x.qty} × ${comMoney(x.price)}</small></div><button onclick="comRemove(${x.product_id})">Xóa</button></div>`).join(''):'<p>Giỏ hàng đang trống.</p>';const t=document.getElementById('comCartTotal');if(t)t.textContent=comMoney(total)}
