...
function comProductCards(list=[]){return (Array.isArray(list)?list:[]).map(p=>`<article class="com-card"><a class="com-card-img" href="${comProductUrl(p)}"><img src="${esc(p.image_url||'/assets/marketing-demo.webp')}" alt="${esc(p.name||'Sản phẩm')}" loading="lazy"></a><div class="com-card-body">${p.badge?`<span class="com-badge">${esc(p.badge)}</span>`:''}<h3><a href="${comProductUrl(p)}">${esc(p.name||'Sản phẩm')}</a></h3><p class="com-desc">${esc(p.short_description||'')}</p><div class="com-price"><strong>${comMoney(p.price)}</strong>${Number(p.compare_at_price)>Number(p.price)?`<del>${comMoney(p.compare_at_price)}</del>`:''}</div><div class="com-stock ${p.stock_status==='out_of_stock'?'out':''}">${p.stock_status==='out_of_stock'?'Tạm hết hàng':'Còn hàng'}</div><div class="com-actions"><button class="com-add" type="button" onclick="comAdd(${Number(p.id)},false)" ${p.stock_status==='out_of_stock'?'disabled':''}>Thêm giỏ</button><a class="com-detail" href="${comProductUrl(p)}">Xem</a></div></div></article>`).join('')}
