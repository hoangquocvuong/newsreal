...
async function renderCommerceStore(site={}){
 ...
 // /san-pham/ detail route remains connected to the commerce backend while the homepage is rebuilt.
 const detailSlug=decodeURIComponent((location.pathname.match(/\/san-pham\/([^/]+)/)||[])[1]||'');
 if(detailSlug){try{const r=await fetch(comApi('catalog')),d=await r.json();if(r.ok){COM_DATA=d;const dp=(d.products||[]).find(x=>String(x.slug)===detailSlug||String(x.id)===detailSlug);if(dp){comRenderProductDetail(dp);return}}}catch(e){}}
 const settings=sxSettings(site),brand=esc(settings.store_name||site.name||'Cửa hàng Online');
 root.innerHTML=`<section class="commerce-reset-v22" data-commerce-reset="true"><style>.commerce-reset-v22{min-height:72vh;display:grid;place-items:center;padding:48px 20px;background:#f8fafc;font-family:Inter,system-ui,-apple-system,Segoe UI,sans-serif}.commerce-reset-v22__box{width:min(720px,100%);padding:48px;border:1px solid #e2e8f0;border-radius:24px;background:#fff;text-align:center;box-shadow:0 18px 50px rgba(15,23,42,.07)}.commerce-reset-v22__mark{display:inline-flex;padding:7px 12px;border-radius:999px;background:#eef2ff;color:#3730a3;font-size:12px;font-weight:800;letter-spacing:.04em}.commerce-reset-v22 h1{margin:18px 0 10px;font-size:clamp(30px,5vw,48px);letter-spacing:-1.5px;color:#0f172a}.commerce-reset-v22 p{margin:0 auto;max-width:560px;color:#64748b;line-height:1.7}.commerce-reset-v22 small{display:block;margin-top:22px;color:#94a3b8}@media(max-width:640px){.commerce-reset-v22{padding:24px 12px}.commerce-reset-v22__box{padding:34px 20px;border-radius:18px}}</style><div class="commerce-reset-v22__box"><span class="commerce-reset-v22__mark">COMMERCE RESET</span><h1>${brand}</h1><p>Template bán hàng cũ đã được gỡ khỏi storefront. Khu vực này đang để trống có chủ đích để xây template bán hàng mới từ đầu.</p><small>Backend sản phẩm, danh mục, đơn hàng và Admin vẫn được giữ nguyên.</small></div></section>`;
}
