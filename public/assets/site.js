...
async function nrBootMain(){
 try{
   const demoTemplateKey=window.NR_DEMO_THEME&&(/^tin-tuc-[1-4]$/.test(window.NR_DEMO_THEME)||/^mau-[1-5]$/.test(window.NR_DEMO_THEME)||/^dich-vu-\d+$/.test(window.NR_DEMO_THEME)||/^game-\d+$/.test(window.NR_DEMO_THEME)||/^san-pham-\d+$/.test(window.NR_DEMO_THEME))?window.NR_DEMO_THEME:'';
...
function nrProductSettings(site={}){const v=site.template_settings&&typeof site.template_settings==='object'?site.template_settings:(()=>{try{return JSON.parse(site.template_settings_json||'{}')}catch(e){return {}}})();return {cta:v.product_cta_label||'Xem nơi bán',note:v.product_affiliate_note||'Website có thể nhận hoa hồng khi người xem mua hàng qua liên kết giới thiệu.'}}
