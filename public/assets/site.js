...
function nrGamePosts(){return (SITE_DATA?.posts||[]).filter(x=>['game','base'].includes(String(x.type||'').toLowerCase())||nrGameExtra(x).game_group)}
function nrGameSectionSlots(site,key,fallback){return nrStructureSlots(site,'game-1',key,fallback)}
function nrGameLevel(post){const e=nrGameExtra(post);return String(e.game_level||e.level||post.level||'TH18').toUpperCase()}
function nrGameCard(post){const e=nrGameExtra(post),level=nrGameLevel(post),premium=String(e.access_tier||e.game_access||'free').toLowerCase()==='premium',img=post.image||GAME_CLASH_LEVEL_ART[level]||GAME_CLASH_LEVEL_ART.TH18,purpose=e.game_purpose||e.purpose||'War',style=e.game_style||e.style||'Compact',defense=e.game_defense||e.defense||'Anti 3 Star',href=post.demo_url||post.url||'#';return `<article class="coc-card"><a class="coc-thumb" href="${esc(href)}"><img src="${esc(img)}" alt="${esc(post.title||level+' Clash of Clans Base')}" loading="lazy"><span class="coc-level">${esc(level)}</span>${premium?'<span class="coc-premium">PREMIUM</span>':'<span class="coc-free">FREE</span>'}</a><div class="coc-card-body"><h3><a href="${esc(href)}">${esc(post.title||level+' Base Layout')}</a></h3><div class="coc-tags"><span>${esc(purpose)}</span><span>${esc(style)}</span><span>${esc(defense)}</span></div><div class="coc-stats"><span>⭐ ${Number(post.rating||4.8).toFixed(1)}</span><span>👁 ${Number(post.views||120).toLocaleString('en-US')}</span><span>⬇ ${Number(e.downloads||post.downloads||35).toLocaleString('en-US')}</span></div></div></article>`}
function renderGameClash1(site={}){const root=document.querySelector('main');if(!root)return;const posts=nrGamePosts(),isDemo=window.NR_DEMO_THEME==='game-1',prefix=isDemo?(window.NR_DEMO_PREFIX||'/demo/game/clash-of-clans'):'',rel=isDemo&&location.pathname.startsWith(prefix)?(location.pathname.slice(prefix.length)||'/'):location.pathname,group=name=>posts.filter(p=>String(nrGameExtra(p).game_group||p.category||'').toLowerCase().includes(name)),th=group('town hall'),bh=group('builder hall'),ch=group('clan capital');if(/^\/base\/[^/]+\.html$/i.test(rel)){const slug=rel.split('/').pop().replace(/\.html$/i,''),post=posts.find(p=>String(p.demo_url||p.url||'').includes('/'+slug+'.html'))||posts[0];if(post){const e=nrGameExtra(post),level=nrGameLevel(post),img=post.image||GAME_CLASH_LEVEL_ART[level]||GAME_CLASH_LEVEL_ART.TH18,premium=String(e.access_tier||'free').toLowerCase()==='premium';root.innerHTML=`<div class="coc-site"><header class="coc-top"><div class="coc-wrap"><a class="coc-brand" href="${prefix||'/'}"><b>COC</b><span>BASE PORTAL</span></a><nav><a href="${prefix||'/'}#town-hall">Town Hall</a><a href="${prefix||'/'}#builder-hall">Builder Hall</a><a href="${prefix||'/'}#clan-capital">Clan Capital</a></nav></div></header><main class="coc-detail coc-wrap"><div class="coc-breadcrumb"><a href="${prefix||'/'}">Home</a> / ${esc(level)} / ${esc(post.title||'Base')}</div><div class="coc-detail-grid"><article><img class="coc-detail-image" src="${esc(img)}" alt="${esc(post.title||'Clash of Clans Base')}"><div class="coc-detail-copy"><div class="coc-tags"><span>${esc(level)}</span><span>${esc(e.game_purpose||'War')}</span><span>${esc(e.game_style||'Compact')}</span><span>${esc(e.game_defense||'Anti 3 Star')}</span></div><h1>${esc(post.title||level+' Base Layout')}</h1><div class="coc-stats big"><span>⭐ ${Number(post.rating||4.8).toFixed(1)}</span><span>👁 ${Number(post.views||0).toLocaleString('en-US')}</span><span>⬇ ${Number(post.downloads||e.downloads||0).toLocaleString('en-US')}</span></div><div class="coc-article-body">${post.content||'<p>Nội dung base đang được cập nhật.</p>'}</div></div></article><aside class="coc-detail-side"><div class="coc-access-card ${premium?'premium':''}"><small>${premium?'PREMIUM BASE':'FREE BASE'}</small><h3>${esc(level)} · ${esc(e.game_purpose||'War')}</h3><p>Style: <b>${esc(e.game_style||'Compact')}</b><br>Defense: <b>${esc(e.game_defense||'Anti 3 Star')}</b></p><a href="${esc(premium?(e.premium_link||'#'):(e.copy_link||'#'))}">${premium?'Mở quyền Premium':'Copy Base Link'}</a></div><div class="coc-side-note"><b>Base details</b><span>Year ${esc(e.game_year||'2026')}</span><span>${esc(e.game_group||post.category||'Town Hall')}</span></div></aside></div></main><footer class="coc-footer"><div class="coc-wrap"><b>COC BASE PORTAL</b><span>Game website template · HoangVuongTech</span></div></footer></div>`;return}}const accessMode=/premium-bases/i.test(rel)?'premium':/free-bases/i.test(rel)?'free':'all';const visible=arr=>accessMode==='all'?arr:arr.filter(p=>String(nrGameExtra(p).access_tier||'free').toLowerCase()===accessMode);const fill=(arr,key,n)=>{const slots=nrGameSectionSlots(site,key,n);let out=arr.slice(0,slots);while(out.length<slots)out.push({__nr_empty:1});return out},cards=(arr,key,n)=>fill(arr,key,n).map(p=>p.__nr_empty?`<div class="coc-card coc-empty"><div class="coc-empty-art">+</div><b>Vị trí base</b><small>Đăng bài trong Trang quản trị</small></div>`:nrGameCard(p)).join(''),admin=isDemo?nrDemoAdminUrl('game-1','newpost'):'/admin?tab=newpost';root.innerHTML=`<div class="coc-site"><header class="coc-top"><div class="coc-wrap"><a class="coc-brand" href="#"><b>COC</b><span>BASE PORTAL</span></a><nav><a href="#town-hall">Town Hall</a><a href="#builder-hall">Builder Hall</a><a href="#clan-capital">Clan Capital</a><a href="${prefix}/free-bases">Free</a><a href="${prefix}/premium-bases">Premium</a></nav><a class="coc-admin" href="${esc(admin)}" target="_blank" rel="noopener">+ Đăng base</a></div></header><section class="coc-hero" data-structure-key="hero"><div class="coc-wrap coc-hero-grid"><div><span class="coc-kicker">CLASH OF CLANS · BASE LAYOUT PORTAL</span><h1>Find the best base.<br><em>Copy. Defend. Win.</em></h1><p>Giao diện chuyên cho website đăng và bán base Clash of Clans: Town Hall, Builder Hall, Clan Capital, Free/Premium và bộ lọc chiến thuật.</p><div class="coc-hero-actions"><a href="#town-hall">Browse Town Hall</a><a class="soft" href="${prefix}/free-bases">Free Bases</a></div><div class="coc-proof"><span><b>TH2–TH18</b><small>Town Hall</small></span><span><b>BH2–BH10</b><small>Builder Hall</small></span><span><b>CH1–CH10</b><small>Clan Capital</small></span></div></div><div class="coc-hero-art"><img src="${GAME_CLASH_LEVEL_ART.TH18}" alt="TH18 Clash of Clans base layout"><div><b>TH18 Legend Base</b><span>War · Anti 3 Star · Compact</span></div></div></div></section><section class="coc-filter-strip" data-structure-key="filters"><div class="coc-wrap"><button>TH18</button><button>TH17</button><button>War</button><button>Farming</button><button>Hybrid</button><button>Legend</button><button>CWL</button><button>Troll</button></div></section><section id="town-hall" class="coc-section" data-structure-key="town-hall"><div class="coc-wrap"><div class="coc-heading"><div><span>TOWN HALL</span><h2>Latest Town Hall Bases</h2><p>War, Farming, Hybrid, Trophy, Legend, CWL và Troll layouts.</p></div></div><div class="coc-grid" data-contract-grid="1">${cards(visible(th),'town-hall',12)}</div></div></section><section id="builder-hall" class="coc-section coc-dark" data-structure-key="builder-hall"><div class="coc-wrap"><div class="coc-heading light"><div><span>BUILDER BASE</span><h2>Builder Hall Bases</h2><p>Base layouts cho BH2 đến BH10, tối ưu defense và trophy pushing.</p></div></div><div class="coc-grid" data-contract-grid="1">${cards(visible(bh),'builder-hall',8)}</div></div></section><section id="clan-capital" class="coc-section" data-structure-key="clan-capital"><div class="coc-wrap"><div class="coc-heading"><div><span>CLAN CAPITAL</span><h2>Clan Capital & District Layouts</h2><p>Capital Peak và các district phổ biến với level rõ ràng.</p></div></div><div class="coc-grid" data-contract-grid="1">${cards(visible(ch),'clan-capital',8)}</div></div></section><section id="free-premium" class="coc-access" data-structure-key="access"><div class="coc-wrap coc-access-grid"><div><small>FREE BASES</small><h2>Duyệt nhanh và copy ngay</h2><p>Danh sách miễn phí có filter level, purpose, year và sort.</p></div><div class="premium"><small>PREMIUM BASES</small><h2>Base tuyển chọn cho người chơi nghiêm túc</h2><p>Card Premium, quyền truy cập, CTA mua/copy và thống kê dùng chung data contract.</p></div></div></section><footer class="coc-footer"><div class="coc-wrap"><b>COC BASE PORTAL</b><span>Game website template · HoangVuongTech</span></div></footer></div>`;try{nrApplyStructureGeometry(site,'game-1');nrAuditStructureContract(site,'game-1')}catch(e){}}
