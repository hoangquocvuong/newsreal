...
async function renderCommerceStore(site={}){comEnsureMobileCss();const root=document.querySelector('main');if(!root)return;document.querySelectorAll('footer.footer,footer.public-footer').forEach(x=>x.remove());const settings=sxSettings(site),brand=esc(settings.store_name||site.name||'Mộc Store'),hero=settings.hero_image||'/assets/marketing-demo.webp',admin=sxAdminNewPostUrl('dich-vu-5').replace('tab=newpost','tab=commerce-products');root.innerHTML=`<div class="commerce21"><div class="c21-ann">MIỄN PHÍ VẬN CHUYỂN THEO CHÍNH SÁCH · ĐỔI TRẢ RÕ RÀNG · HỖ TRỢ NHANH</div><header class="c21-head"><div class="c21-wrap c21-headrow"><a class="c21-brand" href="#">${brand}<em>+</em></a><div class="c21-search"><input id="comSearch" placeholder="Tìm sản phẩm, danh mục..."><button onclick="comFilter()">Tìm</button></div><div class="c21-actions"><a class="admin" href="${admin}" target="_blank">Quản trị</a><button class="bag" onclick="document.getElementById('comCart').classList.add('open')">🛒 Giỏ hàng (<span id="comCartCount">0</span>)</button></div></div><nav class="c21-nav"><div class="c21-wrap"><a class="hot" href="#products">Mới & nổi bật</a><a href="#categories">Danh mục</a><a href="#products">Sản phẩm</a><a href="#products">Bán chạy</a><a href="#support">Giao hàng & đổi trả</a></div></nav></header><section class="c21-hero"><div class="c21-wrap c21-hero-grid"><article class="c21-hero-main"><div><span class="c21-kicker">MUA SẮM THÔNG MINH</span><h1>${esc(settings.hero_title||'Sản phẩm tốt. Giá hợp lý. Mua thật dễ.')}</h1><p>${esc(settings.hero_description||'Khám phá những lựa chọn nổi bật cho công nghệ, thời trang, gia dụng và cuộc sống mỗi ngày.')}</p><a class="c21-cta" href="#products">Mua ngay →</a></div><img src="${esc(hero)}" alt="${brand}"></article><aside class="c21-promos"><article class="c21-promo"><small>ƯU ĐÃI HÔM NAY</small><h3>Chọn nhanh<br>món phù hợp.</h3><p>Cập nhật sản phẩm mới mỗi ngày.</p></article><article class="c21-promo"><small>MUA SẮM AN TÂM</small><h3>Đặt hàng<br>trong vài bước.</h3><p>Thông tin rõ ràng, đặt hàng nhanh.</p></article></aside></div></section><section class="c21-category-section" id="categories"><div class="c21-wrap"><div class="c21-section-head"><div><h2>Danh mục nổi bật</h2><p>Chọn nhanh nhóm sản phẩm bạn quan tâm.</p></div></div><div id="c21CategoryMenu" class="c21-categories"><span>Đang tải danh mục...</span></div></div></section><section class="c21-wrap c21-trust" id="support"><div><b>🚚 Giao hàng linh hoạt</b><span>Nhiều phương thức vận chuyển</span></div><div><b>↩ Đổi trả rõ ràng</b><span>Chính sách minh bạch</span></div><div><b>🔒 Thanh toán an toàn</b><span>COD · Chuyển khoản · VietQR</span></div><div><b>💬 Hỗ trợ nhanh</b><span>Tư vấn trước và sau mua</span></div></section><section class="c21-products" id="products"><div class="c21-wrap"><div class="c21-section-head"><div><h2>Gợi ý dành cho bạn</h2><p>Những lựa chọn nổi bật từ cửa hàng.</p></div></div><div id="comCats" class="c21-pills"></div><div class="com-toolbar"><div><b id="comResultCount">0 sản phẩm</b></div><select id="comSort" onchange="comFilter()"><option value="featured">Đề xuất</option><option value="newest">Mới nhất</option><option value="price_asc">Giá thấp → cao</option><option value="price_desc">Giá cao → thấp</option></select></div><div id="comGrid" class="com-grid"><p>Đang tải sản phẩm...</p></div></div></section><section class="c21-wrap"><div class="c21-newsletter"><div><h2>Tìm thấy món đồ dành cho bạn.</h2><p>Duyệt toàn bộ danh mục hoặc tìm kiếm sản phẩm ngay phía trên.</p></div><a href="#products">Xem tất cả sản phẩm</a></div></section><footer class="c21-footer"><div class="c21-wrap"><div class="c21-footgrid"><div><h4>${brand}</h4><p>${esc(settings.footer_intro||'Cửa hàng trực tuyến hiện đại, mua sắm nhanh và thuận tiện trên mọi thiết bị.')}</p></div><div><h4>Mua sắm</h4><a href="#products">Sản phẩm</a><a href="#categories">Danh mục</a></div><div><h4>Hỗ trợ</h4><a href="#support">Giao hàng</a><a href="#support">Đổi trả</a><a href="#support">Thanh toán</a></div><div><h4>Liên hệ</h4><p>${esc(site.phone||'Hotline đang cập nhật')}</p><p>${esc(site.email||'Email đang cập nhật')}</p></div></div><div class="c21-copy">© 2026 ${brand}. All rights reserved.</div></div></footer></div><aside class="com-cart" id="comCart"><div class="com-cart-head"><b>Giỏ hàng</b><button onclick="document.getElementById('comCart').classList.remove('open')">✕</button></div><div class="com-cart-list" id="comCartList"></div><div class="com-cart-foot"><div class="com-order-summary"><p>Tạm tính <b id="comCartTotal">0đ</b></p></div><form class="com-checkout" onsubmit="comCheckout(event)"><h3>Thông tin nhận hàng</h3><input name="customer_name" required placeholder="Họ và tên *"><input name="phone" required placeholder="Số điện thoại *"><input name="email" type="email" placeholder="Email"><div class="com-address-grid"><input name="province" required placeholder="Tỉnh / Thành phố *"><input name="district" required placeholder="Quận / Huyện *"><input name="ward" required placeholder="Phường / Xã *"></div><input name="address" required placeholder="Số nhà, tên đường *"><h3>Vận chuyển</h3><label><input type="radio" name="shipping_method" value="standard" checked> Giao hàng tiêu chuẩn</label><label><input type="radio" name="shipping_method" value="express"> Giao nhanh</label><h3>Thanh toán</h3><div class="com-payment-options" id="comPaymentOptions"></div><textarea name="note" placeholder="Ghi chú đơn hàng"></textarea><button type="submit">Xác nhận đặt hàng</button><div id="comCheckoutMsg"></div></form></div></aside>`;try{const r=await fetch(comApi('catalog')),d=await r.json();if(!r.ok)throw new Error(d.error||'Không tải được sản phẩm');COM_DATA=d;const detailSlug=decodeURIComponent((location.pathname.match(/\/san-pham\/([^/]+)/)||[])[1]||'');if(detailSlug){const dp=d.products.find(x=>String(x.slug)===detailSlug||String(x.id)===detailSlug);if(dp){comRenderProductDetail(dp);return}}const cats=d.categories||[];document.getElementById('comCats').innerHTML=`<button class="com-cat active" data-slug="">Tất cả</button>`+cats.map(c=>`<button class="com-cat" data-slug="${esc(c.slug)}">${esc(c.name)}</button>`).join('');const icons=['✦','◉','◇','♡','⌂','□','⌁','◎','＋'];const menu=document.getElementById('c21CategoryMenu');menu.innerHTML=cats.length?cats.slice(0,9).map((c,i)=>`<button type="button" data-menu-cat="${esc(c.slug)}"><span class="ico">${icons[i%icons.length]}</span>${esc(c.name)}</button>`).join(''):'<span>Chưa có danh mục</span>';document.querySelectorAll('.com-cat').forEach(b=>b.onclick=()=>{document.querySelectorAll('.com-cat').forEach(x=>x.classList.remove('active'));b.classList.add('active');comFilter()});menu.querySelectorAll('[data-menu-cat]').forEach(b=>b.onclick=()=>{document.querySelector(`.com-cat[data-slug="${CSS.escape(b.dataset.menuCat)}"]`)?.click();document.getElementById('products')?.scrollIntoView({behavior:'smooth'})});const ps=[];if(Number(d.settings.cod_enabled??1))ps.push('<label><input type="radio" name="payment_method" value="cod" checked> Thanh toán khi nhận hàng (COD)</label>');if(Number(d.settings.bank_enabled??1))ps.push('<label><input type="radio" name="payment_method" value="bank"> Chuyển khoản / VietQR</label>');if(Number(d.settings.store_pickup_enabled))ps.push('<label><input type="radio" name="payment_method" value="pickup"> Thanh toán tại cửa hàng</label>');if(Number(d.settings.online_enabled))ps.push('<label><input type="radio" name="payment_method" value="online"> Thanh toán online</label>');document.getElementById('comPaymentOptions').innerHTML=ps.join('');document.getElementById('comSearch').oninput=comFilter;comFilter();comCartRender()}catch(e){document.getElementById('comGrid').innerHTML='<p>'+esc(e.message)+'</p>'}}
