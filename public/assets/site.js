...
/* =========================================================
   V20.8.5 — GAME PORTAL / CLASH OF CLANS CONTRACT
...
function nrGameHeader(prefix,admin){const home=prefix||'/';return `<header class="coc-top"><div class="coc-wrap"><a class="coc-brand" href="${home}"><b>COC</b><span>BASE PORTAL</span></a><nav class="coc-main-menu" aria-label="COC Base Portal"><a href="${home}">Home</a><button type="button" data-coc-popup="news">News</button><button type="button" data-coc-popup="find-source">Find Source</button><button type="button" data-coc-popup="donate">Donate</button><button type="button" data-coc-popup="saved">Saved</button><button type="button" data-coc-popup="more">More</button></nav>${admin?`<a class="coc-admin" href="${esc(admin)}" target="_blank" rel="noopener">+ Đăng base</a>`:''}<button class="coc-mobile-more" type="button" data-coc-popup="more" aria-label="Open menu">☰</button></div></header>`}
function nrGamePopupShell(prefix,posts){return `<div id="coc-popup-layer" class="coc-popup-layer" aria-hidden="true"><button class="coc-popup-overlay" type="button" data-coc-popup-close="1" aria-label="Close"></button><section class="coc-popup-panel" role="dialog" aria-modal="true"><div class="coc-popup-head"><div><small>COC BASE PORTAL</small><h3 id="coc-popup-title">Explore CocBasePro</h3></div><button type="button" class="coc-popup-close" data-coc-popup-close="1" aria-label="Close">×</button></div><div id="coc-popup-body" class="coc-popup-body"></div></section></div>`}
function nrGamePopupContent(type,prefix,posts){const list=(posts||[]).slice(0,8);if(type==='more')return {title:'Explore CocBasePro',html:nrGameMoreGrid(prefix)};if(type==='news')return {title:'News & Base Updates',html:`<div class="coc-popup-list">${list.map(p=>`<a href="${esc(p.demo_url||p.url||'#')}"><img src="${esc(p.image||'')}" alt=""><span><b>${esc(p.title||'Clash of Clans base')}</b><small>${esc(nrGameLevel(p))} · ${esc(nrGameExtra(p).game_purpose||'Base')}</small></span></a>`).join('')}</div>`};if(type==='find-source')return {title:'Find Source',html:`<div class="coc-find-source"><p>Tìm nhanh base theo tên, TH/BH/CH hoặc từ khóa.</p><input id="coc-source-search" type="search" placeholder="Ví dụ: TH15 Farming, Dragon Cliffs..."><div id="coc-source-results" class="coc-source-results"></div></div>`};if(type==='saved'){const saved=nrGameSavedSlugs(),items=(posts||[]).filter(p=>saved.includes(nrGameSlug(p)));return {title:'Saved Bases',html:items.length?`<div class="coc-popup-list">${items.map(p=>`<a href="${esc(p.demo_url||p.url||'#')}"><img src="${esc(p.image||'')}" alt=""><span><b>${esc(p.title||'Base')}</b><small>${esc(nrGameLevel(p))}</small></span></a>`).join('')}</div>`:'<div class="coc-popup-empty">Chưa có base nào được lưu. Nhấn biểu tượng bookmark trên card để lưu.</div>'}};if(type==='donate')return {title:'Support CocBasePro',html:'<div class="coc-popup-copy"><h4>Support is always optional.</h4><p>Tất cả base đều miễn phí. Donate chỉ hỗ trợ chi phí máy chủ, hosting và cập nhật dữ liệu; không mở khóa bất kỳ base nào.</p></div>'};if(type==='events')return {title:'Clash of Clans Events',html:'<div class="coc-popup-copy"><h4>Events & seasonal layouts</h4><p>Khu vực dành cho Clan Games, CWL, Raid Weekend và các bộ sưu tập theo mùa.</p></div>'};if(type==='rankings'){const top=(posts||[]).slice().sort((a,b)=>nrGameMetric(b,'views')-nrGameMetric(a,'views')).slice(0,8);return {title:'Popular Base Rankings',html:`<div class="coc-popup-list">${top.map((p,i)=>`<a href="${esc(p.demo_url||p.url||'#')}"><strong>#${i+1}</strong><span><b>${esc(p.title||'Base')}</b><small>◉ ${Math.round(nrGameMetric(p,'views')).toLocaleString()} views</small></span></a>`).join('')}</div>`}};if(type==='hero-skins')return {title:'Hero Skins',html:'<div class="coc-popup-copy"><h4>Hero Skins Showcase</h4><p>Chuyên mục Hero Skins được giữ lại theo cấu trúc menu của CocBasePro gốc.</p></div>'};if(type==='about')return {title:'About CocBasePro',html:'<div class="coc-popup-copy"><p>Thư viện chia sẻ base Clash of Clans miễn phí cho Town Hall, Builder Hall và Clan Capital.</p></div>'};if(type==='privacy')return {title:'Privacy',html:'<div class="coc-popup-copy"><p>Bookmark được lưu cục bộ cho demo. Hệ thống production có thể đồng bộ tài khoản/Firebase hoặc backend riêng.</p></div>'};if(type==='apps')return {title:'Mobile Apps',html:'<div class="coc-popup-copy"><h4>COC Base Portal on mobile</h4><p>Giữ vị trí Android/iOS Apps trong menu như portal gốc.</p></div>'};return {title:'CocBasePro',html:''}}
function nrGameBindUI(posts,prefix){if(window.__NR_COC_UI_ABORT__)try{window.__NR_COC_UI_ABORT__.abort()}catch(e){}const ctl=new AbortController();window.__NR_COC_UI_ABORT__=ctl;const sig={signal:ctl.signal};const layer=document.getElementById('coc-popup-layer'),body=document.getElementById('coc-popup-body'),title=document.getElementById('coc-popup-title');const close=()=>{if(!layer)return;layer.classList.remove('show');layer.setAttribute('aria-hidden','true');document.documentElement.classList.remove('coc-popup-open')};const open=type=>{if(!layer||!body||!title)return;const c=nrGamePopupContent(type,prefix,posts);title.textContent=c.title;body.innerHTML=c.html;layer.classList.add('show');layer.setAttribute('aria-hidden','false');document.documentElement.classList.add('coc-popup-open');if(type==='find-source'){const input=document.getElementById('coc-source-search'),out=document.getElementById('coc-source-results');if(input&&out){const render=()=>{const q=input.value.trim().toLowerCase();const found=(posts||[]).filter(p=>!q||(`${p.title||''} ${nrGameLevel(p)} ${nrGameExtra(p).game_purpose||''}`).toLowerCase().includes(q)).slice(0,8);out.innerHTML=found.map(p=>`<a href="${esc(p.demo_url||p.url||'#')}">${esc(nrGameLevel(p))} · ${esc(p.title||'Base')}</a>`).join('')||'<span>Không tìm thấy base.</span>'};input.addEventListener('input',render);render()}}};document.addEventListener('click',e=>{const pop=e.target.closest('[data-coc-popup]');if(pop){e.preventDefault();const type=pop.dataset.cocPopup;if(layer?.classList.contains('show')&&body?.dataset.popupType===type){close();return}if(body)body.dataset.popupType=type;open(type);return}if(e.target.closest('[data-coc-popup-close]')){e.preventDefault();close();return}const bm=e.target.closest('[data-coc-bookmark]');if(bm){e.preventDefault();e.stopPropagation();const slug=bm.dataset.cocBookmark;let saved=nrGameSavedSlugs();saved=saved.includes(slug)?saved.filter(x=>x!==slug):[...saved,slug];localStorage.setItem('coc_saved_bases_v1',JSON.stringify(saved));document.querySelectorAll(`[data-coc-bookmark="${CSS.escape(slug)}"]`).forEach(x=>x.classList.toggle('active',saved.includes(slug)));return}const vote=e.target.closest('[data-coc-vote]');if(vote){e.preventDefault();e.stopPropagation();const slug=vote.dataset.cocVote,key='coc_vote_'+slug,done=localStorage.getItem(key)==='1';localStorage.setItem(key,done?'0':'1');vote.classList.toggle('active',!done);const b=vote.querySelector('b');if(b)b.textContent=done?(Number(b.textContent||4.8)-.1).toFixed(1):(Math.min(5,Number(b.textContent||4.8)+.1)).toFixed(1);return}},sig);document.addEventListener('keydown',e=>{if(e.key==='Escape')close()},sig);document.querySelectorAll('.coc-copy-link').forEach(a=>a.addEventListener('click',()=>{const slug=a.dataset.cocCopySlug;if(!slug)return;const k='coc_download_'+slug,n=Number(localStorage.getItem(k)||0)+1;localStorage.setItem(k,String(n));document.querySelectorAll(`[data-coc-download-count="${CSS.escape(slug)}"]`).forEach(el=>{const base=Number((el.textContent||'').replace(/,/g,''))||0;el.textContent=(base+1).toLocaleString()})},sig))}
