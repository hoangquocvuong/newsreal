...
/* V20.9.25 — Personal blog, corporate & EV charging template family */
function renderShowcaseTemplate(preset,site={}){
 ...
 const configs={
 personal_blog_1:{kind:'blog',brand:'LINH JOURNAL',eyebrow:'WRITING · LIFE · WORK',title:'Những điều đáng nhớ,\nviết theo cách của riêng mình.',desc:'Một không gian cá nhân tối giản để chia sẻ góc nhìn, kiến thức và hành trình nghề nghiệp.',cta:'Đọc bài mới',accent:'violet',cards:[['Sống chậm giữa lịch trình bận rộn','Lifestyle'],['7 bài học sau 5 năm làm tự do','Career'],['Góc làm việc giúp mình tập trung hơn','Notes']]},
 personal_blog_2:{kind:'creator',brand:'AN CREATIVE',eyebrow:'CREATOR · DESIGN · STORIES',title:'Create. Share.\nBuild your own audience.',desc:'Blog + portfolio cho creator, freelancer và người làm nội dung muốn xây thương hiệu cá nhân.',cta:'Khám phá portfolio',accent:'orange',cards:[['Behind the scenes: một dự án branding','Case Study'],['Bộ công cụ mình dùng mỗi ngày','Creator'],['Cách xây newsletter đầu tiên','Growth']]},
 corporate_modern_1:{kind:'corp',brand:'NOVATECH',eyebrow:'DIGITAL SOLUTIONS FOR BUSINESS',title:'Giải pháp số giúp doanh nghiệp\ntăng trưởng bền vững.',desc:'Website công ty hiện đại với dịch vụ, năng lực, dự án và CTA tư vấn rõ ràng.',cta:'Nhận tư vấn',accent:'blue',cards:[['Tư vấn chuyển đổi số','Strategy'],['Thiết kế & phát triển sản phẩm','Product'],['Vận hành & tối ưu dữ liệu','Operations']]},
 corporate_industry_2:{kind:'industry',brand:'VIETCORE INDUSTRIES',eyebrow:'ENGINEERING · CONSTRUCTION · PRODUCTION',title:'Năng lực thật.\nCông trình bền vững.',desc:'Hồ sơ năng lực online dành cho công ty sản xuất, xây dựng và kỹ thuật.',cta:'Nhận hồ sơ năng lực',accent:'slate',cards:[['Nhà máy & dây chuyền','Production'],['Thi công công nghiệp','Construction'],['Cơ điện & bảo trì','Engineering']]},
 service_ev_charge_5:{kind:'ev',brand:'EV CHARGE HUB',eyebrow:'ĐIỂM SẠC · LẮP ĐẶT · KHẢO SÁT',title:'Giải pháp điểm sạc\nxe điện thuận tiện hơn.',desc:'Mẫu website dịch vụ cung cấp, lắp đặt và tra cứu điểm sạc xe điện VinFast. Không phải website chính thức của VinFast.',cta:'Đăng ký khảo sát',accent:'cyan',cards:[['Điểm sạc công cộng','24/7'],['Lắp đặt sạc tại nhà','Khảo sát tận nơi'],['Giải pháp cho doanh nghiệp','Bãi xe · showroom']]}
 };
 const c=configs[preset]||configs.personal_blog_1;
 const icons=c.kind==='ev'?['⚡','⌁','▣']:c.kind==='corp'?['◈','◎','↗']:c.kind==='industry'?['▤','⌂','⚙']:['✦','◇','↗'];
 root.innerHTML=`<div class="sx-site sx-${c.kind}" data-accent="${c.accent}"><header class="sx-header"><div class="sx-wrap"><a class="sx-brand" href="#">${c.brand}</a><nav><a href="#services">${c.kind==='blog'||c.kind==='creator'?'Bài viết':'Dịch vụ'}</a><a href="#about">Giới thiệu</a><a href="#contact">Liên hệ</a></nav><a class="sx-head-cta" href="#contact">${c.kind==='ev'?'Tìm điểm sạc':'Bắt đầu'}</a></div></header><section class="sx-hero"><div class="sx-wrap sx-hero-grid"><div><span class="sx-kicker">${c.eyebrow}</span><h1>${c.title.replace(/\n/g,'<br>')}</h1><p>${c.desc}</p><div class="sx-actions"><a href="#services">${c.cta}</a><a class="ghost" href="#about">Xem thêm →</a></div><div class="sx-proof"><b>${c.kind==='ev'?'24/7':'10+'}</b><span>${c.kind==='ev'?'Hỗ trợ tra cứu':'Năm kinh nghiệm'}</span><b>${c.kind==='blog'||c.kind==='creator'?'120+':'50+'}</b><span>${c.kind==='blog'||c.kind==='creator'?'Bài viết':'Dự án'}</span></div></div><div class="sx-visual"><div class="sx-orb"></div><div class="sx-visual-card"><small>${c.kind==='ev'?'TRẠM SẠC GẦN BẠN':c.kind==='blog'||c.kind==='creator'?'FEATURED STORY':'FEATURED PROJECT'}</small><strong>${c.cards[0][0]}</strong><span>${c.cards[0][1]}</span></div><div class="sx-mini one">${icons[0]}</div><div class="sx-mini two">${icons[1]}</div></div></div></section><section id="services" class="sx-section"><div class="sx-wrap"><div class="sx-heading"><span>${c.kind==='blog'||c.kind==='creator'?'NỘI DUNG NỔI BẬT':'GIẢI PHÁP'}</span><h2>${c.kind==='ev'?'Sạc điện dễ tìm, dễ triển khai':c.kind==='blog'||c.kind==='creator'?'Bài viết mới nhất':'Dịch vụ tạo ra giá trị thực'}</h2></div><div class="sx-grid">${c.cards.map((x,i)=>`<article><div class="sx-icon">${icons[i]}</div><small>${x[1]}</small><h3>${x[0]}</h3><p>${c.kind==='blog'||c.kind==='creator'?'Nội dung được trình bày rõ ràng, dễ đọc trên mọi thiết bị.':'Quy trình rõ ràng, thông tin dễ hiểu và CTA chuyển đổi phù hợp.'}</p><a href="#">Xem chi tiết →</a></article>`).join('')}</div></div></section><section id="about" class="sx-band"><div class="sx-wrap sx-band-grid"><div><span>VÌ SAO CHỌN MẪU NÀY</span><h2>Gọn gàng trên desktop,\nđẹp và dễ dùng trên mobile.</h2></div><ul><li>✓ Responsive mobile-first</li><li>✓ Tối ưu cấu trúc SEO</li><li>✓ Quản trị nội dung dễ dùng</li><li>✓ CTA rõ ràng, tốc độ tốt</li></ul></div></section><section id="contact" class="sx-contact"><div class="sx-wrap"><span>SẴN SÀNG BẮT ĐẦU?</span><h2>${c.kind==='ev'?'Đưa dịch vụ điểm sạc của bạn lên online.':'Biến thương hiệu của bạn thành một website chuyên nghiệp.'}</h2><a href="#">${c.cta} →</a></div></section><footer class="sx-footer"><div class="sx-wrap"><b>${c.brand}</b><span>Demo template by HoangVuongTech</span></div></footer></div>`;
}
