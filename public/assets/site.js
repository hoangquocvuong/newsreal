...
const NR_FAST_NAV_VERSION='20.9.4';
...
async function nrBootMain(){
 try{
   ...
   // V20.9.4: the public Clash of Clans showroom is a self-contained demo package.
...
function nrGamePosts(){const live=(SITE_DATA?.posts||[]).filter(x=>['game','base'].includes(String(x.type||'').toLowerCase())||nrGameExtra(x).game_group);let local=[];try{if(typeof GAME_REAL_SAMPLE_POSTS!=='undefined'&&Array.isArray(GAME_REAL_SAMPLE_POSTS))local=GAME_REAL_SAMPLE_POSTS}catch(e){}return window.NR_DEMO_THEME==='game-1'?local:live}
