...
function renderServiceProvider(site={},key='dich-vu-1'){
 ...
 const card=(x,cl='')=>{const e=ex(x),im=x.image||x.image_url||x.thumbnail||'';return `<article class="tel-card ${cl}">${im?`<a class="tel-card-img" href="#dang-ky"><img src="${esc(im)}" alt="${esc(x.title)}" loading="lazy"></a>`:''}<div class="tel-card-body"><span class="tel-pill">${esc(x.category||'Dịch vụ')}</span><h3>${esc(x.title||'Gói dịch vụ')}</h3><div class="tel-speed">${esc(e.service_speed_down||'Tư vấn theo nhu cầu')}</div><p class="tel-desc">${esc(e.service_promo||String(x.content||'').replace(/<[^>]+>/g,' ').slice(0,180)||'Thông tin đang cập nhật')}</p><div class="tel-card-bottom"><div class="tel-price"><small>Chỉ từ</small><strong>${esc(e.service_price||'Liên hệ')}</strong></div><a class="tel-cta svc-lead-open" href="#dang-ky" data-package="${esc(x.title||'')}" data-category="${esc(x.category||'')}">Đăng ký</a></div></div></article>`};
