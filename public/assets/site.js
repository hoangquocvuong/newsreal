...
/* =========================================================
   V20.8.7 — GAME PORTAL / CLASH OF CLANS CONTRACT
...
function nrGameSlug(post){const raw=String(post?.demo_url||post?.url||post?.slug||post?.title||'base');const tail=(raw.split('/').pop()||raw).replace(/\.html?(?:[?#].*)?$/i,'');return tail.toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'')||'base'}
function nrGameHash(v){let h=2166136261>>>0;for(const c of String(v||'')){h^=c.charCodeAt(0);h=Math.imul(h,16777619)>>>0}return h>>>0}
function nrGameMetric(post,key){const h=nrGameHash(nrGameSlug(post));if(key==='rating'){const n=Number(post?.rating||0);return n>0?n:(4.5+((h%5)/10))}if(key==='views'){const n=Number(post?.views||0);return n>0?n:(650+(h%4800))}if(key==='downloads'){const e=nrGameExtra(post),n=Number(post?.downloads||e.downloads||0);return n>0?n:(120+(h%980))}return 0}
function nrGameSavedSlugs(){try{return JSON.parse(localStorage.getItem('coc_saved_bases_v1')||'[]')}catch(e){return []}}
function nrGameIsSaved(post){return nrGameSavedSlugs().includes(nrGameSlug(post))}
function nrGameStatsBar(post,cls=''){const slug=nrGameSlug(post),rating=nrGameMetric(post,'rating').toFixed(1),views=Math.round(nrGameMetric(post,'views')).toLocaleString(),downs=Math.round(nrGameMetric(post,'downloads')).toLocaleString();return `<div class="coc-base-actions ${cls}" data-coc-stats="${esc(slug)}"><button type="button" class="coc-stat-btn coc-vote-btn" data-coc-vote="${esc(slug)}" title="Vote base"><span>★</span><b>${rating}</b></button><span class="coc-stat-item" title="Views"><span>◉</span><b>${views}</b></span><span class="coc-stat-item" title="Downloads"><span>⇩</span><b data-coc-download-count="${esc(slug)}">${downs}</b></span><button type="button" class="coc-stat-btn coc-bookmark-btn ${nrGameIsSaved(post)?'active':''}" data-coc-bookmark="${esc(slug)}" title="Save base" aria-label="Save base"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M6 3h12v18l-6-4-6 4V3z"/></svg><b>Save</b></button></div>`}
function nrGameMoreGrid(prefix){return `<div class="coc-more-grid"><a href="${prefix}/bases?th=all"><span>🏰</span><b>Town Hall</b></a><a href="${prefix}/bases?bh=all"><span>🛖</span><b>Builder Hall</b></a><button type="button" data-coc-popup="events"><span>📅</span><b>Events</b></button><a href="${prefix}/bases?ch=all"><span>🏯</span><b>Capital Hall</b></a><button type="button" data-coc-popup="rankings"><span>🏆</span><b>Rankings</b></button><button type="button" data-coc-popup="hero-skins"><span>⭐</span><b>Hero Skins</b></button><button type="button" data-coc-popup="about"><span>ⓘ</span><b>About</b></button><button type="button" data-coc-popup="privacy"><span>🛡</span><b>Privacy</b></button><button type="button" data-coc-popup="apps"><span>📱</span><b>Apps</b></button></div>`}
function nrGameNavIcon(name){const icons={home:'<path d="M12 3 3 10h2v10h5v-6h4v6h5V10h2L12 3z"/>',news:'<path d="M5 4h14a1 1 0 0 1 1 1v14a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1V5a1 1 0 0 1 1-1zM8 8h8M8 12h8M8 16h5"/>',finder:'<path d="M8 3H4a1 1 0 0 0-1 1v4M16 3h4a1 1 0 0 1 1 1v4M8 21H4a1 1 0 0 1-1-1v-4M16 21h4a1 1 0 0 0 1-1v-4M7 9.2 12 6l5 3.2v5.6L12 18l-5-3.2V9.2z"/>',donate:'<path d="M12 21s-7-4.35-9.33-8.13C.68 9.64 2.08 5.5 5.75 4.66 7.83 4.18 9.67 5.12 12 7.38c2.33-2.26 4.17-3.2 6.25-2.72 3.67.84 5.07 4.98 3.08 8.21C19 16.65 12 21 12 21z"/>',saved:'<path d="M6 2h12v20l-6-4-6 4V2z"/>',more:'<path d="M4 6h16v2H4V6zm0 5h16v2H4v-2zm0 5h16v2H4v-2z"/>'};return icons[name]||icons.more}
function nrGameNavButton(id,label,icon,attrs=''){const fill=icon==='news'||icon==='finder'?'none':'currentColor',stroke=fill==='none'?' stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"':'';return `<button id="${id}" class="nav-item nav-btn" type="button" ${attrs}><svg class="nav-icon" viewBox="0 0 24 24" fill="${fill}"${stroke}>${nrGameNavIcon(icon)}</svg><span>${label}</span></button>`}
function nrGameLoadOriginalAssets(){if(!document.getElementById('nr-coc-original-css')){const l=document.createElement('link');l.id='nr-coc-original-css';l.rel='stylesheet';l.href='/assets/coc-original.css?v=20.8.7';document.head.appendChild(l)}if(!document.getElementById('nr-coc-original-runtime')){const j=document.createElement('script');j.id='nr-coc-original-runtime';j.src='/assets/coc-original-runtime.js?v=20.8.7';j.defer=true;document.head.appendChild(j)}}
function nrGameOriginalGrid(type,prefix){const t=String(type||'th').toLowerCase(),range=t==='th'?[3,18]:[1,10],label=t==='th'?'TH':t==='bh'?'BH':'CH';let out='';for(let n=range[0];n<=range[1];n++){const key=label+n,img=GAME_CLASH_LEVEL_ART[key]||GAME_CLASH_LEVEL_ART.TH18||'';out+=`<div class="card"><img class="thumb" loading="lazy" alt="${label} ${n} Base" src="${img}" data-coc-level="${t}:${n}"><div class="card-title">${label} ${n}</div><div class="card-actions"><a class="free-link" href="${prefix}/bases?${t}=${n}&access=free">FREE</a><a class="premium-link" href="${prefix}/bases?${t}=${n}&access=premium">PREMIUM</a></div></div>`}return out}
function nrGameOriginalHome(prefix){return `<div class="app-content"><h1 id="town-hall">Town Hall</h1><div class="grid" id="th-grid">${nrGameOriginalGrid('th',prefix)}</div><h1 id="builder-hall">Builder Hall</h1><div class="grid" id="bh-grid">${nrGameOriginalGrid('bh',prefix)}</div><h1 id="capital-hall">Clan Capital</h1><div class="grid" id="ch-grid">${nrGameOriginalGrid('ch',prefix)}</div></div>`}
function nrGameHeader(prefix,admin){const home=prefix||'/';return `<header id="header"><div class="top-page-wrapper"></div><div class="header-bg" id="header-section"><div class="widget Header" id="Header1"><div class="blog-title"><a href="${home}"><img alt="Coc Bases Layout" src="https://blogger.googleusercontent.com/img/a/AVvXsEilaejQexQCh9iR7RGfJsOYI4ofX5EZ8H74VhcLlzm7JVYzRUBUo6rhss-OxAjkwIlA-YpL87jB624ldJiJfVM5f1J3h8NbnYlKe_m3urojRBBGJ-L7Fqhc8UEA1o_-_t8-4BzN32Q4KoHX8Qyd-TXG_HR-kUaUG_jUkK2djIJBZgIczs55GmszGeB4rVXo=s409"></a></div></div></div></header><nav id="mobile-nav" aria-label="COC Base Portal"><a class="nav-item" href="${home}"><svg class="nav-icon" viewBox="0 0 24 24" fill="currentColor">${nrGameNavIcon('home')}</svg><span>Home</span></a>${nrGameNavButton('nav-news-btn','News','news','data-coc-action="news"')}${nrGameNavButton('nav-ai-btn','Find Source','finder','data-coc-action="find-source"')}<a aria-label="Support CoC Base Pro" class="nav-item nav-donate" href="#donate-center" id="nav-donate-btn" role="button" data-coc-action="donate"><svg class="nav-icon" viewBox="0 0 24 24" fill="currentColor">${nrGameNavIcon('donate')}</svg><span>Donate</span></a>${nrGameNavButton('nav-saved-btn','Saved','saved','data-more-action="saved" data-coc-action="saved"')}${nrGameNavButton('nav-more-btn','More','more','aria-expanded="false" aria-controls="mobile-more-sheet" data-coc-action="more"')}</nav>`}
function nrGameMoreGrid(prefix){return `<div class="more-sheet-grid"><a class="more-sheet-item" href="${prefix}/bases?th=all"><span>🏠</span><b>Town Hall</b></a><a class="more-sheet-item" href="${prefix}/bases?bh=all"><span>🛠️</span><b>Builder Hall</b></a><button class="more-sheet-item" type="button" data-more-action="events"><span>📅</span><b>Events</b></button><a class="more-sheet-item" href="${prefix}/bases?ch=all"><span>🏛️</span><b>Capital Hall</b></a><button class="more-sheet-item" type="button" data-more-action="rankings"><span>🏆</span><b>Rankings</b></button><button class="more-sheet-item" type="button" data-more-action="hero-skins"><span>⭐</span><b>Hero Skins</b></button><button class="more-sheet-item" type="button" data-more-action="about"><span>ℹ️</span><b>About</b></button><button class="more-sheet-item" type="button" data-more-action="apps"><span>📱</span><b>Apps</b></button><button class="more-sheet-item" type="button" data-more-action="privacy"><span>🛡️</span><b>Privacy</b></button></div>`}
function nrGamePopupContent(type,prefix,posts){const list=(posts||[]).slice(0,10);if(type==='news')return {title:'News & Base Updates',html:`<div class="coc-popup-list">${list.map(p=>`<a href="${esc(p.demo_url||p.url||'#')}"><img src="${esc(p.image||'')}" alt=""><span><b>${esc(p.title||'Clash of Clans base')}</b><small>${esc(nrGameLevel(p))} · ${esc(nrGameExtra(p).game_purpose||'Base')}</small></span></a>`).join('')}</div>`};if(type==='find-source')return {title:'Find Source',html:`<div class="coc-find-source"><p>Tìm nhanh base theo tên, TH/BH/CH, purpose hoặc district.</p><input id="coc-source-search" type="search" placeholder="Ví dụ: TH15 Farming, Dragon Cliffs..."><div id="coc-source-results" class="coc-source-results"></div></div>`};if(type==='saved'){const saved=nrGameSavedSlugs(),items=(posts||[]).filter(p=>saved.includes(nrGameSlug(p)));return {title:'Saved Bases',html:items.length?`<div class="coc-popup-list">${items.map(p=>`<a href="${esc(p.demo_url||p.url||'#')}"><img src="${esc(p.image||'')}" alt=""><span><b>${esc(p.title||'Base')}</b><small>${esc(nrGameLevel(p))}</small></span></a>`).join('')}</div>`:'<div class="coc-popup-empty">Chưa có base nào được lưu. Nhấn bookmark trên card để lưu.</div>'}};if(type==='events')return {title:'Clash of Clans Events',html:'<div class="coc-popup-copy"><h4>Events & seasonal layouts</h4><p>Gold Pass, Clan Games, Clan War League, Raid Weekend và các bộ sưu tập theo mùa.</p></div>'};if(type==='rankings'){const top=(posts||[]).slice().sort((a,b)=>nrGameMetric(b,'views')-nrGameMetric(a,'views')).slice(0,10);return {title:'Popular Base Rankings',html:`<div class="coc-popup-list">${top.map((p,i)=>`<a href="${esc(p.demo_url||p.url||'#')}"><strong>#${i+1}</strong><span><b>${esc(p.title||'Base')}</b><small>◉ ${Math.round(nrGameMetric(p,'views')).toLocaleString()} views</small></span></a>`).join('')}</div>`}};if(type==='hero-skins')return {title:'Hero Skins',html:'<div class="coc-popup-copy"><h4>Hero Skins Showcase</h4><p>Giữ chuyên mục Hero Skins trong menu theo portal CocBasePro gốc.</p></div>'};if(type==='about')return {title:'About CocBasePro',html:'<div class="coc-popup-copy"><p>Thư viện chia sẻ base Clash of Clans miễn phí cho Town Hall, Builder Hall và Clan Capital.</p></div>'};if(type==='privacy')return {title:'Privacy',html:'<div class="coc-popup-copy"><p>Bookmark của showroom được lưu cục bộ. Production có thể đồng bộ Firebase/backend theo user ID.</p></div>'};if(type==='apps')return {title:'Mobile Apps',html:'<div class="coc-popup-copy"><h4>COC Base Portal Apps</h4><p>Vị trí Android/iOS Apps được giữ trong More menu giống cấu trúc gốc.</p></div>'};return {title:'CocBasePro',html:''}}
function nrGamePopupShell(prefix,posts){return `<div id="mobile-more-sheet" class="coc-more-sheet" aria-hidden="true"><button class="more-sheet-overlay" type="button" data-close-more="1" aria-label="Close menu"></button><section class="more-sheet-panel" role="dialog" aria-modal="true" aria-labelledby="mobile-more-title"><div class="more-sheet-handle"></div><div class="more-sheet-head"><h3 id="mobile-more-title">Explore CocBasePro</h3><button class="more-sheet-close" type="button" data-close-more="1" aria-label="Close menu">×</button></div>${nrGameMoreGrid(prefix)}</section></div><div id="simple-popup" class="coc-simple-popup" aria-hidden="true"><button class="overlay" type="button" data-close-simple="1" aria-label="Close"></button><section class="box" role="dialog" aria-modal="true"><div class="coc-popup-head"><div><small>COC BASE PORTAL</small><h3 id="sp-title">CocBasePro</h3></div><button class="close-btn" type="button" data-close-simple="1" aria-label="Close">×</button></div><div id="sp-text" class="coc-popup-body"></div></section></div><div id="cbp-support-popup" class="coc-support-popup" aria-hidden="true"><button class="cbp-support-overlay" type="button" data-close-support="1" aria-label="Close"></button><section class="cbp-support-panel" role="dialog" aria-modal="true" aria-labelledby="cbp-support-title"><button class="cbp-support-close" type="button" data-close-support="1" aria-label="Close">×</button><header class="cbp-support-header"><div class="cbp-support-heart">♥</div><div><span class="cbp-support-kicker">COMMUNITY SUPPORTED</span><h3 id="cbp-support-title">Support CoC Base Pro</h3><p>Help keep every base link free for the Clash of Clans community.</p></div></header><div class="cbp-support-goal-card"><div><small>COMMUNITY MODEL</small><strong>All bases stay free</strong></div><p>Donate chỉ hỗ trợ server, hosting và cập nhật dữ liệu. Không mở khóa base và không tạo quyền Premium.</p></div></section></div>`}
function nrGameBindUI(posts,prefix){if(window.__NR_COC_UI_ABORT__)try{window.__NR_COC_UI_ABORT__.abort()}catch(e){}const ctl=new AbortController();window.__NR_COC_UI_ABORT__=ctl;const sig={signal:ctl.signal};const nav=document.getElementById('mobile-nav'),more=document.getElementById('mobile-more-sheet'),moreBtn=document.getElementById('nav-more-btn'),simple=document.getElementById('simple-popup'),simpleTitle=document.getElementById('sp-title'),simpleText=document.getElementById('sp-text'),support=document.getElementById('cbp-support-popup');const syncNav=()=>{const opened=more?.classList.contains('show');document.documentElement.classList.toggle('mobile-menu-open',!!opened);if(nav){nav.setAttribute('aria-hidden',opened?'true':'false');nav.classList.toggle('menu-suppressed',!!opened)}if(moreBtn){moreBtn.classList.toggle('is-active',!!opened);moreBtn.setAttribute('aria-expanded',opened?'true':'false')}};const closeMore=()=>{more?.classList.remove('show');more?.setAttribute('aria-hidden','true');syncNav()};const openMore=()=>{closeSimple();closeSupport();more?.classList.add('show');more?.setAttribute('aria-hidden','false');syncNav()};const closeSimple=()=>{simple?.classList.remove('show');simple?.setAttribute('aria-hidden','true');document.documentElement.classList.remove('coc-simple-open')};const openSimple=type=>{closeMore();closeSupport();if(!simple||!simpleTitle||!simpleText)return;const c=nrGamePopupContent(type,prefix,posts);simpleTitle.textContent=c.title;simpleText.innerHTML=c.html;simple.classList.add('show');simple.setAttribute('aria-hidden','false');document.documentElement.classList.add('coc-simple-open');if(type==='find-source'){const input=document.getElementById('coc-source-search'),out=document.getElementById('coc-source-results');if(input&&out){const render=()=>{const q=input.value.trim().toLowerCase();const found=(posts||[]).filter(p=>!q||(`${p.title||''} ${nrGameLevel(p)} ${nrGameExtra(p).game_purpose||''} ${nrGameExtra(p).game_defense||''}`).toLowerCase().includes(q)).slice(0,10);out.innerHTML=found.map(p=>`<a href="${esc(p.demo_url||p.url||'#')}">${esc(nrGameLevel(p))} · ${esc(p.title||'Base')}</a>`).join('')||'<span>Không tìm thấy base.</span>'};input.addEventListener('input',render);render()}}};const closeSupport=()=>{support?.classList.remove('show');support?.setAttribute('aria-hidden','true');document.documentElement.classList.remove('cbp-support-open')};const openSupport=()=>{closeMore();closeSimple();support?.classList.add('show');support?.setAttribute('aria-hidden','false');document.documentElement.classList.add('cbp-support-open')};const closeAll=()=>{closeMore();closeSimple();closeSupport()};const closeThen=cb=>{closeMore();requestAnimationFrame(()=>requestAnimationFrame(()=>typeof cb==='function'&&cb()))};document.addEventListener('click',e=>{const action=e.target.closest('[data-coc-action]');if(action){e.preventDefault();const type=action.dataset.cocAction;if(type==='more'){more?.classList.contains('show')?closeMore():openMore();return}if(type==='donate'){openSupport();return}openSimple(type);return}if(e.target.closest('[data-close-more]')){e.preventDefault();closeMore();return}if(e.target.closest('[data-close-simple]')){e.preventDefault();closeSimple();return}if(e.target.closest('[data-close-support]')){e.preventDefault();closeSupport();return}const mi=e.target.closest('[data-more-action]');if(mi&&mi.closest('#mobile-more-sheet')){e.preventDefault();e.stopPropagation();const type=mi.dataset.moreAction;closeThen(()=>openSimple(type));return}const pop=e.target.closest('[data-coc-popup]');if(pop){e.preventDefault();const type=pop.dataset.cocPopup;if(type==='donate')openSupport();else openSimple(type);return}const bm=e.target.closest('[data-coc-bookmark]');if(bm){e.preventDefault();e.stopPropagation();const slug=bm.dataset.cocBookmark;let saved=nrGameSavedSlugs();saved=saved.includes(slug)?saved.filter(x=>x!==slug):[...saved,slug];localStorage.setItem('coc_saved_bases_v1',JSON.stringify(saved));document.querySelectorAll(`[data-coc-bookmark="${CSS.escape(slug)}"]`).forEach(x=>x.classList.toggle('active',saved.includes(slug)));return}const vote=e.target.closest('[data-coc-vote]');if(vote){e.preventDefault();e.stopPropagation();const slug=vote.dataset.cocVote,key='coc_vote_'+slug,done=localStorage.getItem(key)==='1';localStorage.setItem(key,done?'0':'1');vote.classList.toggle('active',!done);const b=vote.querySelector('b');if(b)b.textContent=done?(Number(b.textContent||4.8)-.1).toFixed(1):(Math.min(5,Number(b.textContent||4.8)+.1)).toFixed(1);return}},sig);document.addEventListener('keydown',e=>{if(e.key==='Escape')closeAll()},sig);window.addEventListener('pageshow',closeAll,sig);document.querySelectorAll('.coc-copy-link').forEach(a=>a.addEventListener('click',()=>{const slug=a.dataset.cocCopySlug;if(!slug)return;const k='coc_download_'+slug,n=Number(localStorage.getItem(k)||0)+1;localStorage.setItem(k,String(n));document.querySelectorAll(`[data-coc-download-count="${CSS.escape(slug)}"]`).forEach(el=>{const base=Number((el.textContent||'').replace(/,/g,''))||0;el.textContent=(base+1).toLocaleString()})},sig));syncNav()}
function nrGameFooter(prefix=''){return `<footer class="coc-footer coc-footer-full"><div class="coc-wrap coc-footer-grid"><div><a class="coc-brand coc-footer-brand" href="${prefix||'/'}"><b>COC</b><span>BASE PORTAL</span></a><p>Kho chia sẻ base Clash of Clans miễn phí cho cộng đồng: Town Hall, Builder Hall và Clan Capital.</p></div><div><b>BASE LAYOUTS</b><a href="${prefix}/bases?th=all">Town Hall Bases</a><a href="${prefix}/bases?bh=all">Builder Hall Bases</a><a href="${prefix}/bases?ch=all">Clan Capital Bases</a></div><div><b>KHÁM PHÁ</b><button type="button" data-coc-popup="news">News</button><button type="button" data-coc-popup="rankings">Rankings</button><button type="button" data-coc-popup="hero-skins">Hero Skins</button><button type="button" data-coc-popup="events">Events</button></div><div><b>HỖ TRỢ</b><button type="button" data-coc-popup="find-source">Find Source</button><button type="button" data-coc-popup="saved">Saved Bases</button><button type="button" data-coc-popup="donate">Donate</button><button type="button" data-coc-popup="privacy">Privacy</button></div></div><div class="coc-footer-bottom coc-wrap"><span>Community Clash of Clans base sharing</span><span>Not affiliated with Supercell.</span></div></footer>`}
function nrGameHomeExtras(prefix,posts){const recent=posts.slice(0,4);return `<section id="news" class="coc-community-section"><div class="coc-wrap"><div class="coc-heading"><div><span>NEWS & UPDATES</span><h2>Latest base updates</h2></div></div><div class="coc-community-grid">${recent.map(p=>`<a class="coc-community-card" href="${p.demo_url||p.url||'#'}"><img src="${esc(p.image||'')}" alt="${esc(p.title||'Clash of Clans base')}" loading="lazy"><b>${esc(p.title||'Clash of Clans Base')}</b></a>`).join('')}</div></div></section>`}
function nrGameBaseCard(post){const e=nrGameExtra(post),level=nrGameLevel(post),href=post.demo_url||post.url||'#';return `<article class="coc-card" data-coc-base="${esc(nrGameSlug(post))}"><a class="coc-thumb" href="${esc(href)}"><img src="${esc(post.image||nrGameArt(level.slice(0,2).toLowerCase(),Number(level.replace(/\D/g,''))))}" alt="${esc(post.title||level+' Base')}" loading="lazy" decoding="async"><span class="coc-level">${esc(level)}</span></a><div class="coc-card-body"><h3><a href="${esc(href)}">${esc(post.title||level+' Base Layout')}</a></h3><div class="coc-tags"><span>${esc(e.game_purpose||'Base')}</span><span>${esc(e.game_style||'Original')}</span><span>${esc(e.game_defense||'Balanced Defense')}</span></div>${nrGameStatsBar(post,'coc-card-stats')}</div></article>`}
...
function nrGameDetail(post,prefix,posts=[]){const e=nrGameExtra(post),level=nrGameLevel(post),copy=e.copy_link||'#',related=nrGameRelatedPosts(post,posts),slug=nrGameSlug(post);return `<div class="coc-site">${nrGameHeader(prefix,'')}${nrGamePopupShell(prefix,posts)}<main class="coc-detail coc-wrap"><div class="coc-breadcrumb"><a href="${prefix||'/'}">Home</a> / ${esc(level)} / ${esc(post.title||'Base')}</div><div class="coc-detail-grid"><article><img class="coc-detail-image" src="${esc(post.image||GAME_CLASH_LEVEL_ART[level]||GAME_CLASH_LEVEL_ART.TH18)}" alt="${esc(post.title||level+' Base Layout')}">${nrGameStatsBar(post,'coc-detail-stats')}<div class="coc-detail-copy"><div class="coc-tags"><span>${esc(level)}</span><span>${esc(e.game_purpose||'Base')}</span><span>${esc(e.game_style||'Original')}</span><span>${esc(e.game_defense||'Balanced Defense')}</span></div><h1>${esc(post.title||level+' Base Layout')}</h1><div class="coc-article-body">${post.content||''}</div></div></article><aside class="coc-detail-side"><div class="coc-access-card"><small>BASE LINK</small><h3>${esc(level)} · ${esc(e.game_purpose||'Base')}</h3><a class="coc-copy-link" data-coc-copy-slug="${esc(slug)}" href="${esc(copy)}" target="_blank" rel="noopener">Copy Base Link</a></div></aside></div>${related.length?`<section class="coc-related"><div class="coc-heading"><div><span>RELATED BASES</span><h2>Bài viết liên quan</h2></div></div><div class="coc-related-grid">${related.map(nrGameBaseCard).join('')}</div></section>`:''}</main>${nrGameFooter(prefix)}</div>`}
function renderGameClash1(site={}){nrGameLoadOriginalAssets();const root=document.querySelector('main');if(!root)return;const posts=nrGamePosts(),isDemo=window.NR_DEMO_THEME==='game-1',prefix=isDemo?(window.NR_DEMO_PREFIX||'/demo/game/clash-of-clans'):'',rel=isDemo&&location.pathname.startsWith(prefix)?(location.pathname.slice(prefix.length)||'/'):location.pathname,admin=isDemo?nrDemoAdminUrl('game-1','newpost'):'/admin?tab=newpost';if(/^\/base\/[^/]+\.html$/i.test(rel)){const slug=rel.split('/').pop().replace(/\.html$/i,''),post=posts.find(p=>String(p.demo_url||p.url||'').includes('/'+slug+'.html'))||posts[0];if(post){root.innerHTML=nrGameDetail(post,prefix,posts);nrGameBindUI(posts,prefix);return}}if(/^\/(bases|free-bases|premium-bases)/i.test(rel)){if(/\/(free|premium)-bases/i.test(rel))history.replaceState({},'',`${prefix}/bases${location.search||'?th=all'}`);root.innerHTML=`<div class="coc-site">${nrGameHeader(prefix,admin)}${nrGamePopupShell(prefix,posts)}${nrGameFilterPage(posts,prefix)}${nrGameFooter(prefix)}</div>`;nrGameBindFastFilters(posts,prefix);nrGameBindUI(posts,prefix);if(!window.__NR_COC_POPSTATE__){window.__NR_COC_POPSTATE__=1;window.addEventListener('popstate',()=>{const b=document.querySelector('[data-coc-browser="1"]');if(b){b.outerHTML=nrGameFilterPage(posts,prefix);nrGameBindFastFilters(posts,prefix)}})}return}root.innerHTML=`<div class="coc-site original-coc-ui">${nrGameHeader(prefix,admin)}${nrGamePopupShell(prefix,posts)}${nrGameOriginalHome(prefix)}${nrGameHomeExtras(prefix,posts)}${nrGameFooter(prefix)}</div>`;nrGameBindUI(posts,prefix);try{nrApplyStructureGeometry(site,'game-1');nrAuditStructureContract(site,'game-1')}catch(e){}}
