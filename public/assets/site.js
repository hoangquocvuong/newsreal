...
function nrGamePosts(){const live=(SITE_DATA?.posts||[]).filter(x=>['game','base'].includes(String(x.type||'').toLowerCase())||nrGameExtra(x).game_group);let local=[];try{if(typeof GAME_REAL_SAMPLE_POSTS!=='undefined'&&Array.isArray(GAME_REAL_SAMPLE_POSTS))local=GAME_REAL_SAMPLE_POSTS}catch(e){}if(window.NR_DEMO_THEME==='game-1')return live.length?live:local;return live}
