...
/* =========================================================
   V20.8.6 — GAME PORTAL / CLASH OF CLANS CONTRACT
...
function nrGameHeader(prefix,admin){const home=prefix||'/';return `<header class="coc-top"><div class="coc-wrap"><a class="coc-brand" href="${home}"><b>COC</b><span>BASE PORTAL</span></a>${admin?`<a class="coc-admin" href="${esc(admin)}" target="_blank" rel="noopener">+ Đăng base</a>`:''}</div></header><nav id="mobile-nav" class="coc-global-nav" aria-label="COC Base Portal"><a class="nav-item" href="${home}"><svg class="nav-icon" viewBox="0 0 24 24" fill="currentColor">${nrGameNavIcon('home')}</svg><span>Home</span></a>${nrGameNavButton('nav-news-btn','News','news','data-coc-action="news"')}${nrGameNavButton('nav-ai-btn','Find Source','finder','data-coc-action="find-source"')}<button id="nav-donate-btn" class="nav-item nav-btn nav-donate" type="button" data-coc-action="donate"><svg class="nav-icon" viewBox="0 0 24 24" fill="currentColor">${nrGameNavIcon('donate')}</svg><span>Donate</span></button>${nrGameNavButton('nav-saved-btn','Saved','saved','data-coc-action="saved"')}${nrGameNavButton('nav-more-btn','More','more','aria-expanded="false" aria-controls="mobile-more-sheet" data-coc-action="more"')}</nav>`}
...
function renderGameClash1(site={}){const root=document.querySelector('main');if(!root)return;const posts=nrGamePosts(),isDemo=window.NR_DEMO_THEME==='game-1',prefix=isDemo?(window.NR_DEMO_PREFIX||'/demo/game/clash-of-clans'):'',rel=isDemo&&location.pathname.startsWith(prefix)?(location.pathname.slice(prefix.length)||'/'):location.pathname,admin=isDemo?nrDemoAdminUrl('game-1','newpost'):'/admin?tab=newpost';if(/^\/base\/[^/]+\.html$/i.test(rel)){const slug=rel.split('/').pop().replace(/\.html$/i,''),post=posts.find(p=>String(p.demo_url||p.url||'').includes('/'+slug+'.html'))||posts[0];if(post){root.innerHTML=nrGameDetail(post,prefix,posts);nrGameBindUI(posts,prefix);return}}if(/^\/(bases|free-bases|premium-bases)/i.test(rel)){if(/\/(free|premium)-bases/i.test(rel))history.replaceState({},'',`${prefix}/bases${location.search||'?th=all'}`);root.innerHTML=`<div class="coc-site">${nrGameHeader(prefix,admin)}${nrGamePopupShell(prefix,posts)}${nrGameFilterPage(posts,prefix)}${nrGameFooter(prefix)}</div>`;nrGameBindFastFilters(posts,prefix);nrGameBindUI(posts,prefix);if(!window.__NR_COC_POPSTATE__){window.__NR_COC_POPSTATE__=1;window.addEventListener('popstate',()=>{const b=document.querySelector('[data-coc-browser="1"]');if(b){b.outerHTML=nrGameFilterPage(posts,prefix);nrGameBindFastFilters(posts,prefix)}})}return}root.innerHTML=`<div class="coc-site">${nrGameHeader(prefix,admin)}${nrGamePopupShell(prefix,posts)}<section class="coc-hero"><div class="coc-wrap coc-hero-grid"><div><span class="coc-kicker">CLASH OF CLANS · COMMUNITY BASE LAYOUTS</span><h1>Choose your Hall.<br><em>Find a base fast.</em></h1><p>Kho chia sẻ base Clash of Clans miễn phí cho cộng đồng. Trang chủ chỉ chọn cấp nhà; bộ lọc chi tiết mở sau khi bạn chọn TH, BH hoặc CH.</p><div class="coc-proof"><span><b>TH2–TH18</b><small>Town Hall</small></span><span><b>BH2–BH10</b><small>Builder Hall</small></span><span><b>CH1–CH10</b><small>Clan Capital</small></span></div></div><div class="coc-hero-art"><img src="${GAME_CLASH_LEVEL_ART.TH18.replace('/s600/','/s1600/')}" alt="TH18"><div><b>Choose Hall Level</b><span>Fast filters · Direct copy link</span></div></div></div></section><section id="town-hall" class="coc-section" data-structure-key="town-hall"><div class="coc-wrap"><div class="coc-heading"><div><span>TOWN HALL</span><h2>Choose Town Hall</h2></div></div><div class="coc-level-grid" data-contract-grid="1">${nrGameLevelCards('th',prefix)}</div></div></section><section id="builder-hall" class="coc-section coc-dark" data-structure-key="builder-hall"><div class="coc-wrap"><div class="coc-heading light"><div><span>BUILDER BASE</span><h2>Choose Builder Hall</h2></div></div><div class="coc-level-grid" data-contract-grid="1">${nrGameLevelCards('bh',prefix)}</div></div></section><section id="clan-capital" class="coc-section" data-structure-key="clan-capital"><div class="coc-wrap"><div class="coc-heading"><div><span>CLAN CAPITAL</span><h2>Choose Capital Hall</h2></div></div><div class="coc-level-grid" data-contract-grid="1">${nrGameLevelCards('ch',prefix)}</div></div></section>${nrGameHomeExtras(prefix,posts)}${nrGameFooter(prefix)}</div>`;nrGameBindUI(posts,prefix);try{nrApplyStructureGeometry(site,'game-1');nrAuditStructureContract(site,'game-1')}catch(e){}}
