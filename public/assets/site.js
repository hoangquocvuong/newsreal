...
function comDetail(id){const p=COM_DATA.products.find(x=>Number(x.id)===Number(id));if(!p)return;let attrs={};try{attrs=JSON.parse(p.attributes_json||'{}')}catch{};const html=`${p.description||'<p>'+esc(p.short_description||'')+'</p>'}<hr>${Object.entries(attrs).map(([k,v])=>`<p><b>${esc(k.replaceAll('_',' '))}:</b> ${esc(v)}</p>`).join('')}`;const m=document.createElement('div');m.className='com-cart open';m.id='comDetailPanel';m.innerHTML=`<div class="com-cart-head"><b>${esc(p.name)}</b><button onclick="this.closest('.com-cart').remove()">✕</button></div><div class="com-cart-list"><div class="com-card-img"><img src="${esc(p.image_url)}"></div><h2>${comMoney(p.price)}</h2>${html}</div><div class="com-cart-foot"><button class="com-add" style="width:100%" onclick="comAdd(${p.id});this.closest('.com-cart').remove()">Thêm vào giỏ hàng</button></div>`;document.body.appendChild(m)}
