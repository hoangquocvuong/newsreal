...
async function renderCommerceStore(site={}){
 ...
 const settings=sxSettings(site),brand=String(site.shop_name||site.name||'MIRA STORE');
 const money=n=>new Intl.NumberFormat('vi-VN').format(Number(n||0))+'đ';
 const fallback=[
  {id:'demo-1',name:'Tai nghe AirBeat Pro',cat:'Công nghệ',price:1290000,old:1690000,img:'https://images.unsplash.com/photo-1505740420928-5e560c06d30e?auto=format&fit=crop&w=900&q=82',badge:'BÁN CHẠY'},
  {id:'demo-2',name:'Đồng hồ Urban Classic',cat:'Phụ kiện',price:1890000,old:2390000,img:'https://images.unsplash.com/photo-1523275335684-37898b6baf30?auto=format&fit=crop&w=900&q=82',badge:'MỚI'},
  {id:'demo-3',name:'Sneaker Motion 02',cat:'Thời trang',price:1590000,old:1990000,img:'https://images.unsplash.com/photo-1542291026-7eec264c27ff?auto=format&fit=crop&w=900&q=82',badge:'-20%'},
  {id:'demo-4',name:'Camera Mini Studio',cat:'Công nghệ',price:3490000,old:3990000,img:'https://images.unsplash.com/photo-1516035069371-29a1b244cc32?auto=format&fit=crop&w=900&q=82',badge:'NỔI BẬT'},
  {id:'demo-5',name:'Túi Everyday Canvas',cat:'Thời trang',price:790000,old:990000,img:'https://images.unsplash.com/photo-1553062407-98eeb64c6a62?auto=format&fit=crop&w=900&q=82',badge:'MỚI'},
  {id:'demo-6',name:'Serum Daily Glow',cat:'Mỹ phẩm',price:590000,old:750000,img:'https://images.unsplash.com/photo-1620916566398-39f1143ab7be?auto=format&fit=crop&w=900&q=82',badge:'YÊU THÍCH'},
  {id:'demo-7',name:'Đèn bàn Nordic',cat:'Nội thất',price:890000,old:1190000,img:'https://images.unsplash.com/photo-1507473885765-e6ed057f782c?auto=format&fit=crop&w=900&q=82',badge:'-25%'},
  {id:'demo-8',name:'Bình giữ nhiệt Flow',cat:'Gia dụng',price:390000,old:490000,img:'https://images.unsplash.com/photo-1602143407151-7111542de6e8?auto=format&fit=crop&w=900&q=82',badge:'BÁN CHẠY'}
 ];
 const raw=Array.isArray(SITE_DATA?.commerce_products)?SITE_DATA.commerce_products:[];
 const products=raw.length?raw.map((x,i)=>({id:x.id||i,name:x.name||x.title||'Sản phẩm',cat:x.category_name||x.category||'Khác',price:Number(x.sale_price||x.price||0),old:Number(x.compare_price||x.old_price||0),img:x.cover_image||x.image||getImages(x)[0]||fallback[i%fallback.length].img,badge:x.badge||''})):fallback;
 const cats=['Tất cả',...new Set(products.map(x=>x.cat).filter(Boolean))];
 const card=x=>`<article class="nv-card" data-cat="${esc(x.cat)}" data-name="${esc(x.name.toLowerCase())}"><button class="nv-heart" type="button" aria-label="Yêu thích">♡</button><a class="nv-photo" href="#product-${esc(x.id)}"><img src="${esc(x.img)}" alt="${esc(x.name)}" loading="lazy">${x.badge?`<span>${esc(x.badge)}</span>`:''}</a><div class="nv-card-body"><small>${esc(x.cat)}</small><h3>${esc(x.name)}</h3><div class="nv-price"><b>${money(x.price)}</b>${x.old>x.price?`<del>${money(x.old)}</del>`:''}</div><button class="nv-add" type="button" data-add="${esc(x.id)}">Thêm vào giỏ <span>＋</span></button></div></article>`;
 root.innerHTML=`<div class="nova23" data-commerce-v2="true"><style>
 .nova23{--ink:#151515;--muted:#6d6d6d;--line:#e9e6df;--cream:#f6f2e9;--lime:#dfff52;--orange:#ff5b35;background:#fff;color:var(--ink);font-family:Inter,system-ui,-apple-system,Segoe UI,sans-serif;margin-left:calc(50% - 50vw);margin-right:calc(50% - 50vw);min-height:100vh}.nova23 *{box-sizing:border-box}.nv-wrap{width:min(1280px,calc(100% - 48px));margin:auto}.nv-top{background:#151515;color:#fff;font-size:12px}.nv-top .nv-wrap{height:34px;display:flex;align-items:center;justify-content:space-between}.nv-top a{color:#fff;text-decoration:none}.nv-head{border-bottom:1px solid var(--line);background:#fff;position:sticky;top:0;z-index:40}.nv-head-main{height:82px;display:grid;grid-template-columns:210px minmax(280px,1fr) auto;gap:30px;align-items:center}.nv-logo{font:900 25px/1 Inter,sans-serif;letter-spacing:-1.4px;color:#111;text-decoration:none}.nv-logo i{font-style:normal;color:var(--orange)}.nv-search{height:46px;border:1.5px solid #d8d5ce;border-radius:999px;display:flex;align-items:center;padding:0 8px 0 18px;background:#faf9f6}.nv-search input{border:0;outline:0;background:transparent;width:100%;font:inherit}.nv-search button{border:0;border-radius:999px;background:#111;color:#fff;padding:10px 19px;font-weight:800}.nv-tools{display:flex;gap:8px}.nv-tool{height:44px;border:1px solid var(--line);background:#fff;border-radius:12px;padding:0 14px;font-weight:800;display:flex;align-items:center;gap:7px}.nv-nav{border-top:1px solid #f2f0eb}.nv-nav .nv-wrap{height:48px;display:flex;align-items:center;gap:30px}.nv-nav a{font-size:13px;font-weight:800;text-decoration:none;color:#252525}.nv-nav a:first-child{background:#111;color:#fff;border-radius:9px;padding:9px 13px}.nv-hero{padding:26px 0 16px}.nv-hero-grid{display:grid;grid-template-columns:minmax(0,1.7fr) minmax(280px,.7fr);gap:16px}.nv-mainhero{min-height:410px;border-radius:28px;background:#e7efdd;overflow:hidden;position:relative;padding:58px;display:flex;align-items:center}.nv-mainhero:after{content:"";position:absolute;right:-4%;bottom:-22%;width:48%;aspect-ratio:1;border-radius:50%;background:#cdddba}.nv-mainhero-copy{position:relative;z-index:2;max-width:560px}.nv-eyebrow{font-size:12px;font-weight:900;letter-spacing:.12em}.nv-mainhero h1{font-size:clamp(42px,5vw,72px);line-height:.98;letter-spacing:-4px;margin:18px 0}.nv-mainhero p{font-size:16px;color:#4f594b;max-width:470px;line-height:1.7}.nv-cta{display:inline-flex;margin-top:18px;background:#111;color:#fff;text-decoration:none;border-radius:999px;padding:14px 21px;font-weight:900}.nv-mainhero img{position:absolute;right:2%;bottom:0;width:42%;height:88%;object-fit:cover;border-radius:160px 160px 0 0;z-index:1}.nv-promos{display:grid;gap:16px}.nv-promo{border-radius:28px;padding:28px;min-height:197px;display:flex;flex-direction:column;justify-content:flex-end;overflow:hidden;position:relative}.nv-promo:first-child{background:#f8dfe5}.nv-promo:last-child{background:#e2e6fa}.nv-promo small{font-weight:900}.nv-promo b{font-size:25px;letter-spacing:-1px;max-width:220px;margin-top:7px}.nv-benefits{display:grid;grid-template-columns:repeat(4,1fr);border:1px solid var(--line);border-radius:18px;margin-top:8px}.nv-benefit{padding:19px 22px;display:flex;gap:12px;align-items:center;border-right:1px solid var(--line)}.nv-benefit:last-child{border:0}.nv-benefit b{display:block;font-size:13px}.nv-benefit small{color:var(--muted)}.nv-section{padding:54px 0}.nv-section-head{display:flex;justify-content:space-between;align-items:end;gap:20px;margin-bottom:22px}.nv-section-head h2{font-size:32px;letter-spacing:-1.5px;margin:0}.nv-section-head p{color:var(--muted);margin:6px 0 0}.nv-cats{display:flex;gap:10px;flex-wrap:wrap}.nv-cat{border:1px solid var(--line);background:#fff;border-radius:999px;padding:10px 15px;font-weight:800;cursor:pointer}.nv-cat.active{background:#111;color:#fff;border-color:#111}.nv-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:18px}.nv-card{position:relative;min-width:0}.nv-photo{display:block;height:310px;border-radius:20px;overflow:hidden;background:#f4f4f2;position:relative}.nv-photo img{width:100%;height:100%;object-fit:cover;transition:.35s}.nv-card:hover .nv-photo img{transform:scale(1.035)}.nv-photo span{position:absolute;left:12px;top:12px;background:#111;color:#fff;border-radius:999px;padding:7px 9px;font-size:10px;font-weight:900}.nv-heart{position:absolute;z-index:3;right:12px;top:12px;width:36px;height:36px;border:0;border-radius:50%;background:#fff;font-size:20px}.nv-card-body{padding:14px 3px}.nv-card-body small{color:#8a8a8a;font-weight:700}.nv-card h3{font-size:16px;margin:7px 0 10px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.nv-price{display:flex;align-items:center;gap:9px}.nv-price b{font-size:18px}.nv-price del{color:#999;font-size:12px}.nv-add{width:100%;margin-top:13px;height:42px;border:1px solid #222;border-radius:11px;background:#fff;font-weight:900;display:flex;justify-content:space-between;align-items:center;padding:0 13px}.nv-add:hover{background:#111;color:#fff}.nv-story{background:#151515;color:#fff;padding:65px 0}.nv-story-grid{display:grid;grid-template-columns:1fr 1fr;gap:60px;align-items:center}.nv-story img{width:100%;height:420px;object-fit:cover;border-radius:26px}.nv-story h2{font-size:46px;line-height:1.05;letter-spacing:-2.5px;margin:0 0 18px}.nv-story p{color:#bdbdbd;line-height:1.8}.nv-newsletter{background:var(--lime);padding:45px 0}.nv-newsletter .nv-wrap{display:flex;justify-content:space-between;align-items:center;gap:30px}.nv-newsletter h2{margin:0;font-size:30px;letter-spacing:-1px}.nv-newsletter form{display:flex;background:#fff;border-radius:999px;padding:5px;width:min(430px,100%)}.nv-newsletter input{border:0;outline:0;padding:0 15px;width:100%;font:inherit}.nv-newsletter button{border:0;background:#111;color:#fff;border-radius:999px;padding:12px 18px;font-weight:900}.nv-footer{background:#f7f5ef;padding:50px 0 25px}.nv-footer-grid{display:grid;grid-template-columns:2fr repeat(3,1fr);gap:40px}.nv-footer b{display:block;margin-bottom:14px}.nv-footer a,.nv-footer p{display:block;color:#666;text-decoration:none;font-size:13px;line-height:2}.nv-cart-toast{position:fixed;right:22px;bottom:22px;background:#111;color:#fff;border-radius:14px;padding:14px 18px;font-weight:800;z-index:90;opacity:0;transform:translateY(10px);pointer-events:none;transition:.25s}.nv-cart-toast.show{opacity:1;transform:none}
 @media(max-width:900px){.nv-wrap{width:min(100% - 28px,1280px)}.nv-head-main{grid-template-columns:1fr auto;height:auto;padding:15px 0;gap:12px}.nv-search{grid-column:1/-1;grid-row:2}.nv-nav{display:none}.nv-tool span{display:none}.nv-hero-grid{grid-template-columns:1fr}.nv-promos{grid-template-columns:1fr 1fr}.nv-mainhero{min-height:360px;padding:36px}.nv-mainhero h1{font-size:46px}.nv-benefits{grid-template-columns:1fr 1fr}.nv-benefit:nth-child(2){border-right:0}.nv-benefit:nth-child(-n+2){border-bottom:1px solid var(--line)}.nv-grid{grid-template-columns:repeat(2,1fr)}.nv-photo{height:260px}.nv-story-grid{grid-template-columns:1fr}.nv-footer-grid{grid-template-columns:1fr 1fr}}
 @media(max-width:560px){.nv-top .nv-wrap span:last-child{display:none}.nv-logo{font-size:21px}.nv-tool{padding:0 11px}.nv-mainhero{min-height:350px;padding:28px 24px;align-items:flex-start}.nv-mainhero h1{font-size:39px;letter-spacing:-2.4px}.nv-mainhero p{font-size:14px;max-width:62%}.nv-mainhero img{width:48%;height:56%}.nv-promos{grid-template-columns:1fr 1fr}.nv-promo{min-height:135px;padding:18px}.nv-promo b{font-size:18px}.nv-benefits{display:none}.nv-section{padding:38px 0}.nv-section-head{display:block}.nv-section-head h2{font-size:27px}.nv-cats{margin-top:16px;overflow:auto;flex-wrap:nowrap;padding-bottom:4px}.nv-cat{white-space:nowrap}.nv-grid{gap:11px}.nv-photo{height:190px;border-radius:15px}.nv-card h3{font-size:14px}.nv-price{display:block}.nv-price b{font-size:15px}.nv-add{font-size:12px}.nv-story{padding:45px 0}.nv-story h2{font-size:34px}.nv-story img{height:280px}.nv-newsletter .nv-wrap{display:block}.nv-newsletter form{margin-top:18px}.nv-footer-grid{grid-template-columns:1fr 1fr;gap:25px}.nv-footer-grid>div:first-child{grid-column:1/-1}}
 </style><div class="nv-top"><div class="nv-wrap"><span>Miễn phí vận chuyển đơn từ 500.000đ</span><span>Đổi trả 7 ngày · Hỗ trợ 08:00–22:00</span></div></div><header class="nv-head"><div class="nv-wrap nv-head-main"><a class="nv-logo" href="#">${esc(brand.split(' ')[0])}<i>+</i>STORE</a><form class="nv-search" id="nvSearch"><input id="nvSearchInput" placeholder="Tìm sản phẩm bạn yêu thích..."><button>Tìm kiếm</button></form><div class="nv-tools"><button class="nv-tool" type="button">♡ <span>Yêu thích</span></button><button class="nv-tool" type="button">🛍 <span>Giỏ hàng</span> <b id="nvCartCount">0</b></button></div></div><nav class="nv-nav"><div class="nv-wrap"><a href="#catalog">☰ Danh mục</a><a href="#catalog">Sản phẩm mới</a><a href="#catalog">Bán chạy</a><a href="#catalog">Ưu đãi</a><a href="#story">Về cửa hàng</a><a href="#footer">Liên hệ</a></div></nav></header><section class="nv-hero"><div class="nv-wrap"><div class="nv-hero-grid"><div class="nv-mainhero"><div class="nv-mainhero-copy"><span class="nv-eyebrow">BỘ SƯU TẬP MỚI · 2026</span><h1>Mỗi ngày một lựa chọn đáng thích.</h1><p>Sản phẩm được tuyển chọn cho phong cách sống hiện đại — đẹp, hữu ích và vừa túi tiền.</p><a class="nv-cta" href="#catalog">Khám phá ngay →</a></div><img src="https://images.unsplash.com/photo-1542291026-7eec264c27ff?auto=format&fit=crop&w=900&q=85" alt="Bộ sưu tập mới"></div><div class="nv-promos"><div class="nv-promo"><small>ƯU ĐÃI TRONG TUẦN</small><b>Phụ kiện chọn lọc đến -30%</b></div><div class="nv-promo"><small>NEW DROP</small><b>Những món mới vừa lên kệ</b></div></div></div><div class="nv-benefits"><div class="nv-benefit"><span>🚚</span><div><b>Giao hàng linh hoạt</b><small>Toàn quốc</small></div></div><div class="nv-benefit"><span>↩</span><div><b>Đổi trả dễ dàng</b><small>Trong 7 ngày</small></div></div><div class="nv-benefit"><span>✓</span><div><b>Thanh toán an toàn</b><small>COD · Chuyển khoản</small></div></div><div class="nv-benefit"><span>☏</span><div><b>Hỗ trợ nhanh</b><small>Khi bạn cần</small></div></div></div></div></section><section class="nv-section" id="catalog"><div class="nv-wrap"><div class="nv-section-head"><div><h2>Sản phẩm dành cho bạn</h2><p>Khám phá những lựa chọn đang được yêu thích.</p></div><div class="nv-cats">${cats.map((x,i)=>`<button class="nv-cat ${i===0?'active':''}" type="button" data-filter="${esc(x)}">${esc(x)}</button>`).join('')}</div></div><div class="nv-grid" id="nvGrid">${products.slice(0,8).map(card).join('')}</div></div></section><section class="nv-story" id="story"><div class="nv-wrap nv-story-grid"><img src="https://images.unsplash.com/photo-1441986300917-64674bd600d8?auto=format&fit=crop&w=1200&q=82" alt="Không gian cửa hàng"><div><span class="nv-eyebrow">CHỌN LỌC CÓ CHỦ ĐÍCH</span><h2>Một cửa hàng nhỏ, nhiều lựa chọn hay.</h2><p>Chúng tôi ưu tiên những sản phẩm có thiết kế tốt, công năng rõ ràng và mức giá hợp lý. Mỗi bộ sưu tập được sắp xếp để khách hàng tìm món phù hợp nhanh hơn, không phải lạc giữa hàng nghìn lựa chọn.</p><a class="nv-cta" href="#catalog">Xem bộ sưu tập →</a></div></div></section><section class="nv-newsletter"><div class="nv-wrap"><div><small>ƯU ĐÃI & SẢN PHẨM MỚI</small><h2>Nhận tin hay, không nhận thư rác.</h2></div><form onsubmit="event.preventDefault()"><input type="email" placeholder="Email của bạn"><button>Đăng ký</button></form></div></section><footer class="nv-footer" id="footer"><div class="nv-wrap nv-footer-grid"><div><a class="nv-logo" href="#">${esc(brand.split(' ')[0])}<i>+</i>STORE</a><p>Không gian mua sắm hiện đại dành cho shop nhỏ và thương hiệu online.</p></div><div><b>Mua sắm</b><a href="#catalog">Sản phẩm mới</a><a href="#catalog">Bán chạy</a><a href="#catalog">Ưu đãi</a></div><div><b>Hỗ trợ</b><a href="#">Hướng dẫn mua hàng</a><a href="#">Đổi trả</a><a href="#">Vận chuyển</a></div><div><b>Liên hệ</b><p>${esc(site.phone||'0900 000 000')}</p><p>${esc(site.email||site.contact_email||'hello@store.vn')}</p></div></div></footer><div class="nv-cart-toast" id="nvToast">Đã thêm sản phẩm vào giỏ hàng ✓</div></div>`;
 let cart=0,active='Tất cả',term='';const grid=document.getElementById('nvGrid');
 const redraw=()=>{const arr=products.filter(x=>(active==='Tất cả'||x.cat===active)&&(!term||x.name.toLowerCase().includes(term))).slice(0,8);grid.innerHTML=arr.length?arr.map(card).join(''):'<p>Không tìm thấy sản phẩm phù hợp.</p>';bindCards()};
 const bindCards=()=>document.querySelectorAll('[data-add]').forEach(b=>b.onclick=()=>{cart++;document.getElementById('nvCartCount').textContent=cart;const t=document.getElementById('nvToast');t.classList.add('show');setTimeout(()=>t.classList.remove('show'),1600)});
 document.querySelectorAll('[data-filter]').forEach(b=>b.onclick=()=>{document.querySelectorAll('[data-filter]').forEach(x=>x.classList.remove('active'));b.classList.add('active');active=b.dataset.filter;redraw()});
 document.getElementById('nvSearch').onsubmit=e=>{e.preventDefault();term=document.getElementById('nvSearchInput').value.trim().toLowerCase();redraw()};bindCards();
}
