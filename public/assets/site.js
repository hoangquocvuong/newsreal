...
async function renderCommerceStore(site={}){
 ...
 const products=raw.length?raw.map((x,i)=>({id:String(x.id||i),slug:x.slug||String(x.id||i),name:x.name||x.title||'Sản phẩm',cat:x.category_name||x.category||'Khác',price:Number(x.sale_price||x.price||0),old:Number(x.compare_at_price||x.compare_price||x.old_price||0),img:x.cover_image||x.image_url||x.image||getImages(x)[0]||fallback[i%fallback.length].img,images:Array.isArray(x.images)?x.images:[],badge:x.badge||'',stock:Number(x.stock??x.stock_quantity??10),desc:x.description||x.short_description||fallback[0].desc})):fallback;
 ...
 function detail(id){const x=get(id),box=document.getElementById('v24Detail');if(!x){box.classList.remove('open');return}const related=products.filter(p=>p.cat===x.cat&&String(p.id)!==String(x.id)).slice(0,4);document.getElementById('v24DetailBody').innerHTML=`<div class="v24-wrap v24-detailgrid"><div><img class="v24-detailimg" src="${esc(x.img)}" alt="${esc(x.name)}"></div><div><small>${esc(x.cat)}</small><h1>${esc(x.name)}</h1><div class="v24-detailprice"><b>${money(x.price)}</b>${x.old>x.price?`<del>${money(x.old)}</del>`:''}</div><span class="v24-stock">✓ ${x.stock>0?'Còn hàng':'Liên hệ cửa hàng'}</span><p class="v24-desc">${esc(x.desc)}</p><div class="v24-buy"><button class="buy" data-detail-add="${esc(x.id)}">Thêm vào giỏ hàng</button><button data-detail-buy="${esc(x.id)}">Mua ngay</button></div><button class="v24-tool" data-wish="${esc(x.id)}">♡ Lưu yêu thích</button></div></div><div class="v24-wrap v24-related"><h2>Sản phẩm liên quan</h2><div class="v24-grid">${related.map(card).join('')||'<p>Chưa có sản phẩm liên quan.</p>'}</div></div>`;box.classList.add('open');document.querySelector('[data-detail-add]').onclick=()=>{add(x.id);openCart()};document.querySelector('[data-detail-buy]').onclick=()=>{add(x.id);openCart();checkout()};bindDynamic()}
