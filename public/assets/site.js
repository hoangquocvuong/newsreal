...
function nrGameDetailSidebar(post,copy){
 ...
 return `<div class="coc-access-card coc-access-card-rich"><small>BASE LINK</small><h3>${esc(level)} · ${esc(purpose)}</h3><p class="coc-side-intro">Mở layout trực tiếp trong Clash of Clans. Kiểm tra đúng cấp nhà trước khi sao chép.</p><a href="${esc(copy)}" data-coc-download="${esc(nrGameBaseSlug(post))}" target="_blank" rel="noopener">Copy Base Link</a></div><section class="coc-side-info"><div class="coc-side-box-title"><small>BASE INFORMATION</small><strong>Thông tin layout</strong></div>${row('Group',group)}${row('Level',level)}${row(thirdLabel,purpose)}${row('Year',year)}${row('Style',style)}${row('Defense',defense)}</section><section class="coc-side-guide"><div class="coc-side-box-title"><small>QUICK GUIDE</small><strong>Cách sử dụng</strong></div><ol><li>Nhấn <b>Copy Base Link</b>.</li><li>Cho phép mở Clash of Clans.</li><li>Chọn một Layout Slot trống.</li><li>Kiểm tra lại layout rồi lưu.</li></ol><p>Base được chia sẻ miễn phí cho cộng đồng. Bạn có thể dùng <b>Saved</b> để đánh dấu và xem lại sau.</p></section>`
}
