...
function startLeadTitleAlert(count,label='YÊU CẦU MỚI'){
 __leadAlertActive=true;
 clearInterval(__leadTitleTimer);
 let on=false;
 __leadTitleTimer=setInterval(()=>{
  on=!on;
  document.title=on?`🔔 ${count} ${label}!`:`NEWSREAL CONTROL`;
 },900);
}
...
// V20.9.26.5 — HoangVuongTech sales consultation chat (Master Control only)
let __salesChats=[],__salesChatActive=0,__salesChatTimer=null;
const scEsc=s=>String(s??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));
const scDate=v=>{try{return new Date(String(v).replace(' ','T')+'Z').toLocaleString('vi-VN',{timeZone:'Asia/Ho_Chi_Minh'})}catch{return v||''}};
function renderSalesChatList(){const host=document.getElementById('salesChatList');if(!host)return;const unread=__salesChats.reduce((n,x)=>n+Number(x.unread_master||0),0);const ub=document.getElementById('salesChatUnread');if(ub)ub.textContent=String(unread);host.innerHTML=__salesChats.length?__salesChats.map(x=>`<div class="sales-chat-item ${Number(x.id)===__salesChatActive?'active':''}" data-id="${x.id}"><b><span>${scEsc(x.customer_name||'Khách hàng')}</span>${Number(x.unread_master||0)?`<i class="unread">${x.unread_master}</i>`:''}</b><small>${scEsc(x.phone||'Chưa có SĐT')} · ${scEsc(x.status||'new')}</small>${x.template_name?`<span>Đang xem: ${scEsc(x.template_name)}</span>`:''}<span>${scEsc((x.last_message||'').slice(0,85))}</span><small>${scDate(x.last_message_at)}</small></div>`).join(''):'<div class="empty-state">Chưa có khách nhắn tư vấn.</div>';host.querySelectorAll('[data-id]').forEach(el=>el.onclick=()=>openSalesChat(Number(el.dataset.id)))}
async function loadSalesChats({silent=false}={}){try{const d=await mapi('sales-chats');const oldUnread=__salesChats.reduce((n,x)=>n+Number(x.unread_master||0),0);__salesChats=d.chats||[];renderSalesChatList();const nowUnread=__salesChats.reduce((n,x)=>n+Number(x.unread_master||0),0);if(nowUnread>oldUnread&&oldUnread>=0){startLeadTitleAlert(nowUnread,'TIN NHẮN CHAT MỚI');const tools=document.getElementById('salesChatTools');tools?.classList.add('sales-chat-attention');setTimeout(()=>tools?.classList.remove('sales-chat-attention'),7000)}if(__salesChatActive){const active=__salesChats.find(x=>Number(x.id)===__salesChatActive);const ta=document.querySelector('#salesChatThread .sales-chat-reply textarea');const isTyping=!!ta&&(document.activeElement===ta||ta.value.trim().length>0);if(Number(active?.unread_master||0)>0&&!isTyping)await openSalesChat(__salesChatActive,true)}}catch(e){if(!silent){const h=document.getElementById('salesChatList');if(h)h.innerHTML='<div class="empty-state">Không tải được chat: '+scEsc(e.message)+'</div>'}}}
async function openSalesChat(id,silent=false){__salesChatActive=id;renderSalesChatList();const host=document.getElementById('salesChatThread');if(!host)return;const oldTa=host.querySelector('.sales-chat-reply textarea');const draft=oldTa?.value||'';const hadFocus=document.activeElement===oldTa;try{const d=await mapi('sales-chat-thread?id='+id),t=d.thread||{},list=d.messages||[];host.innerHTML=`<div class="sales-chat-thread-head"><h3>${scEsc(t.customer_name||'Khách hàng')}</h3><small>${scEsc(t.phone||'Chưa có SĐT')}${t.email?' · '+scEsc(t.email):''}</small>${t.template_name?`<small style="display:block">Template: <b>${scEsc(t.template_name)}</b></small>`:''}<small style="display:block">Nguồn: ${scEsc(t.source_title||t.source_url||'HoangVuongTech')}</small><div class="sales-chat-thread-actions"><button class="smallbtn sc-status" data-status="contacted">Đang tư vấn</button><button class="smallbtn sc-status" data-status="won">Đã chốt</button><button class="smallbtn sc-status" data-status="lost">Không phù hợp</button><button class="smallbtn sc-status" data-status="closed">Kết thúc</button></div></div><div class="sales-chat-messages">${list.map(x=>`<div class="sales-chat-msg ${x.sender==='master'?'master':'customer'}">${scEsc(x.message)}<small>${scDate(x.created_at)}</small></div>`).join('')}</div><form class="sales-chat-reply"><textarea rows="2" placeholder="Nhập câu trả lời cho khách..."></textarea><button>Gửi</button></form>`;const box=host.querySelector('.sales-chat-messages');box.scrollTop=box.scrollHeight;const replyTa=host.querySelector('.sales-chat-reply textarea');if(draft&&replyTa)replyTa.value=draft;if(hadFocus&&replyTa){replyTa.focus();replyTa.setSelectionRange(replyTa.value.length,replyTa.value.length)}host.querySelectorAll('.sc-status').forEach(b=>b.onclick=async()=>{await mapi('sales-chat-update',{method:'POST',body:JSON.stringify({id,status:b.dataset.status})});await loadSalesChats({silent:true})});host.querySelector('form').onsubmit=async e=>{e.preventDefault();const ta=e.currentTarget.querySelector('textarea'),message=ta.value.trim();if(!message)return;const btn=e.currentTarget.querySelector('button');btn.disabled=true;try{await mapi('sales-chat-reply',{method:'POST',body:JSON.stringify({id,message})});ta.value='';await openSalesChat(id,true);await loadSalesChats({silent:true})}catch(err){alert(err.message)}finally{btn.disabled=false}};const row=__salesChats.find(x=>Number(x.id)===id);if(row)row.unread_master=0;renderSalesChatList()}catch(e){if(!silent)host.innerHTML='<div class="empty-state">Không mở được cuộc trò chuyện: '+scEsc(e.message)+'</div>'}}
document.getElementById('refreshSalesChats')?.addEventListener('click',()=>loadSalesChats());
document.addEventListener('newsreal:master-ready',()=>{loadSalesChats({silent:true});clearInterval(__salesChatTimer);__salesChatTimer=setInterval(()=>loadSalesChats({silent:true}),5000)});
setTimeout(()=>{if(!masterDashboard?.classList.contains('hidden'))loadSalesChats({silent:true})},1900);
