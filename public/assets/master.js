...
function tmNormalizeStructureDraft(raw,contentType='generic'){
 ...
 p.version=Math.max(5,Number(p.version||5));p.content_type=p.content_type||contentType;p.geometry_locked=Number(p.geometry_locked||0)?1:0;p.sidebars=Array.isArray(p.sidebars)?p.sidebars:[];p.sections=Array.isArray(p.sections)?p.sections:[];
 ...
 p.sections=p.sections.map((x,i)=>{const type=String(x?.type||'section'),d=tmStructureDefaults(type),slots=Math.max(0,Number(x?.slots||x?.limit||0)),dc=Math.max(1,Number(x?.desktop_columns||x?.columns||1));return {...x,key:String(x?.key||`section-${i+1}`),type,slots,desktop_columns:dc,tablet_columns:Math.max(1,Number(x?.tablet_columns||Math.min(2,dc))),mobile_columns:Math.max(1,Number(x?.mobile_columns||1)),fill_policy:x?.fill_policy||(d.bind_required?'complete_rows':'natural'),column_mode:['computed','fixed'].includes(String(x?.column_mode||''))?String(x.column_mode):'computed',desktop_rows:Math.max(0,Number(x?.desktop_rows||0)),content_source:x?.content_source||d.content_source,bind_required:x?.bind_required===false||Number(x?.bind_required)===0?0:d.bind_required,empty_policy:x?.empty_policy||(d.bind_required?'slots':'message')}});return p;
}
