...
if(route==='commerce/categories'){
 ...
 if(request.method==='DELETE'){await env.DB.prepare(`DELETE FROM commerce_categories WHERE id=? AND site_id=?`).bind(Number(u.searchParams.get('id')||0),site.id).run();return json({ok:true})}
}
