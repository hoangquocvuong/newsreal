...
if(route==='commerce/products'){
 await commerceSeedFromSamples();if(request.method==='GET'){if(!user)return json({error:'Chưa đăng nhập'},401);const {results=[]}=await env.DB.prepare(`SELECT p.*,c.name category_name FROM commerce_products p LEFT JOIN commerce_categories c ON c.id=p.category_id AND c.site_id=p.site_id WHERE p.site_id=? ORDER BY p.id DESC`).bind(site.id).all();return json({ok:true,products:results})}if(!user)return json({error:'Chưa đăng nhập'},401);const b=await body(request);if(request.method==='POST'){const name=String(b.name||'').trim();if(!name)return json({error:'Thiếu tên sản phẩm'},400);const categoryId=Number(b.category_id)||null;if(categoryId){const ownedCategory=await env.DB.prepare(`SELECT id FROM commerce_categories WHERE id=? AND site_id=? LIMIT 1`).bind(categoryId,site.id).first();if(!ownedCategory)return json({error:'Danh mục không thuộc website này'},400)}const r=await env.DB.prepare(`INSERT INTO commerce_products(site_id,category_id,name,slug,sku,short_description,description,image_url,gallery_json,price,compare_at_price,stock_qty,stock_status,attributes_json,variants_json,badge,is_featured,is_active) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).bind(site.id,categoryId,name,commerceSlug(b.slug||name+'-'+Date.now()),String(b.sku||''),String(b.short_description||''),String(b.description||''),String(b.image_url||''),JSON.stringify(b.gallery||[]),commerceMoney(b.price),commerceMoney(b.compare_at_price),Math.max(0,Number(b.stock_qty||0)),String(b.stock_status||'in_stock'),JSON.stringify(b.attributes||{}),JSON.stringify(commerceCleanVariants(b.variants)),String(b.badge||''),b.is_featured?1:0,b.is_active===false?0:1).run();return json({ok:true,id:r.meta.last_row_id})}if(request.method==='PUT'){const id=Number(b.id||0),categoryId=Number(b.category_id)||null;if(categoryId){const ownedCategory=await env.DB.prepare(`SELECT id FROM commerce_categories WHERE id=? AND site_id=? LIMIT 1`).bind(categoryId,site.id).first();if(!ownedCategory)return json({error:'Danh mục không thuộc website này'},400)}await env.DB.prepare(`UPDATE commerce_products SET category_id=?,name=?,sku=?,short_description=?,description=?,image_url=?,price=?,compare_at_price=?,stock_qty=?,stock_status=?,attributes_json=?,variants_json=?,badge=?,is_featured=?,is_active=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND site_id=?`).bind(categoryId,String(b.name||''),String(b.sku||''),String(b.short_description||''),String(b.description||''),String(b.image_url||''),commerceMoney(b.price),commerceMoney(b.compare_at_price),Math.max(0,Number(b.stock_qty||0)),String(b.stock_status||'in_stock'),JSON.stringify(b.attributes||{}),JSON.stringify(commerceCleanVariants(b.variants)),String(b.badge||''),b.is_featured?1:0,b.is_active===false?0:1,id,site.id).run();return json({ok:true})}if(request.method==='DELETE'){const id=Number(u.searchParams.get('id')||0);await env.DB.prepare(`DELETE FROM commerce_products WHERE id=? AND site_id=?`).bind(id,site.id).run();return json({ok:true})}
}
if(route==='commerce/categories'){
 await commerceEnsure();if(!user)return json({error:'Chưa đăng nhập'},401);if(request.method==='GET'){const {results=[]}=await env.DB.prepare(`SELECT * FROM commerce_categories WHERE site_id=? ORDER BY sort_order,id`).bind(site.id).all();return json({ok:true,categories:results})}const b=await body(request);
