...
if(route==='commerce/categories'){
 if(!user)return json({error:'Chưa đăng nhập'},401);if(request.method==='GET'){try{await commerceEnsureAdminCategories();const {results=[]}=await env.DB.prepare(`SELECT c.*,(SELECT COUNT(*) FROM commerce_products p WHERE p.site_id=c.site_id AND p.category_id=c.id) product_count FROM commerce_categories c WHERE c.site_id=? AND c.is_active=1 ORDER BY c.sort_order,c.id`).bind(site.id).all();return json({ok:true,categories:results})}catch(e){return json({ok:true,categories:[],fallback:true})}}await commerceEnsureAdminCategories();const b=await body(request);
 ...
 if(request.method==='DELETE'){const id=Number(u.searchParams.get('id')||0),targetId=Number(u.searchParams.get('target_id')||0);if(!id)return json({error:'Thiếu danh mục cần xóa'},400);const count=Number((await env.DB.prepare(`SELECT COUNT(*) n FROM commerce_products WHERE site_id=? AND category_id=?`).bind(site.id,id).first())?.n||0);if(count>0){if(!targetId||targetId===id)return json({error:'Danh mục đang có sản phẩm',code:'CATEGORY_NOT_EMPTY',product_count:count},409);const target=await env.DB.prepare(`SELECT id FROM commerce_categories WHERE site_id=? AND id=? AND is_active=1`).bind(site.id,targetId).first();if(!target)return json({error:'Danh mục chuyển đến không hợp lệ'},400);await env.DB.prepare(`UPDATE commerce_products SET category_id=?,updated_at=CURRENT_TIMESTAMP WHERE site_id=? AND category_id=?`).bind(targetId,site.id,id).run()}await env.DB.prepare(`DELETE FROM commerce_categories WHERE id=? AND site_id=?`).bind(id,site.id).run();return json({ok:true,moved_products:count})}
}
