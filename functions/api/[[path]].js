...
function defaultTemplateStructure(key){
 ...
 const proSettings=[{key:'hero_image',label:'Ảnh Hero trang chủ',type:'image',default:'',help:'Ảnh lớn ở đầu trang. Tải ảnh JPG, PNG hoặc WEBP tối đa 8 MB.'},{key:'hero_title',label:'Tiêu đề Hero',type:'text',default:''},{key:'hero_description',label:'Mô tả Hero',type:'textarea',default:''},{key:'hero_cta_primary',label:'Nhãn nút Hero chính',type:'text',default:''},{key:'hero_cta_secondary',label:'Nhãn nút Hero phụ',type:'text',default:''},{key:'footer_intro',label:'Giới thiệu ngắn ở Footer',type:'textarea',default:'Thông tin ngắn về doanh nghiệp hoặc thương hiệu.'},{key:'footer_address',label:'Địa chỉ hiển thị ở Footer',type:'text',default:''},{key:'footer_phone',label:'Hotline Footer',type:'text',default:''},{key:'footer_email',label:'Email Footer',type:'text',default:''},{key:'footer_facebook',label:'Facebook Footer',type:'url',default:''},{key:'footer_zalo',label:'Zalo Footer',type:'text',default:''},{key:'privacy_url',label:'Link Chính sách bảo mật',type:'url',default:'#'},{key:'terms_url',label:'Link Điều khoản sử dụng',type:'url',default:'#'},{key:'footer_copyright',label:'Dòng bản quyền',type:'text',default:'© 2026. Bản quyền thuộc doanh nghiệp.'},{key:'contact_title',label:'Tiêu đề khối liên hệ',type:'text',default:'Liên hệ với chúng tôi'},{key:'contact_intro',label:'Mô tả khối liên hệ',type:'textarea',default:'Để lại thông tin hoặc liên hệ trực tiếp để được hỗ trợ.'},{key:'contact_address',label:'Địa chỉ',type:'text',default:''},{key:'working_hours',label:'Giờ làm việc',type:'text',default:'Thứ 2 – Thứ 7 · 08:00 – 18:00'},{key:'map_url',label:'Link bản đồ',type:'url',placeholder:'https://maps.google.com/...'},{key:'contact_cta',label:'Nhãn nút gửi liên hệ',type:'text',default:'Gửi yêu cầu'},{key:'hero_note',label:'Dòng giới thiệu ngắn ở Hero',type:'textarea',default:''}];
 const lionSettings=[...proSettings,{key:'troupe_intro',label:'Giới thiệu về đoàn lân',type:'textarea',default:'Đoàn Lân Sư Rồng chuyên biểu diễn khai trương, lễ hội và sự kiện.'},{key:'experience_years',label:'Kinh nghiệm / số năm hoạt động',type:'text',default:'15+ năm'},{key:'event_count',label:'Số sự kiện đã thực hiện',type:'text',default:'800+ sự kiện'},{key:'member_count',label:'Quy mô thành viên',type:'text',default:'35+ thành viên'},{key:'award_1',label:'Danh hiệu / thành tích 1',type:'text',default:''},{key:'award_2',label:'Danh hiệu / thành tích 2',type:'text',default:''},{key:'award_3',label:'Danh hiệu / thành tích 3',type:'text',default:''},{key:'service_area_note',label:'Khu vực nhận biểu diễn',type:'text',default:'Hải Phòng và các tỉnh lân cận'}];
 const laptopSettings=[...proSettings,{key:'store_hotline',label:'Hotline tư vấn laptop',type:'text',default:'0900 000 000'},{key:'store_zalo',label:'Zalo tư vấn',type:'text',default:'0900 000 000'},{key:'store_email',label:'Email cửa hàng',type:'email',default:'laptop@example.com'}];
 const p={
...
,
  ...
  'san-pham-1':{version:1,layout_contract:'universal-layout-v1',content_type:'product',geometry_locked:1,route_contract:'product-catalog-v1',card_contract:'product-title-price-v1',article_contract:'product-detail-v1',sidebars:[],settings_schema:[{key:'product_cta_label',label:'Nhãn nút mua hàng',type:'text',default:'Mua Ngay'},{key:'product_affiliate_note',label:'Ghi chú affiliate',type:'textarea',default:'Website có thể nhận hoa hồng khi người xem mua hàng qua liên kết giới thiệu.'}],sections:[sec('hero','section','Product Store',{content_source:'none',bind_required:0}),sec('categories','section','Danh mục sản phẩm',{content_source:'none',bind_required:0}),sec('featured','category','Sản phẩm nổi bật',{slots:10,desktop_columns:5,tablet_columns:3,mobile_columns:2,fill_policy:'complete_rows',bind_required:1}),sec('electronics','category','Điện tử & Công nghệ',{category:'Điện tử & Công nghệ',slots:5,desktop_columns:5,tablet_columns:3,mobile_columns:2,fill_policy:'complete_rows',bind_required:1}),sec('home','category','Nhà cửa & Đời sống',{category:'Nhà cửa & Đời sống',slots:5,desktop_columns:5,tablet_columns:3,mobile_columns:2,fill_policy:'complete_rows',bind_required:1}),sec('beauty','category','Thời trang & Làm đẹp',{category:'Thời trang & Làm đẹp',slots:5,desktop_columns:5,tablet_columns:3,mobile_columns:2,fill_policy:'complete_rows',bind_required:1}),sec('baby','category','Mẹ & Bé',{category:'Mẹ & Bé',slots:5,desktop_columns:5,tablet_columns:3,mobile_columns:2,fill_policy:'complete_rows',bind_required:1})]},
  'blog-ca-nhan-1':{version:7,layout_contract:'universal-layout-v1',content_type:'news',geometry_locked:1,settings_schema:proSettings,sections:[sec('hero','section','Giới thiệu tác giả',{content_source:'none',bind_required:0}),sec('popular','latest','Đang được đọc nhiều',{slots:6,desktop_columns:3,tablet_columns:2,mobile_columns:1,bind_required:1}),cat(1,'Thương hiệu cá nhân',{slots:6,desktop_columns:3}),cat(2,'Freelance',{slots:6,desktop_columns:3}),cat(3,'Năng suất',{slots:6,desktop_columns:3}),cat(4,'Sách & công cụ',{slots:6,desktop_columns:3}),cat(5,'Lối sống sáng tạo',{slots:6,desktop_columns:3}),cat(6,'Góc nhìn',{slots:6,desktop_columns:3}),sec('contact','section','Liên hệ',{content_source:'none',bind_required:0})]},
  'blog-ca-nhan-2':{version:7,layout_contract:'universal-layout-v1',content_type:'news',geometry_locked:1,settings_schema:proSettings,sections:[sec('hero','section','Giới thiệu người sáng tạo',{content_source:'none',bind_required:0}),sec('stories','latest','Bài nổi bật & câu chuyện dự án',{slots:8,desktop_columns:4,tablet_columns:2,mobile_columns:1,bind_required:1}),cat(1,'Kinh doanh nội dung',{slots:6,desktop_columns:3}),cat(2,'Video & sản xuất',{slots:6,desktop_columns:3}),cat(3,'Chiến lược nội dung',{slots:6,desktop_columns:3}),cat(4,'Câu chuyện dự án',{slots:6,desktop_columns:3}),cat(5,'Công cụ',{slots:6,desktop_columns:3}),cat(6,'Kiếm tiền nội dung',{slots:6,desktop_columns:3}),sec('contact','section','Liên hệ hợp tác',{content_source:'none',bind_required:0})]},
  'doanh-nghiep-1':{version:8,layout_contract:'universal-layout-v1',content_type:'news',geometry_locked:1,settings_schema:proSettings,sections:[sec('hero','section','Giới thiệu doanh nghiệp',{content_source:'none',bind_required:0}),sec('services','services','Giải pháp',{content_source:'none',bind_required:0}),sec('case','section','Dự án tiêu biểu',{content_source:'none',bind_required:0}),cat(1,'Chuyển đổi số',{slots:6,desktop_columns:3}),cat(2,'Vận hành',{slots:6,desktop_columns:3}),cat(3,'Dữ liệu & phân tích',{slots:6,desktop_columns:3}),cat(4,'Khách hàng',{slots:6,desktop_columns:3}),cat(5,'Nhân sự',{slots:6,desktop_columns:3}),cat(6,'Tin doanh nghiệp',{slots:6,desktop_columns:3}),sec('contact','section','Liên hệ doanh nghiệp',{content_source:'none',bind_required:0})]},
  'doanh-nghiep-2':{version:8,layout_contract:'universal-layout-v1',content_type:'news',geometry_locked:1,settings_schema:proSettings,sections:[sec('hero','section','Hồ sơ năng lực',{content_source:'none',bind_required:0}),sec('capabilities','services','Năng lực kỹ thuật',{content_source:'none',bind_required:0}),sec('projects','section','Dự án',{content_source:'none',bind_required:0}),cat(1,'Sản xuất',{slots:6,desktop_columns:3}),cat(2,'Cơ khí & CNC',{slots:6,desktop_columns:3}),cat(3,'Quản lý chất lượng',{slots:6,desktop_columns:3}),cat(4,'An toàn lao động',{slots:6,desktop_columns:3}),cat(5,'Dự án',{slots:6,desktop_columns:3}),cat(6,'Bảo trì',{slots:6,desktop_columns:3}),sec('contact','section','Nhận báo giá',{content_source:'none',bind_required:0})]},
  'dich-vu-5':{version:11,layout_contract:'universal-layout-v1',content_type:'commerce',geometry_locked:1,settings_schema:laptopSettings,lead_contract:'service-lead-v1',route_contract:'commerce-v1',card_contract:'commerce-product-card-v1',commerce_contract:'universal-commerce-v1',article_contract:'service-detail-v2',sections:[sec('hero','section','Cửa hàng Online',{content_source:'none',bind_required:0}),sec('products','section','Sản phẩm nổi bật',{content_source:'none',bind_required:0}),cat(1,'Công nghệ',{slots:6,desktop_columns:3}),cat(2,'Thời trang',{slots:6,desktop_columns:3}),cat(3,'Mỹ phẩm',{slots:6,desktop_columns:3}),cat(4,'Gia dụng',{slots:6,desktop_columns:3}),cat(5,'Nội thất',{slots:6,desktop_columns:3}),cat(6,'Cẩm nang mua sắm',{slots:6,desktop_columns:3}),sec('contact','section','Hỗ trợ khách hàng',{content_source:'none',bind_required:0})]},
  'dich-vu-6':{version:1,layout_contract:'universal-layout-v1',content_type:'service',geometry_locked:1,settings_schema:lionSettings,lead_contract:'service-lead-v1',gallery_contract:'multi-image-slider-v1',pricing_contract:'service-package-detail-v1',sections:[sec('hero','section','Lân Sư Rồng',{content_source:'none',bind_required:0}),sec('packages','category','Gói múa lân',{category:'Gói múa lân',slots:6,desktop_columns:3,tablet_columns:2,mobile_columns:1,bind_required:1}),cat(2,'Múa rồng',{slots:3,desktop_columns:3}),cat(3,'Trống hội',{slots:3,desktop_columns:3}),sec('trust','section','Kinh nghiệm & danh hiệu',{content_source:'none',bind_required:0}),cat(4,'Sự kiện đã thực hiện',{slots:6,desktop_columns:3}),cat(5,'Tin hoạt động',{slots:6,desktop_columns:3}),cat(6,'Kiến thức & phong tục',{slots:6,desktop_columns:3}),sec('contact','section','Liên hệ báo giá',{content_source:'none',bind_required:0})]},
...
async function ensureTemplateCatalog(env){
  ...
  const seeds=[
    ...
    {key:'blog-ca-nhan-1',name:'Blog cá nhân · Nhật ký tối giản',category:'blog-ca-nhan',preset:'personal_blog_1',price:1499000,renewal:1999000,sort:1,image:'https://images.unsplash.com/photo-1499750310107-5fef28a66643?auto=format&fit=crop&w=1200&q=82',demo:'/demo/blog-ca-nhan/mau-1/',badge:'BLOG CÁ NHÂN',description:'Mẫu blog cá nhân tối giản cho blogger, chuyên gia và freelancer.',features:'Hero giới thiệu cá nhân\nBài viết nổi bật\nTrang giới thiệu / liên hệ\nTối ưu SEO nội dung',accent:'violet'},
    {key:'blog-ca-nhan-2',name:'Blog cá nhân · Góc người sáng tạo',category:'blog-ca-nhan',preset:'personal_blog_2',price:1499000,renewal:1999000,sort:2,image:'https://images.unsplash.com/photo-1542744173-8e7e53415bb0?auto=format&fit=crop&w=1200&q=82',demo:'/demo/blog-ca-nhan/mau-2/',badge:'CREATOR',description:'Website creator kết hợp blog, portfolio và newsletter.',features:'Blog + portfolio\nNewsletter\nChuyên mục nội dung\nCTA hợp tác',accent:'orange'},
    {key:'doanh-nghiep-1',name:'Doanh nghiệp · Giải pháp hiện đại',category:'doanh-nghiep',preset:'corporate_modern_1',price:1699000,renewal:2199000,sort:1,image:'https://images.unsplash.com/photo-1521737711867-e3b97375f902?auto=format&fit=crop&w=1200&q=82',demo:'/demo/doanh-nghiep/mau-1/',badge:'DOANH NGHIỆP',description:'Website công ty hiện đại với dịch vụ, năng lực, dự án và CTA tư vấn.',features:'Giới thiệu công ty\nDịch vụ & năng lực\nDự án\nForm tư vấn',accent:'blue'},
    {key:'doanh-nghiep-2',name:'Doanh nghiệp · Hồ sơ năng lực chuyên nghiệp',category:'doanh-nghiep',preset:'corporate_industry_2',price:1799000,renewal:2299000,sort:2,image:'https://images.unsplash.com/photo-1497366216548-37526070297c?auto=format&fit=crop&w=1200&q=82',demo:'/demo/doanh-nghiep/mau-2/',badge:'HỒ SƠ NĂNG LỰC',description:'Website doanh nghiệp cao cấp với hồ sơ năng lực, dịch vụ, dự án, tin tức và tuyển dụng.',features:'Hồ sơ năng lực\nSản phẩm & dịch vụ\nDự án tiêu biểu\nTin tức & tuyển dụng',accent:'slate'},
    {key:'dich-vu-5',name:'Cửa hàng Online · E-commerce',category:'ban-hang',preset:'universal_commerce_5',price:5000000,renewal:5000000,sort:5,image:'https://images.unsplash.com/photo-1496181133206-80ce9b88a853?auto=format&fit=crop&w=1200&q=86',demo:'/demo/ban-hang/cua-hang-online/',badge:'E-COMMERCE',description:'Website bán hàng tổng quát với sản phẩm, giỏ hàng, checkout, thanh toán và quản lý đơn.',features:'Catalog đa ngành\nSearch, filter, cart & checkout\nThanh toán & vận chuyển\nĐơn hàng, tồn kho & Admin',accent:'blue'},
    {key:'dich-vu-6',name:'Lân Sư Rồng · Dịch vụ biểu diễn cao cấp',category:'dich-vu',preset:'service_lion_dance_6',price:2999000,renewal:2999000,sort:6,image:'/assets/demo/dich-vu-6-preview.png',demo:'/demo/dich-vu/mua-lan-su-rong/',badge:'LÂN SƯ RỒNG',description:'Website quảng bá đoàn Lân Sư Rồng với gói 2–7 đầu lân, múa rồng, trống hội, danh hiệu, sự kiện, album slideshow và form báo giá.',features:'Gói 2–7 đầu lân\nMúa rồng & trống hội\nNăng lực & danh hiệu\nAlbum slideshow + nhận báo giá',accent:'red'},
  ...
  // V20.9.26.1 — Professional template Admin contracts: exact taxonomy + contact settings.
  const professionalEditors={
    ...
    'doanh-nghiep-1':{id:'news',label:'Bài viết doanh nghiệp',content_type:'news',categories:['Giới thiệu & năng lực','Sản phẩm & dịch vụ','Dự án & đối tác','Tin tức & sự kiện','Tuyển dụng','Hoạt động & văn hóa'],contentLabel:'Nội dung bài viết / dự án',contentHelp:'Dùng chuyên mục để đưa bài vào đúng khu vực trên trang chủ doanh nghiệp.',custom_fields:[{key:'company_author',label:'Bộ phận / tác giả',type:'text',placeholder:'Phòng Marketing'},{key:'project_client',label:'Khách hàng / dự án',type:'text',placeholder:'Tùy chọn'}]},
    'doanh-nghiep-2':{id:'news',label:'Bài viết kỹ thuật & dự án',content_type:'news',categories:['Giới thiệu & năng lực','Sản phẩm & dịch vụ','Dự án tiêu biểu','Tin tức & sự kiện','Nhân sự & tuyển dụng','Trách nhiệm xã hội'],contentLabel:'Nội dung kỹ thuật / dự án',contentHelp:'Chọn chuyên mục chính xác để bài hiển thị đúng khối năng lực, dự án hoặc chuyên san.',custom_fields:[{key:'project_location',label:'Địa điểm dự án',type:'text'},{key:'project_scope',label:'Phạm vi công việc',type:'text'}]},
    'dich-vu-5':{id:'service',label:'Cẩm nang cửa hàng',content_type:'service',categories:['Cẩm nang mua sắm','Khuyến mãi','Hướng dẫn sử dụng','Tin cửa hàng'],contentLabel:'Bài viết / cẩm nang',contentHelp:'Chọn đúng chuyên mục để sản phẩm hoặc bài tư vấn hiển thị đúng khu vực.',custom_fields:[{key:'service_price',label:'Giá bán / giá tham khảo',type:'text',placeholder:'Ví dụ: 18.990.000đ'},{key:'service_area',label:'Cấu hình / phiên bản',type:'text',placeholder:'Ví dụ: Core Ultra 5 · RAM 16GB · SSD 512GB'},{key:'service_cta',label:'Nhãn nút tư vấn',type:'text',placeholder:'Yêu cầu tư vấn'}]},
    'dich-vu-6':{id:'service',label:'Dịch vụ Lân Sư Rồng',content_type:'service',categories:['Gói múa lân','Múa rồng','Trống hội','Sự kiện đã thực hiện','Tin hoạt động','Kiến thức & phong tục'],contentLabel:'Nội dung gói dịch vụ / bài viết',contentHelp:'Tải nhiều ảnh để tạo album/slideshow. Với gói dịch vụ, nhập rõ quy mô đội hình, nhân sự, đạo cụ, hiệu ứng và giá tham khảo.',custom_fields:[{key:'service_price',label:'Giá gói / Giá tham khảo',type:'text',placeholder:'Từ 3.500.000đ'},{key:'lion_count',label:'Số đầu lân / Rồng',type:'text',placeholder:'2 đầu lân'},{key:'drum_count',label:'Trống & bộ gõ',type:'text',placeholder:'1 trống cái + chập chõa'},{key:'performers_count',label:'Số người tham gia',type:'text',placeholder:'7–9 người'},{key:'performance_duration',label:'Thời lượng',type:'text',placeholder:'20–30 phút'},{key:'fireworks',label:'Pháo sáng',type:'text',placeholder:'Tùy chọn'},{key:'confetti',label:'Pháo kim tuyến',type:'text',placeholder:'Có / Không'},{key:'couplets',label:'Câu đối / liễn',type:'text',placeholder:'01 bộ'},{key:'service_area',label:'Khu vực phục vụ',type:'text',placeholder:'Hải Phòng / Hà Nội / Toàn quốc'},{key:'service_cta',label:'Nhãn CTA',type:'text',placeholder:'Liên hệ báo giá'}]}
  };
  for(const [key,ep] of Object.entries(professionalEditors)){try{await env.DB.prepare(`UPDATE template_catalog SET editor_profile=?,structure_profile=? WHERE template_key=?`).bind(JSON.stringify(ep),JSON.stringify(defaultTemplateStructure(key)),key).run()}catch(e){}}
...
async function buildTemplatePreviewBlueprint(env,templateKey,site={}){
  ...
  if(professional?.articles?.length){
    ...
    const cats=Array.isArray(ep?.categories)&&ep.categories.length?ep.categories:['Điện tử & Công nghệ','Nhà cửa & Đời sống','Thời trang & Làm đẹp','Mẹ & Bé','Thể thao & Du lịch','Phụ kiện & Khác'];
    const names={
    ...
    const sections=Array.isArray(sp?.sections)?sp.sections:[];const needs=new Map();let totalNeed=limit;
    for(const sec of sections){const slots=Math.max(0,Number(sec?.slots||0));if(!slots)continue;const cat=String(sec?.category||'').trim();if(cat)needs.set(cat,Math.max(needs.get(cat)||0,slots));totalNeed=Math.max(totalNeed,slots)}
    posts=[];let id=940000,idx=0;
    const slug=v=>String(v||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/đ/g,'d').replace(/[^a-z0-9]+/g,'-').replace(/^-+|-+$/g,'');
    for(const cat of cats){const pool=names[cat]||['Sản phẩm nổi bật'];const need=Math.max(1,Number(needs.get(cat)||5));for(let i=0;i<need;i++){const title=pool[i%pool.length]+(i>=pool.length?' '+(i+1):'');const price=199000+((idx*173000)%4200000);const old=Math.round(price*1.28/1000)*1000;const image=`/assets/product-affiliate/product-${(idx%6)+1}.svg`;const brand=['Nova','Aster','MobiGear','HomeLab','UrbanX','Kiddo'][idx%6];const extra={product_price:String(price),product_old_price:String(old),product_brand:brand,product_sku:'PA-'+String(idx+1).padStart(4,'0'),product_affiliate_url:'#',product_badge:idx%4===0?'BÁN CHẠY':idx%4===1?'GIẢM GIÁ':'',product_rating:(4.6+(idx%4)*.1).toFixed(1),product_sold:String(230+idx*91),product_promo:'Ưu đãi mẫu dành cho showroom. Chủ website tự chỉnh giá, voucher và link mua trong Trang quản trị.',product_specs:'Thương hiệu: '+brand+'\nBảo hành: 12 tháng\nTình trạng: Mới'};posts.push({id:id++,type:'product',title,category:cat,image,content:`<p>${title} là nội dung mẫu để trình bày catalog thương mại. Chủ website có thể viết review, hướng dẫn chọn mua và nội dung SEO trong Trang quản trị.</p><h2>Điểm nổi bật</h2><p>Giao diện hiển thị hình ảnh, giá, ưu đãi, thông số và liên kết mua hàng theo dữ liệu do chủ website nhập.</p>`,status:'published',featured:idx<10?1:0,verified:1,listing_code:'PRODUCT-'+String(idx+1).padStart(3,'0'),views:320+idx*47,demo_url:`/demo/san-pham/mau-1/san-pham/${slug(title)}.html`,gallery:`/assets/product-affiliate/product-${((idx+1)%6)+1}.svg, /assets/product-affiliate/product-${((idx+2)%6)+1}.svg`,extra_json:JSON.stringify(extra),is_sample:1,sample_key:`san-pham-1:${idx+1}`,__nr_blueprint:1});idx++;}}
    while(posts.length<totalNeed){const x=posts[posts.length%Math.max(1,posts.length)];if(!x)break;posts.push({...x,id:id++,title:x.title+' · Lựa chọn '+(posts.length+1),sample_key:`san-pham-1:${posts.length+1}`})}
...
if(route==='site'&&request.method==='GET'){
 ...
 try{await installDefaultTemplateSamples(env,site.id,{source:__siteTrial?'trial-backfill':'live-handover-backfill'})}catch(e){console.log('sample parity backfill:',e?.message||e)}
...
// V20.9.27.44 — Universal Commerce Foundation. Tenant commerce is isolated from HoangVuongTech template payments.
async function commerceEnsure(){
 const qs=[
 `CREATE TABLE IF NOT EXISTS commerce_categories(id INTEGER PRIMARY KEY AUTOINCREMENT,site_id INTEGER NOT NULL,name TEXT NOT NULL,slug TEXT NOT NULL,description TEXT NOT NULL DEFAULT '',image_url TEXT NOT NULL DEFAULT '',fields_schema_json TEXT NOT NULL DEFAULT '[]',sort_order INTEGER NOT NULL DEFAULT 0,is_active INTEGER NOT NULL DEFAULT 1,created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,UNIQUE(site_id,slug))`,
 `CREATE TABLE IF NOT EXISTS commerce_products(id INTEGER PRIMARY KEY AUTOINCREMENT,site_id INTEGER NOT NULL,category_id INTEGER,name TEXT NOT NULL,slug TEXT NOT NULL,sku TEXT NOT NULL DEFAULT '',short_description TEXT NOT NULL DEFAULT '',description TEXT NOT NULL DEFAULT '',image_url TEXT NOT NULL DEFAULT '',gallery_json TEXT NOT NULL DEFAULT '[]',price INTEGER NOT NULL DEFAULT 0,compare_at_price INTEGER NOT NULL DEFAULT 0,cost_price INTEGER NOT NULL DEFAULT 0,stock_qty INTEGER NOT NULL DEFAULT 0,stock_status TEXT NOT NULL DEFAULT 'in_stock',attributes_json TEXT NOT NULL DEFAULT '{}',variants_json TEXT NOT NULL DEFAULT '[]',badge TEXT NOT NULL DEFAULT '',is_featured INTEGER NOT NULL DEFAULT 0,is_active INTEGER NOT NULL DEFAULT 1,is_sample INTEGER NOT NULL DEFAULT 0,created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,UNIQUE(site_id,slug))`,
 `CREATE TABLE IF NOT EXISTS commerce_orders(id INTEGER PRIMARY KEY AUTOINCREMENT,site_id INTEGER NOT NULL,order_code TEXT NOT NULL UNIQUE,customer_name TEXT NOT NULL,phone TEXT NOT NULL,email TEXT NOT NULL DEFAULT '',address TEXT NOT NULL DEFAULT '',province TEXT NOT NULL DEFAULT '',district TEXT NOT NULL DEFAULT '',ward TEXT NOT NULL DEFAULT '',shipping_method TEXT NOT NULL DEFAULT 'standard',note TEXT NOT NULL DEFAULT '',subtotal INTEGER NOT NULL DEFAULT 0,discount_amount INTEGER NOT NULL DEFAULT 0,shipping_fee INTEGER NOT NULL DEFAULT 0,total_amount INTEGER NOT NULL DEFAULT 0,payment_method TEXT NOT NULL DEFAULT 'cod',payment_status TEXT NOT NULL DEFAULT 'unpaid',order_status TEXT NOT NULL DEFAULT 'new',items_json TEXT NOT NULL DEFAULT '[]',coupon_code TEXT NOT NULL DEFAULT '',created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)`,
 `CREATE TABLE IF NOT EXISTS commerce_coupons(id INTEGER PRIMARY KEY AUTOINCREMENT,site_id INTEGER NOT NULL,code TEXT NOT NULL,discount_type TEXT NOT NULL DEFAULT 'fixed',discount_value INTEGER NOT NULL DEFAULT 0,min_order INTEGER NOT NULL DEFAULT 0,max_uses INTEGER NOT NULL DEFAULT 0,used_count INTEGER NOT NULL DEFAULT 0,starts_at TEXT NOT NULL DEFAULT '',ends_at TEXT NOT NULL DEFAULT '',is_active INTEGER NOT NULL DEFAULT 1,created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,UNIQUE(site_id,code))`,
 `CREATE TABLE IF NOT EXISTS commerce_settings(site_id INTEGER PRIMARY KEY,currency TEXT NOT NULL DEFAULT 'VND',cod_enabled INTEGER NOT NULL DEFAULT 1,bank_enabled INTEGER NOT NULL DEFAULT 1,store_pickup_enabled INTEGER NOT NULL DEFAULT 1,online_enabled INTEGER NOT NULL DEFAULT 0,bank_name TEXT NOT NULL DEFAULT '',bank_account TEXT NOT NULL DEFAULT '',bank_holder TEXT NOT NULL DEFAULT '',bank_qr_url TEXT NOT NULL DEFAULT '',shipping_flat_fee INTEGER NOT NULL DEFAULT 0,free_shipping_from INTEGER NOT NULL DEFAULT 0,updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)`];
 for(const q of qs)try{await env.DB.prepare(q).run()}catch(e){}
 for(const q of [`ALTER TABLE commerce_orders ADD COLUMN ward TEXT NOT NULL DEFAULT ''`,`ALTER TABLE commerce_orders ADD COLUMN shipping_method TEXT NOT NULL DEFAULT 'standard'`,`ALTER TABLE commerce_categories ADD COLUMN fields_schema_json TEXT NOT NULL DEFAULT '[]'`])try{await env.DB.prepare(q).run()}catch(e){}
}
function parseJsonSafe(v,fallback){try{return typeof v==='string'?JSON.parse(v):v??fallback}catch(e){return fallback}}
function commerceSlug(v=''){return String(v||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/đ/g,'d').replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'').slice(0,100)||('sp-'+Date.now())}
function commerceMoney(v){const n=Number(String(v??0).replace(/[^0-9]/g,''));return Number.isFinite(n)?Math.max(0,Math.round(n)):0}
async function commerceSeedFromSamples(){
 await commerceEnsure();
 const standardCats=['Công nghệ','Điện thoại','Thời trang','Mỹ phẩm','Gia dụng','Nội thất','Phụ kiện','Thể thao','Khác'];
 let ci=0;for(const name of standardCats)await env.DB.prepare(`INSERT OR IGNORE INTO commerce_categories(site_id,name,slug,sort_order,is_active) VALUES(?,?,?,?,1)`).bind(site.id,name,commerceSlug(name),ci++).run();
 const c=await env.DB.prepare(`SELECT count(*) n FROM commerce_products WHERE site_id=?`).bind(site.id).first();if(Number(c?.n||0)>0)return;
 const samples=[
 ['Laptop Air Pro 14','Công nghệ',17990000,20490000,'https://images.unsplash.com/photo-1496181133206-80ce9b88a853?auto=format&fit=crop&w=1000&q=84','Bán chạy'],
 ['Điện thoại Nova X','Điện thoại',12990000,14990000,'https://images.unsplash.com/photo-1511707171634-5f897ff02aa9?auto=format&fit=crop&w=1000&q=84','Mới'],
 ['Áo khoác Urban Essential','Thời trang',899000,1099000,'https://images.unsplash.com/photo-1551028719-00167b16eac5?auto=format&fit=crop&w=1000&q=84','Nổi bật'],
 ['Serum dưỡng sáng Premium','Mỹ phẩm',529000,649000,'https://images.unsplash.com/photo-1620916566398-39f1143ab7be?auto=format&fit=crop&w=1000&q=84','Giảm giá'],
 ['Máy pha cà phê Home Barista','Gia dụng',2490000,2890000,'https://images.unsplash.com/photo-1495474472287-4d71bcdd2085?auto=format&fit=crop&w=1000&q=84',''],
 ['Ghế thư giãn Nordic','Nội thất',2890000,3290000,'https://images.unsplash.com/photo-1506439773649-6e0eb8cfb237?auto=format&fit=crop&w=1000&q=84',''],
 ['Tai nghe Wireless ANC','Phụ kiện',2190000,2590000,'https://images.unsplash.com/photo-1505740420928-5e560c06d30e?auto=format&fit=crop&w=1000&q=84','Bán chạy'],
 ['Giày chạy bộ Performance','Thể thao',1590000,1890000,'https://images.unsplash.com/photo-1542291026-7eec264c27ff?auto=format&fit=crop&w=1000&q=84','Mới']
 ];
 let n=0;for(const [name,cat,price,old,img,badge] of samples){const cr=await env.DB.prepare(`SELECT id FROM commerce_categories WHERE site_id=? AND slug=?`).bind(site.id,commerceSlug(cat)).first();await env.DB.prepare(`INSERT OR IGNORE INTO commerce_products(site_id,category_id,name,slug,sku,short_description,description,image_url,price,compare_at_price,stock_qty,stock_status,attributes_json,badge,is_featured,is_active,is_sample) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).bind(site.id,cr?.id||null,name,commerceSlug(name)+'-sample','SAMPLE-'+String(++n).padStart(3,'0'),'Sản phẩm mẫu có thể sửa hoặc xóa trực tiếp trong Admin.','<p>Đây là nội dung mẫu để cửa hàng luôn có giao diện hoàn chỉnh ngay khi khởi tạo.</p>',img,price,old,20,'in_stock','{}',badge,n<=3?1:0,1,1).run()}
}
if(route==='commerce/catalog'&&request.method==='GET'){
 await commerceSeedFromSamples();const q=String(u.searchParams.get('q')||'').trim(),cat=String(u.searchParams.get('category')||'').trim(),sort=String(u.searchParams.get('sort')||'featured');let where=`p.site_id=? AND p.is_active=1`,bind=[site.id];if(q){where+=` AND (p.name LIKE ? OR p.sku LIKE ? OR p.short_description LIKE ?)`;bind.push('%'+q+'%','%'+q+'%','%'+q+'%')}if(cat){where+=` AND c.slug=?`;bind.push(cat)}const order=sort==='price_asc'?'p.is_sample ASC,p.price ASC':sort==='price_desc'?'p.is_sample ASC,p.price DESC':sort==='newest'?'p.is_sample ASC,p.id DESC':'p.is_sample ASC,p.is_featured DESC,p.id DESC';const {results=[]}=await env.DB.prepare(`SELECT p.*,c.name category_name,c.slug category_slug FROM commerce_products p LEFT JOIN commerce_categories c ON c.id=p.category_id WHERE ${where} ORDER BY ${order} LIMIT 120`).bind(...bind).all();const {results:categories=[]}=await env.DB.prepare(`SELECT * FROM commerce_categories WHERE site_id=? AND is_active=1 ORDER BY sort_order,id`).bind(site.id).all();let settings=await env.DB.prepare(`SELECT * FROM commerce_settings WHERE site_id=?`).bind(site.id).first();if(!settings)settings={cod_enabled:1,bank_enabled:1,store_pickup_enabled:1,online_enabled:0,shipping_flat_fee:0,free_shipping_from:0};return json({ok:true,products:results,categories,settings},200,publicCache(20,60));
}
if(route==='commerce/stats'&&request.method==='GET'){
 if(!user)return json({error:'Chưa đăng nhập'},401);await commerceSeedFromSamples();
 const productStats=await env.DB.prepare(`SELECT COUNT(*) total, SUM(CASE WHEN is_active=1 THEN 1 ELSE 0 END) active, SUM(CASE WHEN stock_status='out_of_stock' OR stock_qty<=0 THEN 1 ELSE 0 END) out_of_stock FROM commerce_products WHERE site_id=? AND COALESCE(is_sample,0)=0`).bind(site.id).first()||{};
 const orderStats=await env.DB.prepare(`SELECT COUNT(*) orders, COALESCE(SUM(total_amount),0) revenue FROM commerce_orders WHERE site_id=?`).bind(site.id).first()||{};
 const {results:recent=[]}=await env.DB.prepare(`SELECT p.id,p.name,p.price,p.stock_qty,p.is_active,c.name category_name FROM commerce_products p LEFT JOIN commerce_categories c ON c.id=p.category_id WHERE p.site_id=? AND COALESCE(p.is_sample,0)=0 ORDER BY p.id DESC LIMIT 10`).bind(site.id).all();
 return json({ok:true,total:Number(productStats.total||0),active:Number(productStats.active||0),out_of_stock:Number(productStats.out_of_stock||0),orders:Number(orderStats.orders||0),revenue:Number(orderStats.revenue||0),recent});
}
if(route==='commerce/checkout'&&request.method==='POST'){
 await commerceEnsure();const b=await body(request),name=String(b.customer_name||'').trim(),phone=String(b.phone||'').trim(),items=Array.isArray(b.items)?b.items:[];if(!name||!phone||!items.length)return json({error:'Vui lòng nhập họ tên, số điện thoại và chọn sản phẩm'},400);const ids=[...new Set(items.map(x=>Number(x.product_id||0)).filter(Boolean))];if(!ids.length)return json({error:'Giỏ hàng không hợp lệ'},400);const marks=ids.map(()=>'?').join(',');const {results=[]}=await env.DB.prepare(`SELECT id,name,sku,price,stock_qty,stock_status,is_active,variants_json FROM commerce_products WHERE site_id=? AND id IN (${marks})`).bind(site.id,...ids).all();const map=new Map(results.map(x=>[Number(x.id),x]));let subtotal=0,clean=[];for(const it of items){const p=map.get(Number(it.product_id));const qty=Math.max(1,Math.min(99,Number(it.qty||1)));if(!p||!Number(p.is_active)||p.stock_status==='out_of_stock'||Number(p.stock_qty)<qty)return json({error:`${p?.name||'Sản phẩm'} không đủ hàng`},409);let unitPrice=Number(p.price),variant=null;const vk=String(it.variant_key||'');if(vk){let vs=[];try{vs=JSON.parse(p.variants_json||'[]')}catch(e){};variant=vs.find(v=>String(v.sku||v.name||v.label||'')===vk);if(!variant)return json({error:`Biến thể của ${p.name} không còn tồn tại`},409);const vst=Number(variant.stock_qty??variant.stock??0);if(vst<qty)return json({error:`${p.name} · ${variant.name||variant.label||vk} không đủ hàng`},409);unitPrice=commerceMoney(variant.price||p.price)}subtotal+=unitPrice*qty;clean.push({product_id:p.id,name:p.name,price:unitPrice,qty,variant_key:vk,variant_name:variant?.name||variant?.label||'',sku:variant?.sku||p.sku||''})}let settings=await env.DB.prepare(`SELECT * FROM commerce_settings WHERE site_id=?`).bind(site.id).first()||{cod_enabled:1,bank_enabled:1,store_pickup_enabled:1,shipping_flat_fee:0,free_shipping_from:0};const method=String(b.payment_method||'cod');if(method==='cod'&&!Number(settings.cod_enabled)||method==='bank'&&!Number(settings.bank_enabled)||method==='pickup'&&!Number(settings.store_pickup_enabled))return json({error:'Phương thức thanh toán này chưa được cửa hàng bật'},400);let discount=0,coupon=String(b.coupon_code||'').trim().toUpperCase();if(coupon){const cp=await env.DB.prepare(`SELECT * FROM commerce_coupons WHERE site_id=? AND upper(code)=? AND is_active=1 LIMIT 1`).bind(site.id,coupon).first();if(cp&&subtotal>=Number(cp.min_order||0)&&(!Number(cp.max_uses)||Number(cp.used_count)<Number(cp.max_uses))){discount=cp.discount_type==='percent'?Math.round(subtotal*Math.min(100,Number(cp.discount_value||0))/100):Math.min(subtotal,Number(cp.discount_value||0));await env.DB.prepare(`UPDATE commerce_coupons SET used_count=used_count+1 WHERE id=?`).bind(cp.id).run()}else coupon=''}let shipping=Number(settings.shipping_flat_fee||0);if(Number(settings.free_shipping_from||0)>0&&subtotal>=Number(settings.free_shipping_from))shipping=0;const total=Math.max(0,subtotal-discount+shipping),code='DH'+Date.now().toString().slice(-10);const r=await env.DB.prepare(`INSERT INTO commerce_orders(site_id,order_code,customer_name,phone,email,address,province,district,ward,shipping_method,note,subtotal,discount_amount,shipping_fee,total_amount,payment_method,items_json,coupon_code) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).bind(site.id,code,name,phone,String(b.email||''),String(b.address||''),String(b.province||''),String(b.district||''),String(b.ward||''),String(b.shipping_method||'standard'),String(b.note||''),subtotal,discount,shipping,total,method,JSON.stringify(clean),coupon).run();return json({ok:true,id:r.meta.last_row_id,order_code:code,total_amount:total,payment_method:method,bank:method==='bank'?{bank_name:settings.bank_name||'',account:settings.bank_account||'',holder:settings.bank_holder||'',qr_url:settings.bank_qr_url||''}:null,message:'Đặt hàng thành công'});
}
function commerceCleanVariants(v){const a=Array.isArray(v)?v:[];return a.slice(0,200).map((x,i)=>({name:String(x.name||x.label||('Biến thể '+(i+1))).slice(0,120),options:x.options&&typeof x.options==='object'?x.options:{},sku:String(x.sku||'').slice(0,100),price:commerceMoney(x.price),stock_qty:Math.max(0,Number(x.stock_qty??x.stock??0)),image_url:String(x.image_url||x.image||'').slice(0,1000)}))}
if(route==='commerce/products'){
 await commerceSeedFromSamples();if(request.method==='GET'){if(!user)return json({error:'Chưa đăng nhập'},401);const {results=[]}=await env.DB.prepare(`SELECT p.*,c.name category_name FROM commerce_products p LEFT JOIN commerce_categories c ON c.id=p.category_id WHERE p.site_id=? ORDER BY p.id DESC`).bind(site.id).all();return json({ok:true,products:results})}if(!user)return json({error:'Chưa đăng nhập'},401);const b=await body(request);if(request.method==='POST'){const name=String(b.name||'').trim();if(!name)return json({error:'Thiếu tên sản phẩm'},400);let resolvedCategoryId=Number(b.category_id)||0;const categoryName=String(b.category_name||'').trim();if(!resolvedCategoryId&&categoryName){await commerceEnsure();const slug=commerceSlug(categoryName);await env.DB.prepare(`INSERT OR IGNORE INTO commerce_categories(site_id,name,slug,sort_order,is_active) VALUES(?,?,?,?,1)`).bind(site.id,categoryName,slug,99).run();const cr=await env.DB.prepare(`SELECT id FROM commerce_categories WHERE site_id=? AND slug=? LIMIT 1`).bind(site.id,slug).first();resolvedCategoryId=Number(cr?.id||0)}if(!resolvedCategoryId)return json({error:'Vui lòng chọn danh mục'},400);const r=await env.DB.prepare(`INSERT INTO commerce_products(site_id,category_id,name,slug,sku,short_description,description,image_url,gallery_json,price,compare_at_price,stock_qty,stock_status,attributes_json,variants_json,badge,is_featured,is_active) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).bind(site.id,resolvedCategoryId,name,commerceSlug(b.slug||name+'-'+Date.now()),String(b.sku||commerceAutoSku(name)),String(b.short_description||''),String(b.description||''),String(b.image_url||''),JSON.stringify(b.gallery||[]),commerceMoney(b.price),commerceMoney(b.compare_at_price),Math.max(0,Number(b.stock_qty||0)),String(b.stock_status||'in_stock'),JSON.stringify(b.attributes||{}),JSON.stringify(commerceCleanVariants(b.variants)),String(b.badge||''),b.is_featured?1:0,b.is_active===false?0:1).run();const created=await env.DB.prepare(`SELECT p.*,c.name category_name,c.slug category_slug FROM commerce_products p LEFT JOIN commerce_categories c ON c.id=p.category_id WHERE p.id=? AND p.site_id=?`).bind(r.meta.last_row_id,site.id).first();return json({ok:true,id:r.meta.last_row_id,product:created})}if(request.method==='PUT'){const id=Number(b.id||0);await env.DB.prepare(`UPDATE commerce_products SET category_id=?,name=?,sku=?,short_description=?,description=?,image_url=?,gallery_json=?,price=?,compare_at_price=?,stock_qty=?,stock_status=?,attributes_json=?,variants_json=?,badge=?,is_featured=?,is_active=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND site_id=?`).bind(Number(b.category_id)||null,String(b.name||''),String(b.sku||''),String(b.short_description||''),String(b.description||''),String(b.image_url||''),JSON.stringify(b.gallery||parseJsonSafe(b.gallery_json,[])||[]),commerceMoney(b.price),commerceMoney(b.compare_at_price),Math.max(0,Number(b.stock_qty||0)),String(b.stock_status||'in_stock'),JSON.stringify(b.attributes||{}),JSON.stringify(commerceCleanVariants(b.variants)),String(b.badge||''),b.is_featured?1:0,b.is_active===false?0:1,id,site.id).run();return json({ok:true})}if(request.method==='DELETE'){const id=Number(u.searchParams.get('id')||0);await env.DB.prepare(`DELETE FROM commerce_products WHERE id=? AND site_id=?`).bind(id,site.id).run();return json({ok:true})}
}
async function commerceEnsureAdminCategories(){
 await commerceEnsure();
 const defaults=['Công nghệ','Điện thoại','Thời trang','Mỹ phẩm','Gia dụng','Nội thất','Phụ kiện','Thể thao','Khác'];
 let i=0;for(const name of defaults)await env.DB.prepare(`INSERT OR IGNORE INTO commerce_categories(site_id,name,slug,sort_order,is_active) VALUES(?,?,?,?,1)`).bind(site.id,name,commerceSlug(name),i++).run();
}
function commerceAutoSku(name=''){const base=commerceSlug(name).replace(/-/g,'').toUpperCase().slice(0,12)||'SP';return `SP-${base}-${Date.now().toString(36).toUpperCase().slice(-6)}`}
if(route==='commerce/categories'){
 if(!user)return json({error:'Chưa đăng nhập'},401);if(request.method==='GET'){try{const {results=[]}=await env.DB.prepare(`SELECT * FROM commerce_categories WHERE site_id=? AND is_active=1 ORDER BY sort_order,id`).bind(site.id).all();return json({ok:true,categories:results})}catch(e){return json({ok:true,categories:[],fallback:true})}}await commerceEnsureAdminCategories();const b=await body(request);
 const cleanFields=()=>{const a=Array.isArray(b.fields_schema)?b.fields_schema:[];return a.slice(0,60).map((f,i)=>({key:commerceSlug(f.key||f.label||('field-'+i)).replace(/-/g,'_'),label:String(f.label||f.key||'Thuộc tính').slice(0,80),type:['text','number','select','multiselect','boolean','color','size','date'].includes(String(f.type))?String(f.type):'text',options:Array.isArray(f.options)?f.options.map(x=>String(x).slice(0,80)).slice(0,50):[],required:!!f.required,filterable:!!f.filterable,show_on_card:!!f.show_on_card,show_in_specs:f.show_in_specs!==false,variant:!!f.variant}))};
 if(request.method==='POST'){const name=String(b.name||'').trim();if(!name)return json({error:'Thiếu tên danh mục'},400);const slug=commerceSlug(b.slug||name);await env.DB.prepare(`INSERT OR IGNORE INTO commerce_categories(site_id,name,slug,description,fields_schema_json,sort_order,is_active) VALUES(?,?,?,?,?,?,?)`).bind(site.id,name,slug,String(b.description||''),JSON.stringify(cleanFields()),Number(b.sort_order||99),1).run();const category=await env.DB.prepare(`SELECT * FROM commerce_categories WHERE site_id=? AND slug=? LIMIT 1`).bind(site.id,slug).first();if(!category)return json({error:'Không thể tạo danh mục'},500);return json({ok:true,id:category.id,category})}
 if(request.method==='PUT'){const id=Number(b.id||0);await env.DB.prepare(`UPDATE commerce_categories SET name=?,slug=?,description=?,fields_schema_json=?,sort_order=?,is_active=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND site_id=?`).bind(String(b.name||''),commerceSlug(b.slug||b.name),String(b.description||''),JSON.stringify(cleanFields()),Number(b.sort_order||0),b.is_active===false?0:1,id,site.id).run();return json({ok:true})}
 if(request.method==='DELETE'){const id=Number(u.searchParams.get('id')||0);const used=await env.DB.prepare(`SELECT COUNT(*) n FROM commerce_products WHERE site_id=? AND category_id=?`).bind(site.id,id).first();if(Number(used?.n||0)>0)return json({error:'Danh mục đang có sản phẩm. Hãy chuyển sản phẩm sang danh mục khác trước khi xóa.'},409);await env.DB.prepare(`DELETE FROM commerce_categories WHERE id=? AND site_id=?`).bind(id,site.id).run();return json({ok:true})}
}
if(route==='commerce/order-track'&&request.method==='GET'){
 await commerceEnsure();const code=String(u.searchParams.get('code')||'').trim().toUpperCase(),phone=String(u.searchParams.get('phone')||'').trim();if(!code||!phone)return json({error:'Nhập mã đơn và số điện thoại'},400);const x=await env.DB.prepare(`SELECT order_code,customer_name,phone,province,district,ward,subtotal,discount_amount,shipping_fee,total_amount,payment_method,payment_status,order_status,items_json,created_at,updated_at FROM commerce_orders WHERE site_id=? AND upper(order_code)=? AND phone=? LIMIT 1`).bind(site.id,code,phone).first();if(!x)return json({error:'Không tìm thấy đơn hàng'},404);return json({ok:true,order:x});
}
if(route==='commerce/orders'){
 await commerceEnsure();if(!user)return json({error:'Chưa đăng nhập'},401);if(request.method==='GET'){const {results=[]}=await env.DB.prepare(`SELECT * FROM commerce_orders WHERE site_id=? ORDER BY id DESC LIMIT 500`).bind(site.id).all();return json({ok:true,orders:results})}if(request.method==='PUT'){const b=await body(request);await env.DB.prepare(`UPDATE commerce_orders SET order_status=?,payment_status=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND site_id=?`).bind(String(b.order_status||'new'),String(b.payment_status||'unpaid'),Number(b.id||0),site.id).run();return json({ok:true})}
}
if(route==='commerce/settings'){
 await commerceEnsure();if(!user)return json({error:'Chưa đăng nhập'},401);if(request.method==='GET'){let x=await env.DB.prepare(`SELECT * FROM commerce_settings WHERE site_id=?`).bind(site.id).first();return json({ok:true,settings:x||{cod_enabled:1,bank_enabled:1,store_pickup_enabled:1,online_enabled:0,shipping_flat_fee:0,free_shipping_from:0}})}if(request.method==='PUT'){const b=await body(request);await env.DB.prepare(`INSERT INTO commerce_settings(site_id,cod_enabled,bank_enabled,store_pickup_enabled,online_enabled,bank_name,bank_account,bank_holder,bank_qr_url,shipping_flat_fee,free_shipping_from,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP) ON CONFLICT(site_id) DO UPDATE SET cod_enabled=excluded.cod_enabled,bank_enabled=excluded.bank_enabled,store_pickup_enabled=excluded.store_pickup_enabled,online_enabled=excluded.online_enabled,bank_name=excluded.bank_name,bank_account=excluded.bank_account,bank_holder=excluded.bank_holder,bank_qr_url=excluded.bank_qr_url,shipping_flat_fee=excluded.shipping_flat_fee,free_shipping_from=excluded.free_shipping_from,updated_at=CURRENT_TIMESTAMP`).bind(site.id,b.cod_enabled?1:0,b.bank_enabled?1:0,b.store_pickup_enabled?1:0,b.online_enabled?1:0,String(b.bank_name||''),String(b.bank_account||''),String(b.bank_holder||''),String(b.bank_qr_url||''),commerceMoney(b.shipping_flat_fee),commerceMoney(b.free_shipping_from)).run();return json({ok:true})}
}
...
// V47 — DELETE must run before POST/PUT body validation. Previously a DELETE
// request was rejected as "Thiếu Tiêu đề/Mô tả" before it could delete the row
// or write the sample tombstone, so sample posts appeared impossible to delete.
if(request.method==='DELETE'){
 const id=+u.searchParams.get('id');
 if(!id)return json({error:'Thiếu mã nội dung'},400);
 const deleting=await env.DB.prepare(`SELECT id,coalesce(is_sample,0) is_sample,coalesce(sample_key,'') sample_key FROM posts WHERE id=? AND site_id=? LIMIT 1`).bind(id,site.id).first();
 if(!deleting)return json({error:'Nội dung không tồn tại'},404);
 if(Number(deleting.is_sample)===1&&String(deleting.sample_key||'').trim()){
  await ensureSampleTombstones(env);
  await env.DB.prepare(`INSERT INTO site_sample_tombstones(site_id,sample_key,deleted_at) VALUES(?,?,CURRENT_TIMESTAMP) ON CONFLICT(site_id,sample_key) DO UPDATE SET deleted_at=CURRENT_TIMESTAMP`).bind(site.id,String(deleting.sample_key).trim()).run();
 }
 await env.DB.prepare(`DELETE FROM posts WHERE id=? AND site_id=?`).bind(id,site.id).run();
 return json({ok:true,deleted_id:id,sample_tombstoned:Number(deleting.is_sample)===1});
}
const b=await body(request);
const missing=[];
