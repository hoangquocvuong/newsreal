...
if(route==='commerce/categories'){
 if(!user)return json({error:'Chưa đăng nhập'},401);await commerceEnsureAdminCategories();if(request.method==='GET'){const {results=[]}=await env.DB.prepare(`SELECT * FROM commerce_categories WHERE site_id=? AND is_active=1 ORDER BY sort_order,id`).bind(site.id).all();return json({ok:true,categories:results})}const b=await body(request);
