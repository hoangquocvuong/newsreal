...
async function commerceEnsure(){
 const qs=[
 `CREATE TABLE IF NOT EXISTS commerce_categories(id INTEGER PRIMARY KEY AUTOINCREMENT,site_id INTEGER NOT NULL,name TEXT NOT NULL,slug TEXT NOT NULL,description TEXT NOT NULL DEFAULT '',image_url TEXT NOT NULL DEFAULT '',sort_order INTEGER NOT NULL DEFAULT 0,is_active INTEGER NOT NULL DEFAULT 1,created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,UNIQUE(site_id,slug))`,
 ...
 for(const q of [`ALTER TABLE commerce_orders ADD COLUMN ward TEXT NOT NULL DEFAULT ''`,`ALTER TABLE commerce_orders ADD COLUMN shipping_method TEXT NOT NULL DEFAULT 'standard'`])try{await env.DB.prepare(q).run()}catch(e){}
}
...
if(route==='commerce/categories'){
 await commerceEnsure();if(!user)return json({error:'Chưa đăng nhập'},401);if(request.method==='GET'){const {results=[]}=await env.DB.prepare(`SELECT * FROM commerce_categories WHERE site_id=? ORDER BY sort_order,id`).bind(site.id).all();return json({ok:true,categories:results})}const b=await body(request);if(request.method==='POST'){const name=String(b.name||'').trim();if(!name)return json({error:'Thiếu tên danh mục'},400);const r=await env.DB.prepare(`INSERT INTO commerce_categories(site_id,name,slug,description,sort_order,is_active) VALUES(?,?,?,?,?,?)`).bind(site.id,name,commerceSlug(b.slug||name),String(b.description||''),Number(b.sort_order||0),b.is_active===false?0:1).run();return json({ok:true,id:r.meta.last_row_id})}if(request.method==='DELETE'){await env.DB.prepare(`DELETE FROM commerce_categories WHERE id=? AND site_id=?`).bind(Number(u.searchParams.get('id')||0),site.id).run();return json({ok:true})}
}
