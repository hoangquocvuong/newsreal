...
,
  ...
  'dich-vu-5':{version:10,layout_contract:'universal-layout-v1',content_type:'commerce',geometry_locked:1,settings_schema:laptopSettings,lead_contract:'service-lead-v1',route_contract:'commerce-v1',card_contract:'commerce-product-card-v1',commerce_contract:'universal-commerce-v1',article_contract:'service-detail-v2',sections:[sec('hero','section','Cửa hàng Online',{content_source:'none',bind_required:0}),sec('products','section','Laptop nổi bật',{content_source:'none',bind_required:0}),cat(1,'Laptop văn phòng',{slots:6,desktop_columns:3}),cat(2,'Laptop gaming',{slots:6,desktop_columns:3}),cat(3,'Laptop mỏng nhẹ',{slots:6,desktop_columns:3}),cat(4,'Laptop đồ họa',{slots:6,desktop_columns:3}),cat(5,'MacBook',{slots:6,desktop_columns:3}),cat(6,'Cẩm nang laptop',{slots:6,desktop_columns:3}),sec('contact','section','Nhận tư vấn chọn laptop',{content_source:'none',bind_required:0})]},
...
async function ensureTemplateCatalog(env){
  ...
  const seeds=[
    ...
    {key:'dich-vu-5',name:'Cửa hàng Online · E-commerce',category:'dich-vu',preset:'service_laptop_store_5',price:5000000,renewal:5000000,sort:5,image:'https://images.unsplash.com/photo-1496181133206-80ce9b88a853?auto=format&fit=crop&w=1200&q=86',demo:'/demo/dich-vu/cua-hang-laptop/',badge:'E-COMMERCE',description:'Website bán hàng tổng quát với sản phẩm, giỏ hàng, checkout, thanh toán và quản lý đơn.',features:'Danh mục laptop\nCấu hình & giá rõ ràng\nẢnh laptop thực tế\nHộp yêu cầu tư vấn',accent:'blue'},
...
async function commerceEnsure(){
 ...
 `CREATE TABLE IF NOT EXISTS commerce_orders(id INTEGER PRIMARY KEY AUTOINCREMENT,site_id INTEGER NOT NULL,order_code TEXT NOT NULL UNIQUE,customer_name TEXT NOT NULL,phone TEXT NOT NULL,email TEXT NOT NULL DEFAULT '',address TEXT NOT NULL DEFAULT '',province TEXT NOT NULL DEFAULT '',district TEXT NOT NULL DEFAULT '',note TEXT NOT NULL DEFAULT '',subtotal INTEGER NOT NULL DEFAULT 0,discount_amount INTEGER NOT NULL DEFAULT 0,shipping_fee INTEGER NOT NULL DEFAULT 0,total_amount INTEGER NOT NULL DEFAULT 0,payment_method TEXT NOT NULL DEFAULT 'cod',payment_status TEXT NOT NULL DEFAULT 'unpaid',order_status TEXT NOT NULL DEFAULT 'new',items_json TEXT NOT NULL DEFAULT '[]',coupon_code TEXT NOT NULL DEFAULT '',created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)`,
