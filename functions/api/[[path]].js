...
async function installDefaultTemplateSamples(env,siteId,opts={}){
  ...
  await env.DB.prepare(`INSERT INTO site_template_state(site_id,sample_pack_installed_at,sample_pack_template_key,sample_pack_version,updated_at)
    ...
    ON CONFLICT(site_id) DO UPDATE SET sample_pack_installed_at=CURRENT_TIMESTAMP,sample_pack_template_key=excluded.sample_pack_template_key,sample_pack_version=7,updated_at=CURRENT_TIMESTAMP`)
...
async function commerceSeedFromSamples(){
 await commerceEnsure();const c=await env.DB.prepare(`SELECT count(*) n FROM commerce_products WHERE site_id=?`).bind(site.id).first();if(Number(c?.n||0)>0)return;
 const {results=[]}=await env.DB.prepare(`SELECT id,title,category,image,price,content,extra_json FROM posts WHERE site_id=? AND status='published' ORDER BY coalesce(is_sample,0) DESC,id DESC LIMIT 40`).bind(site.id).all();
 let order=0;for(const r of results){let cat=String(r.category||'Sản phẩm nổi bật').trim();let slug=commerceSlug(cat);await env.DB.prepare(`INSERT OR IGNORE INTO commerce_categories(site_id,name,slug,sort_order) VALUES(?,?,?,?)`).bind(site.id,cat,slug,order++).run();const cr=await env.DB.prepare(`SELECT id FROM commerce_categories WHERE site_id=? AND slug=?`).bind(site.id,slug).first();let ex={};try{ex=JSON.parse(String(r.extra_json||'{}'))}catch(e){};let price=commerceMoney(ex.service_price||r.price);if(price<10000)price=12990000+(Number(r.id||0)%12)*1000000;await env.DB.prepare(`INSERT OR IGNORE INTO commerce_products(site_id,category_id,name,slug,sku,short_description,description,image_url,price,compare_at_price,stock_qty,stock_status,attributes_json,badge,is_featured,is_active,is_sample) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).bind(site.id,cr?.id||null,String(r.title||'Sản phẩm'),commerceSlug(r.title+'-'+r.id),'SP-'+String(r.id).padStart(5,'0'),String(ex.service_area||'Sản phẩm mẫu có thể chỉnh sửa trong Admin'),String(r.content||''),String(r.image||''),price,Math.round(price*1.08),5+(Number(r.id||0)%17),'in_stock',JSON.stringify({Thông_số:String(ex.service_area||'Tùy chỉnh theo ngành hàng')}),Number(r.id||0)%4===0?'Bán chạy':'',Number(r.id||0)%5===0?1:0,1,1).run()}
}
