...
function defaultTemplateStructure(key){
 ...
 const newsStd={route_contract:'news-v2',card_contract:'title-only-v1',article_contract:'article-first-v1',article_sidebar:{enabled:1,sticky:1,internal_scroll:0},homepage_top:{min_stories:5}};
 const p={
  'tin-tuc-1':{...newsStd,version:6,content_type:'news',geometry_locked:1,sidebars:[side('.news-home-sidebar',[sw('popular','ĐỌC NHIỀU','ranked',6,'.news-side-box:nth-child(1)'),sw('categories','CHUYÊN MỤC','categories',8,'.news-side-box:nth-child(2)'),sw('latest','TIN MỚI','latest',5,'.news-side-box:nth-child(3)')])],sections:[sec('breaking','breaking','Mới nhất'),sec('hero','hero','Nổi bật',{slots:3}),sec('topics','topics','Chuyên mục'),sec('latest','latest','Tin mới nhất',{slots:12,desktop_columns:4,desktop_rows:3,tablet_columns:2,mobile_columns:1,fill_policy:'complete_rows'}),cat(1,'Kinh tế',{slots:12,desktop_columns:4,desktop_rows:3}),cat(2,'Công nghệ',{slots:12,desktop_columns:4,desktop_rows:3}),cat(3,'Du lịch',{slots:12,desktop_columns:4,desktop_rows:3}),cat(4,'Sức khỏe',{slots:12,desktop_columns:4,desktop_rows:3}),cat(5,'Bất động sản',{slots:12,desktop_columns:4,desktop_rows:3}),cat(6,'Đời sống',{slots:12,desktop_columns:4,desktop_rows:3}),sec('explore','explore','Nội dung khác',{slots:9,desktop_columns:3,desktop_rows:3,tablet_columns:2,mobile_columns:1,fill_policy:'complete_rows'}),sec('newsletter','newsletter','Nội dung của bạn, thương hiệu của bạn')]},
  'tin-tuc-2':{...newsStd,version:7,content_type:'news',geometry_locked:1,sidebars:[side('.np-home-sidebar',[sw('popular','ĐỌC NHIỀU','ranked',6,'.news-side-box:nth-child(1)'),sw('categories','CHUYÊN MỤC','categories',8,'.news-side-box:nth-child(2)'),sw('latest','TIN MỚI','latest',5,'.news-side-box:nth-child(3)')])],sections:[sec('ticker','ticker','Tin nóng'),sec('hero','hero','Nổi bật',{slots:3}),sec('latest','latest','Tin mới nhất',{slots:8,desktop_columns:2,desktop_rows:4,tablet_columns:2,mobile_columns:1,fill_policy:'complete_rows'}),cat(1,'Kinh tế',{slots:12,desktop_columns:4,desktop_rows:3}),cat(2,'Công nghệ',{slots:12,desktop_columns:4,desktop_rows:3}),cat(3,'Du lịch',{slots:12,desktop_columns:4,desktop_rows:3}),cat(4,'Sức khỏe',{slots:12,desktop_columns:4,desktop_rows:3}),cat(5,'Bất động sản',{slots:12,desktop_columns:4,desktop_rows:3})]},
  'tin-tuc-3':{...newsStd,version:8,content_type:'news',geometry_locked:1,sidebars:[],sections:[sec('editors-pick','hero',"EDITOR'S PICK · CÂU CHUYỆN ĐÁNG ĐỌC",{slots:5,desktop_columns:5,tablet_columns:2,mobile_columns:1,fill_policy:'complete_rows'}),sec('trending','trending','Trending now',{slots:12,desktop_columns:6,desktop_rows:2,tablet_columns:3,mobile_columns:2,fill_policy:'complete_rows'}),cat(1,'Kinh tế',{slots:10,desktop_columns:5,desktop_rows:2}),cat(2,'Công nghệ',{slots:10,desktop_columns:5,desktop_rows:2}),cat(3,'Du lịch',{slots:10,desktop_columns:5,desktop_rows:2}),cat(4,'Sức khỏe',{slots:10,desktop_columns:5,desktop_rows:2}),cat(5,'Bất động sản',{slots:10,desktop_columns:5,desktop_rows:2}),sec('weekend','special','Đọc chậm, hiểu sâu hơn',{eyebrow:'WEEKEND READ',slots:8,desktop_columns:4,desktop_rows:2,tablet_columns:2,mobile_columns:1,column_mode:'fixed',fill_policy:'complete_rows'})]},
...
function normalizeStructureProfile(raw,key,contentType='generic'){
 ...
 p.version=Math.max(4,Number(p.version||1));p.content_type=String(p.content_type||contentType||'generic');
...
async function ensureTemplateCatalog(env){
  ...
  const seeds=[
    ...
    {key:'dich-vu-1',name:'Dịch vụ Mẫu 1 · Internet, Truyền hình & Camera',category:'dich-vu',preset:'service_fpt_1',price:1499000,renewal:1999000,sort:1,image:'/assets/demo/dich-vu-1-preview.svg',demo:'/demo/dich-vu/mau-1/',badge:'DỊCH VỤ',description:'Landing website dịch vụ viễn thông với bảng gói cước, Internet, truyền hình, camera và CTA đăng ký rõ ràng.',features:'Gói Internet\nTruyền hình\nCamera\nCTA tư vấn & đăng ký',accent:'blue'},
  ...
  try{await env.DB.prepare(`UPDATE template_catalog SET editor_profile=? WHERE template_key='dich-vu-1' AND (coalesce(editor_profile,'')='' OR editor_profile NOT LIKE '%"content_type":"service"%')`).bind(JSON.stringify({id:'service',label:'Dịch vụ',content_type:'service',categories:['Internet FPT','Truyền hình FPT','Camera FPT','Combo Internet + Truyền hình','Combo Internet + Camera','Khuyến mãi'],contentLabel:'Mô tả gói dịch vụ',contentHelp:'Trình bày quyền lợi, điều kiện, khu vực áp dụng và hướng dẫn đăng ký.',custom_fields:[{key:'service_price',label:'Giá / tháng',type:'text',placeholder:'Ví dụ: 180.000đ/tháng'},{key:'service_speed',label:'Tốc độ / thông số',type:'text',placeholder:'Ví dụ: 300 Mbps'},{key:'service_promo',label:'Ưu đãi',type:'text',placeholder:'Ví dụ: Tặng modem Wi-Fi 6'},{key:'service_cta',label:'Nhãn nút CTA',type:'text',placeholder:'Ví dụ: Đăng ký ngay'}]})).run()}catch(e){}
  try{const sp={version:5,content_type:'service',geometry_locked:1,route_contract:'service-v1',card_contract:'service-card-v1',article_contract:'service-detail-v1',sidebars:[],sections:[{key:'hero',type:'section',title:'Giải pháp kết nối cho gia đình',slots:0,desktop_columns:1,tablet_columns:1,mobile_columns:1,content_source:'none',bind_required:0,empty_policy:'slots'},{key:'internet',type:'category',title:'Internet FPT',category:'Internet FPT',slots:3,desktop_columns:3,tablet_columns:2,mobile_columns:1,desktop_rows:1,content_source:'category',bind_required:1,empty_policy:'slots'},{key:'television',type:'category',title:'Truyền hình FPT',category:'Truyền hình FPT',slots:3,desktop_columns:3,tablet_columns:2,mobile_columns:1,desktop_rows:1,content_source:'category',bind_required:1,empty_policy:'slots'},{key:'camera',type:'category',title:'Camera FPT',category:'Camera FPT',slots:3,desktop_columns:3,tablet_columns:2,mobile_columns:1,desktop_rows:1,content_source:'category',bind_required:1,empty_policy:'slots'},{key:'combo',type:'category',title:'Combo tiết kiệm',category:'Combo Internet + Truyền hình',slots:3,desktop_columns:3,tablet_columns:2,mobile_columns:1,desktop_rows:1,content_source:'category',bind_required:1,empty_policy:'slots'},{key:'benefits',type:'benefits',title:'Vì sao chọn chúng tôi',slots:0,desktop_columns:4,tablet_columns:2,mobile_columns:1,content_source:'none',bind_required:0,empty_policy:'slots'},{key:'contact',type:'section',title:'Đăng ký tư vấn',slots:0,desktop_columns:1,tablet_columns:1,mobile_columns:1,content_source:'none',bind_required:0,empty_policy:'slots'}]};await env.DB.prepare(`UPDATE template_catalog SET structure_profile=? WHERE template_key='dich-vu-1' AND (coalesce(structure_profile,'')='' OR structure_profile NOT LIKE '%"content_type":"service"%')`).bind(JSON.stringify(sp)).run()}catch(e){}
  // V15.2: backfill đúng khung riêng cho 9 template hiện tại.
  for(const k of ['mau-1','mau-2','mau-3','mau-4','mau-5','tin-tuc-1','tin-tuc-2','tin-tuc-3','tin-tuc-4']){try{const row=await env.DB.prepare(`SELECT structure_profile FROM template_catalog WHERE template_key=?`).bind(k).first();let cur={};try{cur=JSON.parse(String(row?.structure_profile||'{}'))}catch(e){}const def=defaultTemplateStructure(k);const requiredVersion=Math.max(1,Number(def?.version||1));if(!row?.structure_profile||Number(cur?.version||0)<requiredVersion||!Array.isArray(cur?.sections)||!cur.sections.some(x=>Number(x?.slots||0)>0)){await env.DB.prepare(`UPDATE template_catalog SET structure_profile=? WHERE template_key=?`).bind(JSON.stringify(def),k).run()}}catch(e){}}
