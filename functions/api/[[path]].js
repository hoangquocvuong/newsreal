...
if(route==='commerce/categories'){
 await commerceEnsureAdminCategories();if(!user)return json({error:'Chưa đăng nhập'},401);if(request.method==='GET'){const {results=[]}=await env.DB.prepare(`SELECT * FROM commerce_categories WHERE site_id=? ORDER BY sort_order,id`).bind(site.id).all();return json({ok:true,categories:results})}const b=await body(request);
 ...
 if(request.method==='POST'){const name=String(b.name||'').trim();if(!name)return json({error:'Thiếu tên danh mục'},400);const r=await env.DB.prepare(`INSERT INTO commerce_categories(site_id,name,slug,description,fields_schema_json,sort_order,is_active) VALUES(?,?,?,?,?,?,?)`).bind(site.id,name,commerceSlug(b.slug||name),String(b.description||''),JSON.stringify(cleanFields()),Number(b.sort_order||0),b.is_active===false?0:1).run();return json({ok:true,id:r.meta.last_row_id})}
