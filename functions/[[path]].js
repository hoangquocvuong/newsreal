...
function demoInject(html,demo,trialCtx=null){
 ...
 html=html.replace(/\/assets\/style\.css\?v=[^\"'&<]+/g,'/assets/style.css?v=20.9.23.4').replace(/\/assets\/site\.js\?v=[^\"'&<]+/g,'/assets/site.js?v=20.9.23.4');
 ...
 const boot=`<meta name="robots" content="noindex,follow"><meta name="newsreal-demo-build" content="20.9.23.4"><script>window.NR_DEMO_THEME=${JSON.stringify(demo)};window.NR_DEMO_PREFIX=${JSON.stringify(prefix)};window.NR_TRIAL_TOKEN=${JSON.stringify(trialCtx?.trial_token||'')};window.NR_TRIAL_TENANT=${JSON.stringify(trialCtx?.domain||'')};window.NR_DEMO_TENANT=window.NR_TRIAL_TENANT||'batdongsan2027.org.uk';
...
function demoCenterHtml(siteName,templates=[],category=''){
 ...
 const cards=templates.map((t,idx)=>{
   ...
   return `<article class="demo-pro-card ${accent}">
    <div class="demo-pro-shot ${t.category==='game'?'game-live-preview':''}">
     ${t.category==='game'?gameMarketplacePreviewHtml('is-card'):`<img src="${esc(image)}" alt="${esc(t.name)}">`}
    ...
    <div class="demo-pro-body commercial-card">
     ...
     <div class="demo-pro-actions">
       ...
       <a class="demo-choose" href="${choose}" target="_blank" rel="noopener" data-market-choose><span data-market-choose-main>Đăng ký · <b data-market-choose-price>${new Intl.NumberFormat('vi-VN').format(Math.round((renewal||first)/1000))}K</b></span></a>
 ...
 <link rel="stylesheet" href="/assets/style.css?v=20.9.23.4">  <link rel="icon" type="image/png" sizes="16x16" href="/favicons/favicon-16x16.png">
  <meta name="msapplication-TileColor" content="#ffffff">
  <meta name="theme-color" content="#ffffff">
</head>
...
<script type="application/ld+json">${JSON.stringify({"@context":"https://schema.org","@graph":[{"@type":"Organization","@id":"https://hoangvuongtech.com/#organization","name":"HoangVuongTech","url":"https://hoangvuongtech.com/","email":"hoangquocvuong.hp89@gmail.com","telephone":"+84389986287","logo":"https://hoangvuongtech.com/favicons/favicon-16x16.png","contactPoint":{"@type":"ContactPoint","telephone":"+84389986287","contactType":"customer service","areaServed":"VN","availableLanguage":["vi"]}},{"@type":"WebSite","@id":"https://hoangvuongtech.com/#website","url":"https://hoangvuongtech.com/","name":"HoangVuongTech","publisher":{"@id":"https://hoangvuongtech.com/#organization"}},{"@type":"Service","name":"Thiết kế website trọn gói","provider":{"@id":"https://hoangvuongtech.com/#organization"},"areaServed":"VN","url":"https://hoangvuongtech.com/templates/"}]})}</script>`;
 ...
 // V20.9.23.4 — Product/Affiliate public showroom is a fully self-contained route.
 ...
 if(publicProductShowroom){
   ...
   const title=isProductDetail?`${relTitle} | Product Store · Affiliate`:'Product Store · Affiliate | Demo website bán hàng';
   const description=isProductDetail?`Chi tiết sản phẩm demo ${relTitle} trên giao diện Product Store · Affiliate.`:'Demo giao diện website catalog, review sản phẩm và affiliate với giá, voucher, rating, gallery và trang chi tiết sản phẩm.';
   let html=inject(INDEX_HTML,metaTags({title,description,image:'/assets/demo/san-pham-1-preview-real.webp',url:origin+rawPath,type:isProductDetail?'article':'website'}));
