...
function demoInject(html,demo,trialCtx=null){
 ...
 html=html.replace(/\/assets\/style\.css\?v=[^\"'&<]+/g,'/assets/style.css?v=20.9.20.7').replace(/\/assets\/site\.js\?v=[^\"'&<]+/g,'/assets/site.js?v=20.9.13');
...
function demoCenterHtml(siteName,templates=[],category=''){
   ...
   return `<article class="demo-pro-card ${accent}">
    ...
    <div class="demo-pro-body commercial-card">
     ...
     <div class="demo-pro-actions">
       ...
       <a class="demo-choose" href="${choose}" target="_blank" rel="noopener" data-market-choose><span data-market-choose-main>Chọn mẫu này</span><small data-market-choose-note hidden></small></a>
 ...
 <link rel="stylesheet" href="/assets/style.css?v=20.9.20.7">  <link rel="icon" type="image/png" sizes="16x16" href="/favicons/favicon-16x16.png">
  <meta name="msapplication-TileColor" content="#ffffff">
  <meta name="theme-color" content="#ffffff">
</head>
  ...
  <main class="demo-center">
   ...
   (()=>{
     ...
     document.addEventListener('click',e=>{
       ...
       if(voucher){e.preventDefault();const card=voucher.closest('.commercial-card'),box=card?.querySelector('[data-market-price]');if(!box)return;const list=Number(box.dataset.listPrice||0),price=Number(box.dataset.voucherPrice||0)||Math.max(0,list-500000),fmt=n=>new Intl.NumberFormat('vi-VN').format(n)+'đ',del=box.querySelector('[data-market-list]'),cur=box.querySelector('[data-market-current]'),st=box.querySelector('[data-market-voucher-status]'),choose=card?.querySelector('[data-market-choose]'),chooseMain=choose?.querySelector('[data-market-choose-main]'),chooseNote=choose?.querySelector('[data-market-choose-note]');if(del){del.hidden=false;del.textContent=fmt(list)}if(cur)cur.textContent=fmt(price);if(st)st.textContent='✓ Đã áp dụng voucher thành công';voucher.textContent='✓ ĐÃ ÁP DỤNG VOUCHER GIẢM 500K';voucher.disabled=true;if(chooseMain)chooseMain.textContent='Đăng ký mẫu này';if(chooseNote){const priceK=new Intl.NumberFormat('vi-VN').format(Math.round(price/1000))+'K',savingK=new Intl.NumberFormat('vi-VN').format(Math.round(Math.max(0,list-price)/1000))+'K';chooseNote.innerHTML='<span class="market-cta-price">'+priceK+'/năm</span><span class="market-cta-sep"> · </span><span class="market-cta-voucher">✓ Đã áp dụng giảm '+savingK+'</span>';chooseNote.hidden=false}card?.classList.add('voucher-applied');return;}
