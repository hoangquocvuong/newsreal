...
function postUrl(base,p){const cat=p.type==='news'?'tin-tuc':(p.transaction==='rent'?'cho-thue':(p.transaction==='sale'?'mua-ban':'bat-dong-san'));return `${base}/${cat}/${slugify(p.title)}-p${p.id}`}
...
function demoInject(html,demo,trialCtx=null){
 ...
 html=html.replace(/\/assets\/style\.css\?v=\d+/g,'/assets/style.css?v=20.0.1').replace(/\/assets\/site\.js\?v=[^\"'&<]+/g,'/assets/site.js?v=20.4.2');
 ...
 const boot=`<meta name="robots" content="noindex,follow"><meta name="newsreal-demo-build" content="20.4.2"><script>window.NR_DEMO_THEME=${JSON.stringify(demo)};window.NR_DEMO_PREFIX=${JSON.stringify(prefix)};window.NR_TRIAL_TOKEN=${JSON.stringify(trialCtx?.trial_token||'')};window.NR_TRIAL_TENANT=${JSON.stringify(trialCtx?.domain||'')};window.NR_DEMO_TENANT=window.NR_TRIAL_TENANT||'batdongsan2027.org.uk';
window.nrTrialAdminUrl=function(tab=''){
 if(!window.NR_TRIAL_TOKEN){
   const a=new URL('/admin',location.origin);
   if(tab)a.searchParams.set('tab',tab);
   return a.toString();
 }
 const a=new URL('https://hoangvuongtech.com/admin');
 if(window.NR_TRIAL_TENANT)a.searchParams.set('tenant',window.NR_TRIAL_TENANT);
 a.searchParams.set('nr_trial',window.NR_TRIAL_TOKEN);
 a.searchParams.set('template',String(window.NR_DEMO_THEME||''));
 if(tab)a.searchParams.set('tab',tab);
 return a.toString();
};
window.nrTrialUrl=function(raw){
 ...
 try{
   ...
   if(x.pathname==='/admin'||x.pathname.startsWith('/admin/'))return window.nrTrialAdminUrl(x.searchParams.get('tab')||'');
   if(x.origin!==location.origin)return raw;
   if(x.pathname.startsWith('/api/')||x.pathname.startsWith('/assets/')||x.pathname.startsWith('/control-center'))return raw;
   x.searchParams.set('nr_trial',window.NR_TRIAL_TOKEN);
...
window.nrApplyEstateDemoShell=function(){
 ...
 const propertyTypes=[
  ['Căn hộ / Chung cư','Chung cư'],['Nhà riêng','Nhà riêng'],['Nhà mặt phố','Nhà phố'],['Biệt thự / Liền kề','Biệt thự'],
 ...
 const navHtml=[['Trang chủ','/'],['Bất động sản','/bat-dong-san/'],['Nhà đất bán','/mua-ban/'],['Cho thuê','/cho-thue/']].map(([label,url])=>'<a href="'+window.nrEstateDemoUrl(url)+'">'+label+'</a>').join('')+typeMenu+'<a href="'+window.nrEstateDemoUrl('/#news')+'">Tin tức</a>';

 document.querySelectorAll('.header nav.nav,#mainNav,#detailNav,#favoritesNav').forEach(nav=>nav.innerHTML=navHtml);

 const admin='https://batdongsan2027.org.uk/admin?tab=newpost&template='+encodeURIComponent(key);
...
document.addEventListener('DOMContentLoaded',()=>{
 ...
 async function buy(){try{const d=await (await fetch('/api/trial/convert-request',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({token:window.NR_TRIAL_TOKEN})})).json();location.href=d.register_url||('/?template='+encodeURIComponent(window.NR_DEMO_THEME)+'&trial_token='+encodeURIComponent(window.NR_TRIAL_TOKEN)+'#dang-ky')}catch(e){location.href='/?template='+encodeURIComponent(window.NR_DEMO_THEME)+'&trial_token='+encodeURIComponent(window.NR_TRIAL_TOKEN)+'#dang-ky'}}
...
function demoCenterHtml(siteName,templates=[],category=''){
    ...
    <div class="demo-pro-body commercial-card">
     ...
     <div class="commercial-gift-box">
       ...
       <div class="commercial-includes">
        <span><i>✓</i><b>Tên miền riêng</b><small>Miễn phí 1 năm</small></span>
        <span><i>✓</i><b>Hosting website</b><small>Miễn phí 1 năm</small></span>
 ...
 <link rel="stylesheet" href="/assets/style.css?v=20.0.1">  <link rel="icon" type="image/png" sizes="16x16" href="/favicons/favicon-16x16.png">
  <meta name="msapplication-TileColor" content="#ffffff">
  <meta name="theme-color" content="#ffffff">
</head>
...
<script type="application/ld+json">${JSON.stringify({"@context":"https://schema.org","@graph":[{"@type":"Organization","@id":"https://hoangvuongtech.com/#organization","name":"HoangVuongTech","url":"https://hoangvuongtech.com/","email":"hoangquocvuong.hp89@gmail.com","telephone":"+84389986287"},{"@type":"WebSite","@id":"https://hoangvuongtech.com/#website","url":"https://hoangvuongtech.com/","name":"HoangVuongTech","publisher":{"@id":"https://hoangvuongtech.com/#organization"}},{"@type":"Service","name":"Website bất động sản trọn gói","provider":{"@id":"https://hoangvuongtech.com/#organization"},"areaServed":"VN","url":"https://hoangvuongtech.com/templates/bat-dong-san/"}]})}</script>`;
 ...
 if(path==='/listings'){
   const tx=u.searchParams.get('transaction')||'';u.searchParams.delete('transaction');const base=tx==='rent'?'/cho-thue/':tx==='sale'?'/mua-ban/':'/bat-dong-san/';const qs=u.searchParams.toString();return Response.redirect(origin+(demo?demoPrefixForPath(rawPath,demo):'')+base+(qs?'?'+qs:''),301);
 }
 const m=path.match(/^\/(mua-ban|cho-thue|bat-dong-san|tin-tuc)\/([^/]+)-p(\d+)$/i);
 if(m){
   ...
   if(!p && demo && /^mau-[1-5]$/.test(demo) && Number(m[3])>=910000){
     p={id:+m[3],title:String(m[2]||'Bất động sản demo').replace(/-/g,' '),type:'property',image:'',content:'',transaction:m[1].toLowerCase()==='cho-thue'?'rent':'sale'};
   }
 ...
 if(['/mua-ban','/cho-thue','/bat-dong-san'].includes(path)){
   const tx=path==='/mua-ban'?'sale':path==='/cho-thue'?'rent':'',label=tx==='sale'?'Mua bán nhà đất':tx==='rent'?'Bất động sản cho thuê':'Bất động sản';
