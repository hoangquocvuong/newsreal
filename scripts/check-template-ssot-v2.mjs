...
for(const key of PROFESSIONAL_TEMPLATE_KEYS){
 ...
 ok(c.hero&&c.hero.selector&&Array.isArray(c.hero.fields)&&(c.hero.fixed_demo===true||c.hero.fields.length>=7),`${key} hero blueprint`);
...
ok(lion.hero.mode==='fixed-demo-slider'&&lion.hero.fixed_demo===true&&lion.hero.slides===3&&lion.hero.image_fields.length===0,'lion fixed demo Hero contract');
...
ok(site.includes("heroImages=['https://images.unsplash.com/photo-1675605849420-8637ece6a0a1")&&site.includes("heroStat4='24/7'"),'lion live Hero is fixed to showroom demo');
